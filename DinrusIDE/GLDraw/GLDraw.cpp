...
void GLDraw::PutImage(Точка p, const Рисунок& img, const Прям& src)
{
	...
	LLOG("PutImage " << img.GetSerialId() << ' ' << p.x << ", " << p.y << ", "<< img.дайРазм());
	LLOG("SysImage cache pixels " << sTextureCache.дайРазм() << ", count " << sTextureCache.дайСчёт());

	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, GetTextureForImage(img));
	
	if(src == img.дайРазм()) {
		Прям r(p, img.дайРазм());
		...
		РазмерПЗ iszf = img.дайРазм();
		Прям s = src & img.дайРазм();
		Прям r(p, s.дайРазм());
		ПрямПЗ h;
...
void GLDraw::устЦвет(Цвет c)
{
	glColor3f(c.дайК() / 255.0, c.дайЗ() / 255.0, c.дайС() / 255.0);
}

void GLDraw::PutRect(const Прям& r, Цвет color)
{
	LTIMING("PutRect");
