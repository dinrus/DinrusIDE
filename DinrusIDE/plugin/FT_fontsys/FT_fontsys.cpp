...
struct FtFontStyle {
	...
	operator bool() const { return path.дайСчёт() || data; }
	
	FtFontStyle()         { data = NULL; size = 0; }
};

struct FtFontData : Движимое<FtFontData> {
	FtFontStyle style[4];
};

static Вектор<FtFontData>& sFontData()
{
	return Single<Вектор<FtFontData> >();
}

void sSetFont(int Индекс, const Ткст& path, const byte *data, int size, dword style)
{
	Вектор<FtFontData>& fd = sFontData();
	FtFontStyle& f = fd.по(Индекс).style[style & 3];
	f.path = path;
	f.data = data;
	f.size = size;
	if(!fd.по(Индекс).style[0])
		sSetFont(Индекс, path, data, size, 0);
	if(!fd.по(0).style[0])
...
void GetStdFontSys(Ткст& имя, int& height)
{
	имя = "xxxx";
}

FtFontStyle GetFontStyle(Шрифт font)
{
	Вектор<FtFontData>& fd = sFontData();
	int face = font.GetFace();
	if(face >= 0 && face < fd.дайСчёт()) {
...
Ткст GetFontDataSys(Шрифт font, const char *table, int offset, int size)
{
	...
	if(пусто_ли(f.path)) {
		if(table) {
			ПамЧтенПоток in(f.data, f.size);
			return GetFontDataSysSys(in, 0, table, offset, size);
		}
		return Ткст(f.data, min((int)f.size, 100*1024*1024));
	}
	if(table) {
		ФайлВвод in(f.path);
		return GetFontDataSysSys(in, 0, table, offset, size);
	}
	return загрузиФайл(f.path);
}
...
FT_Face CreateFTFace(Шрифт fnt)
{
	sInitFt();
	...
	FT_Face face;
	if(пусто_ли(f.path) ? FT_New_Memory_Face(sFTlib, f.data, f.size, 0, &face)
	                  : FT_New_Face(sFTlib, f.path, 0, &face))
		return NULL;

	FT_Set_Pixel_Sizes(face, 0, fnt.дайВысоту());
	return face;
}
...
static bool sLoadGlyph(Шрифт fnt, FT_Face face, int chr)
{
...
Вектор<FaceInfo> GetAllFacesSys()
{
	...
	Вектор<FaceInfo> list;
	for(int i = 0; i < __countof(basic_fonts); i++) {
		FaceInfo& fi = list.добавь();
		fi.имя = basic_fonts[i];
		fi.info = (i == 3) ? Шрифт::SCALEABLE|Шрифт::FIXEDPITCH : Шрифт::SCALEABLE;
	}

//	const Вектор<FtFontData>& fd = sFontData();
//	for(int i = __countof(basic_fonts); i < fd.дайСчёт(); i++) {
//		FaceInfo& fi = list.добавь();
//	}
...
bool RenderOutline(const FT_Outline& outline, FontGlyphConsumer& path, double xx, double yy)
{
	...
	for(n = 0; n < outline.n_contours; n++) {
		...
		path.Move(ТочкаПЗ(ft_dbl(v_start.x) + xx, -ft_dbl(v_start.y) + yy));
		while(point < limit) {
			point++;
			tags++;
			...
			case FT_CURVE_TAG_ON:
				path.Строка(ТочкаПЗ(ft_dbl(point->x) + xx, -ft_dbl(point->y) + yy));
				continue;
			...
			Do_Conic:
				if(point < limit) {
					...
					if(tag == FT_CURVE_TAG_ON) {
						path.Quadratic(ТочкаПЗ(ft_dbl(v_control.x) + xx, -ft_dbl(v_control.y) + yy),
						               ТочкаПЗ(ft_dbl(vec.x) + xx, -ft_dbl(vec.y) + yy));
						continue;
					}
					...
					path.Quadratic(ТочкаПЗ(ft_dbl(v_control.x) + xx, -ft_dbl(v_control.y) + yy),
					               ТочкаПЗ(ft_dbl(v_middle.x) + xx, -ft_dbl(v_middle.y) + yy));
					v_control = vec;
					goto Do_Conic;
				}
				path.Quadratic(ТочкаПЗ(ft_dbl(v_control.x) + xx, -ft_dbl(v_control.y) + yy),
				               ТочкаПЗ(ft_dbl(v_start.x) + xx, -ft_dbl(v_start.y) + yy));
				goto закрой;
			...
			default:
				...
				if(point <= limit) {
					...
					vec.x = point->x;
					vec.y = point->y;
					path.Cubic(ТочкаПЗ(ft_dbl(vec1.x) + xx, -ft_dbl(vec1.y) + yy),
					           ТочкаПЗ(ft_dbl(vec2.x) + xx, -ft_dbl(vec2.y) + yy),
					           ТочкаПЗ(ft_dbl(vec.x) + xx, -ft_dbl(vec.y) + yy));
					continue;
				}
				path.Cubic(ТочкаПЗ(ft_dbl(vec1.x) + xx, -ft_dbl(vec1.y) + yy),
