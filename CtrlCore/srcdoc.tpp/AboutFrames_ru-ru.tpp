...
формируют внешний вид и функциональность 
области, расположенной между внешней 
границей Ctrl и его показом.&]
[s0; &]
[s0; Фреймы упорядочиваются и у каждого 
имеется собственный индекс..У Ctrl всегда 
есть фрейм с индексом 0, и, как правило, 
он используется для обеспечения декоративно
го бордюра Ctrl. Если Ctrl не требуется 
никакого декоративного бордюра, фрейм 
0 можно установить в NullFrame(). Например, 
 StaticRect по умолчанию имеет NullFrame() в 
качестве frame0:&]
[s0; &]
[s7; -|TopWindow  win;&]
[s7; -|StaticRect ctrl;&]
...
прямоугольник представляет вид контрола. 
[* FrameAddSize] представляет собой противоположный
 процесс: он используется для определения 
требуемого размера Ctrl, чтобы получить 
заданый размер вида, другими словами, 
...
фрейма люьому существующему контролу 
Ctrl.&]
[s0;3 ]]