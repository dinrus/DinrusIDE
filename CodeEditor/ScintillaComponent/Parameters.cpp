...
struct WinMenuKeyDefinition //more or less matches accelerator table definition, easy copy/paste
{
    ...
    bool isAlt;
    bool isShift;
    const char * specialName;      //Used when no real menu name exists (in case of toggle for example)
};
...
int strVal(const char *str, int base)
{
    if (!str) return -1;
    if (!str[0]) return 0;
...
int decStrVal(const char *str)
{
    return strVal(str, 10);
}

int hexStrVal(const char *str)
{
    return strVal(str, 16);
}

int getKwClassFromName(const char *str)
{
...
std::wstring LocalizationSwitcher::getLangFromXmlFileName(const wchar *fn) const
{
...
std::wstring LocalizationSwitcher::getXmlFilePathFromLangName(const wchar *langName) const
{
    for (size_t i = 0, len = _localizationList.size(); i < len ; ++i)
    {
        if (0 == wcsicmp(langName, _localizationList[i].first.Begin()))
...
{
    wchar * fn = ::PathFindFileNameW(xmlFullPath.Begin());
...
bool LocalizationSwitcher::switchToLang(const wchar *lang2switch) const
{
    wstring langPath = getXmlFilePathFromLangName(lang2switch);
    if (langPath.IsEmpty())
        return false;

    return ::CopyFileW(langPath.Begin(), _nativeLangPath.Begin(), FALSE) != FALSE;
}


String ThemeSwitcher::getThemeFromXmlFileName(const char *xmlFullPath) const
{
    if (!xmlFullPath || !xmlFullPath[0])
        return String();
    String fn(::PathFindFileName(xmlFullPath));
    PathRemoveExtension(const_cast<char *>(fn.Begin()));
    return fn;
}
...
NppParameters::NppParameters()
{
    ...
    String notepadStylePath(_nppPath);
    pathAppend(notepadStylePath, notepadStyleFile);

    _asNotepadStyle = (PathFileExists(notepadStylePath.Begin()) == TRUE);
...
bool NppParameters::reloadStylers(const char* stylePath)
{
    delete _pXmlUserStylerDoc;

    const char* stylePathToLoad = stylePath != nullptr ? stylePath : _stylerPath.Begin();
...
{
    // use user path
    String nativeLangPath(_localizationSwitcher._nativeLangPath);

    // if "nativeLang.xml" does not exist, use npp path
    if (!PathFileExists(nativeLangPath.Begin()))
    {
        nativeLangPath = _nppPath;
        pathAppend(nativeLangPath, String(TEXT("nativeLang.xml")));
        if (!PathFileExists(nativeLangPath.Begin()))
            return false;
    }
    ...
    bool loadOkay = _pXmlNativeLangDocA->LoadUnicodeFilePath(nativeLangPath.Begin());
    if (!loadOkay)
    {
        delete _pXmlNativeLangDocA;
...
String NppParameters::getSpecialFolderLocation(int folderKind)
{
    char path[MAX_PATH];
...
String NppParameters::getSettingsFolder()
{
    if (_isLocal)
        return _nppPath;

    String settingsFolderPath = getSpecialFolderLocation(CSIDL_APPDATA);
...
bool NppParameters::load()
{
    ...
    String localConfPath(_nppPath);
    pathAppend(localConfPath, localConfFile);

    // Test if localConf.xml exist
    _isLocal = (PathFileExists(localConfPath.Begin()) == TRUE);
    ...
    if (_isLocal)
    {
        ...
        {
            String progPath = getSpecialFolderLocation(CSIDL_PROGRAM_FILES);
            char nppDirLocation[MAX_PATH];
            wcscpy_s(nppDirLocation, _nppPath.Begin());
    ...
    String cloudChoicePath{_userPath};
    ...
    if (::PathFileExists(cloudChoicePath.Begin()))
    {
        ...
        if (!cloudChoiceStrW.IsEmpty() && ::PathFileExists(cloudChoiceStrW.Begin()))
        {
    ...
    {
        if (!::PathIsDirectory(_cmdSettingsDir.Begin()))
        {
    ...
    //--------------------------//
    String langs_xml_path(_userPath);
    pathAppend(langs_xml_path, TEXT("langs.xml"));

    BOOL doRecover = FALSE;
    if (::PathFileExists(langs_xml_path.Begin()))
    {
        WIN32_FILE_ATTRIBUTE_DATA attributes;

        if (GetFileAttributesEx(langs_xml_path.Begin(), GetFileExInfoStandard, &attributes) != 0)
        {
    ...
    if (doRecover)
    {
        String srcLangsPath(_nppPath);
        pathAppend(srcLangsPath, TEXT("langs.model.xml"));
        ::CopyFile(srcLangsPath.Begin(), langs_xml_path.Begin(), FALSE);
    }
    ...
    //---------------------------//
    String configPath(_userPath);
    pathAppend(configPath, TEXT("config.xml"));

    String srcConfigPath(_nppPath);
    pathAppend(srcConfigPath, TEXT("config.model.xml"));

    if (!::PathFileExists(configPath.Begin()))
        ::CopyFile(srcConfigPath.Begin(), configPath.Begin(), FALSE);
    ...
    if (!PathFileExists(_stylerPath.Begin()))
    {
        String srcStylersPath(_nppPath);
        pathAppend(srcStylersPath, TEXT("stylers.model.xml"));

        ::CopyFile(srcStylersPath.Begin(), _stylerPath.Begin(), TRUE);
    }

    if (_nppGUI._themeName.IsEmpty() || (!PathFileExists(_nppGUI._themeName.Begin())))
        _nppGUI._themeName.assign(_stylerPath);

    _pXmlUserStylerDoc = new TiXmlDocument(_nppGUI._themeName.Begin());
    ...
    loadOkay = _pXmlNativeLangDocA->LoadUnicodeFilePath(nativeLangPath.Begin());
    if (!loadOkay)
    {
        delete _pXmlNativeLangDocA;
        _pXmlNativeLangDocA = nullptr;
    ...
    //---------------------------------//
    String toolbarIconsPath(_userPath);
    ...
    if (!PathFileExists(_shortcutsPath.Begin()))
    {
        String srcShortcutsPath(_nppPath);
        pathAppend(srcShortcutsPath, TEXT("shortcuts.xml"));

        ::CopyFile(srcShortcutsPath.Begin(), _shortcutsPath.Begin(), TRUE);
    }
    ...
    if (!PathFileExists(_contextMenuPath.Begin()))
    {
        String srcContextMenuPath(_nppPath);
        pathAppend(srcContextMenuPath, TEXT("contextMenu.xml"));

        ::CopyFile(srcContextMenuPath.Begin(), _contextMenuPath.Begin(), TRUE);
    }

    _pXmlContextMenuDocA = new TiXmlDocumentA();
    loadOkay = _pXmlContextMenuDocA->LoadUnicodeFilePath(_contextMenuPath.Begin());
    if (!loadOkay)
    {
    ...
    //-------------------------------------------------------------//
    String enableSelectFgColorPath = _userPath;
    pathAppend(enableSelectFgColorPath, TEXT("enableSelectFgColor.xml"));

    if (PathFileExists(enableSelectFgColorPath.Begin()))
    {
        _isSelectFgColorEnabled = true;
    }


    String filePath, filePath2, issueFileName;

    filePath = _nppPath;
    issueFileName = nppLogNetworkDriveIssue;
    issueFileName += TEXT(".xml");
    pathAppend(filePath, issueFileName);
    _doNppLogNetworkDriveIssue = (PathFileExists(filePath.Begin()) == TRUE);
    if (!_doNppLogNetworkDriveIssue)
    {
        filePath2 = _userPath;
        pathAppend(filePath2, issueFileName);
        _doNppLogNetworkDriveIssue = (PathFileExists(filePath2.Begin()) == TRUE);
    }
    ...
    issueFileName += TEXT(".xml");
    pathAppend(filePath, issueFileName);
    _doNppLogNulContentCorruptionIssue = (PathFileExists(filePath.Begin()) == TRUE);
    if (!_doNppLogNulContentCorruptionIssue)
    {
        filePath2 = _userPath;
        pathAppend(filePath2, issueFileName);
        _doNppLogNulContentCorruptionIssue = (PathFileExists(filePath2.Begin()) == TRUE);
    }
...
void NppParameters::setWorkSpaceFilePath(int i, const char* wsFile)
{
    if (i < 0 || i > 2 || !wsFile)
        return;
...
const char* NppParameters::getUserDefinedLangNameFromExt(char *ext, char *fullName) const
{
    if ((!ext) || (!ext[0]))
        return nullptr;
    ...
    {
        extVect.clear();
        cutString(_userLangArray[i]->_ext.Begin(), extVect);

        // Force to use dark mode UDL in dark mode or to use  light mode UDL in light mode
        for (size_t j = 0, len = extVect.size(); j < len; ++j)
        {
            if (!generic_stricmp(extVect[j].Begin(), ext) || (_tcschr(fullName, '.') && !generic_stricmp(extVect[j].Begin(), fullName)))
            {
                ...
                if (((NppDarkMode::isEnabled() && _userLangArray[i]->_isDarkModeTheme)) ||
                    ...
                    return _userLangArray[i]->_name.Begin();
            }
        }
    }
    ...
    if (iMatched >= 0)
    {
        return _userLangArray[iMatched]->_name.Begin();
    }
...
int NppParameters::getExternalLangIndexFromName(const char* externalLangName) const
{
    WcharMbcsConvertor& wmc = WcharMbcsConvertor::getInstance();
    for (int i = 0 ; i < _nbExternalLang ; ++i)
    {
        if (!lstrcmp(externalLangName, wmc.char2wchar(_externalLangArray[i]->_name.Begin(), CP_ACP)))
            return i;
    }
    return -1;
}
...
UserLangContainer* NppParameters::getULCFromName(const char *userLangName)
{
    for (int i = 0 ; i < _nbUserLang ; ++i)
    {
        if (0 == lstrcmp(userLangName, _userLangArray[i]->_name.Begin()))
...
static int CALLBACK EnumFontFamExProc(const LOGFONT* lpelfe, const TEXTMETRIC*, dword, LPARAM lParam)
{
    ...
    const char* lfFaceName = ((ENUMLOGFONTEX*)lpelfe)->elfLogFont.lfFaceName;
    ...
    {
        if (0 == lstrcmp(strVect[i].Begin(), lfFaceName))
        ...
        if (generic_stricmp(menuEntryName.Begin(), purgeMenuItemString(menuEntryString).Begin()) == 0)
        {
            ...
            do {
                ...
                else
                {
                    ...
                    if (generic_stricmp(menuItemName.Begin(), purgeMenuItemString(cmdStr).Begin()) == 0)
                    {
    ...
    for (int i = 0; i < nbPlugins; ++i)
    {
        ...
        if (generic_stricmp(pluginName.Begin(), purgeMenuItemString(menuItemString).Begin()) == 0)
        {
            ...
            for (int j = 0; j < nbPluginCmd; ++j)
            {
                ...
                if (generic_stricmp(pluginCmdName.Begin(), purgeMenuItemString(pluginCmdStr).Begin()) == 0)
                {
...
bool NppParameters::getContextMenuFromXmlTree(Menu* mainMenuHadle, Menu* pluginsMenu)
{
    ...
    if (contextMenuRoot)
    {
            ...
            childNode = childNode->NextSibling("Item") )
        {
            ...
            if (idStr)
            {
                _contextMenuItems.push_back(MenuItemUnit(id, displayAs.Begin(), folderName.Begin()));
            }
            else
            {
                ...
                if (!menuEntryName.IsEmpty() && !menuItemName.IsEmpty())
                {
                    ...
                    if (cmd != -1)
                        _contextMenuItems.push_back(MenuItemUnit(cmd, displayAs.Begin(), folderName.Begin()));
                }
                else
                {
                    ...
                    if (pluginsMenu && !pluginName.IsEmpty() && !pluginCmdName.IsEmpty())
                    {
                        ...
                        if (pluginCmdId != -1)
                            _contextMenuItems.push_back(MenuItemUnit(pluginCmdId, displayAs.Begin(), folderName.Begin()));
                    }
                }
            }
        }
...
void NppParameters::setWorkingDir(const char * newPath)
{
    ...
    else
    {
        if (PathFileExists(_nppGUI._defaultDirExp))
            ...
            _currentDirectory = _nppPath.Begin();
    }
}
...
bool NppParameters::getSessionFromXmlTree(TiXmlDocument *pSessionDoc, Session& session)
{
    ...
    int index = 0;
    const char *str = actView->Attribute(TEXT("activeView"), &index);
    if (str)
    {
    ...
    for (size_t k = 0; k < nbView; ++k)
    {
        if (viewRoots[k])
        {
            ...
            {
                const char *fileName = (childNode->ToElement())->Attribute(TEXT("filename"));
                if (fileName)
                {
                    Position position;
                    const char* posStr = (childNode->ToElement())->Attribute(TEXT("firstVisibleLine"));
                    if (posStr)
                    ...
                    const char* mapPosStr = (childNode->ToElement())->Attribute(TEXT("mapFirstVisibleDisplayLine"));
                    if (mapPosStr)
                    ...
                    const char *boolStr = (childNode->ToElement())->Attribute(TEXT("mapIsWrap"));
                    if (boolStr)
                    ...
                    int encoding = -1;
                    const char *encStr = (childNode->ToElement())->Attribute(TEXT("encoding"), &encoding);
                    const char *backupFilePath = (childNode->ToElement())->Attribute(TEXT("backupFilePath"));
                    ...
                    const char *boolStrReadOnly = (childNode->ToElement())->Attribute(TEXT("userReadOnly"));
                    if (boolStrReadOnly)
                    ...
                    {
                        const char* lineNumberStr = (markNode->ToElement())->Attribute(TEXT("line"));
                        if (lineNumberStr)
                        {
                    ...
                    {
                        const char *lineNumberStr = (foldNode->ToElement())->Attribute(TEXT("line"));
                        if (lineNumberStr)
                        {
    ...
    if (fileBrowserRoot)
    {
        const char *selectedItemPath = (fileBrowserRoot->ToElement())->Attribute(TEXT("latestSelectedItem"));
        if (selectedItemPath)
        {
        ...
        {
            const char *fileName = (childNode->ToElement())->Attribute(TEXT("foldername"));
            if (fileName)
            {
...
void NppParameters::feedFileListParameters(TiXmlNode *node)
{
    ...
    // nbMaxFile value
    int nbMaxFile;
    const char *strVal = (historyRoot->ToElement())->Attribute(TEXT("nbMaxFile"), &nbMaxFile);
    ...
    for (TiXmlNode *childNode = historyRoot->FirstChildElement(TEXT("File"));
        ...
        childNode = childNode->NextSibling(TEXT("File")) )
    {
        const char *filePath = (childNode->ToElement())->Attribute(TEXT("filename"));
        if (filePath)
        {
            _LRFileList[_nbRecentFile] = new String(filePath);
            ++_nbRecentFile;
        }
    }
}
...
void NppParameters::feedFileBrowserParameters(TiXmlNode *node)
{
    ...
    for (TiXmlNode *childNode = fileBrowserRoot->FirstChildElement(TEXT("root"));
        ...
        childNode = childNode->NextSibling(TEXT("root")) )
    {
        const char *filePath = (childNode->ToElement())->Attribute(TEXT("foldername"));
        if (filePath)
        {
...
void NppParameters::feedProjectPanelsParameters(TiXmlNode *node)
{
    ...
    {
        int index = 0;
        const char *idStr = (childNode->ToElement())->Attribute(TEXT("id"), &index);
        if (idStr && (index >= 0 && index <= 2))
        {
            const char *filePath = (childNode->ToElement())->Attribute(TEXT("workSpaceFile"));
            if (filePath)
            {
...
void NppParameters::feedFindHistoryParameters(TiXmlNode *node)
{
    ...
    if ((_findHistory._nbMaxFindHistoryPath > 0) && (_findHistory._nbMaxFindHistoryPath <= NB_MAX_FINDHISTORY_PATH))
    {
        ...
        {
            const char *filePath = (childNode->ToElement())->Attribute(TEXT("name"));
            if (filePath)
            {
                _findHistory._findHistoryPaths.push_back(String(filePath));
            }
        }
    }
    ...
    if ((_findHistory._nbMaxFindHistoryFilter > 0) && (_findHistory._nbMaxFindHistoryFilter <= NB_MAX_FINDHISTORY_FILTER))
    {
        ...
        {
            const char *fileFilter = (childNode->ToElement())->Attribute(TEXT("name"));
            if (fileFilter)
            {
                _findHistory._findHistoryFilters.push_back(String(fileFilter));
            }
        }
    }
    ...
    if ((_findHistory._nbMaxFindHistoryFind > 0) && (_findHistory._nbMaxFindHistoryFind <= NB_MAX_FINDHISTORY_FIND))
    {
        ...
        {
            const char *fileFind = (childNode->ToElement())->Attribute(TEXT("name"));
            if (fileFind)
            {
                _findHistory._findHistoryFinds.push_back(String(fileFind));
            }
        }
    }
    ...
    if ((_findHistory._nbMaxFindHistoryReplace > 0) && (_findHistory._nbMaxFindHistoryReplace <= NB_MAX_FINDHISTORY_REPLACE))
    {
        ...
        {
            const char *fileReplace = (childNode->ToElement())->Attribute(TEXT("name"));
            if (fileReplace)
            {
                _findHistory._findHistoryReplaces.push_back(String(fileReplace));
            }
        }
    }

    const char *boolStr = (findHistoryRoot->ToElement())->Attribute(TEXT("matchWord"));
    if (boolStr)
...
void NppParameters::feedShortcut(TiXmlNode *node)
{
    ...
    {
        int id;
        const char *idStr = (childNode->ToElement())->Attribute(TEXT("id"), &id);
        if (idStr)
        {
...
void NppParameters::getActions(TiXmlNode *node, Macro & macro)
{
    ...
    {
        int type;
        const char *typeStr = (childNode->ToElement())->Attribute(TEXT("type"), &type);
        if ((!typeStr) || (type > 3))
            continue;
        ...
        const char *sParam = (childNode->ToElement())->Attribute(TEXT("sParam"));
        if (!sParam)
            sParam = TEXT("");
...
void NppParameters::feedPluginCustomizedCmds(TiXmlNode *node)
{
    ...
    {
        const char *moduleName = (childNode->ToElement())->Attribute(TEXT("moduleName"));
        if (!moduleName)
            continue;

        int internalID = -1;
        const char *internalIDStr = (childNode->ToElement())->Attribute(TEXT("internalID"), &internalID);
...
void NppParameters::feedScintKeys(TiXmlNode *node)
{
    ...
    {
        int scintKey;
        const char *keyStr = (childNode->ToElement())->Attribute(TEXT("ScintID"), &scintKey);
        if (!keyStr)
            continue;
        ...
        for (int32_t i = 0; i < static_cast<int32_t>(len); ++i)
        {
            ...
            if (skmOrig.getScintillaKeyID() == (unsigned long)scintKey && skmOrig.getMenuCmdID() == menuID)
            {
                ...
                {
                    const char *str = (nextNode->ToElement())->Attribute(TEXT("Ctrl"));
                    if (!str)
                        continue;
...
bool NppParameters::feedBlacklist(TiXmlNode *node)
{
    ...
    {
        const char *name = (childNode->ToElement())->Attribute(TEXT("name"));
        if (name)
        {
...
bool NppParameters::getShortcuts(TiXmlNode *node, Shortcut & sc)
{
    ...
    const char *name = (node->ToElement())->Attribute(TEXT("name"));
    if (!name)
        name = TEXT("");

    bool isCtrl = false;
    const char *isCtrlStr = (node->ToElement())->Attribute(TEXT("Ctrl"));
    if (isCtrlStr)
        isCtrl = (lstrcmp(TEXT("yes"), isCtrlStr) == 0);

    bool isAlt = false;
    const char *isAltStr = (node->ToElement())->Attribute(TEXT("Alt"));
    if (isAltStr)
        isAlt = (lstrcmp(TEXT("yes"), isAltStr) == 0);

    bool isShift = false;
    const char *isShiftStr = (node->ToElement())->Attribute(TEXT("Shift"));
    if (isShiftStr)
        isShift = (lstrcmp(TEXT("yes"), isShiftStr) == 0);

    int key;
    const char *keyStr = (node->ToElement())->Attribute(TEXT("Key"), &key);
    if (!keyStr)
        return false;
...
std::pair<unsigned char, unsigned char> NppParameters::feedUserLang(TiXmlNode *node)
{
    ...
    {
        const char* name = (childNode->ToElement())->Attribute(TEXT("name"));
        const char* ext = (childNode->ToElement())->Attribute(TEXT("ext"));
        const char* darkModeTheme = (childNode->ToElement())->Attribute(TEXT("darkModeTheme"));
        const char* udlVersion = (childNode->ToElement())->Attribute(TEXT("udlVersion"));
...
LangType NppParameters::getLangFromExt(const char *ext)
{
    int i = getNbLang();
    i--;
    while (i >= 0)
    {
        ...
        if (isInList(ext, list.Begin()))
            return l->getLangID();
    }
    return L_TEXT;
}

void NppParameters::setCloudChoice(const char *pathChoice)
{
    String cloudChoicePath = getSettingsFolder();
...
void NppParameters::removeCloudChoice()
{
    ...
    if (PathFileExists(cloudChoicePath.Begin()))
    {
        ::DeleteFile(cloudChoicePath.Begin());
    }
}
...
bool NppParameters::isCloudPathChanged() const
{
    ...
    {
        char c = _initialCloudChoice.at(_initialCloudChoice.size()-1);
        if (c == '\\' || c == '/')
        {
    ...
    {
        char c = _nppGUI._cloudPath.at(_nppGUI._cloudPath.size() - 1);
        if (c == '\\' || c == '/')
        {
...
bool NppParameters::writeSettingsFilesOnCloudForThe1stTime(const String& cloudSettingsPath)
{
    ...
    // config.xml
    String cloudConfigPath = cloudSettingsPath;
    pathAppend(cloudConfigPath, TEXT("config.xml"));
    if (!::PathFileExists(cloudConfigPath.Begin()) && _pXmlUserDoc)
    {
        isOK = _pXmlUserDoc->SaveFile(cloudConfigPath.Begin());
        if (!isOK)
            return false;
    }

    // stylers.xml
    String cloudStylersPath = cloudSettingsPath;
    pathAppend(cloudStylersPath, TEXT("stylers.xml"));
    if (!::PathFileExists(cloudStylersPath.Begin()) && _pXmlUserStylerDoc)
    {
        isOK = _pXmlUserStylerDoc->SaveFile(cloudStylersPath.Begin());
        if (!isOK)
            return false;
    }

    // langs.xml
    String cloudLangsPath = cloudSettingsPath;
    pathAppend(cloudLangsPath, TEXT("langs.xml"));
    if (!::PathFileExists(cloudLangsPath.Begin()) && _pXmlUserDoc)
    {
        isOK = _pXmlDoc->SaveFile(cloudLangsPath.Begin());
        if (!isOK)
            return false;
    }

    // userDefineLang.xml
    String cloudUserLangsPath = cloudSettingsPath;
    pathAppend(cloudUserLangsPath, TEXT("userDefineLang.xml"));
    if (!::PathFileExists(cloudUserLangsPath.Begin()) && _pXmlUserLangDoc)
    {
        isOK = _pXmlUserLangDoc->SaveFile(cloudUserLangsPath.Begin());
        if (!isOK)
            return false;
    }

    // shortcuts.xml
    String cloudShortcutsPath = cloudSettingsPath;
    pathAppend(cloudShortcutsPath, TEXT("shortcuts.xml"));
    if (!::PathFileExists(cloudShortcutsPath.Begin()) && _pXmlShortcutDoc)
    {
        isOK = _pXmlShortcutDoc->SaveFile(cloudShortcutsPath.Begin());
        if (!isOK)
            return false;
    }

    // contextMenu.xml
    String cloudContextMenuPath = cloudSettingsPath;
    pathAppend(cloudContextMenuPath, TEXT("contextMenu.xml"));
    if (!::PathFileExists(cloudContextMenuPath.Begin()) && _pXmlContextMenuDocA)
    {
        isOK = _pXmlContextMenuDocA->SaveUnicodeFilePath(cloudContextMenuPath.Begin());
        if (!isOK)
            return false;
    }

    // nativeLang.xml
    String cloudNativeLangPath = cloudSettingsPath;
    pathAppend(cloudNativeLangPath, TEXT("nativeLang.xml"));
    if (!::PathFileExists(cloudNativeLangPath.Begin()) && _pXmlNativeLangDocA)
    {
        isOK = _pXmlNativeLangDocA->SaveUnicodeFilePath(cloudNativeLangPath.Begin());
        if (!isOK)
            return false;
    }
...
void NppParameters::writeDefaultUDL()
{
    ...
    {
        if (::PathFileExists(_userDefineLangPath.Begin()))
        {
            ::DeleteFile(_userDefineLangPath.Begin());
        }
    }
    ...
    for (auto udl : _pXmlUserLangsDoc)
    {
        if (udl._isDirty && udl._udlXmlDoc != nullptr && udl._udlXmlDoc != _pXmlUserLangDoc)
        {
            if (udl._indexRange.second == udl._indexRange.first) // no more udl for this xmldoc container
            {
                ...
                const char* docFilePath = udl._udlXmlDoc->Value();
...
void NppParameters::insertMacro(TiXmlNode *macrosRoot, const MacroShortcut & macro)
{
    ...
    for (size_t i = 0, len = macro._macro.size(); i < len ; ++i)
    {
        ...
        actionNode->ToElement()->SetAttribute(TEXT("sParam"), action._sParameter.Begin());
    }
}
...
void NppParameters::insertUserCmd(TiXmlNode *userCmdRoot, const UserCommand & userCmd)
{
    ...
    cmdRoot->InsertEndChild(TiXmlText(userCmd._cmd.Begin()));
}
...
{
    const char *pathName = fileName?fileName:_sessionPath.Begin();
    ...
    if (root)
    {
        ...
        for (size_t k = 0; k < nbElem ; ++k)
        {
            ...
            for (size_t i = 0, len = viewElems[k].viewFiles->size(); i < len ; ++i)
            {
                ...
                (fileNameNode->ToElement())->SetAttribute(TEXT("lang"), viewSessionFiles[i]._langName.Begin());
                ...
                (fileNameNode->ToElement())->SetAttribute(TEXT("filename"), viewSessionFiles[i]._fileName.Begin());
                (fileNameNode->ToElement())->SetAttribute(TEXT("backupFilePath"), viewSessionFiles[i]._backupFilePath.Begin());
        ...
        if (session._includeFileBrowser)
        {
            ...
            fileBrowserRootNode->ToElement()->SetAttribute(TEXT("latestSelectedItem"), session._fileBrowserSelectedItem.Begin());
            for (const auto& root : session._fileBrowserRoots)
            {
                ...
                (fileNameNode->ToElement())->SetAttribute(TEXT("foldername"), root.Begin());
            }
        }
    }
...
void NppParameters::feedUserSettings(TiXmlNode *settingsRoot)
{
    ...
    if (prefixNode)
    {
        const char *udlVersion = _userLangArray[_nbUserLang - 1]->_udlVersion.Begin();
        ...
        {
            char names[SCE_USER_TOTAL_KEYWORD_GROUPS][7] = {TEXT("words1"), TEXT("words2"), TEXT("words3"), TEXT("words4")};
            for (int i = 0 ; i < 4 ; ++i)
            {
...
void NppParameters::feedUserKeywordList(TiXmlNode *node)
{
    ...
    {
        const char * keywordsName = (childNode->ToElement())->Attribute(TEXT("name"));
        ...
        if (valueNode)
        {
            ...
            if (!lstrcmp(udlVersion, TEXT("")) && !lstrcmp(keywordsName, TEXT("Delimiters")))   // support for old style (pre 2.0)
            {
                ...
                wcscpy_s(_userLangArray[_nbUserLang - 1]->_keywordLists[SCE_USER_KWLIST_DELIMITERS], temp.Begin());
            }
            else if (!lstrcmp(keywordsName, TEXT("Comment")))
            {
                kwl = (valueNode)?valueNode->Value():TEXT("");
                //int len = _tcslen(kwl);
                basic_string<char> temp{TEXT(" ")};
                ...
                wcscpy_s(_userLangArray[_nbUserLang - 1]->_keywordLists[SCE_USER_KWLIST_COMMENTS], temp.Begin());
            }
            else
            {
                kwl = (valueNode)?valueNode->Value():TEXT("");
...
void NppParameters::feedUserStyles(TiXmlNode *node)
{
    ...
    {
        const char *styleName = (childNode->ToElement())->Attribute(TEXT("name"));
        if (styleName)
        {
...
bool NppParameters::feedStylerArray(TiXmlNode *node)
{
         ...
         childNode = childNode->NextSibling(TEXT("LexerType")) )
    {
        ...
        const char *lexerName = element->Attribute(TEXT("name"));
        const char *lexerDesc = element->Attribute(TEXT("desc"));
        const char *lexerUserExt = element->Attribute(TEXT("ext"));
        const char *lexerExcluded = element->Attribute(TEXT("excluded"));
        if (lexerName)
        {
    ...
    {
        TiXmlElement *element = childNode->ToElement();
        const char *styleIDStr = element->Attribute(TEXT("styleID"));
...
bool NppParameters::writeFileBrowserSettings(const Vector<String> & rootPaths, const String& latestSelectedItemPath) const
{
    ...
    if (rootPaths.size() != 0)
    {
        fileBrowserRootNode.SetAttribute(TEXT("latestSelectedItem"), latestSelectedItemPath.Begin());
        ...
        for (size_t i = 0; i < len; ++i)
        {
            ...
            (fbRootNode.ToElement())->SetAttribute(TEXT("foldername"), rootPaths[i].Begin());
...
LangType NppParameters::getLangIDFromStr(const char *langName)
{
    ...
    {
        const char * name = ScintillaEditView::_langNameInfoArray[lang]._langName;
...
void NppParameters::feedKeyWordsParameters(TiXmlNode *node)
{
        ...
        langNode = langNode->NextSibling(TEXT("Language")) )
    {
        if (_nbLang < NB_LANG)
        {
            ...
            const char* name = element->Attribute(TEXT("name"));
            if (name)
            {
                ...
                {
                    const char *indexName = (kwNode->ToElement())->Attribute(TEXT("name"));
                    TiXmlNode *kwVal = kwNode->FirstChild();
                    const char *keyWords = TEXT("");
                    if ((indexName) && (kwVal))
...
void NppParameters::feedGUIParameters(TiXmlNode *node)
{
    ...
    {
        TiXmlElement* element = childNode->ToElement();
        const char* nm = element->Attribute(TEXT("name"));
        if (nullptr == nm)
            continue;

        if (!lstrcmp(nm, TEXT("ToolBar")))
        {
            const char* val = element->Attribute(TEXT("visible"));
            if (val)
            {
        ...
        else if (!lstrcmp(nm, TEXT("TabBar")))
        {
            ...
            const char* val = element->Attribute(TEXT("dragAndDrop"));
            if (val)
            {
        ...
        {
            const char* val = element->Attribute(TEXT("matchCase"));
            if (val)
            {
                if (lstrcmp(val, TEXT("yes")) == 0)
        ...
        else if (!lstrcmp(nm, TEXT("TagsMatchHighLight")))
        {
            ...
            if (n)
            {
                ...
                if (val)
                {
                    ...
                    const char *tahl = element->Attribute(TEXT("TagAttrHighLight"));
                    if (tahl)
        ...
        else if (!lstrcmp(nm, TEXT("UserDefineDlg")))
        {
            ...
            const char* val = element->Attribute(TEXT("position"));
            if (val)
            {
        ...
        {
            int i;
            const char* val = element->Attribute(TEXT("size"), &i);
            if (val)
                _nppGUI._tabSize = i;
        ...
        {
            int i;
            const char* val = element->Attribute(TEXT("width"), &i);
            if (val)
        ...
        {
            const char* val = element->Attribute(TEXT("enableMultiSelection"));
            if (val)
            {
                if (lstrcmp(val, TEXT("yes")) == 0)
        ...
        else if (!lstrcmp(nm, TEXT("AppPosition")))
        {
            ...
            const char* val = element->Attribute(TEXT("isMaximized"));
            if (val)
        ...
        {
            const char* val = element->Attribute(TEXT("wrappedLines"));
            if (val)
            {
        ...
        else if (!lstrcmp(nm, TEXT("NewDocDefaultSettings")))
        {
            ...
            const char* val = element->Attribute(TEXT("openAnsiAsUTF8"));
            if (val)
        ...
        else if (!lstrcmp(nm, TEXT("langsExcluded")))
        {
            ...
            const char* val = element->Attribute(TEXT("langMenuCompact"));
            if (val)
        ...
        {
            const char* val = element->Attribute(TEXT("lineNumber"));
            if (val)
        ...
        else if (!lstrcmp(nm, TEXT("Backup")))
        {
            ...
            const char *bDir = element->Attribute(TEXT("useCustumDir"));
            if (bDir)
            {
                _nppGUI._useDir = (lstrcmp(bDir, TEXT("yes")) == 0);
            }
            const char *pDir = element->Attribute(TEXT("dir"));
            if (pDir)
                _nppGUI._backupDir = pDir;

            const char *isSnapshotModeStr = element->Attribute(TEXT("isSnapshotMode"));
        ...
        {
            const char *bDir = element->Attribute(TEXT("fg"));
            if (bDir)
        ...
        else if (!lstrcmp(nm, TEXT("auto-completion")))
        {
            ...
            const char * optName = element->Attribute(TEXT("autoCIgnoreNumbers"));
            if (optName)
        ...
        {
            const char * optName = element->Attribute(TEXT("htmlXmlTag"));
            if (optName)
                 ...
                 subChildNode = subChildNode->NextSibling(TEXT("UserDefinePair")) )
            {
                int open = -1;
                int openVal = 0;
                const char *openValStr = (subChildNode->ToElement())->Attribute(TEXT("open"), &openVal);
                if (openValStr && (openVal >= 0 && openVal < 128))
                    open = openVal;

                int close = -1;
                int closeVal = 0;
                const char *closeValStr = (subChildNode->ToElement())->Attribute(TEXT("close"), &closeVal);
        ...
        {
            const char * value = element->Attribute(TEXT("value"));
            if (value && value[0])
            {
            ...
            const char * path = element->Attribute(TEXT("defaultDirPath"));
            if (path && path[0])
            {
        ...
        {
            const char * value = element->Attribute(TEXT("short"));
        ...
        {
            const char * value = element->Attribute(TEXT("useDefault"));
            if (value && value[0])
            {
                if (lstrcmp(value, TEXT("yes")) == 0)
            ...
            const char *charsAddedW = element->Attribute(TEXT("charsAdded"));
            if (charsAddedW)
            {
                WcharMbcsConvertor& wmc = WcharMbcsConvertor::getInstance();
        ...
        else if (!lstrcmp(nm, TEXT("searchEngine")))
        {
            ...
            const char * searchEngineCustom = element->Attribute(TEXT("searchEngineCustom"));
        ...
        {
            const char* optNameMonoFont = element->Attribute(TEXT("monospacedFontFindDlg"));
            if (optNameMonoFont)
                _nppGUI._monospacedFontFindDlg = (lstrcmp(optNameMonoFont, TEXT("yes")) == 0);

            const char* optStopFillingFindField = element->Attribute(TEXT("stopFillingFindField"));
            if (optStopFillingFindField)
                _nppGUI._stopFillingFindField = (lstrcmp(optStopFillingFindField, TEXT("yes")) == 0);

            const char* optFindDlgAlwaysVisible = element->Attribute(TEXT("findDlgAlwaysVisible"));
            if (optFindDlgAlwaysVisible)
                _nppGUI._findDlgAlwaysVisible = (lstrcmp(optFindDlgAlwaysVisible, TEXT("yes")) == 0);

            const char* optConfirmReplaceOpenDocs = element->Attribute(TEXT("confirmReplaceInAllOpenDocs"));
            if (optConfirmReplaceOpenDocs)
                _nppGUI._confirmReplaceInAllOpenDocs = (lstrcmp(optConfirmReplaceOpenDocs, TEXT("yes")) == 0);

            const char* optReplaceStopsWithoutFindingNext = element->Attribute(TEXT("replaceStopsWithoutFindingNext"));
        ...
        {
            const char * optName = element->Attribute(TEXT("fileSwitcherWithoutExtColumn"));
            if (optName)
            ...
            const char * optNamePath = element->Attribute(TEXT("fileSwitcherWithoutPathColumn"));
            if (optNamePath)
            ...
            const char * optNameBackSlashEscape = element->Attribute(TEXT("backSlashIsEscapeCharacterForSql"));
            if (optNameBackSlashEscape && !lstrcmp(optNameBackSlashEscape, TEXT("no")))
                _nppGUI._backSlashIsEscapeCharacterForSql = false;

            const char * optNameWriteTechnologyEngine = element->Attribute(TEXT("writeTechnologyEngine"));
            ...
            const char * optNameFolderDroppedOpenFiles = element->Attribute(TEXT("isFolderDroppedOpenFiles"));
            if (optNameFolderDroppedOpenFiles)
                _nppGUI._isFolderDroppedOpenFiles = (lstrcmp(optNameFolderDroppedOpenFiles, TEXT("yes")) == 0);

            const char * optDocPeekOnTab = element->Attribute(TEXT("docPeekOnTab"));
            if (optDocPeekOnTab)
                _nppGUI._isDocPeekOnTab = (lstrcmp(optDocPeekOnTab, TEXT("yes")) == 0);

            const char * optDocPeekOnMap = element->Attribute(TEXT("docPeekOnMap"));
            if (optDocPeekOnMap)
                _nppGUI._isDocPeekOnMap = (lstrcmp(optDocPeekOnMap, TEXT("yes")) == 0);

            const char* optSortFunctionList = element->Attribute(TEXT("sortFunctionList"));
            if (optSortFunctionList)
                _nppGUI._shouldSortFunctionList = (lstrcmp(optSortFunctionList, TEXT("yes")) == 0);

            const char* saveDlgExtFilterToAllTypes = element->Attribute(TEXT("saveDlgExtFilterToAllTypes"));
            if (saveDlgExtFilterToAllTypes)
                _nppGUI._setSaveDlgExtFiltToAllTypes = (lstrcmp(saveDlgExtFilterToAllTypes, TEXT("yes")) == 0);

            const char * optMuteSounds = element->Attribute(TEXT("muteSounds"));
            if (optMuteSounds)
                _nppGUI._muteSounds = lstrcmp(optMuteSounds, TEXT("yes")) == 0;

            const char * optEnableFoldCmdToggable = element->Attribute(TEXT("enableFoldCmdToggable"));
        ...
        {
            auto parseYesNoBoolAttribute = [&element](const char* name, bool defaultValue = false)->bool {
...
void NppParameters::feedScintillaParam(TiXmlNode *node)
{
    ...
    // Line Number Margin
    const char *nm = element->Attribute(TEXT("lineNumberMargin"));
    if (nm)
    {
    ...
    {
        const char* currentLineModeStr = element->Attribute(TEXT("currentLineIndicator"));
...
void NppParameters::feedDockingManager(TiXmlNode *node)
{
        ...
        childNode = childNode->NextSibling(TEXT("PluginDlg")) )
    {
        ...
        const char *name = dlgElement->Attribute(TEXT("pluginName"));

        int id;
        const char *idStr = dlgElement->Attribute(TEXT("id"), &id);
        if (name && idStr)
        {
            int curr = 0; // on left
            int prev = 0; // on left
            ...
            const char *val = dlgElement->Attribute(TEXT("isVisible"));
            if (val)
            {
...
bool NppParameters::writeScintillaParams()
{
    ...
    const char *pViewName = TEXT("ScintillaPrimaryView");
    TiXmlNode *nppRoot = _pXmlUserDoc->FirstChild(TEXT("NotepadPlus"));
    if (!nppRoot)
    {
        nppRoot = _pXmlUserDoc->InsertEndChild(TiXmlElement(TEXT("NotepadPlus")));
    ...
    const char *pFolderStyleStr = (_svp._folderStyle == FOLDER_STYLE_SIMPLE)?TEXT("simple"):
    ...
    const char *pWrapMethodStr = (_svp._lineWrapMethod == LINEWRAP_ALIGNED)?TEXT("aligned"):
...
void NppParameters::createXmlTreeFromGUIParams()
{
    ...
    // <GUIConfig name="ToolBar" visible="yes">standard</GUIConfig>
    {
        ...
        const char *pStr = (_nppGUI._toolbarShow) ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="StatusBar">show</GUIConfig>
    {
        ...
        const char *pStr = _nppGUI._statusBarShow ? TEXT("show") : TEXT("hide");
        GUIConfigElement->InsertEndChild(TiXmlText(pStr));
    }
    ...
    // <GUIConfig name="TabBar" dragAndDrop="yes" drawTopBar="yes" drawInactiveTab="yes" reduce="yes" closeButton="yes" doubleClick2Close="no" vertical="no" multiLine="no" hide="no" quitOnEmpty="no" iconSetNumber="0" />
    {
        ...
        const char *pStr = (_nppGUI._tabStatus & TAB_DRAWTOPBAR) ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="ScintillaViewsSplitter">vertical</GUIConfig>
    {
        ...
        const char *pStr = _nppGUI._splitterPos == POS_VERTICAL ? TEXT("vertical") : TEXT("horizontal");
        GUIConfigElement->InsertEndChild(TiXmlText(pStr));
    }
    ...
    // <GUIConfig name="UserDefineDlg" position="undocked">hide</GUIConfig>
    {
        ...
        const char *pStr = (_nppGUI._userDefineDlgStatus & UDD_DOCKED) ? TEXT("docked") : TEXT("undocked");
    ...
    // <GUIConfig name = "TabSetting" size = "4" replaceBySpace = "no" / >
    {
        ...
        const char *pStr = _nppGUI._tabReplacedBySpace ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="FinderConfig" wrappedLines="no" purgeBeforeEverySearch="no" showOnlyOneEntryPerFoundLine="yes"/>
    {
        ...
        const char* pStr = _nppGUI._finderLinesAreCurrentlyWrapped ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="noUpdate" intervalDays="15" nextUpdateDate="20161022">no</GUIConfig>
    {
        ...
        element->SetAttribute(TEXT("nextUpdateDate"), _nppGUI._autoUpdateOpt._nextUpdateDate.toString().Begin());
    }

    // <GUIConfig name="Auto-detection">yes</GUIConfig>
    {
        const char *pStr = TEXT("no");
    ...
    // <GUIConfig name="Backup" action="0" useCustumDir="no" dir="" isSnapshotMode="yes" snapshotBackupTiming="7000" />
    {
        ...
        GUIConfigElement->SetAttribute(TEXT("dir"), _nppGUI._backupDir.Begin());
    ...
    // <GUIConfig name="uriCustomizedSchemes">svn://</GUIConfig>
    {
        ...
        GUIConfigElement->InsertEndChild(TiXmlText(_nppGUI._uriSchemes.Begin()));
    }
    ...
    // <GUIConfig name = "auto-completion" autoCAction = "3" triggerFromNbChar = "1" funcParams = "yes" autoCIgnoreNumbers = "yes" / >
    {
        ...
        const char * pStr = _nppGUI._autocIgnoreNumbers ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name = "sessionExt">< / GUIConfig>
    {
        ...
        GUIConfigElement->InsertEndChild(TiXmlText(_nppGUI._definedSessionExt.Begin()));
    }
    ...
    // <GUIConfig name="workspaceExt"></GUIConfig>
    {
        ...
        GUIConfigElement->InsertEndChild(TiXmlText(_nppGUI._definedWorkspaceExt.Begin()));
    }
    ...
    // <GUIConfig name="titleBar" short="no" />
    {
        ...
        const char *pStr = (_nppGUI._shortTitlebar) ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="stylerTheme" path="C:\sources\notepad-plus-plus\PowerEditor\visual.net\..\bin\stylers.xml" />
    {
        ...
        GUIConfigElement->SetAttribute(TEXT("path"), _nppGUI._themeName.Begin());
    }
    ...
    // <GUIConfig name="insertDateTime" path="C:\sources\notepad-plus-plus\PowerEditor\visual.net\..\bin\stylers.xml" />
    {
        ...
        GUIConfigElement->SetAttribute(TEXT("customizedFormat"), _nppGUI._dateTimeFormat.Begin());
        const char* pStr = (_nppGUI._dateTimeReverseDefaultOrder) ? TEXT("yes") : TEXT("no");
    ...
    // <GUIConfig name="wordCharList" useDefault="yes" charsAdded=".$%"  />
    {
        ...
        WcharMbcsConvertor& wmc = WcharMbcsConvertor::getInstance();
        const wchar* charsAddStr = wmc.char2wchar(_nppGUI._customWordChars.Begin(), SC_CP_UTF8);
    ...
    if (_nppGUI._commandLineInterpreter.compare(CMD_INTERPRETER))
    {
        ...
        GUIConfigElement->InsertEndChild(TiXmlText(_nppGUI._commandLineInterpreter.Begin()));
    }
    ...
    // <GUIConfig name="DarkMode" enable="no" colorTone="0" />
    {
        ...
        auto setYesNoBoolAttribute = [&GUIConfigElement](const char* name, bool value) {
            const char* pStr = value ? TEXT("yes") : TEXT("no");
...
bool NppParameters::writeFindHistory()
{
    ...
    {
        (hist_element.ToElement())->SetAttribute(TEXT("name"), _findHistory._findHistoryPaths[i].Begin());
    ...
    {
        (hist_element.ToElement())->SetAttribute(TEXT("name"), _findHistory._findHistoryFilters[i].Begin());
        findHistoryRoot->InsertEndChild(hist_element);
    }
    ...
    {
        (hist_element.ToElement())->SetAttribute(TEXT("name"), _findHistory._findHistoryFinds[i].Begin());
        findHistoryRoot->InsertEndChild(hist_element);
    }
    ...
    {
        (hist_element.ToElement())->SetAttribute(TEXT("name"), _findHistory._findHistoryReplaces[i].Begin());
        findHistoryRoot->InsertEndChild(hist_element);
    }
...
{
    const char *pStr = _nppGUI._printSettings._printLineNumber?TEXT("yes"):TEXT("no");
    element->SetAttribute(TEXT("lineNumber"), pStr);

    element->SetAttribute(TEXT("printOption"), _nppGUI._printSettings._printOption);

    element->SetAttribute(TEXT("headerLeft"), _nppGUI._printSettings._headerLeft.Begin());
    element->SetAttribute(TEXT("headerMiddle"), _nppGUI._printSettings._headerMiddle.Begin());
    element->SetAttribute(TEXT("headerRight"), _nppGUI._printSettings._headerRight.Begin());
    element->SetAttribute(TEXT("footerLeft"), _nppGUI._printSettings._footerLeft.Begin());
    element->SetAttribute(TEXT("footerMiddle"), _nppGUI._printSettings._footerMiddle.Begin());
    element->SetAttribute(TEXT("footerRight"), _nppGUI._printSettings._footerRight.Begin());

    element->SetAttribute(TEXT("headerFontName"), _nppGUI._printSettings._headerFontName.Begin());
    ...
    element->SetAttribute(TEXT("footerFontName"), _nppGUI._printSettings._footerFontName.Begin());
...
{
    const char *pStr = bVal?TEXT("yes"):TEXT("no");
...
String NppParameters:: getWinVersionStr() const
{
    switch (_winVersion)
    {
...
String NppParameters::getWinVerBitStr() const
{
    switch (_platForm)
    {
    case PF_X86:
...
String NppParameters::writeStyles(LexerStylerArray & lexersStylers, StyleArray & globalStylers)
{
        ...
        childNode = childNode->NextSibling(TEXT("LexerType")))
    {
        ...
        if (pLs)
        {
                    ...
                    grChildNode = grChildNode->NextSibling(TEXT("WordsStyle")))
            {
                ...
                const char *styleName = grElement->Attribute(TEXT("name"));
        ...
        childNode = childNode->NextSibling(TEXT("WidgetStyle")))
    {
        ...
        const char *styleName = pElement->Attribute(TEXT("name"));
    ...
    if (!isSaved)
    {
        ...
        if (!savePath.IsEmpty())
        {
            _pXmlUserStylerDoc->SaveFile(savePath.Begin());
            return savePath;
        }
    }
    return TEXT("");
...
bool NppParameters::insertTabInfo(const char *langName, int tabInfo)
{
...
void NppParameters::writeStyle2Element(const Style & style2Write, Style & style2Sync, TiXmlElement *element)
{
    ...
    {
        const char * oldFontName = element->Attribute(TEXT("fontName"));
    ...
    if (!style2Write._keywords.IsEmpty())
    {
        TiXmlNode *teteDeNoeud = element->LastChild();

        if (teteDeNoeud)
            teteDeNoeud->SetValue(style2Write._keywords.Begin());
        else
            element->InsertEndChild(TiXmlText(style2Write._keywords.Begin()));
    }
}
...
void NppParameters::insertUserLang2Tree(TiXmlNode *node, UserLangContainer *userLang)
{
    ...
    rootElement->SetAttribute(TEXT("udlVersion"), udlVersion.Begin());
