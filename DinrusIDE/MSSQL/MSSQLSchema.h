...
#define STRING(x, n)               COLUMN(MsSqlTextType(n), Ткст, x, n, 0)
#define STRING_ARRAY(x, n, items)  COLUMN_ARRAY(MsSqlTextType(n), Ткст, x, n, 0, items)
#define STRING_(x, n)              COLUMN_(MsSqlTextType(n), Ткст, x, n, 0)
#define STRING_ARRAY_(x, n, items) COLUMN_ARRAY_(MsSqlTextType(n), Ткст, x, n, 0, items)

#define LONGRAW(x)                 COLUMN("varbinary(max)", Ткст, x, 0, 0)
#define LONGRAW_(x)                COLUMN_("varbinary(max)", Ткст, x, 0, 0)

#define BLOB(x)                    COLUMN("varbinary(max)", Ткст, x, 0, 0)
#define BLOB_(x)                   COLUMN_("varbinary(max)", Ткст, x, 0, 0)

#ifndef PRIMARY_KEY
#define PRIMARY_KEY                INLINE_ATTRIBUTE("primary ключ")\
                                   ATTRIBUTE("alter table @t add constraint PK_@x primary ключ "\
                                             "(@c)",\
                                             "alter table @t drop constraint PK_@x;")\
                                   ATTRIBUTE("create Индекс PKX_@x on @t(@c);", \
                                             "drop Индекс PKX_@x;")
#endif
...
#define Индекс                      ATTRIBUTE("create Индекс IDX_@x on @t(@c);", \
                                             "drop Индекс IDX_@x;")
#define UNIQUE                     ATTRIBUTE("create unique Индекс UNQ_@x on @t(@c);", \
	                                         "drop Индекс UNQ_@x;")
