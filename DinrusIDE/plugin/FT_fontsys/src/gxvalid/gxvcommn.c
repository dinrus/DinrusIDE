...
/*  by suzuki toshiya, Masatake YAMATO, красный Hat K.K.,                      */
...
#define FT_COMPONENT  trace_gxvcommon
  ...
  /*************************************************************************/
  /*****                                                               *****/
  /*****               scan значение array and get min & max              *****/
  /*****                                                               *****/
  /*************************************************************************/
  /*************************************************************************/
   ...
   * which is used in LookupTable формат 2, 4, 6.
   *
...
#define GXV_UNITSIZE_VALIDATE( FORMAT, UNITSIZE, NUNITS, CORRECTSIZE ) \
  ...
  /* ================= Simple Array фмт 0 Lookup Table ================ */
  static void
                                 ...
                                 GXV_Validator  valid )
  {
    ...
    GXV_NAME_ENTER( "LookupTable формат 0" );
    ...
    for ( i = 0; i < valid->face->num_glyphs; i++ )
    {
      ...
      значение = GXV_LOOKUP_VALUE_LOAD( p, valid->lookupval_sign );
      valid->lookupval_func( i, &значение, valid );
    }
  ...
  /* ================= Segment Single фмт 2 Loolup Table ============== */
  /*
   * Apple spec says:
   *
   ...
   *   included is table-specific.  The значение that indicates binary search
                                 ...
                                 GXV_Validator  valid )
  {
    ...
    GXV_NAME_ENTER( "LookupTable формат 2" );
    ...
    for ( unit = 0, gid = 0; unit < nUnits; unit++ )
    {
      ...
      for ( gid = firstGlyph; gid <= lastGlyph; gid++ )
        valid->lookupval_func( gid, &значение, valid );
    }
  ...
  /* ================= Segment Array фмт 4 Lookup Table =============== */
  static void
                                 ...
                                 GXV_Validator  valid )
  {
    ...
    GXV_NAME_ENTER( "LookupTable формат 4" );
    ...
    for ( unit = 0, gid = 0; unit < nUnits; unit++ )
    {
      ...
      for ( gid = firstGlyph; gid <= lastGlyph; gid++ )
      {
        значение = valid->lookupfmt4_trans( (FT_UShort)( gid - firstGlyph ),
                                         &base_value,
                                         limit,
                                         valid );
  ...
  /* ================= Segment Table фмт 6 Lookup Table =============== */
  static void
                                 ...
                                 GXV_Validator  valid )
  {
    ...
    GXV_NAME_ENTER( "LookupTable формат 6" );
    ...
    for ( unit = 0, prev_glyph = 0; unit < nUnits; unit++ )
    {
      ...
      valid->lookupval_func( glyph, &значение, valid );
    }
  ...
  /* ================= Trimmed Array фмт 8 Lookup Table =============== */
  static void
                                 ...
                                 GXV_Validator  valid )
  {
    ...
    GXV_NAME_ENTER( "LookupTable формат 8" );
    ...
    for ( i = 0; i < glyphCount; i++ )
    {
      GXV_LIMIT_CHECK( 2 );
      значение = GXV_LOOKUP_VALUE_LOAD( p, valid->lookupval_sign );
      valid->lookupval_func( (FT_UShort)( firstGlyph + i ), &значение, valid );
    }
                         ...
                         GXV_Validator  valid )
  {
    ...
    for ( i = 0; i < nnames; i++ )
    {
      if ( FT_Get_Sfnt_Name( valid->face, i, &имя ) != GXV_Err_Ok )
        continue ;

      if ( имя.name_id == name_index )
        goto Out;
    }
  ...
  Out:
    ...
    GXV_TRACE_HEXDUMP_SFNTNAME( имя );
    FT_TRACE1(( ")\n" ));
   ...
   * are permitted.  по present, holes in a defined class are not checked.
   *   -- suzuki toshiya <mpsuzuki@hiroshima-u.ac.jp>
   */
  ...
  */
                        ...
                        GXV_odtect_Range  odtect )
  {
    ...
    odtect->range[ odtect->nRanges ].имя   = (FT_String*)имя;
    odtect->nRanges++;
  }
                       ...
                       GXV_Validator     valid )
  {
    ...
    for ( i = 0; i < odtect->nRanges; i++ )
      for ( j = 0; j < i; j++ )
        ...
        {
          if ( odtect->range[i].имя || odtect->range[j].имя )
