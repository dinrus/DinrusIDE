...
#define FT_PIX_CEIL( x )      FT_PIX_FLOOR( (x) + 63 )
  ...
  /*
   *  Return the highest power of 2 that is <= значение; this correspond to
   *  the highest bit in a given 32-bit значение.
   */
  FT_BASE( FT_UInt32 )
  ft_highpow2( FT_UInt32  значение );
...
#define FT_CMAP_FACE( x )         FT_CMAP( x )->charmap.face
  ...
  typedef struct  FT_CMap_ClassRec_
  {
    ...
    /* Subsequent entries are special ones for формат 14 -- the variant */
...
#define FT_DEFINE_CMAP_CLASS(class_, size_, init_, done_, char_index_,       \
  ...
  /*      of any glyph in this face.  If this значение cannot be known in     */
  /*      advance, or if the face isn't scalable, this should be set to 0. */
  /*      Only relevant for scalable formats.                              */
  /*                                                                       */
  /*    max_contours ::                                                    */
  /*      The maximal number of contours used to store the vectorial       */
  /*      outline of any glyph in this face.  If this значение cannot be      */
  ...
  /*                         bitmap буфер.                                */
  /*                                                                       */
  /*    glyph_transformed :: Boolean.  уст to TRUE when the loaded glyph   */
  ...
  /*                                                                       */
  /*    glyph_hints       :: фмт-specific glyph hints management.       */
  /*                                                                       */
...
#define FT_DRIVER_HAS_HINTER( x )  ( FT_MODULE_CLASS( x )->module_flags & \
  ...
  /*                                                                       */
  /*    module_name :: The module's имя (as an ASCII string).             */
  /*                                                                       */
  /* <Return>                                                              */
...
#define FT_REQUEST_HEIGHT( req )                                            \
  ...
  /* уст the metrics according to a bitmap strike. */
  FT_BASE( void )
  FT_Select_Metrics( FT_Face   face,
                     FT_ULong  strike_index );


  /* уст the metrics according to a size request. */
  FT_BASE( void )
  FT_Request_Metrics( FT_Face          face,
                      FT_Size_Request  req );


  /* сверь a size request against `available_sizes'. */
  FT_BASE( FT_Error )
  ...
  /* освободи the bitmap of a given glyphslot when needed (i.e., only when it */
  /* was allocated with ft_glyphslot_alloc_bitmap).                       */
  FT_BASE( void )
  ft_glyphslot_free_bitmap( FT_GlyphSlot  slot );


  /* Allocate a new bitmap буфер in a glyph slot. */
  FT_BASE( FT_Error )
  ft_glyphslot_alloc_bitmap( FT_GlyphSlot  slot,
                             FT_ULong      size );


  /* уст the bitmap буфер in a glyph slot to a given pointer.  The буфер */
...
#define FT_DRIVER_CLASS( x )  FT_DRIVER( x )->clazz
  ...
  /*    managing and loading font files of a given формат.                 */
  /*                                                                       */
  /*  <Fields>                                                             */
  /*     root         :: содержит the fields of the root module class.     */
  /*                                                                       */
...
#define FT_DEBUG_HOOK_TRUETYPE            0
  ...
  /* уст this debug hook to a non-null pointer to force unpatented hinting */
...
#define FT_DEBUG_HOOK_UNPATENTED_HINTING  1
  ...
  /*                        libraries.  нов modules are added through the  */
  ...
  /*                        FT_GLYPH_FORMAT_OUTLINE формат.                */
  /*                                                                       */
  /*    auto_hinter      :: XXX                                            */
  /*                                                                       */
  ...
  /*                                                                       */
  /*    pic_container    :: содержит global structs and tables, instead    */
  ...
  /*                                                                       */
  /* <Return>                                                              */
  /*    A pointer to the new memory object.  0 in case of Ошибка.           */
  /*                                                                       */
  FT_BASE( FT_Memory )
  FT_New_Memory( void );
