 ...
 *
 * This file contains routines to write output images in GIF формат.
 *
 ...
 *   hash_value[i]	symbol's значение; undefined if empty slot
 * where slot values (i) range from 0 to HSIZE-1.  The symbol значение is
...
typedef struct {
  ...
  /* GIF data packet construction буфер */
...
/* добавь a character to current packet; flush to disk if necessary */
...
/* переустанов compressor and issue a очисть code */
{
...
/* Initialize compressor */
{
  ...
  if (dinfo->hash_code != NULL)
    clear_hash(dinfo);
  /* GIF specifies an initial очисть code */
...
/* Clean up at end */
{
  /* слей out the buffered LZW code */
  if (! dinfo->first_byte)
    output(dinfo, dinfo->waiting_code);
  /* Send an EOF code */
  output(dinfo, dinfo->EOFCode);
  /* слей the bit-packing буфер */
  if (dinfo->cur_bits > 0) {
    CHAR_OUT(dinfo, dinfo->cur_accum & 0xFF);
  }
  /* слей the packet буфер */
  flush_packet(dinfo);
}
...
/* Emit 3 copies of same byte значение --- handy subr for colormap construction */
{
...
/* If colormap == NULL, synthesize a grayscale colormap */
{
  ...
  /* пиши the Logical Screen Descriptor */
  ...
  putc(0, dinfo->pub.output_file); /* фон color index */
  putc(0, dinfo->pub.output_file); /* Reserved (aspect ratio in GIF89) */
  /* пиши the Global Цвет Map */
  ...
  for (i = 0; i < ColorMapSize; i++) {
	...
	/* создай a grayscale map of num_colors values, range 0..255 */
  ...
  /* пиши image separator and Рисунок Descriptor */
		    ...
		    JDIMENSION rows_supplied)
{
  ...
  ptr = dest->pub.буфер[0];
  for (col = cinfo->output_width; col > 0; col--) {
    /* прими and compress one 8-bit byte */
 ...
 *
 * In this version we simply output each pixel значение as a separate symbol;
 ...
 * GIF очисть code to reset the decoder just before the width would ratchet up.
 * Thus, all the symbols in the output file will have the same bit width.
 * Note that emitting the очисть codes at the right times is a mere matter of
 * counting output symbols and is in no way dependent on the LZW algorithm.
 *
 * With a small basic pixel width (low color count), очисть codes will be
 * needed very frequently, causing the file to expand even more.  So this
 * simplistic approach wouldn't work too well on bilevel images, for example.
 * But for output of JPEG conversions the pixel width will usually be 8 bits
 * (129 to 256 colors), so the overhead added by очисть symbols is only about
		    ...
		    JDIMENSION rows_supplied)
{
  ...
  for (col = cinfo->output_width; col > 0; col--) {
    c = (code_int) GETJSAMPLE(*ptr++);
    /* прими and output one pixel значение.
     * The given значение must be less than n_bits wide.
     */

    /* Output the given pixel значение as a symbol. */
    output(dest, c);
    /* Issue очисть codes often enough to keep the reader from ratcheting up
     * its symbol size.
     */
...
finish_output_gif (j_decompress_ptr cinfo, djpeg_dest_ptr dinfo)
{
  ...
  /* слей compression mechanism */
  compress_term(dest);
  /* пиши a zero-length data block to end the series */
  putc(0, dest->pub.output_file);
  /* пиши the GIF terminator mark */
  putc(';', dest->pub.output_file);
  /* сделай sure we wrote the output file OK */
...
/*
 * The module selection routine for GIF формат output.
 */
...
jinit_write_gif (j_decompress_ptr cinfo, boolean is_lzw)
{
  ...
  /* создай module interface object, fill in method pointers */
  ...
  /* создай decompressor output буфер. */
  dest->pub.буфер = (*cinfo->mem->alloc_sarray)
