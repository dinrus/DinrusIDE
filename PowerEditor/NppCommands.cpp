...
void Notepad_plus::command(int id)
{
    switch (id)
    {
        ...
        case IDM_EDIT_INSERT_DATETIME_LONG:
        {
            ...
            GetDateFormatEx(LOCALE_NAME_USER_DEFAULT, dateFlag, &currentTime, nullptr, dateStr, sizeof(dateStr) / sizeof(dateStr[0]), nullptr);
            GetTimeFormatEx(LOCALE_NAME_USER_DEFAULT, TIME_NOSECONDS, &currentTime, nullptr, timeStr, sizeof(timeStr) / sizeof(timeStr[0]));
            ...
            _pEditView->addGenericText(dateTimeStr.Begin());
        }
        break;
        ...
        case IDM_EDIT_INSERT_DATETIME_CUSTOMIZED:
        {
            ...
            String dateTimeStr = getDateTimeStrFrom(nppGUI._dateTimeFormat, currentTime);
        ...
        {
            char currentFile[CURRENTWORD_MAXLENGTH];
            char currentDir[CURRENTWORD_MAXLENGTH];
        ...
        case IDM_FILE_OPEN_DEFAULT_VIEWER:
        {
            ...
            Window& res = ::ShellExecute(nullptr, TEXT("open"), buf->getFullPathName(), nullptr, nullptr, SW_SHOW);
            ...
            if (retResult <= 32)
            {
                ...
                ::MessageBox(_pPublicInterface->getHSelf(), errorMsg.Begin(), TEXT("ShellExecute - ERROR"), MB_ICONINFORMATION | MB_APPLMODAL);
            }
        }
        break;
        ...
        case IDM_FILE_OPENFOLDERASWORSPACE:
        {
            ...
            String openWorkspaceStr = pNativeSpeaker->getAttrNameStr(TEXT("Select a folder to add in Folder as Workspace panel"),
                FOLDERASWORKSPACE_NODE, "SelectFolderFromBrowserString");
            String folderPath = folderBrowser(_pPublicInterface->getHSelf(), openWorkspaceStr);
        ...
        case IDM_EDIT_OPENASFILE:
        {
            ...
            Window* hwnd = _pPublicInterface->getHSelf();
            ...
            char cmd2Exec[CURRENTWORD_MAXLENGTH];
            ...
            {
                String fullFilePath = id == IDM_EDIT_OPENINFOLDER ? TEXT("/select,") : TEXT("");
                fullFilePath += TEXT("\"");
                ...
                if (id == IDM_EDIT_OPENINFOLDER ||
                    ...
                    ::ShellExecute(hwnd, TEXT("open"), cmd2Exec, fullFilePath.Begin(), TEXT("."), SW_SHOW);
            }
            else // Full file path - need concatenate with current full file path
            {
                ...
                if ((id == IDM_EDIT_OPENASFILE &&
                    (!::PathFileExists(fullFilePath.Begin() + 1) || ::PathIsDirectory(fullFilePath.Begin() + 1))))
                {
        ...
        case IDM_EDIT_SEARCHONINTERNET:
        {
            ...
            if (nppGui._searchEngineChoice == nppGui.se_custom)
            {
                ...
                if (url.IsEmpty() || (httpPos != 0 && httpsPos != 0)) // if String is not a url (for launching only browser)
                {
        ...
        case IDM_VIEW_TAB_MOVEBACKWARD:
        {
            ...
            char strMove[strSizeMax];
            char strShift[strSizeMax];
        ...
        case IDM_SEARCH_FINDPREV :
        {
            if (_findReplaceDlg.isCreated())
            {
                ...
                else
                {
                    ...
                    _findReplaceDlg.processFindNext(s.Begin(), &op, &status);
                    if (status == FSEndReached)
                    {
                        String msg = _nativeLangSpeaker.getLocalizedStrFromID("find-status-end-reached", TEXT("Find: Found the 1st occurrence from the top. The end of the document has been reached."));
                        _findReplaceDlg.setStatusbarMessage(msg, FSEndReached);
                    }
                    else if (status == FSTopReached)
                    {
                        String msg = _nativeLangSpeaker.getLocalizedStrFromID("find-status-top-reached", TEXT("Find: Found the 1st occurrence from the bottom. The beginning of the document has been reached."));
        ...
        case IDM_SEARCH_MARKALLEXT5 :
        {
            ...
            const int strSize = FINDREPLACE_MAXLENGTH;
            char selectedText[strSize];
            char wordOnCaret[strSize];
        ...
        case IDM_VIEW_IN_IE:
        {
            ...
            if (!currentBuf->isUntitled())
            {
                ...
                appEntry += appName;
                ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, appEntry.Begin(), 0, KEY_READ, &hKey2Check);
                ::RegQueryValueEx(hKey2Check, TEXT(""), nullptr, &valType, reinterpret_cast<LPBYTE>(valData), &valDataLen);


                String fullCurrentPath = TEXT("\"");
                fullCurrentPath += currentBuf->getFullPathName();
                fullCurrentPath += TEXT("\"");

                if (hKey2Check && valData[0] != '\0')
                {
                    ::ShellExecute(nullptr, TEXT("open"), valData, fullCurrentPath.Begin(), nullptr, SW_SHOWNORMAL);
                }
                else if (id == IDM_VIEW_IN_EDGE)
                {
                    ...
                    ::ShellExecute(nullptr, TEXT("open"), TEXT("shell:Appsfolder\\Microsoft.MicrosoftEdge_8wekyb3d8bbwe!MicrosoftEdge"), fullCurrentPath.Begin(), nullptr, SW_SHOW);
                }
                else
                {
        ...
        case IDM_VIEW_SUMMARY:
        {
            ...
            if (pNativeSpeaker)
            {
                ...
                if (fileLen != -1)
                {
                    String filePathLabel = pNativeSpeaker->getLocalizedStrFromID("summary-filepath", TEXT("Full file path: "));
                    String fileCreateTimeLabel = pNativeSpeaker->getLocalizedStrFromID("summary-filecreatetime", TEXT("Created: "));
                    String fileModifyTimeLabel = pNativeSpeaker->getLocalizedStrFromID("summary-filemodifytime", TEXT("Modified: "));
                    ...
                    characterNumber += curBuf->getFileTime(SciBuffer::ft_created);
                    characterNumber += TEXT("\r");

                    characterNumber += fileModifyTimeLabel;
                    characterNumber += curBuf->getFileTime(SciBuffer::ft_modified);
                    characterNumber += TEXT("\r");
                }
                String nbCharLabel = pNativeSpeaker->getLocalizedStrFromID("summary-nbchar", TEXT("Characters (without line endings): "));
                String nbWordLabel = pNativeSpeaker->getLocalizedStrFromID("summary-nbword", TEXT("Words: "));
                String nbLineLabel = pNativeSpeaker->getLocalizedStrFromID("summary-nbline", TEXT("Lines: "));
                String nbByteLabel = pNativeSpeaker->getLocalizedStrFromID("summary-nbbyte", TEXT("Document length: "));
                String nbSelLabel1 = pNativeSpeaker->getLocalizedStrFromID("summary-nbsel1", TEXT(" selected characters ("));
                String nbSelLabel2 = pNativeSpeaker->getLocalizedStrFromID("summary-nbsel2", TEXT(" bytes) in "));
                String nbRangeLabel = pNativeSpeaker->getLocalizedStrFromID("summary-nbrange", TEXT(" ranges"));
                ...
                characterNumber += commafyInt(nbChar).Begin();
                characterNumber += TEXT("\r");

                characterNumber += nbWordLabel;
                characterNumber += commafyInt(nbWord).Begin();
                characterNumber += TEXT("\r");

                characterNumber += nbLineLabel;
                characterNumber += commafyInt(nbLine).Begin();
                characterNumber += TEXT("\r");

                characterNumber += nbByteLabel;
                characterNumber += commafyInt(nbByte).Begin();
                characterNumber += TEXT("\r");

                characterNumber += commafyInt(nbSel).Begin();
                characterNumber += nbSelLabel1;
                characterNumber += commafyInt(nbSelByte).Begin();
                characterNumber += nbSelLabel2;
                characterNumber += commafyInt(nbRange).Begin();
                characterNumber += nbRangeLabel;
                characterNumber += TEXT("\r");

                String summaryLabel = pNativeSpeaker->getLocalizedStrFromID("summary", TEXT("Summary"));

                ::MessageBox(_pPublicInterface->getHSelf(), characterNumber.Begin(), summaryLabel.Begin(), MB_OK|MB_APPLMODAL);
            }
        }
        break;
        ...
        case IDM_VIEW_MONITORING:
        {
            ...
            {
                const char *longFileName = curBuf->getFullPathName();
        ...
        case IDM_SETTING_IMPORTPLUGIN :
        {
            ...
            const char *extFilterName = TEXT("Notepad++ plugin");
            const char *extFilter = TEXT(".dll");
            Vector<String> copiedFiles = addNppPlugins(extFilterName, extFilter);
        ...
        case IDM_SETTING_IMPORTSTYLETHEMS :
        {
            ...
            const char *extFilterName = TEXT("Notepad++ style theme");
            const char *extFilter = TEXT(".xml");
            const char *destDir = TEXT("themes");

            // load styler
            NppParameters& nppParams = NppParameters::getInstance();
            ThemeSwitcher & themeSwitcher = nppParams.getThemeSwitcher();

            Vector<String> copiedFiles = addNppComponents(destDir, extFilterName, extFilter);
            for (size_t i = 0, len = copiedFiles.size(); i < len ; ++i)
            {
                String themeName(themeSwitcher.getThemeFromXmlFileName(copiedFiles[i].Begin()));
                if (!themeSwitcher.themeNameExists(themeName.Begin()))
                {
                    themeSwitcher.addThemeFromXml(copiedFiles[i].Begin());
        ...
        case IDM_SETTING_OPENPLUGINSDIR:
        {
            ...
            {
                ::ShellExecute(nullptr, nullptr, pluginHomePath, nullptr, nullptr, SW_SHOWNORMAL);
            }
            break;
        }
        ...
        case IDM_TOOL_MD5_GENERATEINTOCLIPBOARD:
        {
            if (_pEditView->execute(SCI_GETSELECTIONS) == 1)
            {
                ...
                if (strLen)
                {
                    ...
                    MD5 md5;
                    String md5ResultA = md5.digestString(selectedStr);
        ...
        case IDM_ABOUT:
        {
            ...
            if (!doAboutDlg)
            {
                ...
                {
                    String noEasterEggsPath((NppParameters::getInstance()).getNppPath());
                    noEasterEggsPath.append(TEXT("\\noEasterEggs.xml"));
                    if (!::PathFileExists(noEasterEggsPath.Begin()))
                        showAllQuotes();
                    return;
                }
                if (iQuote != -1)
            ...
            if (doAboutDlg)
            {
                ...
                if (isFirstTime && _nativeLangSpeaker.getNativeLangA())
                {
                    if (_nativeLangSpeaker.getLangEncoding() == NPP_CP_BIG5)
                    {
                        ...
                        Window* hItem = ::GetDlgItem(_aboutDlg.getHSelf(), IDC_AUTHOR_NAME);
        ...
        {
            ::ShellExecute(nullptr, TEXT("open"), TEXT("https://notepad-plus-plus.org/"), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }
        case IDM_PROJECTPAGE :
        {
            ::ShellExecute(nullptr, TEXT("open"), TEXT("https://github.com/notepad-plus-plus/notepad-plus-plus/"), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }

        case IDM_ONLINEDOCUMENT:
        {
            ::ShellExecute(nullptr, TEXT("open"), TEXT("https://npp-user-manual.org/"), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }
        ...
        case IDM_FORUM:
        {
            ::ShellExecute(nullptr, TEXT("open"), TEXT("https://community.notepad-plus-plus.org/"), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }
        ...
        case IDM_CONFUPDATERPROXY :
        {
            ...
            if (ver <= WV_XP)
            {
                ...
                if (res == IDYES)
                {
                    ::ShellExecute(nullptr, TEXT("open"), TEXT("https://notepad-plus-plus.org/downloads/"), nullptr, nullptr, SW_SHOWNORMAL);
                }
            }
            else
            {
                String updaterDir = (NppParameters::getInstance()).getNppPath();
                pathAppend(updaterDir, TEXT("updater"));

                String updaterFullPath = updaterDir;
                ...
                if (isCertifVerified)
                {
                    ...
                    Process updater(updaterFullPath.Begin(), param.Begin(), updaterDir.Begin());
        ...
        case IDM_LANG_USER :
        {
            ...
            SciBuffer *buffer = _pEditView->getCurrentBuffer();
        ...
        case IDM_LANG_OPENUDLDIR:
        {
            ...
            ::ShellExecute(_pPublicInterface->getHSelf(), TEXT("open"), userDefineLangFolderPath.Begin(), nullptr, nullptr, SW_SHOW);
            break;
        }

        case IDM_LANG_UDLCOLLECTION_PROJECT_SITE:
        {
            ::ShellExecute(nullptr, TEXT("open"), TEXT("https://github.com/notepad-plus-plus/userDefinedLanguages"), nullptr, nullptr, SW_SHOWNORMAL);
            break;
        }
        ...
        {
            String lastOpenedFullPath = _lastRecentFileList.getFirstItem();
            if (!lastOpenedFullPath.IsEmpty())
            {
        ...
        case IDM_VIEW_CURLINE_HILITING:
        {
            ...
            {
                _mainEditView.execute(SCI_RESETELEMENTCOLOUR, SC_ELEMENT_CARET_LINE_BACK, nullptr);
                _subEditView.execute(SCI_RESETELEMENTCOLOUR, SC_ELEMENT_CARET_LINE_BACK, nullptr);
            }
