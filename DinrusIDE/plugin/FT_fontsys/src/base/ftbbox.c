...
#define CHECK_Y( p, bbox )  \
 ...
 /* FT_Pos    a = y4 - 3*y3 + 3*y2 - y1; */
                    ...
                    FT_Pos*  max )
  {
    ...
    /* There are some split points.  найди them. */
    {
      FT_Pos    a = y4 - 3*y3 + 3*y2 - y1;
      FT_Pos    b = y3 - 2*y2 + y1;
      FT_Pos    c = y2 - y1;
      ...
      /* These values must fit into a single 16.16 значение.                  */
      /*                                                                   */
      /* We normalize a, b, and c to `8.16' fixed float values to ensure   */
      /* that its product is held in a `16.16' значение.                      */

      {
        ...
        /* any значение `y', if `n' is the position of the most         */
        ...
        /* corresponds to a значение of `t1' that is in the range       */
        /* 0x40_0000..0x7F_FFFF.                                     */
        /*                                                           */
...
FT_DEFINE_OUTLINE_FUNCS(bbox_interface,
                       ...
                       FT_BBox     *abbox )
  {
         ...
         cbox.yMin < bbox.yMin || cbox.yMax > bbox.yMax )
    {
      ...
      Ошибка = FT_Outline_Decompose( outline, &bbox_interface, &user );
      if ( Ошибка )
        return Ошибка;
