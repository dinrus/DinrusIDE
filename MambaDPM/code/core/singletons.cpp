...
namespace mamba
{
    ...
    public:
        ...
        CURLSetup()
        {
            ...
            {
                LOG_ERROR << "cURL SSL init вызыван слишком поздно, это баг.";
            }
            else if (sslset_res == CURLSSLSET_UNKNOWN_BACKEND || sslset_res == CURLSSLSET_NO_BACKENDS)
            {
                LOG_WARNING << "Не удаётся использовать предпочитаемый бэкэнд SSL (Linux: OpenSSL, OS X: SecureTransport, Win: SChannel)"
                            << std::endl;
                LOG_WARNING << "Пожалуйста, проверьте конфигурацию используемой библиотеки cURL."
                            << std::endl;
            }
#endif
    ...
    MainExecutor::MainExecutor()
    {
        ...
        if (!main_executor.compare_exchange_strong(expected, this))
        {
            throw MainExecutorError(
                "была попытка создать несколько главных исполнителей",
    ...
    Console& Console::instance()
    {
        ...
        {
            throw mamba_error(
                "попытка доступа к консоли, которая ещё не создана",
                mamba_error_code::incorrect_usage
            );
        }
    ...
    void Console::set_singleton(Console& console)
    {
        ...
        {
            throw mamba_error("была попытка создать несколько консолей", mamba_error_code::incorrect_usage);
        }
    }
