 ...
 *    source буфер it is an undocumented property of jdcoefct.c.
 */


/* сбрось code may be used with or without virtual memory adaptation code.
...
drop_request_from_src (j_decompress_ptr dropinfo, j_decompress_ptr srcinfo)
{
  ...
  /* выполни only initialization part.
	      ...
	      jvirt_barray_ptr coef_array, JQUANT_TBL *qtblptr1)
{
  ...
  for (blk_y = 0; blk_y < compptr->height_in_blocks;
    ...
    буфер = (*cinfo->mem->access_virt_barray)
      ((j_common_ptr) cinfo, coef_array, blk_y,
       (JDIMENSION) compptr->v_samp_factor, TRUE);
    for (offset_y = 0; offset_y < compptr->v_samp_factor; offset_y++) {
      block = буфер[offset_y];
...
/* сбрось.  If the dropinfo component number is smaller than the destination's,
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
    ...
    for (; y_wipe_blocks < wipe_bottom;
	 y_wipe_blocks += compptr->v_samp_factor) {
      буфер = (*srcinfo->mem->access_virt_barray)
	((j_common_ptr) srcinfo, src_coef_arrays[ci], y_wipe_blocks,
	 (JDIMENSION) compptr->v_samp_factor, TRUE);
      for (offset_y = 0; offset_y < compptr->v_samp_factor; offset_y++) {
	FMEMZERO(буфер[offset_y] + x_wipe_blocks,
 ...
 */
{
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
	...
	if (x_wipe_blocks > 0) {
	  /* Reflect from left */
	  dst_row_ptr = буфер[offset_y] + x_wipe_blocks;
	  ...
	  /* Reflect from right */
	  dst_row_ptr = буфер[offset_y] + x_wipe_blocks + wipe_width;
	  for (dst_blk_x = wipe_width; dst_blk_x > 0;) {
	    src_row_ptr = dst_row_ptr;	   /* (re)set axis of reflection */
 ...
 */
{
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
	...
	for (blk_x = 0; blk_x * 2 < comp_width; blk_x++) {
	  ptr1 = буфер[offset_y][blk_x];
	  ptr2 = буфер[offset_y][comp_width - blk_x - 1];
	  /* this unrolled loop doesn't need to know which row it's on... */
	  for (k = 0; k < DCTSIZE2; k += 2) {
	...
	if (x_crop_blocks > 0) {
	   ...
	   */
	  for (blk_x = 0; blk_x < compptr->width_in_blocks; blk_x++) {
	    jcopy_block_row(буфер[offset_y] + blk_x + x_crop_blocks,
			    буфер[offset_y] + blk_x,
			    (JDIMENSION) 1);
	  }
	}
      }
...
/* Vertical flip */
{
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
	...
	/* низ-edge blocks will be copied verbatim. */
	src_buffer = (*srcinfo->mem->access_virt_barray)
	  ((j_common_ptr) srcinfo, src_coef_arrays[ci],
	   dst_blk_y + y_crop_blocks,
	   (JDIMENSION) compptr->v_samp_factor, FALSE);
 ...
 */
{
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
	...
	/* низ-edge rows are only mirrored horizontally. */
	src_buffer = (*srcinfo->mem->access_virt_barray)
	  ((j_common_ptr) srcinfo, src_coef_arrays[ci],
	   dst_blk_y + y_crop_blocks,
	   (JDIMENSION) compptr->v_samp_factor, FALSE);
  ...
  for (ci = 0; ci < dstinfo->num_components; ci++) {
	...
	for (dst_blk_x = 0; dst_blk_x < compptr->width_in_blocks;
	  ...
	  for (offset_x = 0; offset_x < compptr->h_samp_factor; offset_x++) {
		...
		/* право-edge blocks are mirrored in y only */
		src_ptr = src_buffer[offset_x]
		  [comp_height - y_crop_blocks - dst_blk_y - offset_y - 1];
		for (i = 0; i < DCTSIZE; i++) {
		  for (j = 0; j < DCTSIZE; j++) {
	      ...
	      if (x_crop_blocks + dst_blk_x < comp_width) {
		/* низ-edge blocks are mirrored in x only */
		src_ptr = src_buffer[compptr->h_samp_factor - offset_x - 1]
		  [dst_blk_y + offset_y + y_crop_blocks];
		for (i = 0; i < DCTSIZE; i++) {
		  for (j = 0; j < DCTSIZE; j++)
		...
		/* по lower right corner, just transpose, no mirroring */
		src_ptr = src_buffer[offset_x]
		  [dst_blk_y + offset_y + y_crop_blocks];
		for (i = 0; i < DCTSIZE; i++)
		  for (j = 0; j < DCTSIZE; j++)
 ...
 * *strptr is advanced over the digit string, and *result is set to its значение.
 */
 ...
 *
 * The crop spec string should have the формат
 *	<width>[{fr}]x<height>[{fr}]{+-}<xoffset>{+-}<yoffset>
 * where width, height, xoffset, and yoffset are unsigned integers.
 * Each of the elements can be omitted to indicate a default значение.
...
/* обрежь off any partial iMCUs on the indicated destination edge */
			      ...
			      jpeg_transform_info *info)
{
   ...
   */
  if (info->crop) {
    /* вставь default values for unset crop parameters */
    ...
    /* Преобр negative crop offsets into regular offsets */
    ...
    /* сохрани x/y offsets measured in iMCUs */
			...
			JDIMENSION new_width, JDIMENSION new_height)
{
  ...
  if (length < 12) return; /* длина of an IFD entry */
  ...
  /* дай first IFD offset (offset to IFD0) */
  if (is_motorola) {
  ...
  /* дай the number of directory entries contained in this IFD */
  if (is_motorola) {
  ...
  /* Search for ExifSubIFD offset Тэг in IFD0 */
  for (;;) {
    ...
    if (tagnum == 0x8769) break; /* found ExifSubIFD offset Тэг */
    if (--number_of_tags == 0) return;
    firstoffset += 12;
  }

  /* дай the ExifSubIFD offset */
  if (is_motorola) {
  ...
  /* дай the number of directory entries contained in this SubIFD */
  if (is_motorola) {
  ...
  do {
    ...
    if (tagnum == 0xA002 || tagnum == 0xA003) {
      if (tagnum == 0xA002)
	new_value = new_width; /* ExifImageWidth Тэг */
      else
	new_value = new_height; /* ExifImageHeight Тэг */
      if (is_motorola) {
	data[offset+2] = 0; /* фмт = unsigned long (4 octets) */
	data[offset+3] = 4;
	...
	data[offset+2] = 4; /* фмт = unsigned long (4 octets) */
	data[offset+3] = 0;
 ...
 *
 * The return значение is the set of virtual coefficient arrays to be written
 * (either the ones allocated by jtransform_request_workspace, or the
 * original source data arrays).  The caller will need to pass this значение
			      ...
			      jpeg_transform_info *info)
{
  ...
  if (info->force_grayscale) {
	...
	srcinfo->comp_info[0].v_samp_factor == srcinfo->max_v_samp_factor) {
      /* We use jpeg_set_colorspace to make sure subsidiary настройки get fixed
...
/* выполни the actual transformation, if any.
 *
...
/* настрой decompression object to save desired markers in memory.
...
jcopy_markers_setup (j_decompress_ptr srcinfo, JCOPY_OPTION option)
{
  ...
  /* сохрани comments except under NONE option */
  if (option != JCOPYOPT_NONE) {
    jpeg_save_markers(srcinfo, JPEG_COM, 0xFFFF);
  }
  /* сохрани all types of APPn markers iff ALL option */
