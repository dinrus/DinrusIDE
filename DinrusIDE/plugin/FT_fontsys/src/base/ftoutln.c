...
#define SCALED( x )  ( ( (x) << shift ) - delta )
                     ...
                     FT_Raster_Params*  params )
  {
    ...
    Ошибка = FT_Err_Cannot_Render_Glyph;
    while ( renderer )
    {
      Ошибка = renderer->raster_render( renderer->raster, params );
      if ( !Ошибка || FT_ERROR_BASE( Ошибка ) != FT_Err_Cannot_Render_Glyph )
        break;
...
#define FT_OUTLINE_GET_CONTOUR( outline, c, first, last )  \
  ...
  /* является a point in some contour?                     */
  /*                                                 */
