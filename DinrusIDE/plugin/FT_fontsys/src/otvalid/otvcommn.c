...
#define FT_COMPONENT  trace_otvcommon
                         ...
                         FT_Int         expected_count )
  {
    ...
    case 2:     /* CoverageFormat2 */
      {
        ...
        FT_UInt  старт, стоп, StartCoverageIndex, last = 0;
        ...
        {
          старт              = FT_NEXT_USHORT( p );
          стоп                = FT_NEXT_USHORT( p );
          StartCoverageIndex = FT_NEXT_USHORT( p );

          if ( старт > стоп || StartCoverageIndex != total )
            FT_INVALID_DATA;

          if ( стоп >= valid->glyph_count )
            FT_INVALID_GLYPH_ID;
    ...
    /* there is no associated count, a значение of -1 tells us not to check. */
  ...
  */
  ...
  /* uses valid->extra1 (if > 0: array значение limit) */
  ...
  /* `ux' in the function's имя is not really correct since only x-1 */
  /* elements are tested                                              */

  /* uses valid->extra1 (array значение limit) */
  ...
  /* `uy' in the function's имя is not really correct since only y-1 */
  /* elements are tested                                              */
