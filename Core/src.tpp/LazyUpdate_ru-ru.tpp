...
[s9;%RU-RU LazyUpdate class provides thread`-safe means for lazy 
evaluation. Typically it is used in connection with mutable member 
variable cache. Mutating operations on such class set LazyUpdate 
to `"invalid`" state. When any method is called to obtain evaluated 
value, it first checks LazyUpdate whether cache is valid using 
BeginUpdate.(such method is usually const, that is why the cache 
needs to be mutable). If BeginUpdate returns true, cache has 
to be updated. At the end of update, EndUpdate has to be called.&]
[s9;%RU-RU BeginUpdate also blocks any other thread once update is 
in progress.&]
[s9;%RU-RU That way, many threads are allowed to invoke read methods 
...
[s9;%RU-RU When the cache is in updated state, BeginUpdate is wait`-free.&]
[s3; &]
[s0; &]
...
nst]&]
[s2;%RU-RU Queries whether LazyUpdate is in invalid state. In that 
case, true is returned and any other thread calling BeginUpdate 
is blocked until EndUpdate (and such blocked thread then returns 
