...
+ By default ComplimentaryMultИПlyWithCarry with r=1024, a=987769338, b=2^32-1, период a*b^r>10^9873
     ...
     struct CMWC(бцел cmwc_r=1024U,бдол cmwc_a=987769338UL)
    {
        ...
        ткст вТкст()
        {
            ...
            рез[i..i+4]="CMWC";
            i+=4;
            Целое.фмт(рез[i..i+16],cmwc_a,"x16");
            i+=16;
            рез[i]='_';
            ++i;
            Целое.фмт(рез[i..i+8],cmwc_r,"x8");
            i+=8;
            foreach (знач; cmwc_q)
            {
                рез[i]='_';
                ++i;
                Целое.фмт(рез[i..i+8],знач,"x8");
                i+=8;
            }
        ...
        т_мера изТкст(ткст s)
        {
            ...
            i+=4;
            assert(s[i..i+16]==Целое.фмт(tmpC,cmwc_a,"x16"),"неожиданный cmwc_a");
            i+=16;
            assert(s[i]=='_',"неожиданный формат, ожидался _");
            i++;
            assert(s[i..i+8]==Целое.фмт(tmpC,cmwc_r,"x8"),"неожиданный cmwc_r");
            i+=8;
