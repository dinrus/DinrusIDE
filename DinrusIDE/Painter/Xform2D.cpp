...
ТочкаПЗ Xform2D::GetScaleXY() const
{
	return ТочкаПЗ(sqrt(x.x * x.x + y.x * y.x), sqrt(x.y * x.y + y.y * y.y));
}

double Xform2D::GetScale() const
{
	ТочкаПЗ d = GetScaleXY();
	return длина(d) / M_SQRT2;
}
...
Xform2D Xform2D::вКарту(ТочкаПЗ s1, ТочкаПЗ s2, ТочкаПЗ s3)
{
	Xform2D s;
	s.x.x = s1.x - s3.x;
	s.y.x = s1.y - s3.y;
...
Xform2D Xform2D::вКарту(ТочкаПЗ s1, ТочкаПЗ s2, ТочкаПЗ s3, ТочкаПЗ t1, ТочкаПЗ t2, ТочкаПЗ t3)
{
	return Inverse(Xform2D::вКарту(s1, s2, s3)) * вКарту(t1, t2, t3);
}
...
Xform2D Inverse(const Xform2D& m)
{
	...
	r.x = ТочкаПЗ(m.y.y, -m.x.y) / det;
	r.y = ТочкаПЗ(-m.y.x, m.x.x) / det;
