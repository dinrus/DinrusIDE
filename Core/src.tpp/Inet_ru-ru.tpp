...
[%-*@3 content`_type] устанавливает соотстветствующий
 заголовочник http (если NULL, он не отправляется
). Если [%-*@3 data] не пуст, отправляется 
заголовочник `"content`-length`", а после него 
отправляются даные [%-*@3 data]. [%-*@3 server] 
устанавливает поле `"server`" заголовочника 
http, если он отстутствует, то устанавливается
 как `"U`+`+ based server`". Если [%-*@3 gzip] равно 
true, функция добавляет заголовочник 
