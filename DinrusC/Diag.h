...
#define NEVER()          ПРОВЕРЬ(0)
#define NEVER_(msg)      ПРОВЕРЬ_(0, msg)
#define XNEVER(d)        if(1) { LOG("NEVER failed"); LOGSRCPOS(); LOG(d); ПРОВЕРЬ(0); } else
...
#define NEVER()          ЛОГ_НОП
#define NEVER_(msg)      ЛОГ_НОП
#define XNEVER(d)        ЛОГ_НОП
