...
startElement(struct solv_xmlparser *xmlp, int state, const char *name, const char **atts)
{
  ...
  switch (state)
    ...
    case STATE_REQUIRESENTRY:
      s->requires_ = adddep(pool, pd, s->requires_, atts, -SOLVABLE_PREREQMARKER);
      break;
...
findKernelFlavor(struct parsedata *pd, Solvable *s)
{
  ...
  if (s->requires_)
    {
      pidp = pd->repo->idarraydata + s->requires_;
      while ((pid = *pidp++) != 0)
	{
	  const char *depname;
