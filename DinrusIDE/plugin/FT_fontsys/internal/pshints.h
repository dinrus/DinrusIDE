...
FT_BEGIN_HEADER
   ...
   *
   *   - открой a new hint recording session by calling the `open' method.
   *     This rewinds the recorder and prepare it for new input.
   *
   *   - For each hint found in the glyph charstring, call the corresponding
   *     method (`stem', `stem3', or `reset').  Note that these functions do
   *     not return an Ошибка code.
   *
   *   - закрой the recording session by calling the `close' method.  It
   *     returns an Ошибка code if the hints were invalid or something
   ...
   *
   *   coords ::
   *     Массив of 2 coordinates in 16.16 формат, used as (position,length)
   *     stem descriptor.
   *
   * @note:
   ...
   *
   *   coords ::
   *     An array of 6 values in 16.16 формат, holding 3 (position,length)
   ...
   *
   *   end_point ::
   *     The Индекс of the last point in the input glyph in which the
   ...
   *
   *   end_point ::
   *     The Индекс of the last point in the input glyph.
   *
   * @return:
   *   FreeType Ошибка code.  0 means success.
   *
   * @note:
   *   The Ошибка code is set to indicate that an Ошибка occurred during the
   *   recording session.
   *
   */
  typedef FT_Error
   ...
   *     functions do not return an Ошибка code.
   *
   *   - закрой the recording session by calling the `close' method.  It
   *     returns an Ошибка code if the hints were invalid or something
   *     strange happened (e.g., memory shortage).
   ...
   *
   *   coords ::
   *     An array of `count' (position,length) pairs in 16.16 формат.
   *
   * @note:
   *   Use vertical coordinates (y) for horizontal stems (dim=0).  Use
   *   horizontal coordinates (x) for vertical stems (dim=1).
   ...
   *
   *   end_point ::
   *     The glyph Индекс of the last point to which the previously defined
   ...
   *   definition), the значение of `end_point' should be 0.
   *
   ...
   *
   *   end_point ::
   *     A glyph Индекс of the last point to which the previously defined or
   *     active hints apply.
   *
   *   bit_count ::
   *     The number of bits in the hint mask.
