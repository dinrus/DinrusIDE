...
/*  ttkern.c                                                               */
/*                                                                         */
/*    грузи the basic TrueType kerning table.  This doesn't handle          */
...
#define TT_KERN_INDEX( g1, g2 )  ( ( (FT_ULong)(g1) << 16 ) | (g2) )
                     ...
                     FT_Stream  stream )
  {
    ...
    Ошибка = face->goto_table( face, TTAG_kern, stream, &table_size );
    if ( Ошибка )
      goto Exit;
      ...
      case 0:
        {
          ...
          if ( face->kern_order_bits & mask )   /* binary search */
          {
            ...
            while ( min < max )
            {
              ...
              ключ = FT_NEXT_ULONG( q );

              if ( ключ == key0 )
              {
                значение = FT_PEEK_SHORT( q );
                goto Found;
              }
              if ( ключ < key0 )
                min = mid + 1;
       ...
       /*
        *  We don't support формат 2 because we haven't seen a single font
