...
void Pdb::MemMenu(ArrayCtrl& array, Bar& bar, const String& exp)
{
	...
	try {
		...
		if(v.rvalue) {
			sep = false;
			bar.Add("Память по 0x" + FormatIntHex((dword)GetInt64(v)), THISBACK1(MemoryGoto, "&" + exp));
		}
		if(!v.rvalue) {
			sep = false;
			bar.Add("Память по &&" + exp, THISBACK1(MemoryGoto, "&" + exp));
...
{
	String s;
	if(EditPDBExpression("Добавить вотч", s, this))
		AddWatch(s);
}
...
void Pdb::EditWatch()
{
	...
	if(EditPDBExpression("Редактировать вотч", s, this)) {
		SetTab(2);
		watches.Set(0, s);
		Data();
	}
