...
#define FT_COMPONENT  trace_ttload
  ...
  /*                                                                       */
  /* <Description>                                                         */
  /*    Looks for a TrueType table by имя.                                */
  /*                                                                       */
  /* <Input>                                                               */
  ...
  /*                                                                       */
  /* <Description>                                                         */
  /*    Looks for a TrueType table by имя, then seek a stream to it.      */
  /*                                                                       */
  /* <Input>                                                               */
  /*    face   :: A face object handle.                                    */
  /*                                                                       */
                      ...
                      FT_ULong*  length )
  {
    ...
    if ( table )
    {
      ...
      if ( FT_STREAM_SEEK( table->смещение ) )
        goto Exit;
    }
    else
      Ошибка = SFNT_Err_Table_Missing;
...
#define FT_STRUCTURE  TT_TableRec
      ...
      FT_FRAME_START( 16 ),
        FT_FRAME_ULONG( Тэг ),
        FT_FRAME_ULONG( Checkсумма ),
        FT_FRAME_ULONG( смещение ),
        FT_FRAME_ULONG( длина ),
      FT_FRAME_END
    };
    ...
    for ( nn = 0; nn < sfnt->num_tables; nn++ )
    {
      ...
      if ( table.смещение + table.длина > stream->size )
      {
      ...
      if ( table.Тэг == TTAG_head || table.Тэг == TTAG_bhed )
      {
        FT_UInt32  magic;


#ifndef TT_CONFIG_OPTION_EMBEDDED_BITMAPS
        if ( table.Тэг == TTAG_head )
#endif
          has_head = 1;
         ...
         * padded to 32-bit lengths, but this doesn't apply to the значение of
         * its `длина' field!
         *
         */
        if ( table.длина < 0x36 )
        ...
        if ( FT_STREAM_SEEK( table.смещение + 12 ) ||
             FT_READ_ULONG( magic )              )
          goto Exit;
      ...
      else if ( table.Тэг == TTAG_SING )
        has_sing = 1;
      else if ( table.Тэг == TTAG_META )
        has_meta = 1;
    }
    ...
    if ( sfnt->num_tables == 0 )
    {
      ...
      Ошибка = SFNT_Err_Unknown_File_Format;
      goto Exit;
    }
...
#define FT_STRUCTURE  SFNT_HeaderRec
    ...
    FT_TRACE2(( "-- фмт version:   0x%08lx\n", sfnt.format_tag ));

    /* check first */
    Ошибка = check_table_dir( &sfnt, stream );
    if ( Ошибка )
    {
    ...
    for ( nn = 0; nn < sfnt.num_tables; nn++ )
    {
      entry->Тэг      = FT_GET_TAG4();
      entry->Checkсумма = FT_GET_ULONG();
      entry->смещение   = FT_GET_LONG();
      entry->длина   = FT_GET_LONG();

      /* ignore invalid tables */
      if ( entry->смещение + entry->длина > stream->size )
        continue;
      else
      {
        FT_TRACE2(( "  %c%c%c%c  %08lx  %08lx  %08lx\n",
                    (FT_Char)( entry->Тэг >> 24 ),
                    (FT_Char)( entry->Тэг >> 16 ),
                    (FT_Char)( entry->Тэг >> 8  ),
                    (FT_Char)( entry->Тэг       ),
                    entry->смещение,
                    entry->длина,
                    ...
                    FT_ULong*  length )
  {
    ...
    if ( FT_STREAM_READ_AT( offset, буфер, size ) )
      goto Exit;
...
#define FT_STRUCTURE  TT_Header
      ...
      FT_FRAME_START( 54 ),
        ...
        FT_FRAME_LONG  ( Checkсумма_Adjust ),
    ...
    Ошибка = face->goto_table( face, tag, stream, 0 );
    if ( Ошибка )
      goto Exit;
...
#define FT_STRUCTURE  TT_MaxProfile
    ...
    Ошибка = face->goto_table( face, TTAG_maxp, stream, 0 );
    if ( Ошибка )
      goto Exit;
    ...
    if ( maxProfile->version >= 0x10000L )
    {
      ...
      /*   the maxFunctionDefs значение is smaller.              */
...
#define FT_STRUCTURE  TT_NameEntryRec
    ...
    Ошибка = face->goto_table( face, TTAG_name, stream, &table_len );
    if ( Ошибка )
      goto Exit;
    ...
    /* Some popular Asian fonts have an invalid `storageOffset' значение   */
    /* (it should be at least "6 + 12*num_names").  However, the string */
    /* offsets, computed as "storageOffset + entry->stringOffset", are  */
    /* valid pointers within the имя table...                          */
    /*                                                                  */
    ...
    {
      FT_ERROR(( "tt_face_load_name: invalid `имя' table\n" ));
      Ошибка = SFNT_Err_Name_Table_Missing;
      goto Exit;
    }

    /* Allocate the array of имя records. */
    ...
    /* грузи the имя records and determine how much storage is needed */
    /* to hold the strings themselves.                                */
    {
      ...
      for ( ; count > 0; count-- )
      {
        ...
        /* check that the имя is not empty */
        if ( entry->stringLength == 0 )
          continue;

        /* check that the имя string is within the table */
                     ...
                     FT_Stream  stream )
  {
    ...
    Ошибка = face->goto_table( face, TTAG_cmap, stream, &face->cmap_size );
    if ( Ошибка )
      goto Exit;
...
#define FT_STRUCTURE  TT_OS2
    ...
    /* and test this значение each time we need to access the table. */
    Ошибка = face->goto_table( face, TTAG_OS2, stream, 0 );
    if ( Ошибка )
      goto Exit;
...
#define FT_STRUCTURE  TT_Postscript
    ...
    Ошибка = face->goto_table( face, TTAG_post, stream, 0 );
    if ( Ошибка )
      return Ошибка;
...
#define FT_STRUCTURE  TT_PCLT
    ...
    /* optional table */
    Ошибка = face->goto_table( face, TTAG_PCLT, stream, 0 );
    if ( Ошибка )
      goto Exit;
                     ...
                     FT_Stream  stream )
  {
    ...
    Ошибка = face->goto_table( face, TTAG_gasp, stream, 0 );
    if ( Ошибка )
      goto Exit;
