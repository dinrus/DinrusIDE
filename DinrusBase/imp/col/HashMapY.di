                     ...
                     : ИКонтейнер!(З)
{
        ...
        ***********************************************************************/
        ...
        final бул replaceKey (К ключ, К замени)
        {
                if (счёт)
                   {
                   auto в = Хэш (ключ, таблица.length);
                   auto hd = таблица[в];
                   auto trail = hd;
                   auto p = hd;
                   ...
                   while (p)
                         ...
                         if (ключ == p.ключ)
                            ...
                            в = Хэш (замени, таблица.length);
                            таблица[в] = p.установи (замени, p.значение, таблица[в]);
                            return да;
                            }
                         else
                            {
                ...
                foreach (бакет; таблица)
                         while (бакет)
                               ...
                               version (ХэшКэш)
                                     ...
                                     auto в = Хэш (бакет.ключ, новЁмк);
                               бакет.следщ = newtab[в];
                               newtab[в] = бакет;
                               бакет = n;
                               }
...
debug (ХэшКарта)
{
        ...
        проц main()
        {
                ...
                while (карта.возьми(знач))
                       Стдвыв.фмтнс ("taking {}, {} лево", знач, карта.размер);
                ...
                for (цел i=счёт; i--;)
                     тест.получи(i, знач);
                Стдвыв.фмтнс ("{} lookups: {}/s", тест.размер, тест.размер/ш.stop);
...
/+
                ...
                for (цел i=5_000_000; i--;)
                     aa.добавь (i, 0);
                Стдвыв.фмтнс ("{} тест iteration: {}/s", aa.размер, aa.размер/ш.stop);
+/
        }
}
