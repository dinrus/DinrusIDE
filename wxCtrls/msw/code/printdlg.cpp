...
#include <wxCtrls/msw/printdlg.h>
#include <wxCtrls/paper.h>
...
wxWindowsPrintNativeData::~wxWindowsPrintNativeData()
{
    HGLOBAL hDevMode = (HGLOBAL)(WXLPARAM) m_devMode;
    if ( hDevMode )
        GlobalFree(hDevMode);
    HGLOBAL hDevNames = (HGLOBAL)(WXLPARAM) m_devNames;
    if ( hDevNames )
...
bool wxWindowsPrintNativeData::TransferFrom( const wxPrintData &data )
{
    ...
    if (!hDevMode)
    {
        ...
        else
        {
            ...
            m_devMode = (void*)(WXLPARAM) hDevMode;
            pd.hDevMode = NULL;
    ...
    m_devNames = (void*) (WXLPARAM) wxCreateDevNames(wxEmptyString, data.GetPrinterName(), wxEmptyString);
...
bool wxWindowsPrintDialog::ConvertToNative( wxPrintDialogData &data )
{
    ...
    pd->hDevMode = (HGLOBAL)(WXLPARAM) native_data->GetDevMode();
    native_data->SetDevMode( (void*) NULL);

    // Shouldn't assert; we should be able to test Ok-ness at a higher level
    //wxASSERT_MSG( (pd->hDevMode), wxT("hDevMode must be non-NULL in ConvertToNative!"));

    pd->hDevNames = (HGLOBAL)(WXLPARAM) native_data->GetDevNames();
...
bool wxWindowsPrintDialog::ConvertFromNative( wxPrintDialogData &data )
{
    ...
    if (pd->hDevMode)
    {
        if (native_data->GetDevMode())
        {
            ...
            GlobalFree( (HGLOBAL)(WXLPARAM) native_data->GetDevMode() );
        }
        native_data->SetDevMode( (void*)(WXLPARAM) pd->hDevMode );
        pd->hDevMode = NULL;
    }
    ...
    if (pd->hDevNames)
    {
        ...
        {
            // Make sure we don't leak memory
            GlobalFree((HGLOBAL)(WXLPARAM) native_data->GetDevNames());
        }
        native_data->SetDevNames((void*)(WXLPARAM) pd->hDevNames);
        pd->hDevNames = NULL;
    }
