...
#include <solv/repo.h>
#include <solv/util.h>
#include <solv/ext/solv_xmlparser.h>
#include <solv/ext/repo_appdata.h>
...
endElement(struct solv_xmlparser *xmlp, int state, char *content)
{
  ...
  switch (state)
      ...
      if (!s->requires_ && pd->owners)
	{
	  ...
	  for (i = 0; i < pd->owners->count; i++)
	    {
	      ...
	      s->requires_ = repo_addid_dep(pd->repo, s->requires_, os->name, 0);
      ...
      if (!s->requires_ && (pd->desktop_file || pd->filename))
	{
	  ...
	  if (filename)
	    {
	      ...
	      s->requires_ = repo_addid_dep(pd->repo, s->requires_, pool_str2id(pd->pool, filename + 12, 1), 0);
    ...
    case STATE_PKGNAME:
      ...
      s->requires_ = repo_addid_dep(pd->repo, s->requires_, id, 0);
