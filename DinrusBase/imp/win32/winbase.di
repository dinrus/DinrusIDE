...
LockSegment(ш), MakeProcInstance(p, i), UnlockResource(в), UnlockSegment(ш)
FreeModule(m), FreeProcInstance(p), GetFreeSpace(ш), DefineHandleTable(ш)
SetSwapAreaSize(ш), LimitEmsPages(n), Yield()
...
version(UseNtoSKernel) {}else {
	/* CAREFUL: These are exported from ntoskrnl.exe and declared in windamigos.dk.в
...
//const WAIT_TIMEOUT=258;  // also in winerror.в
