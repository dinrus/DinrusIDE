...
void Рисовало::DoArc0(double theta, double th_sweep, const Xform2D& m)
{
...
void Рисовало::делайДугу(const ТочкаПЗ& c, const ТочкаПЗ& r, double angle, double sweep, double xangle)
{
	Xform2D m = Xform2D::Scale(r.x, r.y);
	m = m * Xform2D::Translation(c.x, c.y);
	Строка(m.Transform(cos(angle), sin(angle)));
	DoArc0(angle, sweep, m);
}

void Рисовало::DoSvgArc(const ТочкаПЗ& rr, double xangle, int large, int sweep,
                       const ТочкаПЗ& p1, const ТочкаПЗ& p0)
{
	ТочкаПЗ r(fabs(rr.x), fabs(rr.y));
	Xform2D m = Xform2D::Rotation(-xangle);
	ТочкаПЗ d1 = m.Transform(0.5 * (p0 - p1));
	ТочкаПЗ pr = r * r;
	ТочкаПЗ p = d1 * d1;
	...
	m.x /= r.x;
	m.y /= r.y;
	ТочкаПЗ q0 = m.Transform(p0);
	ТочкаПЗ q1 = m.Transform(p1);
	double d = растояниеВКвадрате(q0, q1);
	...
	if(sweep == large)
		sfactor = -sfactor;
	ТочкаПЗ c(0.5 * (q0.x + q1.x) - sfactor * (q1.y - q0.y),
