...
{ // it there are multiple topics, use one that is "close" by path
	if(tl.GetCount() == 0)
		return Null;
	String path = DinrusIDE2 ? DinrusIDE2->editfile : Null;
	int mi = 0;
	int m = 0;
...
{
	if(DinrusIDE2)
		DinrusIDE2->OpenTopic(topic, create, before);
}
...
void AssistEditor::NewTopic(String group, String coderef)
{
	if(!DinrusIDE2)
		return;
	String ef = DinrusIDE2->editfile;
	String n = GetFileTitle(ef);
	DinrusIDE2->EditFile(AppendFileName(PackageDirectory(DinrusIDE2->GetActivePackage()), group + ".tpp"));
	if(!DinrusIDE2->designer)
		return;
	TopicEditor *te = dynamic_cast<TopicEditor *>(&DinrusIDE2->designer->DesignerCtrl());
	if(!te)
		return;
	String scope, item;
	SplitCodeRef(coderef, scope, item);
	if(!te->NewTopicEx(IsNull(scope) ? n : Join(Split(scope, ':'), "_"), GetCodeAnnotation(coderef)))
		DinrusIDE2->EditFile(ef);
}
...
void AssistEditor::EditAnnotation(bool leftclick)
{
	...
	if(leftclick) { // show documentation in help widget
		auto GoToTopic = [&] (int i) {
			if(DinrusIDE2) {
				DinrusIDE2->doc.WhenMatchLabel = [](const WString& lbl, const WString& ref) {
					return ref.ToString() == CleanupTppId(lbl.ToString());
				};
				DinrusIDE2->ShowTopics();
				for(String cr : AnnotationCandidates(coderef))
					if(DinrusIDE2->doc.GoTo(tl[i] + '#' + cr))
						break;
			}
		};
		if(tl.GetCount() > 1) {
	...
	if(tl.GetCount()) {
		for(int i = 0; i < tl.GetCount(); i++)
			bar.Add("Редактировать " + tl[i], THISBACK3(OpenTopic, tl[i] + '#' + coderef, String(), false));
		bar.Separator();
	}
	...
	if(tpp.GetCount()) {
		for(int i = 0; i < tpp.GetCount(); i++) {
			...
			bar.Add("Вставить в " + l, THISBACK3(OpenTopic, l + '#' + tpp[i], coderef, i >= backi));
		}
		bar.Separator();
	}
	bar.Add("Новая тематика справки..", THISBACK2(NewTopic, "src", coderef));
	bar.Add("Новая тематика по реализации..", THISBACK2(NewTopic, "srcimp", coderef));
	bar.Separator();
	bar.Add("Copy code reference id", [=, this] { WriteClipboardText(coderef); });
	bar.Execute();
}