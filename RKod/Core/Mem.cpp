...
void memset8__(void *p, i16x8 данные, size_t len)
{
	ПРОВЕРЬ(len >= 16);
	byte *t = (byte *)p;
	auto Set4 = [&](size_t at) { данные.сохрани(t + at); };
	...
	e -= 128;
#ifdef CPU_SSE2
	if(len >= 1024*1024) { // for really huge данные, bypass the cache
		auto Set4S = [&](int at) { данные.Поток(t + at); };
		while(len >= 64) {
...
void memcpy8__(void *p, const void *q, size_t len)
{
	...
	if(len > 4*1024*1024) { // for really huge данные, call memcpy to bypass the cache
		memcpy(t, s, len);
		return;
	}
	...
	Copy128(0); // align target данные up on the next 16 bytes boundary
	if(len <= 32)
		return;
	const byte *e = t + len;
