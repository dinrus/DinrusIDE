...
const ткст УРЛ =  r"(([в|H][t|T]|[f|F])[t|T][p|P]([s|S]?)\:\/\/|~/|/)?([\w]+:\w+@)?(([а-zA-Z]{1}([\w\-]+\.)+([\w]{2,5}))(:[\d]{1,5})?)?((/?\w+/)+|/?)(\w+\.[\w]{3,4})?([,]\w+)*((\?\w+=\w+)?(&\w+=\w+)*([,]\w*)*)?";
...
//exception
    ...
    //бул устСледОбр( типСледОбр в );
    бул устПроверОбр( типПроверОбр в );
...
//object
    проц _d_monitorrelease(Object в);
    ...
    проц _d_monitordelete(Object в, бул det);
    проц удалиМонитор(Объект о, бул уд);

    проц _d_monitorenter(Object в);
    проц войдиВМонитор(Объект о);

    проц _d_monitorexit(Object в);
    проц выйдиИзМонитора(Объект о);
    проц _d_monitor_devt(Monitor* m, Object в);
    проц событиеМонитора(Монитор* м, Объект о);
    проц rt_attachDisposeEvent(Object в, ДСобыт e);
    проц rt_detachDisposeEvent(Object в, ДСобыт e);
    int _fatexit(ук);
...
//runtime
	...
	проц ртСтоп(); 
    проц  ртСоздайОбработчикСледа( Следопыт в );
    Исключение.ИнфОСледе ртСоздайКонтекстСледа( ук  ptr );
    проц  ртУстановиОбработчикСборки(ОбработчикСборки в);
    ук ртНизСтэка();
    ук ртВерхСтэка();
