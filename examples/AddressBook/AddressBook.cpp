...
#define LAYOUTFILE <examples/AddressBook/AddressBook.lay>
...
AddressBook::AddressBook()
{
	...
	SetMainMenu([=](Bar& bar) {
		bar.Sub("Файл", [=](Bar& bar) {
			bar.Add("Открыть..", CtrlImg::open(), THISFN(Open));
			bar.Add("Сохранить", CtrlImg::save(), THISFN(Save));
			bar.Add("Сохранить как..", CtrlImg::save_as(), THISFN(SaveAs));
			bar.Separator();
			bar.Add("Печать", CtrlImg::print(), THISFN(Print));
			bar.Separator();
			bar.Add("Выход", THISFN(Quit));
		});
	});
#else
	menu.Set([=](Bar& bar) {
...
{
	String qtf;
	qtf = "{{1:1:1:1 Имя:: Фамилия:: Адрес:: Почта";
	for(int i = 0; i < array.GetCount(); i++)
		for(int q = 0; q < 4; q++)
