...
static проц sha256_transform(SHA256_CTX *ctx, const ббайт *данные)
{
	бцел a, b, c, d, e, f, g, h, i, j, t1, t2, m[64];
...
static проц sha256_init(SHA256_CTX *ctx)
{
	ctx->datalen = 0;
	ctx->bitlen[0] = 0;
	ctx->bitlen[1] = 0;
...
static проц sha256_update(SHA256_CTX *ctx, const ббайт *данные, бцел len)
{
	бцел i;
...
static проц sha256_final(SHA256_CTX *ctx, ббайт *hash)
{
	бцел i;
	...
	ctx->данные[63] = ббайт(ctx->bitlen[0]);
	ctx->данные[62] = ббайт(ctx->bitlen[0] >> 8);
	ctx->данные[61] = ббайт(ctx->bitlen[0] >> 16);
	ctx->данные[60] = ббайт(ctx->bitlen[0] >> 24);
	ctx->данные[59] = ббайт(ctx->bitlen[1]);
	ctx->данные[58] = ббайт(ctx->bitlen[1] >> 8);
	ctx->данные[57] = ббайт(ctx->bitlen[1] >> 16);
	ctx->данные[56] = ббайт(ctx->bitlen[1] >> 24);
	sha256_transform(ctx,ctx->данные);
 
	// Since this implementation uses little endian ббайт ordering and SHA uses
