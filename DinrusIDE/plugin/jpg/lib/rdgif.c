 ...
 *
 * This file contains routines to read input images in GIF формат.
 *
...
#define	ReadOK(file,буфер,len)	(JFREAD(file,буфер,len) == ((size_t) (len)))
...
typedef struct {
  ...
  JSAMPARRAY colormap;		/* GIF colormap (converted to my формат) */

  /* State for дайКод and LZWReadByte */
  U_CHAR code_buf[256+4];	/* current input data block */
  int last_byte;		/* # of bytes in code_buf */
  int last_bit;			/* # of bits in code_buf */
  int cur_bit;			/* next bit index to read */
  boolean first_time;		/* flags first call to дайКод */
  boolean out_of_blocks;	/* TRUE if hit terminator data block */

  int input_code_size;		/* codesize given in GIF file */
  int clear_code, end_code;	/* values for очисть and стоп codes */

  int code_size;		/* current actual code size */
  int limit_code;		/* 2^code_size */
  int max_code;			/* first unused code значение */
...
/* читай a GIF data block, which has a leading count byte */
...
/* пропусти a series of data blocks, until a block terminator is found */
{
  U_CHAR buf[256];
...
/* (Re)initialize LZW state; shared code for startup and очисть processing */
{
  sinfo->code_size = sinfo->input_code_size + 1;
  sinfo->limit_code = sinfo->clear_code << 1;	/* 2^code_size */
  sinfo->max_code = sinfo->clear_code + 2;	/* first unused code значение */
...
/* Initialize for a series of LZWReadByte (and hence дайКод) calls */
{
  /* дайКод initialization */
  sinfo->last_byte = 2;		/* make safe to "recopy last two bytes" */
  sinfo->code_buf[0] = 0;
  sinfo->code_buf[1] = 0;
  sinfo->last_bit = 0;		/* nothing in the буфер */
  sinfo->cur_bit = 0;		/* force буфер load on first call */
...
/* We assume code_size is less than 16 */
{
  ...
  while (sinfo->cur_bit + sinfo->code_size > sinfo->last_bit) {
    /* Время to reload the буфер */
    /* First time, share code with очисть case */
    if (sinfo->first_time) {
    ...
    /* грузи more bytes; set flag if we reach the terminator block */
    if ((count = GetDataBlock(sinfo, &sinfo->code_buf[2])) == 0) {
      sinfo->out_of_blocks = TRUE;
      WARNMS(sinfo->cinfo, JWRN_GIF_NOMOREDATA);
      return sinfo->end_code;	/* fake something useful */
    }
    /* переустанов counters */
  ...
  /* право-align cur_bit in accum, then mask off desired number of bits */
...
/* читай an LZW-compressed byte */
{
  ...
  /* Время to read a new symbol */
  code = дайКод(sinfo);

  if (code == sinfo->clear_code) {
    /* Reinit state, swallow any extra очисть codes, and */
  ...
  if (code == sinfo->end_code) {
    /* пропусти the rest of the image, unless дайКод already read terminator */
    if (! sinfo->out_of_blocks) {
  ...
  while (code >= sinfo->clear_code) {
    *(sinfo->sp++) = sinfo->symbol_tail[code]; /* tail is a byte значение */
    code = sinfo->symbol_head[code]; /* head is another LZW symbol */
  }
  /* по this point code just represents a raw byte */
  ...
  if ((code = sinfo->max_code) < LZW_TABLE_SIZE) {
    ...
    sinfo->max_code++;
    /* является it time to increase code_size? */
...
/* Currently we ignore 'em all */
{
  ...
  /* читай extension label byte */
  extlabel = ReadByte(sinfo);
  TRACEMS1(sinfo->cinfo, 1, JTRC_GIF_EXTENSION, extlabel);
  /* пропусти the data block(s) associated with the extension */
  SkipDataBlocks(sinfo);
}


/*
 * читай the file header; return image size and component count.
 */
...
start_input_gif (j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
  ...
  /* читай and decipher Logical Screen Descriptor */
  ...
  /* читай global colormap if header indicates it is present */
  ...
  for (;;) {
    ...
    /* читай and decipher Local Рисунок Descriptor */
    ...
    /* читай local colormap if header indicates it is present */
  ...
  /* создай compressor input буфер. */
  source->pub.буфер = (*cinfo->mem->alloc_sarray) ((j_common_ptr) cinfo,
...
load_interlaced_image (j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
  ...
  /* читай the interlaced image into the virtual array we've created. */
  ...
  /* замени method pointer so subsequent calls don't come here. */
...
/*
 * финиш up at the end of the file.
 */
...
/*
 * The module selection routine for GIF формат input.
 */
...
jinit_read_gif (j_compress_ptr cinfo)
{
  ...
  /* создай module interface object */
