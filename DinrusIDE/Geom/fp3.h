...
struct Pointf3 : Движимое<Pointf3>
{
public:
	Pointf3() {}
	Pointf3(const Обнул&) : x(Null), y(Null), z(Null) {}
	Pointf3(double x, double y, double z) : x(x), y(y), z(z) {}
	Pointf3(ТочкаПЗ p, double z = 0) : x(p.x), y(p.y), z(z) {}
	explicit Pointf3(double t) : x(t), y(t), z(t) {}

	void              сериализуй(Поток& stream)                    { stream % x % y % z; }

	double            operator [] (int i) const                    { return i == 0 ? x : i == 1 ? y : z; }
	double&           operator [] (int i)                          { return i == 0 ? x : i == 1 ? y : z; }

	ТочкаПЗ            XY() const                                   { return ТочкаПЗ(x, y); }
	ТочкаПЗ            YX() const                                   { return ТочкаПЗ(y, x); }

	ТочкаПЗ            YZ() const                                   { return ТочкаПЗ(y, z); }
	ТочкаПЗ            ZY() const                                   { return ТочкаПЗ(y, z); }

	ТочкаПЗ            XZ() const                                   { return ТочкаПЗ(x, z); }
	ТочкаПЗ            ZX() const                                   { return ТочкаПЗ(z, x); }
...
inline bool           пусто_ли(Pointf3 p)                         { return пусто_ли(p.x); }
inline Pointf3        Nvl(Pointf3 p, Pointf3 q)                 { return !пусто_ли(p) ? p : q; }
extern Ткст         какТкст(const Pointf3& p);
...
inline Pointf3        середина(Pointf3 p, Pointf3 q)                 { return Pointf3((p.x + q.x) / 2, (p.y + q.y) / 2, (p.z + q.z) / 2); }
inline Pointf3        середина(Pointf3 p, Pointf3 q, double w)       { return p + (q - p) * w; }
inline double         вКвадрате(Pointf3 p)                        { return p.x * p.x + p.y * p.y + p.z * p.z; }
inline double         длина(Pointf3 p)                         { return sqrt(вКвадрате(p)); }
extern Pointf3        длина(Pointf3 p, double l);
inline double         расстояние(Pointf3 p, Pointf3 q)            { return длина(p - q); }
inline Pointf3        UnitV(Pointf3 p)                          { return длина(p, 1.0); }
inline Pointf3        Scale(Pointf3 p, double d, Pointf3 c = ТочкаПЗ(0, 0)) { return (p - c) * d + c; }
...
extern Pointf3        ортогональ(Pointf3 v, Pointf3 against);
...
Ткст          какТкст(const Plane3& plane);

inline double   расстояние(Plane3 plane, Pointf3 point)       { return (point ^ plane.normal) + plane.delta; }
extern double   пересек(Plane3 p, Pointf3 la, Pointf3 lb);
...
struct Matrixf3 : Движимое<Matrixf3>
{
	Pointf3 x, y, z, a;

	Matrixf3()                                         { *this = Matrixf3_1(); }
	Matrixf3(const Обнул&) : a(Null)                  {}
	Matrixf3(Pointf3 x, Pointf3 y, Pointf3 z, Pointf3 a = Pointf3(0, 0, 0))
		: x(x), y(y), z(z), a(a) {}
	Matrixf3(const Matrixf3& (*фн)())                  { *this = фн(); }
	...
	bool             нуль_ли() const                        { return x.x == 0 && x.y == 0 && x.z == 0 && y.x == 0 && y.y == 0 && y.z == 0 && z.x == 0 && z.y == 0 && z.z == 0 && a.x == 0 && a.y == 0 && a.z == 0; }
	bool             экзПусто_ли() const                { return пусто_ли(a); }

	Matrixf3&        фиксируй(const Pointf3& v)                 { a = v - v.x * x - v.y * y - v.z * z; return *this; }
...
//extern Matrixf3  Matrixf3Mirror (ТочкаПЗ A, ТочкаПЗ B);
...
{
public:
	Box3(const Обнул& = Null) { low[0] = Null; }
	...
	bool    пустой() const              { return low[0] > high[0] || low[1] > high[1] || low[2] > high[2]; }
	void    очисть()                      { low[0] = low[1] = low[2] = BIGDOUBLE; high[0] = high[1] = high[2] = -BIGDOUBLE; }
	void    союз(Pointf3 pt);
...
inline bool пусто_ли(Box3 b) { return пусто_ли(b.low[0]); }
...
public:
	...
	Camera&  Viewport(РазмерПЗ mm)                         { width_mm = mm.cx; height_mm = mm.cy; return *this; }
	Camera&  ViewportOffset(double xc, double yc)       { shift_x = xc; shift_y = yc; return *this; }
	Camera&  ViewportSize(РазмерПЗ px)                     { view_px = px.cx; view_py = px.cy; return *this; }
	...
	bool     TransformClip(Pointf3 a, Pointf3 b, ТочкаПЗ& outa, ТочкаПЗ& outb) const;
	Pointf3  InverseXY(ТочкаПЗ point, double z) const;
