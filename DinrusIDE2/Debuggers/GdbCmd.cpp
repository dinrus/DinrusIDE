...
#define IMAGEFILE  <DinrusIDE2/Debuggers/Debuggers.iml>
...
String Gdb::Cmd(const char *command, bool start)
{
	...
	if(command) {
		...
		PutVerbose(String() << "Команда: " << command);
	}
	String result;
	int ms0 = msecs();
	while(dbg.IsRunning()) {
		...
		if(!dbg.Read(s)) {
			PutVerbose(result);
			PutVerbose("Отладчик остановлен");
	...
	Unlock();
	if(command) {
		PutVerbose(String() << "Время у `" << command <<"` " << ts);
	}
...
String Gdb::FastCmd(const char *command)
{
	...
	if(command) {
		dbg.Write(String(command) + "\n");
		PutVerbose(String() << "Быстрая команда: " << command);
	}
	String result;
	TimeStop ts;
	while(dbg.IsRunning()) {
