 ...
 * This значение is usually set such that a reasonable exposure comes from
 ...
 * tag значение.  A decoder that demands absolute color calibration may use
 ...
 *	 S       Le           ue       ve
 *
 * For the 24-bit COMPRESSION_SGILOG24 color формат, the data is stored as:
 *
 *	     10           14
 *	|----------|--------------|
 *	     Le'          Ce
 ...
 *
 * The 10 bits of the smaller формат may be converted into the 15 bits of
 * the larger формат by multiplying by 4 and adding 13314.  Obviously,
 * a smaller range of magnitudes is covered (about 5 orders of magnitude
 * instead of 38), and the lack of a sign bit means that negative luminances
 * are not allowed.  (Well, they aren't allowed in the real world, either,
 * but they are useful for certain types of image processing.)
 *
 * The desired user формат is controlled by the setting the internal
 * pseudo tag TIFFTAG_SGILOGDATAFMT to one of:
 *  SGILOGDATAFMT_FLOAT       = IEEE 32-bit float XYZ values
 *  SGILOGDATAFMT_16BIT	      = 16-bit integer encodings of logL, u and v
 * Raw data i/o is also possible using:
 *  SGILOGDATAFMT_RAW         = 32-bit unsigned integer with encoded pixel
 * In addition, the following decoding is provided for ease of дисплей:
 *  SGILOGDATAFMT_8BIT        = 8-bit default дайКЗС gamma-corrected values
 *
 ...
 * a PackBits тип of compression.  Since the 24-bit encoding is not
 * adaptive, the 32-bit color формат takes less space in many cases.
 *
 ...
 *
 * The default значение of this tag is SGILOGENCODE_NODITHER for
...
struct logLuvState {
	...
	int                     user_datafmt;   /* user data формат */
	int                     encode_meth;    /* encoding method */
	int                     pixel_size;     /* bytes per pixel */

	uint8*                  tbuf;           /* translation буфер */
	tmsize_t                tbuflen;        /* буфер length */
...
LogL16InitState(TIFF* tif)
{
	...
	default:
		TIFFErrorExt(tif->tif_clientdata, module,
		    "No support for converting user data формат to LogL");
		return (0);
	}
        if( isTiled(tif) )
	...
	if (multiply_ms(sp->tbuflen, sizeof (int16)) == 0 ||
		...
		TIFFErrorExt(tif->tif_clientdata, module, "No space for SGILog translation буфер");
		return (0);
	}
	return (1);
}
...
LogLuvInitState(TIFF* tif)
{
	...
	default:
		TIFFErrorExt(tif->tif_clientdata, module,
		    "No support for converting user data формат to LogLuv");
		return (0);
	}
        if( isTiled(tif) )
            sp->tbuflen = multiply_ms(td->td_tilewidth, td->td_tilelength);
...
LogLuvClose(TIFF* tif)
{
	 ...
	 * regardless of the data формат being used by the application.
	...
	case TIFFTAG_SGILOGDATAFMT:
		...
		default:
			TIFFErrorExt(tif->tif_clientdata, tif->tif_name,
			    "Unknown data формат %d for LogLuv compression",
			    sp->user_datafmt);
			return (0);
		}
