...
void SIC_Print(EscEscape& e)
{
	...
	if(!e[0].IsVoid())
		e.ThrowError("неправильный аргумент при вызове 'print'");
}
...
void SIC_GetTextSize(EscEscape& e)
{
	if(e.GetCount() < 1 || e.GetCount() > 2)
		e.ThrowError("неправильное число аргументов при вызове 'GetTextSize'");
	e.CheckArray(0);
	WString text = e[0];
	Font font = StdFont();
	if(e.GetCount() > 1)
...
void EscDraw::DrawRect(EscEscape& e)
{
	...
	if(e.GetCount() == 5)
		...
		e.ThrowError("неправильное число аргументов при вызове 'DrawRect'");
}
...
void EscDraw::DrawLine(EscEscape& e)
{
	...
	if(e.GetCount() == 6)
		...
		e.ThrowError("неправильное число аргументов при вызове 'DrawLine'");
}

void EscDraw::DrawText(EscEscape& e)
{
	if(e.GetCount() < 3 || e.GetCount() > 6)
		e.ThrowError("неправильное число аргументов при вызове 'DrawText'");
	int x = e.Int(0);
	int y = e.Int(1);
	Font font = StdFont();
...
void EscDraw::DrawSmartText(EscEscape& e)
{
	if(e.GetCount() < 3 || e.GetCount() > 7)
		e.ThrowError("неправильное число аргументов при вызове 'DrawSmartText'");
	int x = e.Int(0);
	int y = e.Int(1);
	int ii = 2;
	String text;
...
void EscDraw::DrawQtf(EscEscape& e)
{
	if(e.GetCount() < 5 || e.GetCount() > 6)
		e.ThrowError("неправильное число аргументов при вызове 'DrawQtf'");
//	int zoom = e.Int(0);
	int x = e.Int(1);
	int y = e.Int(2);
	e.CheckArray(3);
...
void EscDraw::DrawImage(EscEscape& e)
{
	...
	if(cnt == 5)
		...
		e.ThrowError("неправильное число аргументов при вызове 'DrawImage'");
}
...
void EscDraw::DrawImageColor(EscEscape& e)
{
	...
	if(cnt != 4 && cnt != 5 && cnt != 7)
		e.ThrowError("неправильное число аргументов при вызове 'DrawImageColor'");
