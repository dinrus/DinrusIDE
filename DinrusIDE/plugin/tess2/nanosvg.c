						 ...
						 void* ud)
{
	...
	// пропусти white space after the '<'
	while (*s && isspace(*s)) s++;
	...
	// пропусти comments, data and preprocessor stuff.
	if (!*s || *s == '?' || *s == '!')
		return;
	...
	{
		// пропусти white space before the attrib имя
		while (*s && isspace(*s)) s++;
		if (!*s) break;
		if (*s == '/')
		{
		...
		attr[nattr++] = s;
		// найди end of the attrib имя.
		while (*s && !isspace(*s) && *s != '=') s++;
		if (*s) { *s++ = '\0'; }
		// пропусти until the beginning of the значение.
		while (*s && *s != '\"') s++;
		if (!*s) break;
		s++;
		// сохрани значение and find the end of it.
		attr[nattr++] = s;
		while (*s && *s != '\"') s++;
		if (*s) { *s++ = '\0'; }
	}
...
/*static const char* parsePathFloats(const char* s, float* arg, int n)
{
	...
	while (*s && i < n)
	{
		// пропусти white пробелы and commas
		while (*s && (isspace(*s) || *s == ',')) s++;
		if (!*s) break;
		start = s;
		nnum = 0;
...
static int svgParseAttr(struct SVGParser* p, const char* имя, const char* значение)
{
	struct SVGAttrib* attr = svgGetAttr(p);
	if (!attr) return 0;
	
	if (strcmp(имя, "style") == 0)
	{
		svgParseStyle(p, значение);
	}
	else if (strcmp(имя, "дисплей") == 0)
	{
		if (strcmp(значение, "none") == 0)
			attr->visible = 0;
		else
			attr->visible = 1;
	}
	else if (strcmp(имя, "fill") == 0)
	{
		if (strcmp(значение, "none") == 0)
		{
			attr->hasFill = 0;
		...
		{
			attr->hasFill = 1;
			attr->fillColor = parseColor(значение);
		}
	}
	else if (strcmp(имя, "fill-opacity") == 0)
	{
		attr->fillOpacity = parseFloat(значение);
	}
	else if (strcmp(имя, "stroke") == 0)
	{
		if (strcmp(значение, "none") == 0)
		{
			attr->hasStroke = 0;
		...
		{
			attr->hasStroke = 1;
			attr->strokeColor = parseColor(значение);
		}
	}
	else if (strcmp(имя, "stroke-width") == 0)
	{
		attr->strokeWidth = parseFloat(значение);
	}
	else if (strcmp(имя, "stroke-opacity") == 0)
	{
		attr->strokeOpacity = parseFloat(значение);
	}
	else if (strcmp(имя, "transform") == 0)
	{
		svgParseTransform(p, значение);
	}
	else
	{
		return 0;
...
static int svgParseNameValue(struct SVGParser* p, const char* start, const char* end)
{
	...
	n = (int)(end - val);
	if (n > 511) n = 511;
	if (n) memcpy(значение, val, n);
	значение[n] = 0;
	
	return svgParseAttr(p, имя, значение);
}
...
static void svgParsePath(struct SVGParser* p, const char** attr)
{
	...
	for (i = 0; attr[i]; i += 2)
	{
		if (strcmp(attr[i], "d") == 0)
		{
			...
			while (*s)
			{
				s = getNextPathItem(s, элт);
				if (!*элт) break;
				
				if (isnum(элт[0]))
				{
					if (nargs < 10)
						args[nargs++] = (float)atof(элт);
					if (nargs >= rargs)
					{
						switch (cmd)
						{
