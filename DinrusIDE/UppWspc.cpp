...
void WorkspaceWork::AddSeparator()
{
	...
	if(active.IsEmpty()) return;
	String name;
	if(!EditText(name, "Добавить сепаратор", "Имя"))
		return;
...
void WorkspaceWork::DoImportTree(const String& dir, const String& mask, bool sep, Progress& pi, int from)
{
	...
	if(sep && files.GetCount()) {
		if(!DirectoryExists(absPath))
			if(!RealizeDirectory(absPath))
				throw Format("Ошибка при создании папки:&\1%s", absPath);
	...
	for(int i = 0; i < files.GetCount(); i++) {
		...
		if(FileCopy(files[i], AppendFileName(absPath, name))) {
			...
			throw Format("Ошибка при копировании файла:&\1%s", files[i]);
	}
...
{
	bool isaux = IsAux();
	menu.Add("Вставить файл(ы)..", THISBACK1(AddFile, ANY_FILE))
		.Key(K_SHIFT|K_CTRL_I)
		.Help("Вставить файлы с диска (вредно для портабельных пакетов)");
	menu.Add(isaux && GetOutputDir().GetCount(), "Вставить файл(ы) из папки вывода..", THISBACK1(AddFile, OUTPUT_FILE))
		.Help("Открыть файлвыборку в папке вывода / промежуточной папке для текущего пакета");
#ifdef PLATFORM_POSIX
	menu.Add(isaux && GetConfigDir().GetCount(), "Вставить файл(ы) конфигурации папки..", THISBACK1(AddFile, CONFIG_FILE))
		.Help("Открыть файлвыборку в папке вывода / промежуточной папке для текущего пакета");
#endif
	menu.Add(isaux, "Вставить файл(ы) из локальной папки..", THISBACK1(AddFile, LOCAL_FILE))
		.Help("Открыть файлвыборку в локальной папке для текущего пакета");
	menu.Add("Вставить файл(ы) из домашней папки..", THISBACK1(AddFile, HOME_FILE))
...
void WorkspaceWork::SpecialFileMenu(Bar& menu)
{
	...
	menu.Add("Импортировать исходники из дерева папки..", THISBACK(Import));
}
...
void WorkspaceWork::FileMenu(Bar& menu)
{
	...
	menu.Add(filelist.IsCursor(), "Стереть", sel ? THISBACK(RemoveFile) : THISBACK(DelFile))
	...
	if(IsActiveFile()) {
		...
		if(ext == ".tpp" && IsFolder(p)) {
			...
			if(GetRepoKind(p))
				menu.Add("Синхронизовать Репозиторий " + p, THISBACK1(SyncSvnDir, p));
		}
		else {
			if(IsHeaderExt(ext))
				menu.Add("Прекомпилировать заголовок", THISBACK(TogglePCH))
...
{
	String p = SelectPackage("Выбрать пакет");
	if(p.IsEmpty()) return;
		...
		for(int i = 0; i < prj.uses.GetCount(); i++)
			if(UnixPath(prj.uses[i].text) == active) {
				...
				bar.Add("Удалить из '" + pn + '\'', THISBACK1(RemovePackage, pn))
					.Help(Format("Удалить пакет '%s' из раздела использований в '%s'", active, pp));
			}
	}
	if(usecnt > 1) {
		bar.MenuSeparator();
		bar.Add("Удалить все использования", THISBACK1(RemovePackage, String(Null)))
			.Help(Format("Удалить пакет '%s' из всех использований в активном проекте и его подмодулях", active));
	}
}
...
void WorkspaceWork::PackageMenu(Bar& menu)
{
	if(!menu.IsScanKeys()) {
		...
		menu.Add(cando, ~Format("Добавить пакет в '%s'", act), IdeImg::package_add(), THISBACK(AddNormalUses));
		RemovePackageMenu(menu);
		if(menu.IsMenuBar()) {
			bool main = package.GetCursor() == 0;
			
			menu.Add(cando, "Переименовать пакет..", THISBACK(RenamePackage));
			menu.Add(cando && !main, "Удалить пакет", THISBACK(DeletePackage));
			menu.Separator();
