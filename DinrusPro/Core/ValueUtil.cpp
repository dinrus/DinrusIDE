...
struct Реф::ValueRef : public РефМенеджер {
	...
	virtual Значение дайЗначение(const ук укз)            { return *(Значение *) укз; }
	virtual бул  пусто_ли(const ук укз)              { return РНЦП::пусто_ли(*(Значение *) укз); }
	virtual проц  устЗначение(ук укз, const Значение& v)  { *(Значение *) укз = v; }
	virtual проц  устПусто(ук укз)                   { *(Значение *) укз = Null; }
};

Реф::Реф(Ткст& s)  { укз = &s; m = &Single< StdRef<Ткст> >(); }
Реф::Реф(ШТкст& s) { укз = &s; m = &Single< StdRef<ШТкст> >(); }
Реф::Реф(цел& i)     { укз = &i; m = &Single< StdRef<цел> >(); }
Реф::Реф(дол& i)   { укз = &i; m = &Single< StdRef<дол> >(); }
Реф::Реф(дво& d)  { укз = &d; m = &Single< StdRef<дво> >(); }
Реф::Реф(бул& b)    { укз = &b; m = &Single< StdRef<бул> >(); }
Реф::Реф(Дата& d)    { укз = &d; m = &Single< StdRef<Дата> >(); }
Реф::Реф(Время& t)    { укз = &t; m = &Single< StdRef<Время> >(); }
Реф::Реф(Значение& v)   { укз = &v; m = &Single< ValueRef >(); }
...
{
	ДинрусРНЦП::вДжейсон(jio, данные);
}

проц МассивЗнач::Данные::вРяр(РярВВ& io)
{
	ДинрусРНЦП::вРяр(io, данные);
}
...
{
	ДинрусРНЦП::вРяр(xio, ключ);
	ДинрусРНЦП::вРяр(xio, значение);
}
...
цел CompareStrings(const Значение& a, const Значение& b, const LanguageInfo& f)
{
