...
#define FT_COMPONENT  trace_raccess
                             ...
                             FT_Long    *rdata_pos )
  {
    ...
    Ошибка = FT_Stream_Seek( stream, rfork_offset );
    if ( Ошибка )
      return Ошибка;

    Ошибка = FT_Stream_Read( stream, (FT_Byte *)head, 16 );
    if ( Ошибка )
      return Ошибка;
    ...
    *rdata_pos = rfork_offset + ( ( head[0] << 24 ) |
    ...
    Ошибка = FT_Stream_Seek( stream, map_pos );
    if ( Ошибка )
      return Ошибка;

    head2[15] = (FT_Byte)( head[15] + 1 );       /* make it be different */

    Ошибка = FT_Stream_Read( stream, (FT_Byte*)head2, 16 );
    if ( Ошибка )
      return Ошибка;
    ...
    /* пропусти handle to next resource map, the file resource number, and  */
    /* attributes.                                                      */
    ...
    Ошибка = FT_Stream_Seek( stream, map_pos + type_list );
    if ( Ошибка )
      return Ошибка;
                              ...
                              FT_Long    *count )
  {
    ...
    Ошибка = FT_Stream_Seek( stream, map_offset );
    if ( Ошибка )
      return Ошибка;
    ...
    for ( i = 0; i < cnt; ++i )
    {
      ...
      if ( tag_internal == tag )
      {
        *count = subcnt + 1;
        ...
        Ошибка = FT_Stream_Seek( stream, rpos );
        if ( Ошибка )
          return Ошибка;
        ...
        for ( j = 0; j < *count; ++j )
        {
          if ( FT_READ_USHORT( ref[j].res_id ) )
            goto Exit;
          if ( FT_STREAM_SKIP( 2 ) ) /* resource имя */
            goto Exit;
  ...
  /* For fast translation between rule index and rule тип,
  ...
  raccess_get_rule_type_from_rule_index( FT_UInt  rule_index )
  {
    ...
    return raccess_guess_table[rule_index].тип;
  }
                                   ...
                                   FT_Long    *result_offset )
  {
    ...
    Ошибка = raccess_guess_linux_double_from_file_name( library, newpath,
                                                       result_offset );
    if ( !Ошибка )
                               ...
                               FT_Long    *result_offset )
  {
    ...
    /* filler */
    Ошибка = FT_Stream_Skip( stream, 16 );
    if ( Ошибка )
      return Ошибка;
    ...
    for ( i = 0; i < n_of_entries; i++ )
    {
      ...
      {
        Ошибка = FT_Stream_Skip( stream, 4 + 4 );    /* offset + length */
        if ( Ошибка )
          return Ошибка;
      }
    }
                                             ...
                                             FT_Long    *result_offset )
  {
    ...
    Ошибка = raccess_guess_apple_double( library, stream2, file_name,
                                        &nouse, result_offset );
