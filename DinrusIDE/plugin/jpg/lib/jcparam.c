...
/* Преобр a user-specified quality rating to a percentage scaling factor
 * for an underlying quantization table, using our recommended scaling curve.
 * The input 'quality' factor should be 0 (terrible) to 100 (very good).
 */
{
  /* Safety limit on quality factor.  Преобр 0 to 1 to avoid zero divide. */
...
/*
 * переустанов standard Huffman tables
 */
...
jpeg_set_defaults (j_compress_ptr cinfo)
{
   ...
   * multiple images at same param настройки.
   */
  ...
  /* дефолт is no multiple-scan output */
   ...
   */
  cinfo->JFIF_major_version = 1; /* дефолт JFIF version = 1.01 */
...
/*
 * выдели an appropriate JPEG colorspace for in_color_space.
 */
...
#define SET_COMP(index,id,hsamp,vsamp,quant,dctbl,actbl)  \
  ...
  case JCS_UNKNOWN:
    ...
    break;
  case JCS_GRAYSCALE:
    cinfo->write_JFIF_header = TRUE; /* пиши a JFIF marker */
    cinfo->num_components = 1;
    /* JFIF specifies component ИД 1 */
    SET_COMP(0, 0x01, 1,1, 0, 0,0);
    break;
  case JCS_RGB:
    cinfo->write_Adobe_marker = TRUE; /* write Adobe marker to flag дайКЗС */
  ...
  case JCS_BG_RGB:
    ...
    cinfo->num_components = 3;
    /* добавь offset 0x20 to the normal R/G/B component IDs */
  ...
  case JCS_BG_YCC:
    ...
    cinfo->JFIF_major_version = 2;   /* Set JFIF major version = 2 */
    cinfo->num_components = 3;
    /* добавь offset 0x20 to the normal Cb/Cr component IDs */
    /* We default to 2x2 subsamples of chrominance */
    SET_COMP(0, 0x01, 2,2, 0, 0,0);
...
/*
 * создай a recommended progressive-JPEG script.
...
jpeg_simple_progression (j_compress_ptr cinfo)
{
  ...
  if (ncomps == 3 &&
    ...
    /* All-purpose script for other color пробелы. */
    if (ncomps > MAX_COMPS_IN_SCAN)
   ...
   * multiple compressions without changing the настройки.  To avoid a memory
  ...
  if (ncomps == 3 &&
    ...
    /* финиш DC successive approximation */
    scanptr = fill_dc_scans(scanptr, ncomps, 1, 0);
    /* финиш AC successive approximation */
