...
static void
FillTable(TIFFFaxTabEnt *T, int Размер, struct proto *P, int State)
{
    int limit = 1 << Размер;
...
void
WriteTable(FILE* fd, const TIFFFaxTabEnt* T, int Размер, const char* имя)
{
    int i;
    char* sep;

    fprintf(fd, "%s %s TIFFFaxTabEnt %s[%d] = {",
	storage_class, const_class, имя, Размер);
    if (packoutput) {
	sep = "\n";
	for (i = 0; i < Размер; i++) {
	    fprintf(fd, "%s%s%d,%d,%d%s",
		sep, prebrace, T->State, T->устШирину, (int) T->Param, postbrace);
	    if (((i+1) % 10) == 0)
		    sep = ",\n";
	    else
		    sep = ",";
