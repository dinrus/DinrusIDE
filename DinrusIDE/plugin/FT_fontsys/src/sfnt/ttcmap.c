...
#define TT_NEXT_ULONG   FT_NEXT_ULONG
  ...
  /***** assume that the значение "charcode" correspond to following:     *****/
  /*****                                                               *****/
  /*****   - For one byte characters, "charcode" is simply the         *****/
  /*****     character code.                                           *****/
  /*****                                                               *****/
  /*****   - For two byte characters, "charcode" is the 2-byte         *****/
  /*****     character code in big endian формат.  ещё exactly:       *****/
  /*****                                                               *****/
  /*****       (charcode >> 8)    is the first byte значение              *****/
  /*****       (charcode & 0xFF)  is the second byte значение             *****/
  /*****                                                               *****/
  ...
  /*   NAME        OFFSET         TYPE            DESCRIPTION              */
  /*                                                                       */
  /*   формат      0              USHORT          must be 2                */
  /*   length      2              USHORT          table length in bytes    */
  /*   language    4              USHORT          Mac language code        */
  /*   keys        6              USHORT[256]     sub-header keys          */
  /*   subs        518            SUBHEAD[NSUBS]  sub-headers array        */
  /*   glyph_ids   518+NSUB*8     USHORT[]        glyph ИД array           */
  /*                                                                       */
  /* The `keys' table is used to map charcode high-bytes to sub-headers.   */
  /* The значение of `NSUBS' is the number of sub-headers defined in the      */
  ...
  /* by 8.                                                                 */
  /*                                                                       */
  /* Each sub-header has the following формат:                             */
  /*                                                                       */
  /*   NAME        OFFSET      TYPE            DESCRIPTION                 */
  /*                                                                       */
  ...
  /*                                                                       */
  /* * The значение of `offset' is read.  This is a _byte_ distance from the  */
  /*   location of the `offset' field itself into a slice of the           */
  /*   `glyph_ids' table.  Let's call it `slice' (it is a USHORT[] too).   */
  /*                                                                       */
  /* * The значение `slice[char.lo - first]' is read.  If it is 0, there is   */
  /*   no glyph for the charcode.  Otherwise, the значение of `delta' is      */
  ...
  tt_cmap2_validate( FT_Byte*      table,
                     FT_Validator  valid )
  {
    FT_Byte*  p      = table + 2;           /* skip формат */
    FT_UInt   length = TT_PEEK_USHORT( p );
    FT_UInt   n, max_subs;
    FT_Byte*  keys;                         /* keys table */
    FT_Byte*  subs;                         /* sub-headers */
    FT_Byte*  glyph_ids;                    /* glyph ИД array */
    ...
    for ( n = 0; n < 256; n++ )
    {
      ...
      /* значение must be multiple of 8 */
                          ...
                          FT_UInt32  char_code )
  {
    ...
    if ( char_code < 0x10000UL )
    {
      ...
      if ( char_hi == 0 )
      {
        ...
        /* indicates that it is really a valid one-byte значение  */
        /* Otherwise, return 0                                 */
        /*                                                     */
        p += char_lo * 2;
      ...
      else
      {
        ...
        /* check that the high byte isn't a valid one-byte значение */
        if ( sub == subs )
          goto Exit;
      }
      result = sub;
                      ...
                      FT_UInt32  *pcharcode )
  {
    ...
    while ( charcode < 0x10000UL )
    {
      ...
      /* jump to next sub-header, i.e. higher byte значение */
    Next_SubHeader:
  ...
  /*                                                  zero                 */
  /*                                                                       */
  /*   glyphIds      16+NUM_SEGS*8  USHORT[]          array of glyph ИД    */
  /*                                                  ranges               */
  /*                                                                       */
  /* Character codes are modelled by a series of ordered (increasing)      */
  /* intervals called segments.  Each segment has start and end codes,     */
  /* provided by the `startCount' and `endCount' arrays.  Segments must    */
  /* not overlap, and the last segment should always contain the значение     */
  /* 0xFFFF for `endCount'.                                                */
  /*                                                                       */
  ...
  /* charcode within the segment is obtained by adding the значение of        */
  /* `idDelta' directly to the charcode, modulo 65536.                     */
  /*                                                                       */
  /* Otherwise, a glyph index is taken from the glyph IDs sub-array for    */
  /* the segment, and the значение of `idDelta' is added to it.               */
  /*                                                                       */
  /*                                                                       */
                     ...
                     FT_Validator  valid )
  {
    ...
    if ( valid->level >= FT_VALIDATE_PARANOID )
    {
      /* check that we have an even значение here */
      if ( num_segs & 1 )
        FT_INVALID_DATA;
    }
    ...
    /* check last segment; its end count значение must be 0xFFFF */
    if ( valid->level >= FT_VALIDATE_PARANOID )
    {
      p = ends + ( num_segs - 1 ) * 2;
        ...
        if ( start <= last_end && n > 0 )
        {
          ...
          else
          {
            ...
            if ( last_start > start || last_end > end )
              Ошибка |= TT_CMAP_FLAG_UNSORTED;
            else
              Ошибка |= TT_CMAP_FLAG_OVERLAPPING;
          }
        }

        if ( offset && offset != 0xFFFFU )
        {
          p += offset;  /* start of glyph ИД array */
  ...
  /***** says about this формат, but here is my conclusion.            *****/
  /*****                                                               *****/
  /***** The purpose of this формат is to easily map UTF-16 text to    *****/
  /***** glyph indices.  Basically, the `char_code' must be in one of  *****/
  /***** the following formats:                                        *****/
  /*****                                                               *****/
  /*****   - A 16-bit значение that isn't part of the Unicode Surrogates  *****/
  /*****     Area (i.e. U+D800-U+DFFF).                                *****/
  /*****                                                               *****/
  /*****   - A 32-bit значение, made of two surrogate values, i.e.. if    *****/
  /*****     `char_code = (char_hi << 16) | char_lo', then both        *****/
  /*****     `char_hi' and `char_lo' must be in the Surrogates Area.   *****/
  /*****      Area.                                                    *****/
  /*****                                                               *****/
  /***** The `is32' table embedded in the charmap indicates whether a  *****/
  /***** given 16-bit значение is in the surrogates area or not.          *****/
  /*****                                                               *****/
  ...
  /*                                                                       */
  /* This header is followed by `count' groups of the following формат:    */
  /*                                                                       */
  /*   start       0              ULONG       first charcode               */
  /*   end         4              ULONG       last charcode                */
  /*   startId     8              ULONG       start glyph ИД for the группа */
  /*                                                                       */
                             ...
                             FT_Bool     next )
  {
    ...
    if ( next )
    {
      ...
      /* if `char_code' is not in any группа, then `mid' is */
      /* the группа nearest to `char_code'                  */
      /*                                                   */
  ...
  /*   start       0          ULONG      first charcode                    */
  /*   end         4          ULONG      last charcode                     */
  /*   glyphId     8          ULONG      glyph ИД for the whole группа      */
  /*                                                                       */
                      ...
                      FT_Validator  valid )
  {
    ...
    {
      /* we start lastVarSel at 1 because a variant selector значение of 0
       * isn't valid.
       */
...
#include "ttcmapc.h"
  ...
  tt_face_build_cmaps( TT_Face  face )
  {
    ...
    /* only recognize формат 0 */
    if ( TT_NEXT_USHORT( p ) != 0 )
    {
      p -= 2;
      FT_ERROR(( "tt_face_build_cmaps:"
                 " unsupported `cmap' table формат = %d\n",
                 TT_PEEK_USHORT( p ) ));
      return SFNT_Err_Invalid_Table;
    }
    ...
    for ( ; num_cmaps > 0 && p + 8 <= limit; num_cmaps-- )
    {
      ...
      if ( offset && offset <= face->cmap_size - 2 )
      {
        ...
        volatile FT_UInt               формат = TT_PEEK_USHORT( cmap );
        ...
        {
          clazz = *pclazz;
          if ( clazz->формат == формат )
          {
            volatile TT_ValidatorRec  valid;
            volatile FT_Error         Ошибка = SFNT_Err_Ok;
              ...
              *((ft_jmp_buf*)&FT_VALIDATOR( &valid )->jump_buffer) ) == 0 )
            {
              ...
              Ошибка = clazz->validate( cmap, FT_VALIDATOR( &valid ) );
            }

            if ( valid.validator.Ошибка == 0 )
            {
              FT_CMap  ttcmap;
                                 ...
                                 cmap, &charmap, &ttcmap ) )
              {
                ...
                ((TT_CMap)ttcmap)->flags = (FT_Int)Ошибка;
              }
            }
            else
            {
