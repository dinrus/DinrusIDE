...
constexpr int lengthTestArray2 = 4;
static const int testArray2[4] = {3, 4, 5, 6};
...
TEST_CASE("SplitVector") {
	...
	SECTION("InsertFromArray") {
		sv.InsertFromArray(0, testArray2, 0, lengthTestArray2);
		REQUIRE(lengthTestArray2 == sv.Length());
		for (int i=0; i<sv.Length(); i++) {
	...
	SECTION("DeleteOne") {
		sv.InsertFromArray(0, testArray2, 0, lengthTestArray2);
		sv.Delete(2);
		REQUIRE((lengthTestArray2-1) == sv.Length());
		REQUIRE(3 == sv[0]);
		REQUIRE(4 == sv[1]);
		REQUIRE(6 == sv[2]);
	}
	...
	SECTION("GetRangeOverGap") {
		sv.InsertFromArray(0, testArray2, 0, lengthTestArray2);
		REQUIRE(lengthTestArray2 == sv.Length());
		int retrieveArray[lengthTestArray2] = {0};
		sv.GetRange(retrieveArray, 0, lengthTestArray2);
		for (int i=0; i<sv.Length(); i++) {
	...
	SECTION("BufferPointer") {
		...
		constexpr int lengthAfterInsertion = 1 + lengthTestArray2;
