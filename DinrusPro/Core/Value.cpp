...
{
	if(пусто_ли()) return Null;
	return данные.особый_ли(BOOL_V) ? (цел)дайСмолл<бул>() :
	       данные.особый_ли(INT64_V) ? (цел)дайСмолл<дол>() :
	       (цел)дайСмолл<дво>();
}
...
{
	if(пусто_ли()) return Null;
	return данные.особый_ли(BOOL_V) ? (дол)дайСмолл<бул>() :
	       данные.особый_ли(INT_V) ? (дол)дайСмолл<цел>() :
	       (дол)дайСмолл<дво>();
}
...
{
	if(пусто_ли()) return Null;
	return данные.особый_ли(BOOL_V) ? (дво)дайСмолл<бул>() :
	       данные.особый_ли(INT_V) ? (дво)дайСмолл<цел>() :
	       (дво)дайСмолл<дол>();
}
...
{
	if(пусто_ли()) return Null;
	return данные.особый_ли(DOUBLE_V) ? (бул)дайСмолл<дво>() :
	       данные.особый_ли(INT_V) ? (бул)дайСмолл<цел>() :
	       (бул)дайСмолл<дол>();
}
...
Ткст Значение::дайИмя() const
{
	...
	Кортеж<ббайт, const char *> *x = найдиКортеж(tp, __countof(tp), данные.дайОсобый());
