...
#define IMAGEFILE  <DinrusIDE2/LayDes/LayDes.iml>
...
void LayDes::Paint(Draw& w)
{
	...
	if(!IsNull(fileerror))
		w.DrawText(16, 16, "ФАЙЛОВАЯ ОШИБКА: " + fileerror, ArialZ(14).Bold(), Red);
	if(IsNull(currentlayout))
		return;
	w.Offset(-sb.Get());
	LayoutData& l = CurrentLayout();
...
void LayDes::Template(Bar& bar, const String& temp)
{
	...
	bar.Add("Все", THISBACK1(TemplateGroup, TempGroup(temp, String())));
	if((q++ + 2) % 16 == 0)
		bar.Break();
	for(i = 0; i < type.GetCount(); i++) {
		bar.Add(type[i], GetTypeIcon(type[i], w, h, 0, SLtGray),
...
void LayDes::RightDown(Point p, dword keyflags)
{
	...
	int w = 8 * h / 3;
	menu.LeftGap(w + 2);
	menu.Add("Пользовательский класс", THISBACK1(CreateCtrl, ""));
	menu.Separator();
	Index<String> group;
	Vector<String> type;
	int i;
	...
	menu.Add("Все", THISBACK1(Group, String()));
	menu.Add("Шаблоны", THISBACK(Templates));
	if((q++ + 2) % 16 == 0)
		menu.Break();
	for(i = 0; i < type.GetCount(); i++) {
...
void LayDes::AddLayout(bool insert)
{
	String name;
	for(;;) {
		if(!EditText(name, "Добавить новую выкладку", "Выкладка", CharFilterCid))
			return;
		CParser p(name);
		if(p.IsId())
			break;
		Exclamation("Недопустимое имя!");
	}
	int q = list.GetKey();
...
void LayDes::DuplicateLayout()
{
	...
	String name = c.name;
	for(;;) {
		if(!EditText(name, "Дублировать выкладку", "Выкладка", CharFilterCid))
			return;
		CParser p(name);
		if(p.IsId())
			break;
...
void LayDes::RenameLayout()
{
	...
	if(!EditText(name, "Переименовать выкладку", "Выкладка", CharFilterCid))
		return;
	int q = list.GetKey();
...
{
	if(IsNull(currentlayout) || !PromptYesNo("Удалить [* " + DeQtf(layout[currentlayout].name) + "] ?"))
		return;
	int q = list.GetKey();
...
void LayDes::LayoutMenu(Bar& bar)
{
	...
	bar.Add("Добавить новую выкладку..", THISBACK1(AddLayout, false));
	bar.Add("Вставить новую выкладку..", THISBACK1(AddLayout, true));
	bar.Add(iscursor, "Дублировать выкладку..", THISBACK(DuplicateLayout));
	bar.Add(iscursor, "Переименовать выкладку..", THISBACK(RenameLayout));
	bar.Add(iscursor, "Удалить выкладку..", THISBACK(RemoveLayout));
	bar.Separator();
	int q = list.GetKey();
