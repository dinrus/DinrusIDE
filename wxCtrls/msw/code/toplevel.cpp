...
#include <wxCtrls/toplevel.h>
    ...
    #include <wxCtrls/containr.h>        // wxSetFocusToChild()
...
void wxTopLevelWindowMSW::OnActivate(wxActivateEvent& event)
{
    if ( event.GetActive() )
    {
        ...
        // have it
        wxLogTrace(_T("focus"), _T("wxTLW %08x activated."), (int)(WXLPARAM) m_hWnd);
    ...
    else // deactivating
    {
        ...
        wxLogTrace(_T("focus"),
                   ...
                   (int)(WXLPARAM) m_hWnd,
                   (int)(WXLPARAM) (m_winLastFocused ? GetHwndOf(m_winLastFocused)
                                           : NULL));
