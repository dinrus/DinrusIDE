...
NestEditorDlg::NestEditorDlg()
{
	...
	nests.WhenLeftDouble = edit.SetImage(IdeImg::pencil()) ^= [=] {
		if(nests.IsCursor()) {
			...
			if(EditText(h, "Гнездовая папка", "Папка"))
				nests.Set(0, h);
		}
	};
...
bool BaseSetupDlg::Run(String& vars)
{
	...
	while(TopWindow::Run() == IDOK)
	{
		...
		if(varname != vars)
		{
			if(FileExists(varfile) && !PromptOKCancel(Format("Переписать существующую сборку [* \1%s\1]?", varfile)))
				continue;
			if(!SaveVars(varname))
			{
				Exclamation(Format("Ошибка при записи сборки [* \1%s\1].", VarFilePath(varname)));
				continue;
			}
		}
		SetVar("UPP", ~upp);
