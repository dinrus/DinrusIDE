...
#include <solv/repo.h>
#include <solv/util.h>
#include <solv/ext/repo_autopattern.h>
...
repo_add_autopattern(Repo *repo, int flags)
{
  ...
  for (i = 0; i < patq2.count; i += 2)
      ...
      if (!data)
	{
      ...
      s2->vendor = s->vendor;
      /* add link requires */
      s2->requires_ = repo_addid_dep(repo, s2->requires_, pool_rel2id(pool, s->name, s->evr, REL_EQ, 1) , 0);
  ...
  for (i = 0; i < prdq2.count; i += 2)
	...
	continue;		/* eek, not for installed packages, please! */
      ...
      s2->vendor = s->vendor;
      /* add link requires */
      s2->requires_ = repo_addid_dep(repo, s2->requires_, prv, 0);
      if (!autoproduct_id)
