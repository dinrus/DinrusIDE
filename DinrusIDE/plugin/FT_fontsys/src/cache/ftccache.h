...
#define _FTC_FACE_ID_HASH( i )                                                \
  ...
  /*                                                                       */
  /* A node can be anything, depending on the тип of information held by  */
  ...
  typedef struct  FTC_NodeRec_
  {
    ...
    FT_UShort       cache_index;  /* Индекс of cache the node belongs to  */
...
#define FTC_NODE__TOP_FOR_HASH( cache, hash )            \
  ...
  /* compare a node to a given ключ pair */
  typedef FT_Bool
  ...
  typedef struct  FTC_CacheRec_
  {
    ...
    FTC_Manager        manager;
    FT_Memory          memory;
    FT_UInt            Индекс;       /* in manager's table     */
...
#define FTC_CACHE_P( x )  ( (FTC_Cache*)(x) )
  ...
  /* удали all nodes that relate to a given face_id.  This is useful
...
#define FTC_CACHE_LOOKUP_CMP( cache, nodecmp, hash, query, node, Ошибка ) \
  FT_BEGIN_STMNT                                                         \
    ...
    node  = NULL;                                                        \
                                                                         \
    /* иди to the `top' node of the list sharing same masked hash */      \
    ...
    goto _Ok;                                                            \
                                                                         \
  _NewNode:                                                              \
    Ошибка = FTC_Cache_NewNode( _cache, _hash, query, &_node );           \
                                                                         \
  _Ok:                                                                   \
    node = _node;                                                        \
  FT_END_STMNT
...
#define FTC_CACHE_LOOKUP_CMP( cache, nodecmp, hash, query, node, Ошибка ) \
  FT_BEGIN_STMNT                                                         \
    Ошибка = FTC_Cache_Lookup( FTC_CACHE( cache ), hash, query,           \
                              (FTC_Node*)&(node) );                      \
  FT_END_STMNT
