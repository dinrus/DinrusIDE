		...
		if(IsFunction(cm.kind) && li >= 0) { // do local variables
			for(const AnnotationItem& m : editor.locals) {
				...
				if(m.id == ref_id && m.pos.y >= cm.pos.y && m.pos.y <= li && m.pos.y > ppy) {
					...
					found_nest = m.nest;
					found_id = m.id;
					PutAssist("Найден Локальный: " + AsString(m.pos));
					if(ref_pos == m.pos)
						break;
				}
			}
				...
				for(const AnnotationItem& m : f.value.items)
					if(m.id == ref_id &&
					   (IsNull(found_path) ||
						...
						found_nest = m.nest;
						found_id = m.id;
						PutAssist("Найден Глобальный: " + found_path);
					}
...
void Ide::FindDesignerItemReferences(const String& id, const String& name)
{
	...
	Exclamation("Не найдено о применении.");
}
