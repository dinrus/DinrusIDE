...
Вектор<dword> AAGetMap(int& dmin, int& dmax, int dclipmin, int dclipmax,
...
//			LOG("-> " << map[rec] << " + " << map[rec + 1]);
	...
	for(int t = 0; t < (int)map[MAP_COUNT]; t++)
	{
		...
		ПРОВЕРЬ(offs >= times * smin && offs + segspan <= times * smax);
	}
#endif
...
static void BltAAMapRGBA1(dword *dest, const КЗСА *s, const dword *map)
{
...
static void BltAAMapRGBA2(dword *dest, const КЗСА *s, const dword *map)
{
	int count = map[MAP_COUNT];
	map += 4;
	while(count--) {
...
static void BltAAMapRGBA3(dword *dest, const КЗСА *s, const dword *map)
{
	int count = map[MAP_COUNT];
	map += 4;
	while(count--) {
...
static void BltAAMapRGBA4(dword *dest, const КЗСА *s, const dword *map)
{
...
void BltAASet2Fix(КЗСА *dest, const dword *src1, dword w1, const dword *src2, dword w2, int count)
{
	while(count--) {
...
void BltAASet4Fix(КЗСА *dest, const dword *src1, const dword *src2,
...
void BltAAFix2(КЗСА *dest, const dword *src, int count)
{
...
void RescaleImage::создай(Размер _tsz, Raster& _src, const Прям& src_rc)
{
	y = -1;
	src = &_src;
	tsz = _tsz;
	...
	row_buffers.размести(cx4 * segment);
	first = vert[4];
	full = 0;
	offsets = vert.дайОбх(4);
	offset = 0;
	y = 0;
	cii = 0;
...
const КЗСА *RescaleImage::дайСтроку(int ii)
{
	if(cache[0].ii == ii)
		return cache[0].line;
	if(cache[1].ii == ii)
	...
	cache[cii].ii = ii;
	const КЗСА *l = cache[cii].line;
	cii = (cii + 1) % 4;
	return l;
}

void RescaleImage::дай(КЗСА *tgt)
{
	if(y < 0 || offsets >= vert.стоп()) {
		memset(tgt, 0, sizeof(КЗСА) * tsz.cx);
		return;
	}
	offset += *offsets++;
	ПРОВЕРЬ(offset >= 0 && offset + segspan <= size.cy);
	if(bigseg) {
		row_proc(&row_buffers[0 * cx4], дайСтроку(offset + 0 * step), horz);
		row_proc(&row_buffers[1 * cx4], дайСтроку(offset + 1 * step), horz);
		row_proc(&row_buffers[2 * cx4], дайСтроку(offset + 2 * step), horz);
		row_proc(&row_buffers[3 * cx4], дайСтроку(offset + 3 * step), horz);
		...
		for(int next = first + full; next < endoff; next++) {
			if(full >= segment)
				...
				full++;
			row_proc(&row_buffers[next % segment * cx4], дайСтроку(next), horz);
		}
		while(first > offset) {
			if(full < segment)
				full++;
			--first;
			row_proc(&row_buffers[first % segment * cx4], дайСтроку(first), horz);
		}
		ПРОВЕРЬ(offset >= first && endoff <= first + full);
		switch(segment) {
		case 1:
...
bool Rescale(RasterEncoder& tgt, Размер tsz, Raster& src, const Прям& src_rc,
             Врата<int, int> progress)
{
	tgt.создай(tsz, src);
	RescaleImage rs;
	rs.создай(tsz, src, src_rc);
...
Рисунок Rescale(const Рисунок& src, Размер sz, const Прям& src_rc, Врата<int, int> progress)
{
	if(src.дайРазм() == sz && src_rc == sz)
		return src;
...
Рисунок Rescale(const Рисунок& src, Размер sz, Врата<int, int> progress)
{
	return Rescale(src, sz, src.дайРазм(), progress);
}

Рисунок Rescale(const Рисунок& src, int cx, int cy, Врата<int, int> progress)
{
	return Rescale(src, Размер(cx, cy), progress);
}
