...
{
	return (тип & 31) == RASTER_32PREMULTIPLIED && bpos == 0 && gpos == 1 && rpos == 2 && apos == 3;
}
...
void RasterFormat::TailBitsMSB1st(КЗСА *t, const byte *src, int cx, byte shift1, byte andm, byte shift, const КЗСА *palette)
{
	if(cx) {
		byte c = *src;
		while(cx > 0) {
...
void RasterFormat::читай(КЗСА *t, const byte *s, int cx, const КЗСА *palette) const
{
	switch(тип) {
	case RASTER_1:
		{
