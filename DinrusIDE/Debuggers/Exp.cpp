...
Pdb::Val Pdb::Compute(Pdb::Val v1, Pdb::Val v2, int oper)
{
	...
	if(v1.ref || v1.array) {
		...
		case '-': v1.address -= q; break;
		default: ThrowError("Неверная арифметика указателей");
		}
		return v1;
	}
	if(v2.ref || v2.array) {
		...
		if(oper == '+')
			...
			ThrowError("Неверная арифметика указателей");
		return v2;
	}
	Pdb::Val res;
	...
	if(res.type == DBL || res.type == FLT) {
		...
		default: ThrowError("Неверные операнды для операции");
		}
	}
	else {
		int64 a = GetInt64(v1);
...
{
	if(v.ref <= 0)
		ThrowError("Дереференциируются только указатели");
	v = GetRVal(v);
	v.ref--;
	v.rvalue = false;
	return v;
...
{
	if(v.rvalue)
		ThrowError("Нельзя ссылаться на R-value");
	if(v.reference)
		v.reference = false;
	else {
		v.rvalue = true;
...
{
	if(v.type < 0)
		ThrowError("Должно быть структурой, классом или союзом");
	v = Field0(v, field);
	if(IsNull(v.type))
		ThrowError("Неизвестный член \'" + field + "\'");
	return v;
}
...
Pdb::Val Pdb::Term(CParser& p)
{
	...
	if(p.IsChar('\'')) {
		...
		if(s.GetLength() != 1)
			ThrowError("Неверный символный литерал");
		return RValue(s[0]);
	}
	if(p.Char('(')) {
		Val v = Exp0(p);
...
Pdb::Val Pdb::Unary(CParser& p)
{
	...
	if(p.Char('#')) {
		...
		ThrowError("Значение не распознаётся как высокоуровневый тип");
	}
	if(p.Char('*'))
