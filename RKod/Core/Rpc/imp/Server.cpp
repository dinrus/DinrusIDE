...
void ThrowRpcError(int код_, const char *s)
{
	RpcError e;
	e.код_ = код_;
	e.text = s;
...
struct XmlRpcDo {
	...
	Ткст XmlResult();
	Ткст DoXmlRpc();
	Ткст JsonRpcError(int код_, const char *text, const Значение& ид);
	Ткст JsonResult();
...
Ткст XmlRpcDo::JsonRpcError(int код_, const char *text, const Значение& ид)
{
	Json m;
	m("jsonrpc", "2.0");
	МапЗнач err;
