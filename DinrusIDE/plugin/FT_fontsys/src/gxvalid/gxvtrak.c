...
#define FT_COMPONENT  trace_gxvtrak
  ...
  /*************************************************************************/
    ...
    /*
     * referred track table формат specification:
     ...
     * формат           (uint16: 16bit) = 0 is only defined (1996)
...
#define GXV_TRAK_DATA( FIELD )  GXV_TABLE_DATA( trak, FIELD )
                                ...
                                GXV_Validator  valid )
  {
    ...
    for ( i = 0; i < nTracks; i ++ )
    {
      ...
      for ( j = i; j < nTracks; j ++ )
      {
         ...
         t = FT_NEXT_LONG( p );
         if ( t == track )
           GXV_TRACE(( "duplicated entries found for track значение 0x%x\n",
                        track ));
      }
    }
                     ...
                     FT_Validator  ftvalid )
  {
    ...
    /* Reserved фиксирован Значение (always) */
    if ( reserved != 0x0000 )
      FT_INVALID_DATA;
