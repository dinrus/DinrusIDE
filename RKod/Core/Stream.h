...
public:
	...
	bool      открой(const char *имяф, dword mode);

	ФайлПоток(const char *имяф, dword mode);
#endif

#ifdef PLATFORM_POSIX
	bool      открой(const char *имяф, dword mode, mode_t acm = 0644);
	ФайлПоток(const char *имяф, dword mode, mode_t acm = 0644);
