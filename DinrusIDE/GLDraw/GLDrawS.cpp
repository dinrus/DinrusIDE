...
void CheckError()
{
    /* Check for Ошибка conditions. */
    GLenum gl_error = glGetError( );
    if( gl_error != GL_NO_ERROR ) {
        паника(Ткст().конкат() << " OpenGL Ошибка: " << gl_error);
    }
}
...
void initializeGL()
{
	...
	gl_rect.создай(R"(
		...
		ATTRIB_VERTEX, "a_position",
		ATTRIB_COLOR, "a_color" // we need this per vertex because we want to группа rects
	);
...
void GLDraw::FlushPutRect()
{
	...
	Буфер<GLshort> vertex(8 * put_rect.дайСчёт());
	Буфер<GLubyte> color(12 * put_rect.дайСчёт());
	Буфер<GLushort> Индекс(6 * put_rect.дайСчёт());
	GLshort *v = vertex;
	GLubyte *c = color;
	GLushort *n = Индекс;
	for(int i = 0; i < put_rect.дайСчёт(); i++) {
	    ...
	    *v++ = rc.rect.right; *v++ = rc.rect.top;
		GLubyte r = rc.color.дайК();
		GLubyte g = rc.color.дайЗ();
		GLubyte b = rc.color.дайС();
		*c++ = r; *c++ = g; *c++ = b;
		*c++ = r; *c++ = g; *c++ = b;
		*c++ = r; *c++ = g; *c++ = b;
		*c++ = r; *c++ = g; *c++ = b;
	...
	glDisable(GL_BLEND);
	glDrawElements(GL_TRIANGLES, 6 * put_rect.дайСчёт(), GL_UNSIGNED_SHORT, Индекс);
	glEnable(GL_BLEND);
...
void GLDraw::PutRect(const Прям& rect, Цвет color)
{
	LTIMING("PutRect");
	...
	if(inv) {
		...
		color = Цвет(255, 255, 255);
	}
	else
		glDisable(GL_BLEND);

	GLubyte r = color.дайК();
	GLubyte g = color.дайЗ();
	GLubyte b = color.дайС();
	...
	if(color != InvertColor()) {
		RectColor& rc = put_rect.добавь();
		rc.rect = rect;
		rc.color = color;
		if(put_rect.дайСчёт() > 100)
			FlushPutRect();
		return;
	}
#endif
...
static float *sMakePartial(float *partial, Размер isz, const Прям& s)
{
	РазмерПЗ iszf = isz;
	ПрямПЗ h;
...
void GLDraw::PutImage(Точка p, const Рисунок& img, const Прям& src)
{
	LTIMING("PutImage");
...
void GLDraw::PutImage(Точка p, const Рисунок& img, const Прям& src, Цвет color)
{
	FlushPutRect();
...
void GLDraw::иниц(Размер sz, uint64 context_)
{
	SDraw::иниц(sz);
