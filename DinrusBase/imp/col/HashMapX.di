...
deprecated public class ХэшКарта(К, З) : КоллекцияКарт!(К, З), ПараметрыХэш
{
        ...
        ************************************************************************/
        ...
        public final проц добавь (К ключ, З элемент)
        {
                ...
                цел в = хэшУ(ключ);
                ПараССТ hd = таблица[в];
                if (hd is пусто)
                   {
                   таблица[в] = new ПараССТ(ключ, элемент, hd);
                   инкрСчёт();
                   return;
                   }
                else
                ...
                if (таблица !is пусто)
                   ...
                   for (цел i = 0; i < таблица.length; ++i)
                       ...
                       while (p !is пусто)
                             {
                             ПараССТ n = cast(ПараССТ)(p.следщ());
                             цел в = (p.хэшКлюча() & 0x7FFFFFFF) % новЁмк;
                             p.следщ(newtab[в]);
                             newtab[в] = p;
                             p = n;
                             }
        ...
        ************************************************************************/
        ...
        private final проц удали_(З элемент, бул всеСлучаи)
        {
                ...
                for (цел в = 0; в < таблица.length; ++в)
                    {
                    ЯчейкаССТ hd = таблица[в];
                    ЯчейкаССТ p = hd;
                    ЯчейкаССТ trail = p;
                    while (p !is пусто)
                          {
        ...
        ************************************************************************/
        ...
        public override проц читай (ИЧитатель ввод)
        {
                ...
                for (длин=счёт; длин-- > 0;)
                    ...
                    цел в = хэшУ (ключ);
                    таблица[в] = new ПараССТ (ключ, элемент, таблица[в]);
                    }
        }
