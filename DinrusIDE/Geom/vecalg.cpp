...
double расстояние(ТочкаПЗ X, ТочкаПЗ A, ТочкаПЗ B, double *arg)
{
	ТочкаПЗ AB = B - A;
	ТочкаПЗ AX = X - A;
	double ab2 = вКвадрате(AB);
	double abx = AB ^ AX;
	if(abx <= 0 || ab2 == 0)
	{
		if(arg)
...
double расстояние(ТочкаПЗ X, ТочкаПЗ A, ТочкаПЗ B, double bulge, double* arg)
{
	if(bulge == 0)
		return расстояние(X, A, B, arg);
	return VecArcInfo(A, B, bulge).расстояние(X, arg);
}

double расстояние(ТочкаПЗ X, ТочкаПЗ C, double radius, double *arg)
{
	if(arg)
...
bool сКроссами(const ПрямПЗ& R, ТочкаПЗ A, ТочкаПЗ B)
{
	...
	switch(ax >> 1)
	{
	default: ПРОВЕРЬ(0); // invalid choice
	case 0: temp = (B - A) % (R.низЛево() - A); break;
	case 1: temp = (B - A) % (R.низПраво() - A); break;
	case 2: temp = (B - A) % (R.верхЛево() - A); break;
	case 3: temp = (B - A) % (R.верхПраво() - A); break;
	}
...
bool сКроссами(const ПрямПЗ& R, ТочкаПЗ A, ТочкаПЗ B, double bulge)
{
	...
	return VecArcInfo(A, B, bulge).сКроссами(R);
}

bool сКроссами(const ПрямПЗ& R, ТочкаПЗ C, double radius)
{
	ПрямПЗ box = PointfRectf(C);
	box.инфлируй(radius);
	if(R.содержит(box))
		return true;
	if(!box.пересекается(R))
		return false;
	...
	int temp = 0;
	if(вКвадрате(box.верхЛево())     > radius) temp |= 1;
	if(вКвадрате(box.верхПраво())    > radius) temp |= 2;
	if(вКвадрате(box.низЛево())  > radius) temp |= 4;
	if(вКвадрате(box.низПраво()) > radius) temp |= 8;
	if(temp != 15)
		return temp != 0;
	return (box.left * box.right < 0 || box.top * box.bottom < 0);
}

bool ClipLine(ТочкаПЗ& A, ТочкаПЗ& B, const ПрямПЗ& R)
{
	if(пусто_ли(R) || R.пустой()) // null rectangle
		return false;

	if(R.содержит(A) && R.содержит(B)) // both are in
		return true;
...
bool ClipLine(ТочкаПЗ& A, ТочкаПЗ& B, const Прям& R)
{
	if(R.пустой()) // null rectangle
		return false;
...
bool ClipLine(Точка& A, Точка& B, const Прям& R)
{
	if(R.пустой()) // null rectangle
		return false;
...
ПрямПЗ GetBoundingBox(const Массив<ТочкаПЗ>& vertices)
{
	if(vertices.пустой())
		return Null;
	ПрямПЗ rc = PointfRectf(vertices[0]);
	for(int i = 1; i < vertices.дайСчёт(); i++)
...
Прям GetBoundingBox(const Точка *vertices, int vertex_count)
{
	if(vertex_count == 0)
		return Null;
	Прям rc(*vertices, Размер(1, 1));
	const Точка *e = vertices + vertex_count;
	while(++vertices < e)
	{
...
{
	return GetBoundingBox(vertices.старт(), vertices.дайСчёт());
}

template <class C, class VP>
class ConvexHullGenerator {
public:
	typedef typename C::ТипЗнач P;
...
template <class C, class VP>
Вектор<int> ConvexHullGenerator<C, VP>::Generate()
{
	...
	if(upper.пустой())
		return out;
	if(upper.дайСчёт() == 1) {
		out.устСчёт(1, upper[0]);
		return out;
	}
	сортируй(upper, *this);
	int nnpt = upper.дайСчёт();
	lower <<= upper;
	int ue = nnpt;
	int le = nnpt;
	...
	if(le > lb && lower[le - 1] == upper[ue - 1])
		le--;
	out.устСчёт(ue + le - lb);
	копируй(out.старт(), upper.старт(), upper.дайОбх(ue));
	const int *b = lower.дайОбх(lb);
	const int *e = lower.дайОбх(le);
	for(int *o = out.дайОбх(ue); e > b; *o++ = *--e)
		;
	return out;
}
...
{
	Вектор<int>& Индекс = (is_lower ? lower : upper);
	for(;;) {
		const P& l1 = points[Индекс[le - 1]];
		const P& r1 = points[Индекс[rb + 0]];
		if(le - lb >= 2) {
			const P& l2 = points[Индекс[le - 2]];
	...
	копируй(Индекс.дайОбх(le), Индекс.дайОбх(rb), Индекс.дайОбх(re));
	return le + (re - rb);
}

Вектор<int> ConvexHullOrder(const Вектор<Точка>& points)
{
	ConvexHullGenerator<Вектор<Точка>, int64> generator(points);
	return generator.Generate();
}

Вектор<int> ConvexHullOrder(const Массив<ТочкаПЗ>& points)
{
	ConvexHullGenerator<Массив<ТочкаПЗ>, double> generator(points);
	return generator.Generate();
}
...
VecArcIterator::VecArcIterator(ТочкаПЗ start, ТочкаПЗ end, double bulge, Обрвыз1<ТочкаПЗ> lineto)
...
VecArcIterator::VecArcIterator(const VecArc& arc, Обрвыз1<ТочкаПЗ> lineto)
...
void VecArcIterator::иди()
{
	ПРОВЕРЬ(precision >= 0);
	last = arc.A;
	last_clip = (пусто_ли(clip) ? 0 : GetClip(arc.A));
	int end_clip = (пусто_ли(clip) ? 0 : GetClip(arc.B));
	arclen = 0;
	if(!пусто_ли(clip) && arcto)
		arclen = Diagonal(clip);
	double l = arc.VecLine::длина();
	...
	{ // divide in two halves
		...
		int centre_clip = пусто_ли(clip) ? 0 : GetClip(centre);
...
void VecArcIterator::Recurse(ТочкаПЗ next, double l, double h, int depth, int next_clip)
{
	...
	{ // bisect arc
		if(l < arclen)
    	{
			ПРОВЕРЬ(!пусто_ли(clip) && arcto);
			if(2 * расстояние(clip, середина(last, next)) <= l + precision)
				if(arcto(next, h))
				{
					last = next;
					last_clip = next_clip;
		...
		ТочкаПЗ normal = право(next - last) * h / l;
		ТочкаПЗ centre = середина(last, next) + normal;
...
int VecArcIterator::GetClip(ТочкаПЗ point) const
{
...
bool VecTolEq(ТочкаПЗ a, ТочкаПЗ b)
{
...
double VecLine::расстояние(ТочкаПЗ point, double* arg) const
{
	if(пусто_ли(point) || пусто_ли(*this))
		return Null;
	return РНЦП::расстояние(point, A, B, arg);
}

//////////////////////////////////////////////////////////////////////

ТочкаПЗ VecLine::середина() const
{
	return РНЦП::середина(A, B);
}

//////////////////////////////////////////////////////////////////////

ТочкаПЗ VecLine::право() const
{
	return РНЦП::право(Вектор());
}

//////////////////////////////////////////////////////////////////////

ТочкаПЗ VecLine::лево() const
{
	return РНЦП::лево(Вектор());
}

//////////////////////////////////////////////////////////////////////

ТочкаПЗ VecLine::Вектор() const
{
	return B - A;
}
...
VecLine& VecLine::SetClip(const ПрямПЗ& R)
{
	if(пусто_ли(*this)) // null line
		return *this;
...
double VecLine::GetArg(ТочкаПЗ pt) const
{
	ТочкаПЗ AB = Вектор();
	return minmax<double>(((pt - A) ^ AB) / вКвадрате(AB), 0, 1);
}

//////////////////////////////////////////////////////////////////////

ТочкаПЗ VecLine::GetPointAt(ТочкаПЗ A, ТочкаПЗ B, double arg)
{
	return A + (B - A) * arg;
}
...
ТочкаПЗ VecLine::GetPointAt(double arg) const
{
...
ПрямПЗ VecArc::GetBoundingBox() const
{
	ПрямПЗ result = SortRectf(A, B);
	double l = A | B;
	...
	ТочкаПЗ C = РНЦП::середина(A, B) + РНЦП::право(B - A) * ((bulge - r) / l);
	...
	q1 = ((1 << q2) - 1) ^ ((1 << q1) - 1);
	if(q1 & 0x11)
		SetUnion(result, C + ТочкаПЗ(0, +r));
	if(q1 & 0x22)
		SetUnion(result, C + ТочкаПЗ(-r, 0));
	if(q1 & 0x44)
		SetUnion(result, C + ТочкаПЗ(0, -r));
	if(q1 & 0x88)
		SetUnion(result, C + ТочкаПЗ(+r, 0));
...
ТочкаПЗ VecArc::ArcMid(ТочкаПЗ P, ТочкаПЗ Q, double l, double h)
{
	ТочкаПЗ result = РНЦП::середина(P, Q);
	if(l > Vec_tolerance)
		result += РНЦП::право(Q - P) * (h / l);
	return result;
}

//////////////////////////////////////////////////////////////////////

double VecArc::ArcLength(ТочкаПЗ P, ТочкаПЗ Q, double l, double h)
{
	l *= 0.5;
...
ТочкаПЗ VecArc::GetPointAt(double t) const
{
...
ТочкаПЗ VecArc::GetPointAt(ТочкаПЗ P, ТочкаПЗ Q, double l, double h, double t)
{
	if(l <= Vec_tolerance || fabs(h) <= Vec_tolerance)
		return VecLine::GetPointAt(P, Q, t);
	double k = l * 0.5;
	double lambda = h / k;
	ТочкаПЗ C = РНЦП::середина(P, Q);
...
//	double k4 = k2 * k2;
	...
	if(denom > Vec_tolerance)
		C += РНЦП::право(Q - P) * (4 * l2 * (1 - t) / denom);
	return C;
}

//////////////////////////////////////////////////////////////////////

void VecArc::Bisect(ТочкаПЗ P, ТочкаПЗ Q, double l, double h, ТочкаПЗ& centre, double& ll, double& hh)
{
	centre = РНЦП::середина(P, Q) + РНЦП::право(Q - P) * h / l;
	ll = sqrt(l * l / 4 + h * h);
	hh = h * ll / (2 * ll + l);
}
...
/*
VecVertex::VecVertex()
	: ТочкаПЗ(Null), bulge(0)
{
}

//////////////////////////////////////////////////////////////////////

Ткст VecVertex::Dump() const
{
	return ТочкаПЗ::Dump() + фмт(" (%.10g)", bulge);
}
...
void VecCurve::добавь(const VecVertex& vertex)
{
	Основа::добавь(vertex);
}

//////////////////////////////////////////////////////////////////////

void VecCurve::добавь(ТочкаПЗ point)
{
	Основа::добавь(point);
}
...
void VecCurve::добавь(ТочкаПЗ point, double bulge)
{
	Основа::добавь(VecVertex(point, bulge));
}

//////////////////////////////////////////////////////////////////////

void VecCurve::добавь(double x, double y, double bulge)
{
	Основа::добавь(VecVertex(x, y, bulge));
}

//////////////////////////////////////////////////////////////////////

Ткст VecCurve::Dump() const
{
	if(пустой())
		return "Null(VecCurve)";
	Ткст dump = '(' + (*this)[0].Dump();
	for(int i = 1; i < дайСчёт(); i++)
		dump << " -> " << (*this)[i].Dump();
	return dump + ')';
}
...
Вектор<ТочкаПЗ> VecCurve::Check() const
{
	Вектор<ТочкаПЗ> errors;
	int count = дайСчёт();
	if(count < 2)
	...
	{ // special case: segment bulges & vertices should differ
		...
		if(VecTolEq(A, B))
			...
			errors.добавь(VecArc(A, B, B.bulge).ArcMid());
		return errors;
	}
	return errors;
}
...
//////////////////////////////////////////////////////////////////////
// уст: sets arc defined by centre, endpoints & orientation.

void VecArcInfo::уст(ТочкаПЗ _C, ТочкаПЗ _A, ТочкаПЗ _B, bool anticlockwise)
{
	A = _A;
	B = _B;
	C = _C;
	...
	bow = A | B;
	alpha = РНЦП::Bearing(A - C);
	beta = РНЦП::Bearing(B - C);
	radius = B | C;
	circle = (A == B);
...
//////////////////////////////////////////////////////////////////////
// уст: sets up a line segment (degenerated arc).
//////////////////////////////////////////////////////////////////////

void VecArcInfo::уст(ТочкаПЗ _P, ТочкаПЗ _Q)
{
	A        = _P;
	B        = _Q;
	bow      = A | B;
...
//////////////////////////////////////////////////////////////////////
// уст: sets up arc defined by endpoints and a point on arc.
//////////////////////////////////////////////////////////////////////

void VecArcInfo::уст(ТочкаПЗ _P, ТочкаПЗ _Q, ТочкаПЗ X)
{
	circle = false;
	...
	double x2  = РНЦП::вКвадрате(X);
	ТочкаПЗ     AX  = X - _P;
	ТочкаПЗ     BX  = X - _Q;
	double m1x = (x2 - РНЦП::вКвадрате(_P)) / 2;
	double m1y = (x2 - РНЦП::вКвадрате(_Q)) / 2;
	double det = AX % BX;
	ТочкаПЗ     dc(m1x * BX.y - m1y * AX.y, m1y * AX.x - m1x * BX.x);
...
//////////////////////////////////////////////////////////////////////
// уст: sets up arc defined by chord and bulge.
//////////////////////////////////////////////////////////////////////

void VecArcInfo::уст(ТочкаПЗ _P, ТочкаПЗ _Q, double _bulge)
{
	circle = false;
	...
	C      = РНЦП::середина(A, B) + РНЦП::право(B - A) * ((_bulge - radius) / bow);
	alpha  = РНЦП::Bearing(A - C);
	beta   = РНЦП::Bearing(B - C);
...
//////////////////////////////////////////////////////////////////////
// уст: sets up arc defined by centre, radius & angles.
//////////////////////////////////////////////////////////////////////

void VecArcInfo::уст(ТочкаПЗ _C, double r, double a, double b)
{
...
//////////////////////////////////////////////////////////////////////
// уст: sets the arc to a full circle.
//////////////////////////////////////////////////////////////////////

void VecArcInfo::уст(ТочкаПЗ _C, double r)
{
	circle   = true;
	curved   = true;
	reversed = false;
...
ТочкаПЗ VecArcInfo::CentreOfMass() const
{
	if(IsCircle())
		return C;
	ТочкаПЗ M = РНЦП::середина(A, B);
	if(!IsCurved())
		return M;
	double extrusion = bow / (beta - alpha) - radius + fabs(bulge);
	return M + UnitV(IsReversed() ? РНЦП::лево(M) : РНЦП::право(M)) * extrusion;
}

//////////////////////////////////////////////////////////////////////

void VecArcInfo::Move(ТочкаПЗ offset)
{
	if(!пусто_ли(A)) A += offset;
	if(!пусто_ли(B)) B += offset;
	if(!пусто_ли(C)) C += offset;
}

//////////////////////////////////////////////////////////////////////

double VecArcInfo::длина() const
{
	if(IsCircle())
		return TWOPI * radius;
	if(IsCurved())
...
ТочкаПЗ VecArcInfo::GetPointAt(double t) const
{
	if(IsCircle())
...
Ткст Dump(const VecArcInfo& arc)
{
	Ткст output;
	if(arc.IsCircle())
	{
...
//////////////////////////////////////////////////////////////////////
// расстояние: distance between arc and point.

double VecArcInfo::расстояние(ТочкаПЗ X, double* arg) const
{
	if(IsCircle())
	{
		if(arg)
	...
	if(!IsCurved())
		return РНЦП::расстояние(X, A, B, arg);
...
double VecArcInfo::GetMaxDistance(ТочкаПЗ point, ТочкаПЗ *farthest) const
{
	if(IsCircle() || IsCurved() && ContainsBearing(РНЦП::Bearing(C, point)))
	{
		double distance = (point | C) + radius;
		if(farthest)
			*farthest = point + РНЦП::длина(C - point, radius);
		return distance;
	}
...
bool VecArcInfo::сКроссами(const ПрямПЗ& R) const
{
	if(пусто_ли(*this))
		return false;

	if(!IsCurved())
		return РНЦП::сКроссами(R, A, B);

	if(R.содержит(A) || R.содержит(B))
		return true;

	if(!РНЦП::сКроссами(R, C, radius))
		return false;
	...
	ТочкаПЗ K = R.верхЛево();
	ТочкаПЗ L = R.верхПраво();
	ТочкаПЗ M = R.низПраво();
	ТочкаПЗ N = R.низЛево();
...
double VecArcInfo::GetArg(ТочкаПЗ point) const
{
	if(!IsCurved())
		return VecLine::GetArg(point);
	return GetArg(РНЦП::Bearing(point, C));
}

//////////////////////////////////////////////////////////////////////

ПрямПЗ VecArcInfo::GetBoundingBox() const
{
	if(IsCircle())
		return ПрямПЗ(C.x - radius, C.y - radius, C.x + radius, C.y + radius);
	else if(IsCurved())
...
ТочкаПЗ VecArcInfo::GetStartDir() const
{
	if(IsCurved())
	{
		ТочкаПЗ CA = РНЦП::лево(A - C);
		if(IsReversed())
			CA = -CA;
		return CA;
	}
...
ТочкаПЗ VecArcInfo::GetEndDir() const
{
	if(IsCurved())
	{
		ТочкаПЗ CB = РНЦП::лево(B - C);
		if(IsReversed())
			CB = -CB;
		return CB;
	}
...
{
	return РНЦП::Bearing(GetStartDir());
}
...
{
	return РНЦП::Bearing(GetEndDir());
}

//////////////////////////////////////////////////////////////////////

VecArcInfo VecArcInfo::смещение(double dist) const
{
	if(dist == 0)
		return *this;
	ТочкаПЗ last = A + РНЦП::UnitV(РНЦП::право(GetStartDir())) * dist;
	ТочкаПЗ next = B + РНЦП::UnitV(РНЦП::право(GetEndDir())) * dist;
	if(IsCurved())
...
VecArcInfo VecArcInfo::Subset(double start, double end) const
{
	if(IsCircle())
	{
		...
		if(IsReversed())
		{
			info.уст(C, radius, alpha - end * TWOPI, alpha - start * TWOPI);
			info.SetReversed();
		}
		else
			info.уст(C, radius, alpha + start * TWOPI, alpha + end * TWOPI);
		return info;
	}
	else if(IsCurved())
...
void VecIntersection::удали(int Индекс)
{
	ПРОВЕРЬ(Индекс >= 0 && Индекс < count);
	if(--count > Индекс && Индекс == 0)
	{
		t[0] = t[1];
		u[0] = u[1];
	}
...
bool VecIntersection::LL(const ТочкаПЗ& P1, const ТочкаПЗ& Q1, const ТочкаПЗ& P2, const ТочкаПЗ& Q2)
{
...
bool VecIntersection::LC(const ТочкаПЗ& P1, const ТочкаПЗ& Q1, const ТочкаПЗ& C2, double r2)
{
	ТочкаПЗ PQ    = Q1 - P1;
	ТочкаПЗ CP    = P1 - C2;
	double a = вКвадрате(PQ);
...
//			ТочкаПЗ temp = ТочкаПЗ::поляр(C2, r2, u[0] * TWOPI);
...
bool VecIntersection::LA(const ТочкаПЗ& P1, const ТочкаПЗ& Q1, const VecArcInfo &a2)
{
	if(!a2.IsCurved())
...
bool VecIntersection::CC(const ТочкаПЗ& C1, double r1, const ТочкаПЗ& C2, double r2)
{
	if(fabs(r1) <= Vec_tolerance || fabs(r2) <= Vec_tolerance
		|| (C1 | C2) >= r1 + r2 + Vec_tolerance)
		return Nothing();
	ТочкаПЗ CC  = C2 - C1;
	ТочкаПЗ P   = C1 + CC * (0.5 + (r1 * r1 - r2 * r2) / (2 * вКвадрате(CC)));
	ТочкаПЗ CP1 = P - C1;
	ТочкаПЗ CP2 = P - C2;
	double r;
	if(r1 >= r2)
		r = r1 * r1 - вКвадрате(CP1);
	else
		r = r2 * r2 - вКвадрате(CP2);
	double tol2 = Vec_tolerance * Vec_tolerance;

	if(r > tol2)
	{ // two solutions
		ТочкаПЗ D = UnitV(право(CC)) * sqrt(r);
		count = 2;
...
bool VecIntersection::CA(const ТочкаПЗ& C1, double r1, const VecArcInfo &a2)
{
	if(!a2.IsCurved())
	{
		LC(a2.A, a2.B, C1, r1);
