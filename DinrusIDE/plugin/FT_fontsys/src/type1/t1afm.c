...
#define FT_COMPONENT  trace_t1afm
  ...
  /* read a glyph имя and return the equivalent glyph index */
  static FT_Int
  t1_get_index( const char*  имя,
                FT_Offset    len,
                void*        user_data )
  {
    ...
    /* PS string/имя length must be < 16-bit */
    if ( len > 0xFFFFU )
      return 0;
    ...
    for ( n = 0; n < type1->num_glyphs; n++ )
    {
      ...
      if ( gname && gname[0] == имя[0]        &&
           ft_strlen( gname ) == len           &&
           ft_strncmp( gname, имя, len ) == 0 )
        return n;
    }
...
#define KERN_INDEX( g1, g2 )  ( ( (FT_ULong)(g1) << 16 ) | (g2) )
               ...
               AFM_FontInfo  fi )
  {
    ...
    for ( n = 0; n < t1_face->num_charmaps; n++ )
    {
      ...
      {
        Ошибка = FT_Set_Charmap( t1_face, charmap );
        if ( Ошибка )
          goto Exit;
        break;
      }
    ...
    if ( oldcharmap != NULL )
      Ошибка = FT_Set_Charmap( t1_face, oldcharmap );
    if ( Ошибка )
      goto Exit;
                   ...
                   FT_Stream  stream )
  {
    ...
    FT_Error       Ошибка   = T1_Err_Unknown_File_Format;
    ...
    {
      Ошибка = psaux->afm_parser_funcs->init( &parser,
                                             stream->memory,
                                             stream->cursor,
                                             stream->limit );
      ...
      if ( !Ошибка )
      {
        ...
        Ошибка = psaux->afm_parser_funcs->parse( &parser );
    ...
    if ( Ошибка == T1_Err_Unknown_File_Format )
    {
      ...
      if ( stream->size > 6                              &&
        ...
        Ошибка = T1_Read_PFM( t1_face, stream, fi );
    }
