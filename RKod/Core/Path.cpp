...
Время дайВремяф(const char *имяф)
{
#if defined(PLATFORM_WIN32)
	HANDLE handle;
...
ФВремя дайФВремя(const char *имяф)
{
#if defined(PLATFORM_WIN32)
	HANDLE handle;
	handle = CreateFileW(вСисНабсимШ(имяф), GENERIC_READ,
...
bool устФВремя(const char *имяф, ФВремя ft)
{
#if defined(PLATFORM_WIN32)
	HANDLE handle;
...
bool устВремяф(const char *имяф, Время time)
{
...
Ткст дайПутьСимСсылки(const char *linkpath)
{
	...
	int len = readlink(linkpath, buff, _МАКС_ПУТЬ);
