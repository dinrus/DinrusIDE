...
inline size_t соЧанк__(C count, MC min_chunk = CO_PARTITION_MIN, MC max_chunk = CO_PARTITION_MAX)
{
...
void соОтсек(Обх begin, Обх end, const Лямбда& lambda, int min_chunk = CO_PARTITION_MIN, int max_chunk = CO_PARTITION_MAX)
{
...
{
	size_t chunk = соЧанк__(r.дайСчёт(), CO_PARTITION_MIN);
	if(chunk) {
		СоРабота co;
		auto begin = r.begin();
		auto end = r.end();
