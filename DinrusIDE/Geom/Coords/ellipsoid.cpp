...
static EPSGEllipsoid epsg_ellipsoids[] =
{
...

	{
		...
		"\"Australian вКарту Grid Technical Manual\"; National Mapping Council of Australia "
		"особый Publication #7; 1972"
	},
...
//*/
		...
		"US Army вКарту Service Technical Manual; 1943."
	},
...

	{
		...
		0,
		{ { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 }, { 0, 0, 0 } },
		"a = 6377397.155 German legal metres. This is the same значение as the Bessel 1841 figure "
...

	{
		...
		"1865 inch-metre ratio of 39.370432 to obtain metres. (Metric значение then converted "
		"to US survey feet for use in the United States using 39.37 exactly giving "
		"a=20925832.16 ft US).", "US Army вКарту Service Technical Manual No. 7; 1943."
	},
...

	{
		...
		0,
		{ { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 }, { 0, 0, 0 } },
		"Adopts Clarke's значение for a with derived 1/f.  Uses his 1865 ratio of 39.370432 "
...

	{
		...
		"from equator to pole equal to 10000000 metres exactly. (Реф: Strasser).",
...

	{
		...
		{ { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 }, { 0, 0, 0 } },
		"",
		"нов Brunswick Geographic Information Corporation land and water information standards manual"
	},
...

	{
		...
		"rounded with slight Ошибка as 1 Ind ft = 0.3047995m exactly and applied to "
...

	{
		...
		0,
		{ { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 }, { 0, 0, 0 } },
		"The semi-major axis has the same значение as the Bessel 1841 ellipsoid (code 7004) "
...

	{
		...
		"as имя and parameter values do not match; replaced by 7048.", "EPSG"
	},
...
ТочкаПЗ GisEllipsoid::From3D(double x, double y, double z) const
{
	double p = hypot(x, y);
	...
	return ТочкаПЗ((2 / DEGRAD) * atan2(y, p + x), atan2(tn, td) / DEGRAD);
}

const Индекс<int>& GisEllipsoid::EnumEPSG()
{
	static Индекс<int> eem;
	if(eem.пустой())
	{
		eem.резервируй(__countof(epsg_ellipsoids));
		for(int i = 0; i < __countof(epsg_ellipsoids); i++)
			eem.добавь(epsg_ellipsoids[i].code);
	}
	return eem;
}
