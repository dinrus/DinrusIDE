...
GLM_FUNC_QUALIFIER void glm_mat4_rotate(__m128 const in[4], float Angle, float const v[3], __m128 out[4])
{
	...
	результат[1] = TmpB4;
	результат[2] = TmpC4;
	результат[3] = _mm_set_ps(1, 0, 0, 0);

	//mat<4, 4, valType> результат;
	//результат[0] = m[0] * Rotate[0][0] + m[1] * Rotate[0][1] + m[2] * Rotate[0][2];
	//результат[1] = m[0] * Rotate[1][0] + m[1] * Rotate[1][1] + m[2] * Rotate[1][2];
	//результат[2] = m[0] * Rotate[2][0] + m[1] * Rotate[2][1] + m[2] * Rotate[2][2];
	//результат[3] = m[3];
	//return результат;
	sse_mul_ps(in, результат, out);
}
*/
