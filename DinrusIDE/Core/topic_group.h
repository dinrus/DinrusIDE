...
#define TOPIC_VERB 0
		...
		#else
			#Ошибка Too many topic files !
		#endif
	#endif
#endif

#define TOPIC(ид)       { const char *topic = ид;
#define TITLE(x)          const char *title = x;
#define COMPRESSED        static const РНЦП::byte данные[] = {
#define END_TOPIC         }; RegisterTopic__(КАКТКСТ(TOPICFILE), topic, title, данные, __countof(данные)); }

ИНИЦБЛОК_(COMBINE3(TOPIC__, TOPIC_VERB, TOPIC_VERA))
{
	#include TOPICFILE
}
