...
FT_Face FTFace(Шрифт fnt, Ткст *rpath);
...
bool RenderOutline(const FT_Outline& outline, Рисовало& path, double xx, double yy)
{
	FT_Vector   v_last;
	FT_Vector   v_control;
	FT_Vector   v_start;
	...
	for(n = 0; n < outline.n_contours; n++) {
		...
		while(point < limit) {
			...
			case FT_CURVE_TAG_ON:
				path.Строка(ft_dbl(point->x) + xx, -ft_dbl(point->y) + yy);
				continue;
...
void PaintCharacterSys(Рисовало& sw, double x, double y, int ch, Шрифт fnt)
{
	DrawLock __;
	PAINTER_TIMING("симвОп");
