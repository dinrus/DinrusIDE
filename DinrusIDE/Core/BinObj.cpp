		...
		if(binid == "BINARY" || ba || bm) {
			...
			if(ba) {
				...
				if(blk.index < 0 || blk.index > 1000000)
					binscript.ThrowError(Format("неверный индекс массива: %d", blk.index));
				binscript.PassChar(',');
			}
			else if(bm) {
			...
			if(ff.Search(searchpath))
				...
				while(ff.Next());
			if(files.IsEmpty())
				binscript.ThrowError(Format("'%s' не найден или не является файлом", file));
			if(!(blk.flags & Block::FLG_MASK) && files.GetCount() > 1)
				binscript.ThrowError(Format("Несколько файлов лбнаружено (напр., %s, %s) в режиме единичного файла", files[0], files[1]));
			...
			for(int i = 0; i < files.GetCount(); i++) {
				...
				if(f >= 0)
					binscript.ThrowError(Format("%s[%d] уже встречалось на строке %d", blk.ident, blk.index, brow[f].scriptline));
				if(blk.index < 0 && !brow.IsEmpty() || blk.index >= 0 && !brow.IsEmpty() && brow[0].index < 0)
					binscript.ThrowError(Format("%s: смешение элементов массива и немассива", blk.ident));
			...
			binscript.ThrowError("ожидался идентификатор элемента двоичного сценария");
	}
}
