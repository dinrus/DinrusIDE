...
#include <solv/ext/tools_util.h>
#include <solv/ext/repo_susetags.h>
#ifdef ENABLE_COMPLEX_DEPS
#include <solv/ext/pool_parserpmrichdep.h>
#endif
#include <solv/ext/repodata_diskusage.h>

#ifdef _WIN32
#include <solv/strfncs.h>
#endif
  ...
  for (;;)
      ...
      if (!s)
	{
	  ...
	  case CTAG('=', 'R', 'e', 'q'):                                        /* requires */
	    s->requires_ = adddep(pool, &pd, s->requires_, line, -SOLVABLE_PREREQMARKER, pd.kind);
	    continue;
          case CTAG('=', 'P', 'r', 'q'):                                        /* pre-requires / packages required */
	    if (pd.kind)
	      s->requires_ = adddep(pool, &pd, s->requires_, line, 0, 0);		/* pattern: package requires */
	    else
	      s->requires_ = adddep(pool, &pd, s->requires_, line, SOLVABLE_PREREQMARKER, 0); /* pre-requires */
	    continue;
