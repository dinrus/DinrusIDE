...
void memset8__(void *t, i16x8 данные, size_t len);

inline
void memset8(void *p, byte данные, size_t count)
{
	byte *t = (byte *)p;
	if(count < 2) {
		if(count)
...
inline
void memset16(void *p, word данные, size_t count)
{
	word *t = (word *)p;
	if(count < 2) {
		if(count)
...
inline
void memset32(void *p, dword данные, size_t count)
{
	dword *t = (dword *)p;
	if(count < 4) {
		if(count & 2) {
			t[0] = t[1] = t[count - 1] = данные;
			return;
		}
		if(count & 1)
			t[0] = данные;
	...
	i32x4 val4 = i32all(данные);
	if(count >= 16) {
		memset8__(t, val4, 4 * count);
		return;
	}
	auto Set128 = [&](size_t at) { val4.сохрани(t + at); };
...
inline
void memset64(void *p, qword данные, size_t count)
{
	qword *t = (qword *)p;
	if(count < 2) {
		if(count)
	...
	i16x8 val2 = i64all(данные);
	if(count >= 8) {
		memset8__(t, val2, 8 * count);
		return;
	}
	auto Set128 = [&](size_t at) { val2.сохрани(t + at); };
...
inline
void memset128(void *t, m128 данные, size_t count)
{
	if(count)
		memset8__(t, i16x8().грузи(&данные), 16 * count);
}
...
void memcpy8(void *p, const void *q, size_t count)
{
	...
	if(count <= 32) { // improves Ткст::LSet
		auto Copy128 = [&](size_t at) { i16x8(s + at).сохрани(t + at); };
		Copy128(count - 16);
		Copy128(0);
		return;
	}
...
bool inline_memeq8_aligned(const void *p, const void *q, size_t count)
{
	...
	if(count >= 16) // 15..31 is the most important range for Ткст, make it fastest
		return memeq8__(t, s, count);
	if(count > 4) {
		if(count <= 8) // test tail first in case we are searching in sorted list
			return подбери32(s + count - 4) == подбери32(t + count - 4) && подбери32(s) == подбери32(t);
		return подбери64(s + count - 8) == подбери64(t + count - 8) && подбери64(s) == подбери64(t);
	}
	if(count < 2)
		return count ? t[0] == s[0] : true;
	if(подбери16(s + count - 2) != подбери16(t + count - 2))
		return false;
	return count > 2 ? подбери16(s) == подбери16(t) : true;
}
...
bool inline_memeq16_aligned(const void *p, const void *q, size_t count)
{
	...
	if(count <= 2)
		return count ? подбери16(s + count - 1) == подбери16(t + count - 1) && подбери16(s) == подбери16(t) : true;
	if(count <= 8) {
		if(count <= 4)
			return подбери32(s + count - 2) == подбери32(t + count - 2) && подбери32(s) == подбери32(t);
		return подбери64(s + count - 4) == подбери64(t + count - 4) && подбери64(s) == подбери64(t);
	}
...
bool inline_memeq32_aligned(const void *p, const void *q, size_t count)
{
	...
	if(count <= 4) {
		if(count <= 2)
			return count ? подбери32(s + count - 1) == подбери32(t + count - 1) && подбери32(s) == подбери32(t) : true;
		return подбери64(s + count - 2) == подбери64(t + count - 2) && подбери64(s) == подбери64(t);
	}
...
bool inline_memeq64_aligned(const void *p, const void *q, size_t count)
{
	...
	if(count <= 2)
		return count ? подбери64(s + count - 1) == подбери64(t + count - 1) && подбери64(s) == подбери64(t) : true;
...
template <class T>
void memset__(void *p, T данные, size_t count)
{
	T *t = (T *)p;
	while(count >= 8) {
		t[0] = t[1] = t[2] = t[3] = t[4] = t[5] = t[6] = t[7] = данные;
		t += 8;
		count -= 8;
	}
	if(count & 4) {
		t[0] = t[1] = t[2] = t[3] = данные;
		t += 4;
	}
	if(count & 2) {
		t[0] = t[1] = данные;
...
{
	if(count >= 64) {
		memset(p, данные, count);
		return;
	}
	memset__<byte>(p, данные, count);
}

inline
void memset16(void *p, word данные, size_t count)
{
	memset__<word>(p, данные, count);
}

inline
void memset32(void *p, dword данные, size_t count)
{
	memset__<dword>(p, данные, count);
}

inline
void memset64(void *p, qword данные, size_t count)
{
	memset__<qword>(p, данные, count);
}

inline
void memset128(void *p, m128 данные, size_t count)
{
	memset__<m128>(p, данные, count);
}
...
int inline_memcmp_aligned(const char *a, const char *b, size_t count)
{
	...
	if(count & 4) {
		...
		uint32 b32 = *(uint32 *)b;
		if(a32 != b32)
			return эндианРазворот32(a32) < эндианРазворот32(b32) ? -1 : 1;
		a += 4;
		b += 4;
	}
#else
	...
	if(count & 2) {
		...
		uint16 b16 = *(uint16 *)b;
		if(a16 != b16)
			return эндианРазворот16(a16) < эндианРазворот16(b16) ? -1 : 1;
		a += 2;
		b += 2;
	}
	if((count & 1) != 0 && *a != *b)
