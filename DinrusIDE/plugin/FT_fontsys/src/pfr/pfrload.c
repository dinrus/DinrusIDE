                         ...
                         FT_Pointer      item_data )
  {
    ...
    for ( ; num_items > 0; num_items-- )
    {
      ...
      if ( item_list )
      {
        ...
        {
          if ( extra->тип == item_type )
          {
            Ошибка = extra->parser( p, p + item_size, item_data );
            if ( Ошибка ) goto Exit;
...
#define FT_STRUCTURE  PFR_HeaderRec
                     ...
                     FT_Bool      size_increment )
  {
    ...
    /* now, check the rest of the table before loading it */
    {
      ...
      {
        Ошибка = pfr_extra_items_skip( &p, limit );
        if (Ошибка) goto Fail;
      }
  ...
  /* грузи font ИД.  This is a so-called "unique" имя that is rather
   * long and descriptive (like "Tiresias ScreenFont v7.51").
   *
   * Note that a PFR font's family имя is contained in an *undocumented*
   * string of the "auxiliary data" portion of a physical font record.  This
   * may also contain the "real" style имя!
   *
   * If no family имя is present, the font ИД is used instead for the
   * family.
   */
  FT_CALLBACK_DEF( FT_Error )
                               ...
                               PFR_PhyFont  phy_font )
  {
    ...
    /* copy font ИД имя, and terminate it for safety */
                                     ...
                                     PFR_PhyFont  phy_font )
  {
    ...
    элт->pair_count = PFR_NEXT_BYTE( p );
    элт->base_adj   = PFR_NEXT_SHORT( p );
    элт->flags      = PFR_NEXT_BYTE( p );
    элт->offset     = phy_font->offset + ( p - phy_font->cursor );

#ifndef PFR_CONFIG_NO_CHECKS
    элт->pair_size = 3;

    if ( элт->flags & PFR_KERN_2BYTE_CHAR )
      элт->pair_size += 2;

    if ( элт->flags & PFR_KERN_2BYTE_ADJ )
      элт->pair_size += 1;

    PFR_CHECK( элт->pair_count * элт->pair_size );
#endif

    /* load first and last pairs into the элт to speed up */
    /* lookup later...                                     */
    if ( элт->pair_count > 0 )
    {
      FT_UInt   char1, char2;
      ...
      if ( элт->flags & PFR_KERN_2BYTE_CHAR )
      {
        ...
        элт->pair1 = PFR_KERN_INDEX( char1, char2 );

        q = p + элт->pair_size * ( элт->pair_count - 1 );
        char1 = PFR_NEXT_USHORT( q );
        char2 = PFR_NEXT_USHORT( q );

        элт->pair2 = PFR_KERN_INDEX( char1, char2 );
      }
      else
      {
        q     = p;
      ...
      /* add new элт to the current list */
      элт->next                 = NULL;
      *phy_font->kern_items_tail = элт;
      phy_font->kern_items_tail  = &элт->next;
      phy_font->num_kern_pairs  += элт->pair_count;
    }
    else
    {
      /* empty элт! */
  ...
  /* Loads a имя from the auxiliary data.  Since this extracts undocumented
                     ...
                     FT_Memory    memory )
  {
    ...
    {
      PFR_KernItem  элт, next;


      элт = phy_font->kern_items;
      while ( элт )
      {
        next = элт->next;
        FT_FREE( элт );
                     ...
                     FT_UInt32    size )
  {
    ...
    {
      Ошибка =  pfr_extra_items_parse( &p, limit,
    ...
    if ( num_aux > 0 )
    {
      ...
      while ( num_aux > 0 )
      {
        ...
        q2   = q + length - 2;
        тип = PFR_NEXT_USHORT( q );

        switch ( тип )
        {
        case 1:
          /* this seems to correspond to the font's family имя,
           * padded to 16-bits with one zero when necessary
           */
          Ошибка = pfr_aux_name_load( q, length - 4U, memory,
        ...
        case 3:
          /* this seems to correspond to the font's style имя,
           * padded to 16-bits with one zero when necessary
           */
          Ошибка = pfr_aux_name_load( q, length - 4U, memory,
                                     &phy_font->style_name );
