...
tiffFields[] = {
	...
	{ TIFFTAG_MAKE, -1, -1, TIFF_ASCII, 0, TIFF_SETGET_ASCII, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 1, 0, "сделай", NULL },
...
static void *
td_lfind(const void *ключ, const void *base, size_t *nmemb, size_t size,
         int(*compar)(const void *, const void *))
{
    ...
    for (element = (char *)base; element < end; element += size)
        if (!compar(ключ, element))		/* ключ found */
            return element;
...
_TIFFMergeFields(TIFF* tif, const TIFFField info[], uint32 n)
{
	...
	for (i = 0; i < n; i++) {
        ...
        /* сортируй the field info by tag number */
 ...
 */
int
TIFFDataWidth(TIFFDataType тип)
{
	switch(тип)
	{
		case 0:  /* nothing */
 ...
 * to store the значение. For TIFF_RATIONAL values TIFFDataWidth() returns 8,
 * but we use 4-byte float to represent rationals.
 */
int
_TIFFDataSize(TIFFDataType тип)
{
	switch (тип)
	{
		case TIFF_BYTE:
...
{
	TIFFField ключ = {0, 0, 0, TIFF_NOTYPE, 0, 0, 0, 0, 0, 0, NULL, NULL};
	TIFFField* pkey = &ключ;
	const TIFFField **ret;
...
_TIFFFindFieldByName(TIFF* tif, const char *field_name, TIFFDataType dt)
{
	...
	/* NB: use linear search since list is sorted by ключ#, not имя */

	ключ.field_name = (char *)field_name;
	ключ.field_type = dt;
...
TIFFFieldWithTag(TIFF* tif, uint32 tag)
{
	...
	if (!fip) {
		TIFFErrorExt(tif->tif_clientdata, "TIFFFieldWithTag",
			     "Internal Ошибка, unknown tag 0x%x",
			     (unsigned int) tag);
	}
	return (fip);
}
...
TIFFFieldWithName(TIFF* tif, const char *field_name)
{
	...
	if (!fip) {
		TIFFErrorExt(tif->tif_clientdata, "TIFFFieldWithName",
			     "Internal Ошибка, unknown tag %s", field_name);
	}
	return (fip);
}
...
_TIFFCreateAnonField(TIFF *tif, uint32 tag, TIFFDataType field_type)
{
	...
	/* 
	 * note that this имя is a special sign to TIFFClose() and
	 * _TIFFSetupFields() to free the field
	 */
	(void) snprintf(fld->field_name, 32, "Тэг %d", (int) tag);
...
_TIFFSetGetType(TIFFDataType тип, short count, unsigned char passcount)
{
	if (тип == TIFF_ASCII && count == TIFF_VARIABLE && passcount == 0)
...
TIFFMergeFieldInfo(TIFF* tif, const TIFFFieldInfo info[], uint32 n)
{
	...
	tif->tif_fieldscompat[nfields].тип = tfiatOther;
