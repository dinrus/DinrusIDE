...
#define GXV_OPBD_DATA( FIELD )  GXV_TABLE_DATA( opbd, FIELD )
                                 ...
                                 GXV_Validator        valid )
  {
    ...
    for ( i = 0; i < 4; i++ )
    {
      ...
      if ( GXV_OPBD_DATA( формат ) )    /* формат 1, значение is ctrl pt. */
      {
        if ( delta_value == -1 )
          continue;
      ...
      else                              /* формат 0, значение is distance */
        continue;
    }
  }
                               ...
                               GXV_Validator        valid )
  {
    ...
    /* XXX: check range? */
    значение.u = (FT_UShort)( base_value_p->u +
                     ...
                     FT_Validator  ftvalid )
  {
    ...
    GXV_OPBD_DATA( формат ) = FT_NEXT_USHORT( p );
    ...
    GXV_TRACE(( "(формат=0x%04x)\n", GXV_OPBD_DATA( формат ) ));
    if ( 0x0001 < GXV_OPBD_DATA( формат ) )
      FT_INVALID_FORMAT;
