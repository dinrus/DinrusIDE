...
void GatherLinks(Index<String>& link, String topic)
{
	...
	if(link.Find(topic) < 0) {
		...
		SyncTopicFile(txt, topic, ":DinrusIDE2:", p.title);
...
static String sTopicHome = "topic://DinrusIDE2/app/index_ru-ru";
static String s_idehelp = "Справка по DinrusIDE2";
...
static String s_reference = "Справочник";
...
Topic TopicCtrl::AcquireTopic(const String& t)
{
	...
	if(!IsNull(tl.package)) {
		...
		if(q >= 0)
			tl.topic.Set(q, '_');
		Topic t;
		if(tl.topic.StartsWith("topic://DinrusIDE2/app/"))
			t = GetTopic(tl.topic);
		else
...
/*	bar.Add("Highlight search keywords in topic", IdeImg::ShowWords(), THISBACK(ShowWords))
	   .Check(showwords);*/
	bar.Add(!internal && GetCurrent().StartsWith("topic:"), "Показать на весь экран",
	        IdeImg::show(), THISBACK(SShow));
	bar.GapRight();
	bar.Separator();
	bar.Add(!internal && GetCurrent().StartsWith("topic:"), "Редактировать тематику",
...
TopicCtrl::TopicCtrl()
{
	...
	search.Tip("Поиск по всему тексту");
