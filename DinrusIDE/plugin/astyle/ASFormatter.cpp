...
namespace astyle
{
Вектор<const ШТкст*> ASFormatter::headers;
Вектор<const ШТкст*> ASFormatter::nonParenHeaders;
Вектор<const ШТкст*> ASFormatter::preDefinitionHeaders;
Вектор<const ШТкст*> ASFormatter::preCommandHeaders;
Вектор<const ШТкст*> ASFormatter::operators;
Вектор<const ШТкст*> ASFormatter::assignmentOperators;
Вектор<const ШТкст*> ASFormatter::castOperators;
...
ASFormatter::ASFormatter()
{
	...
	ШТкст filename = "tracef.txt";
	char* env = getenv("HOME");
	if (env != NULL)
		filename = ШТкст(env) + ШТкст("/tracef.txt");
	else
	{
		env = getenv("USERPROFILE");
		if (env != NULL)
			filename = ШТкст(env) + ШТкст("\\My Documents\\tracef.txt");
		else
		{
...
{
	static int formatterFileType = 9;      // initialized with an invalid тип
...
void ASFormatter::init(ASSourceIterator *si)
{
	...
	INIT_CONTAINER(preBracketHeaderStack, new Вектор<const ШТкст*>);
	INIT_CONTAINER(bracketTypeStack, new Вектор<BracketType>);
	bracketTypeStack->push_back(NULL_TYPE);
	INIT_CONTAINER(parenStack, new Вектор<int>);
	parenStack->push_back(0);

	currentHeader = NULL;
	currentLine = ШТкст("");
	readyFormattedLine = ШТкст("");
	formattedLine = "";
	currentChar = ' ';
	previousChar = ' ';
	previousCommandChar = ' ';
...
ШТкст ASFormatter::nextLine()
{
	// these are reset with each new line
	const ШТкст *newHeader;
	...
	while (!isLineReady)
	{
		...
		if (isInLineComment)
		{
			...
			if (charNum + 1 == (int) currentLine.дайСчёт())
			{
				isInLineBreak = true;
				isInLineComment = false;
		...
		/* not in MIDDLE of quote or comment or white-space of any тип ... */
		...
		if (isSequenceReached("//"))
		{
			...
			// move comment if пробелы were added or deleted
			if (lineCommentNoIndent == false && spacePadNum != 0)
				adjustComments();
			formattedLineCommentNum = formattedLine.дайСчёт();
		...
		/* not in quote or comment or white-space of any тип ... */
		...
		if (isImmediatelyPostHeader)
		{
			...
			// сделай sure headers are broken from their succeeding blocks
			// (e.g.
			//     if (isFoo) DoBar();
			//  should become
			//     if (isFoo)
			...
			if (parenStack->back() == 0 && currentChar != ';') // allow ;;
			{
				...
				if (IS_A(bracketTypeStack->back(), SINGLE_LINE_TYPE))
				{
					...
					if (isBeforeLineEndComment(blockEnd))
					{
						...
						assert((currentLine.середина(commentStart, 2) == ШТкст("//"))
						       || (currentLine.середина(commentStart, 2) == ШТкст("/*")));
						int commentLength = currentLine.дайСчёт() - commentStart;
						int tabCount = getIndentLength();
						appendSpacePad();
						for (int i=1; i<tabCount; i++)
							formattedLine.конкат(' ');
						formattedLine.конкат(currentLine.середина(commentStart, commentLength));
						currentLine.удали(commentStart, commentLength);
					}
				}
				shouldReparseCurrentChar = true;
				isInLineBreak = true;
		...
		if (!isInTemplate && currentChar == '<')
		{
			...
			const ШТкст *oper;
			for (int i = charNum;
			        i < currentLine.дайСчёт();
			        i += (oper ? oper->дайСчёт() : 1))
			{
		...
		if (currentChar == '{' || currentChar == '}')
		{
			...
			if (currentChar == '}')
			{
				...
				if (!preBracketHeaderStack->пустой())
				{
		...
		if (!isInTemplate)
		{
			if ((newHeader = findHeader(headers)) != NULL)
			{
				...
				goForward(currentHeader->дайСчёт() - 1);
				// if a paren-header is found add a space after it, if needed
				// this checks currentLine, appendSpacePad() checks formattedLine
				if (!isNonParenHeader && charNum < (int) currentLine.дайСчёт() && !isWhiteSpace(currentLine[charNum+1]))
					appendSpacePad();
		...
		else
		{
			...
			if (currentLine.середина(charNum, 3) ==  ШТкст("new") && !isLegalNameChar(currentLine[charNum+3]))
				isInPotentialCalculation = false;
		}
	...
	int readyFormattedLineLength = trim(readyFormattedLine).дайСчёт();
	...
	else								// формат the current formatted line
	{
		isLineReady = false;
 ...
 * options:
 *    true     statement operators will be padded with пробелы around them.
...
* options:
*    true     statement parenthesiss will be padded with пробелы around them.
...
* options:
*    true     statement parenthesis will be padded with пробелы around them.
...
* options:
*    true     statement parenthesis will be unpadded with пробелы removed around them.
...
/**
 * set option to convert tabs to пробелы.
 *
...
bool ASFormatter::isBeforeComment() const
{
	...
	foundComment = (currentLine.середина(peekNum, 2) == ШТкст("/*")
	                || currentLine.середина(peekNum, 2) == ШТкст("//"));
...
bool ASFormatter::isBeforeLineEndComment(int startPos) const
{
	...
	if (peekNum >= 0)
	{
		if (currentLine.середина(peekNum, 2) == ШТкст("//"))
			foundLineEndComment = true;
		else if (currentLine.середина(peekNum, 2) == ШТкст("/*"))
		{
			// comment must be closed on this line with nothing after it
			int endNum = currentLine.найди("*/", peekNum + 2);
			if (endNum >= 0)
...
bool ASFormatter::getNextChar()
{
	...
	int currentLineLength = currentLine.дайСчёт();
	...
	else        // end of line has been reached
	{
		if (sourceIterator->hasMoreLines())
		{
			...
			if (currentLine.дайСчёт() == 0)
			{
				currentLine = ШТкст(" ");        // a null is inserted if this is not done
			}
 ...
 * @param canBreakLine     if true, a registered line-break
 */
void ASFormatter::appendSequence(const ШТкст &sequence, bool canBreakLine)
{
	if (canBreakLine && isInLineBreak)
		breakLine();
	formattedLine.конкат(sequence);
}
 ...
 * opens a:
 * - a definition тип block (such as a class or namespace),
 * - a command block (such as a method block)
 * - a static array
 * this method takes for granted that the current character
 * is an opening bracket.
 *
 * @return    the тип of the opened block.
 */
...
{
	int formattedLineLength = formattedLine.дайСчёт();
...
bool ASFormatter::isOneLineBlockReached() const
{
	...
	for (int i = charNum + 1; i < currentLineLength; ++i)
	{
		...
		if (isInComment)
		{
			if (currentLine.середина(i, 2) == ШТкст("*/"))
			{
				isInComment = false;
				++i;
			}
 ...
 * @param checkBoundry
 */
const ШТкст *ASFormatter::findHeader(const Вектор<const ШТкст*> &headers, bool checkBoundry)
{
...
/**
 * adjust comment position because of adding or deleting пробелы
 * the пробелы are added or deleted to formattedLine
 * spacePadNum contains the adjustment
 */
void ASFormatter::adjustComments(void)
{
	assert(spacePadNum != 0);
	assert(currentLine.середина(charNum, 2) ==  ШТкст("//")
	       || currentLine.середина(charNum, 2) == ШТкст("/*"));


	// block comment must be closed on this line with nothing after it
	if (currentLine.середина(charNum, 2) == ШТкст("/*"))
	{
		int endNum = currentLine.найди("*/", charNum + 2);
		if (endNum < 0)
			return;
		if(ASString_Find_First_Not_Of(currentLine, " \t", endNum + 2) >= 0)
			return;
	...
	int len = formattedLine.дайСчёт();
	// if пробелы were removed, need to add пробелы before the comment
	if (spacePadNum < 0)
	{
		int adjust = -spacePadNum;          // make the number positive
		if (formattedLine[len-1] != '\t')   // don't adjust if a tab
			formattedLine.конкат(ШТкст(' ', adjust));
//		else								// comment out to avoid compiler warning
//			adjust = 0;
//		TRcomment(adjust);                  // trace macro
	}
	// if пробелы were added, need to delete пробелы before the comment, if possible
	else if (spacePadNum > 0)
	{
		...
		if (ASString_Find_Last_Not_Of(formattedLine, " ") < len - adjust - 1
			...
			formattedLine.обрежь(len - adjust);
...
void ASFormatter::appendCharInsideComments(void)
{
	...
	assert(formattedLine.середина(formattedLineCommentNum, 2) == ШТкст("//")
	       || formattedLine.середина(formattedLineCommentNum, 2) == ШТкст("/*"));
	...
	if (end - beg < 3)						// is there room to insert?
		formattedLine.вставь(beg, ШТкст(' ', 3-end+beg));
	if (formattedLine[beg] == '\t')			// don't pad with a tab
		formattedLine.вставь(beg, ' ');
	formattedLine.уст(beg+1, currentChar);
}
 ...
 */
void ASFormatter::padOperators(const ШТкст *newOperator)
{
	...
	goForward(newOperator->дайСчёт() - 1);
	...
	currentChar = (*newOperator)[newOperator->дайСчёт() - 1];
	// pad after operator
	...
	if (shouldPad
	        ...
	        && (currentLine.середина(charNum + 1, 2) != ШТкст("::")))
		appendSpaceAfter();
...
void ASFormatter::padParens(void)
{
	...
	if (currentChar == '(')
	{
		int spacesOutsideToDelete = formattedLine.дайСчёт() - 1;
		int spacesInsideToDelete = 0;

		// compute пробелы outside the opening paren to delete
		if (shouldUnPadParens)
		{
			...
			if (i >= 0)
			{
				...
				ШТкст prevWord = formattedLine.середина(start, end-start+1);
				// if previous word is a header, it will be a paren header
				const ШТкст *prevWordH = ASBeautifier::findHeader(formattedLine, start, headers);
				if (prevWordH != NULL)
				{
					prevIsParenHeader = true;
					TRxtra(*prevWordH);         // trace macro
				}
				else if (prevWord == ШТкст("return") 	// don't unpad return statements
				         || prevWord == ШТкст("*"))     // don't unpad multiply or pointer
				{
					prevIsParenHeader = true;
					TRxtra(prevWord);           // trace macro
				}
				// don't unpad variables
				else if (prevWord == ШТкст("bool")
				         || prevWord ==  ШТкст("int")
				         || prevWord ==  ШТкст("void")
				         || prevWord ==  ШТкст("void*")
				         || (prevWord.дайСчёт() >= 6     // check end of word for _t
				             && prevWord.середина(prevWord.дайСчёт()-2, 2) == ШТкст("_t"))
				         || prevWord ==  ШТкст("BOOL")
				         || prevWord ==  ШТкст("DWORD")
				         || prevWord ==  ШТкст("HWND")
				         || prevWord ==  ШТкст("INT")
				         || prevWord ==  ШТкст("LPSTR")
				         || prevWord ==  ШТкст("VOID")
				         || prevWord ==  ШТкст("LPVOID")
				        )
				{
					prevIsParenHeader = true;
					TRxtra(prevWord);           // trace macro
			...
			{
				formattedLine.удали(i + 1, spacesOutsideToDelete);
		...
		if (shouldUnPadParens)
		{
			...
			{
				currentLine.удали(charNum + 1, spacesInsideToDelete);
	...
	else if (currentChar == ')' /*|| currentChar == ']'*/)
	{
		...
		int spacesInsideToDelete = formattedLine.дайСчёт();
		...
		if (shouldUnPadParens)
		{
			int i = ASString_Find_Last_Not_Of(formattedLine, " \t");
			if (i >= 0)
				spacesInsideToDelete = formattedLine.дайСчёт() - 1 - i;
			if (shouldPadParensInside)
				spacesInsideToDelete--;
			if (spacesInsideToDelete > 0)
			{
				formattedLine.удали(i + 1, spacesInsideToDelete);
				spacePadNum -= spacesInsideToDelete;
			}
		}
...
//                    spacesOutsideToDelete--;            // always leave 1 space
			...
			if (spacesOutsideToDelete > 0)
			{
				currentLine.удали(charNum + 1, spacesOutsideToDelete);
				spacePadNum -= spacesOutsideToDelete;
			}
		}
...
/**
 * формат brackets as attached or broken
 ...
 * the calling function should have a continue statement after calling this method
 *
 * @param bracketType    the тип of bracket to be formatted.
 */
...
/**
 * формат array brackets as attached or broken
 * determine if the brackets can have an inStatement indent
 * currentChar contains the bracket
 * the brackets will be appended to the current formattedLine or a new formattedLine as necessary
 * the calling function should have a continue statement after calling this method
 *
 * @param bracketType            the тип of bracket to be formatted, must be an ARRAY_TYPE.
