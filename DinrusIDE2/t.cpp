...
{
	PutConsole(p.GetFileName() + Format("(%d): t_/tt_ работает только с простыми текстовыми литералами", p.GetLine()));
}
		...
		while(!p.IsEof()) {
			if((p.Id("t_") || p.Id("tt_")) && p.Char('('))
				if(p.IsString()) {
					...
					le.text.GetAdd(LNG_RUSSIAN) = GetENUS(tid);
					le.AddFileLine(p);
					le.added = true;
					if(!p.Char(')'))
						t_error(p);
...
void Ide::SyncT(int kind)
{
	...
	for(int i = 0; i < tfile.GetCount(); i++) {
		...
		pi.SetText("Сохранение " + tf.package + '/' + tf.file);
		if(pi.StepCanceled())
			break;
