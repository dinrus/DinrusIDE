...
struct Размер_ : Движ< Размер_<T> > {
	...
	Размер_&        operator/=(Размер_ p)          { cx  /= p.cx; cy  /= p.cy; return *this; }
	Размер_&        operator/=(T t)              { cx  /= t;    cy  /= t;    return *this; }
	Размер_&        operator<<=(цел sh)          { cx <<= sh;   cy <<= sh;   return *this; }
	Размер_&        operator>>=(цел sh)          { cx >>= sh;   cy >>= sh;   return *this; }
	...
	friend Размер_  operator/(Размер_ a, Размер_ b)  { return a /= b; }
	friend Размер_  operator/(Размер_ a, T b)      { return a /= b; }
	friend Размер_  operator<<(Размер_ a, цел sh)  { return a <<= sh; }
	friend Размер_  operator>>(Размер_ a, цел sh)  { return a >>= sh; }

	friend бул   operator==(Размер_ a, Размер_ b) { return a.cx == b.cx && a.cy == b.cy; }
	friend бул   operator!=(Размер_ a, Размер_ b) { return !(a == b); }

	friend Размер_  мин(Размер_ a, Размер_ b)        { return Размер_(мин(a.cx, b.cx), мин(a.cy, b.cy)); }
	friend Размер_  макс(Размер_ a, Размер_ b)        { return Размер_(макс(a.cx, b.cx), макс(a.cy, b.cy)); }
	...
	проц сериализуй(Поток& s)                  { s % cx % cy; }
	проц вДжейсон(ДжейсонВВ& jio)                  { jio("cx", cx)("cy", cy); }
	проц вРяр(РярВВ& xio)                    { xio.Атр("cx", cx).Атр("cy", cy); }
	
	цел  сравни(const Размер_&) const           { НИКОГДА(); return 0; }
	...
	explicit Размер_(LONG lParam) { cx = (T)(крат)LOWORD(lParam); cy = (T)(крат)HIWORD(lParam); }
#endif
};
...
struct Точка_ : Движ< Точка_<T> > {
	...
	Точка_&       operator/=(Точка_ p)              { x  /= p.x;  y  /= p.y; return *this; }
	Точка_&       operator/=(T t)                   { x  /= t;    y  /= t;    return *this; }
	Точка_&       operator<<=(цел sh)               { x <<= sh;   y <<= sh;   return *this; }
	Точка_&       operator>>=(цел sh)               { x >>= sh;   y >>= sh;   return *this; }
	...
	friend Точка_ operator/(Точка_ a, Sz b)         { return a /= b; }
	friend Точка_ operator/(Точка_ a, T b)          { return a /= b; }
	friend Точка_ operator<<(Точка_ a, цел sh)      { return a <<= sh; }
	friend Точка_ operator>>(Точка_ a, цел sh)      { return a >>= sh; }


	friend бул   operator==(Точка_ a, Точка_ b)    { return a.x == b.x && a.y == b.y; }
	friend бул   operator!=(Точка_ a, Точка_ b)    { return a.x != b.x || a.y != b.y; }

	friend Точка_ мин(Точка_ a, Точка_ b)           { return Точка_(мин(a.x, b.x), мин(a.y, b.y)); }
	friend Точка_ макс(Точка_ a, Точка_ b)           { return Точка_(макс(a.x, b.x), макс(a.y, b.y)); }
	...
	проц сериализуй(Поток& s)                       { s % x % y; }
	проц вДжейсон(ДжейсонВВ& jio)                       { jio("x", x)("y", y); }
	проц вРяр(РярВВ& xio)                         { xio.Атр("x", x).Атр("y", y); }

	цел  сравни(const Точка_&) const               { НИКОГДА(); return 0; }
	цел  полиСравни(const Значение&) const            { НИКОГДА(); return 0; }
	...
	explicit Точка_(LONG lParam)                    { x = (T)(крат)LOWORD(lParam); y = (T)(крат)HIWORD(lParam); }
#endif
};
...
template <>
inline дво GHalf_(дво d) { return d / 2; }

template <class T>
struct Прямоугольник_ : Движ< Прямоугольник_<T> > {
	T      left, top, right, bottom;

	typedef Точка_<T>  Pt;
	typedef Размер_<T>   Sz;

	проц   очисть()                          { left = top = right = bottom = 0; }

	бул   пустой() const                  { return right <= left || bottom <= top; }
	проц   устПусто()                        { left = right = top = bottom = Null; }
	бул   экзПусто_ли() const           { return пусто_ли(left); }
	...
	проц   уст(T l, T t, T r, T b)          { left = l; top = t; right = r; bottom = b; }
	проц   уст(Pt a, Pt b)                  { left = a.x; top = a.y; right = b.x; bottom = b.y; }
	проц   уст(Pt a, Sz sz)                 { уст(a, a + sz); }
	проц   уст(const Прямоугольник_& r)              { уст(r.left, r.top, r.right, r.bottom); }

	проц   устРазм(T cx, T cy)                  { right = left + cx; bottom = top + cy; }
	проц   устРазм(Sz sz)                       { устРазм(sz.cx, sz.cy); }

	проц   инфлируйГориз(T dx)                    { left -= dx; right += dx; }
	проц   инфлируйВерт(T dy)                    { top -= dy; bottom += dy; }
	проц   инфлируй(T dx, T dy)                  { инфлируйГориз(dx); инфлируйВерт(dy); }
	проц   инфлируй(Sz sz)                       { инфлируй(sz.cx, sz.cy); }
	проц   инфлируй(T dxy)                       { инфлируй(dxy, dxy); }
	проц   инфлируй(T l, T t, T r, T b)          { left -= l; top -= t; right += r; bottom += b; }
	проц   инфлируй(const Прямоугольник_& r)              { инфлируй(r.left, r.top, r.right, r.bottom); }

	проц   дефлируйГориз(T dx)                    { инфлируйГориз(-dx); }
	проц   дефлируйВерт(T dy)                    { инфлируйВерт(-dy); }
	проц   дефлируй(T dx, T dy)                  { инфлируй(-dx, -dy); }
	проц   дефлируй(Sz sz)                       { инфлируй(-sz); }
	проц   дефлируй(T dxy)                       { инфлируй(-dxy); }
	проц   дефлируй(T l, T t, T r, T b)          { инфлируй(-l, -t, -r, -b); }
	проц   дефлируй(const Прямоугольник_& r)              { дефлируй(r.left, r.top, r.right, r.bottom); }
	...
	проц   смещениеГориз(T dx)                     { left += dx; right += dx; }
	проц   смещениеВерт(T dy)                     { top += dy; bottom += dy; }
	проц   смещение(T dx, T dy)                   { смещениеГориз(dx); смещениеВерт(dy); }
	проц   смещение(Sz sz)                        { смещение(sz.cx, sz.cy); }
	проц   смещение(Pt p)                         { смещение(p.x, p.y); }
	...
	проц   союз(Pt p);
	проц   союз(const Прямоугольник_& rc);
	проц   пересек(const Прямоугольник_& rc);

	бул   содержит(T x, T y) const;
	бул   содержит(Pt p) const                 { return содержит(p.x, p.y); }
	бул   содержит(const Прямоугольник_& rc) const;
	бул   пересекается(const Прямоугольник_& rc) const;
	...
	бул   operator==(const Прямоугольник_& b) const;
	бул   operator!=(const Прямоугольник_& b) const                 { return !operator==(b); }
	...
	friend бул  operator&&(const Прямоугольник_& a, const Прямоугольник_& b) { return a.пересекается(b); }
	friend бул  operator>=(const Прямоугольник_& a, Pt b)           { return a.содержит(b); }
	friend бул  operator<=(Pt a, const Прямоугольник_& b)           { return b.содержит(a); }
	friend бул  operator<=(const Прямоугольник_& a, const Прямоугольник_& b) { return b.содержит(a); }
	friend бул  operator>=(const Прямоугольник_& b, const Прямоугольник_& a) { return a.содержит(b); }
	...
	проц     сериализуй(Поток& s) { s % left % top % right % bottom; }
	проц     вДжейсон(ДжейсонВВ& jio) { jio("left", left)("top", top)("right", right)("bottom", bottom); }
	проц     вРяр(РярВВ& xio)   { xio.Атр("left", left).Атр("top", top).Атр("right", right).Атр("bottom", bottom); }

	цел      сравни(const Прямоугольник_&) const           { НИКОГДА(); return 0; }
	цел      полиСравни(const Значение&) const       { НИКОГДА(); return 0; }
...
template <>
inline проц Прямоугольник_<дво>::союз(Точка_<дво> p) {
	if(пусто_ли(p)) return;
	if(экзПусто_ли())
	{
		left = right = p.x;
...
template <class T>
проц Прямоугольник_<T>::союз(const Прямоугольник_<T>& r) {
...
template <>
inline проц Прямоугольник_<дво>::союз(const Прямоугольник_<дво>& r) {
	прям_дво_союз(*this, r);
}

template <class T>
проц Прямоугольник_<T>::пересек(const Прямоугольник_<T>& r) {
	if(r.left > left) left = r.left;
	if(r.top > top) top = r.top;
...
template <class T>
бул Прямоугольник_<T>::содержит(T x, T y) const {
	return x >= left && x < right && y >= top && y < bottom;
}

template <>
inline бул Прямоугольник_<дво>::содержит(дво x, дво y) const {
	return x >= left && x <= right && y >= top && y <= bottom;
}

template <class T>
бул Прямоугольник_<T>::содержит(const Прямоугольник_<T>& r) const {
	return r.left >= left && r.top >= top && r.right <= right && r.bottom <= bottom;
}

template <class T>
бул Прямоугольник_<T>::пересекается(const Прямоугольник_<T>& r) const {
...
template <>
inline бул Прямоугольник_<дво>::пересекается(const Прямоугольник_<дво>& r) const {
...
template <>
inline Точка_<дво> Прямоугольник_<дво>::свяжи(Точка_<дво> pt) const {
...
template <class T>
бул Прямоугольник_<T>::operator==(const Прямоугольник_& b) const {
...
typedef Размер_<цел>     Размер;
typedef Точка_<цел>    Точка;
typedef Прямоугольник_<цел>     Прям;

typedef Размер_<крат>   Размер16;
typedef Точка_<крат>  Точка16;
typedef Прямоугольник_<крат>   Прям16;

typedef Размер_<дво>  РазмерПЗ;
typedef Точка_<дво> ТочкаПЗ;
typedef Прямоугольник_<дво>  ПрямПЗ;

typedef Размер_<дол>   Размер64;
typedef Точка_<дол>  Точка64;
typedef Прямоугольник_<дол>   Прям64;

const цел SIZE_V   = 70;
const цел SIZE16_V = 71;
const цел SIZEF_V  = 72;
const цел SIZE64_V = 79;

template<> inline бцел номерТипаЗнач(const Размер*)   { return SIZE_V; }
template<> inline бцел номерТипаЗнач(const Размер16*) { return SIZE16_V; }
template<> inline бцел номерТипаЗнач(const Размер64*) { return SIZE64_V; }
template<> inline бцел номерТипаЗнач(const РазмерПЗ*)  { return SIZEF_V; }

const цел POINT_V   = 73;
const цел POINT16_V = 74;
const цел POINTF_V  = 75;
const цел POINT64_V = 80;

template<> inline бцел номерТипаЗнач(const Точка*)   { return POINT_V; }
template<> inline бцел номерТипаЗнач(const Точка16*) { return POINT16_V; }
template<> inline бцел номерТипаЗнач(const Точка64*) { return POINT64_V; }
template<> inline бцел номерТипаЗнач(const ТочкаПЗ*)  { return POINTF_V; }

const цел RECT_V   = 76;
const цел RECT16_V = 77;
const цел RECTF_V  = 78;
const цел RECT64_V = 81;

template<> inline бцел номерТипаЗнач(const Прям*)   { return RECT_V; }
template<> inline бцел номерТипаЗнач(const Прям16*) { return RECT16_V; }
template<> inline бцел номерТипаЗнач(const Прям64*) { return RECT64_V; }
template<> inline бцел номерТипаЗнач(const ПрямПЗ*)  { return RECTF_V; }

Прям    RectC(цел x, цел y, цел cx, цел cy);
Прям16  Rect16C(крат x, крат y, крат cx, крат cy);
ПрямПЗ   RectfC(дво x, дво y, дво cx, дво cy);

inline Размер& operator*=(Размер& sz, дво a)
{
	sz.cx = цел(sz.cx * a);
	sz.cy = цел(sz.cy * a);
	return sz;
...
{
	sz.cx = цел(sz.cx * a.cx);
	sz.cy = цел(sz.cy * a.cy);
	return sz;
}
...
inline РазмерПЗ operator*(Размер sz, дво a)   { return РазмерПЗ(sz.cx * a, sz.cy * a); }
inline РазмерПЗ operator*(дво a, Размер sz)   { return РазмерПЗ(sz.cx * a, sz.cy * a); }
inline РазмерПЗ operator/(Размер sz, дво a)   { return РазмерПЗ(sz.cx / a, sz.cy / a); }
...
inline Размер16& operator*=(Размер16& sz, дво a)
{
	sz.cx = крат(sz.cx * a);
	sz.cy = крат(sz.cy * a);
	return sz;
...
{
	sz.cx = крат(sz.cx * a.cx);
	sz.cy = крат(sz.cy * a.cy);
	return sz;
}
...
inline РазмерПЗ operator*(Размер16 sz, дво a)   { return РазмерПЗ(sz.cx * a, sz.cy * a); }
inline РазмерПЗ operator*(дво a, Размер16 sz)   { return РазмерПЗ(sz.cx * a, sz.cy * a); }
inline РазмерПЗ operator/(Размер16 sz, дво a)   { return РазмерПЗ(sz.cx / a, sz.cy / a); }
...
inline Прям RectC(цел x, цел y, цел cx, цел cy) {
	return Прям(x, y, x + cx, y + cy);
}

inline Прям16  Rect16C(крат x, крат y, крат cx, крат cy) {
	return Прям16(x, y, x + cx, y + cy);
}

inline ПрямПЗ   RectfC(дво x, дво y, дво cx, дво cy) {
	return ПрямПЗ(x, y, x + cx, y + cy);
}

inline Прям  RectSort(Точка a, Точка b)    { return Прям(мин(a, b), макс(a, b) + 1); }
inline ПрямПЗ RectfSort(ТочкаПЗ a, ТочкаПЗ b) { return ПрямПЗ(мин(a, b), макс(a, b)); }
...
Размер        дайСоотношение(Размер stdsize, цел cx, цел cy);
Размер        дайРазмСхождения(Размер objsize, цел cx, цел cy);
...
inline дво Bearing(const ТочкаПЗ& p) { return направление(p); }
