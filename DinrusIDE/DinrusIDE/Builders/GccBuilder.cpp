...
void   GccBuilder::добавьФлаги(Индекс<Ткст>& cfg)
{
}

Ткст GccBuilder::CompilerName() const
{
	if(!пусто_ли(compiler)) return compiler;
	return "c++";
}

Ткст GccBuilder::CmdLine(const Ткст& package, const Пакет& pkg)
{
...
void GccBuilder::BinaryToObject(Ткст objfile, СиПарсер& binscript, Ткст basedir,
...
bool GccBuilder::постройПакет(const Ткст& package, Вектор<Ткст>& linkfile, Вектор<Ткст>& immfile,
...
bool GccBuilder::CreateLib(const Ткст& product, const Вектор<Ткст>& obj,
...
bool GccBuilder::Link(const Вектор<Ткст>& linkfile, const Ткст& linkoptions, bool createmap)
{
	if(!жди())
		return false;
	PutLinking();
...
bool GccBuilder::Preprocess(const Ткст& package, const Ткст& file, const Ткст& target, bool asmout)
{
	Пакет pkg;
	Ткст packagePath = PackagePath(package);
	pkg.грузи(packagePath);
