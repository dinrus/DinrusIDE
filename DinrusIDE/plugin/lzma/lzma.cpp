...
struct IUppInStream {
	...
	static SRes читай(const ISeqInStream *pp, void *buf, size_t *size) {
		IUppInStream *p = (IUppInStream *)pp;
		int64 h = (size_t)p->in.дай64(buf, *size);
		if(p->in.ошибка_ли() || h < 0) {
			*size = 0;
...
struct IUppOutStream {
	...
	static size_t пиши(const ISeqOutStream *pp, const void *buf, size_t size) {
		IUppOutStream *p = (IUppOutStream *)pp;
		p->out.помести64(buf, size);
		p->len += size;
		return p->out.ошибка_ли() ? -1 : size;
	}

	IUppOutStream(Поток& out) : out(out) {
		s.пиши = пиши;
		len = 0;
	}
};
...
static SRes Decode2(CLzmaDec *state, ISeqOutStream *outStream, ISeqInStream *inStream, UInt64 unpackSize, Gate2<int64, int64> progress)
{
	...
	Байт inBuf[IN_BUF_SIZE];
	Байт outBuf[OUT_BUF_SIZE];
	...
	for(;;) {
		if(inPos == inSize) {
			inSize = IN_BUF_SIZE;
			RINOK(inStream->читай(inStream, inBuf, &inSize));
			inPos = 0;
		}
		SRes res;
		...
		if(outStream)
			if(outStream->пиши(outStream, outBuf, outPos) != outPos)
				return SZ_ERROR_WRITE;
...
static SRes Decode(ISeqOutStream *outStream, ISeqInStream *inStream, Gate2<int64, int64> progress)
{
	...
	/* читай and parse header */
	IUppInStream *is = (IUppInStream *)inStream;
	if(is->in.дай(header, sizeof(header)) != sizeof(header))
		return SZ_ERROR_READ;
...
static SRes Encode(ISeqOutStream *outStream, ISeqInStream *inStream, UInt64 fileSize, ICompressProgress *pg, int lvl)
{
	...
	if(res == SZ_OK) {
		Байт header[LZMA_PROPS_SIZE + 8];
		size_t headerSize = LZMA_PROPS_SIZE;
		int i;
		res = LzmaEnc_WriteProperties(enc, header, &headerSize);
		for(i = 0; i < 8; i++)
			header[headerSize++] = (Байт)(fileSize >> (8 * i));
		if(outStream->пиши(outStream, header, headerSize) != headerSize)
			res = SZ_ERROR_WRITE;
		else {
			if(res == SZ_OK)
...
int64 LZMACompress(Поток& out, Поток& in, int64 size, Gate2<int64, int64> progress, int lvl)
{
	IUppInStream   lin(in);
	...
	return Encode(&lout.s, &lin.s, size, &pg.s, lvl) == SZ_OK
	       && !in.ошибка_ли() && !out.ошибка_ли() ? lout.len : -1;
}

int64 LZMACompress(Поток& out, Поток& in, Gate2<int64, int64> progress, int lvl)
{
	return LZMACompress(out, in, in.GetLeft(), progress, lvl);
}

Ткст LZMACompress(const void *data, int64 len, Gate2<int64, int64> progress, int lvl)
{
	ТкстПоток  out;
...
Ткст LZMACompress(const Ткст& s, Gate2<int64, int64> progress, int lvl)
{
	return LZMACompress(~s, s.дайДлину(), progress, lvl);
}

int64 LZMADecompress(Поток& out, Поток& in, Gate2<int64, int64> progress)
{
	IUppInStream   lin(in);
	IUppOutStream  lout(out);
	return Decode(&lout.s, &lin.s, progress) == SZ_OK && !in.ошибка_ли() && !out.ошибка_ли() ? lout.len : -1;
}

Ткст LZMADecompress(const void *data, int64 len, Gate2<int64, int64> progress)
{
	ТкстПоток  out;
	ПамЧтенПоток in(data, len);
...
Ткст LZMADecompress(const Ткст& s, Gate2<int64, int64> progress)
{
	return LZMADecompress(~s, s.дайДлину(), progress);
}
...
{
	Ткст dstfile = Ткст(srcfile) + ".lzma";
...
bool LZMADecompressFile(const char *srcfile, Gate2<int64, int64> progress)
{
	...
	if(dstfile.заканчиваетсяНа(".lzma"))
