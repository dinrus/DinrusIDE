...
TIFFSetCompressionScheme(TIFF* tif, int scheme)
{
	...
	/*
	 * Don't treat an unknown compression scheme as an Ошибка.
...
TIFFFindCODEC(uint16 scheme)
{
	...
	for (c = _TIFFBuiltinCODECS; c->имя; c++)
		if (c->scheme == scheme)
			return (c);
...
TIFFCodec*
TIFFRegisterCODEC(uint16 scheme, const char* имя, TIFFInitMethod init)
{
	codec_t* cd = (codec_t*)
	    _TIFFmalloc((tmsize_t)(sizeof (codec_t) + sizeof (TIFFCodec) + strlen(имя)+1));

	if (cd != NULL) {
		cd->info = (TIFFCodec*) ((uint8*) cd + sizeof (codec_t));
		cd->info->имя = (char*)
		    ((uint8*) cd->info + sizeof (TIFFCodec));
		strcpy(cd->info->имя, имя);
		...
		TIFFErrorExt(0, "TIFFRegisterCODEC",
		    "No space to register compression scheme %s", имя);
		return NULL;
	}
	return (cd->info);
}
...
TIFFUnRegisterCODEC(TIFFCodec* c)
{
	...
	TIFFErrorExt(0, "TIFFUnRegisterCODEC",
	    "Cannot remove compression scheme %s; not registered", c->имя);
}
...
/**
 * дай list of configured codecs, both built-in and registered by user.
