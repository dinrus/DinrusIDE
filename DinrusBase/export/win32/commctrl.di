...
enum {
	...
	TCN_FIRST = -550U,
	TCN_LAST  = -580U,
	CDN_FIRST = -601U, /* also in commdlg.в */
	CDN_LAST  = -699U,
	TBN_FIRST = -700U,
	TBN_LAST  = -720U,
	UDN_FIRST = -721U,
...
int Header_GetItemCount(HWND ш) {
	return SendMessage(ш, HDM_GETITEMCOUNT, 0, 0);
}

int Header_InsertItem(HWND ш, int i, CPtr!(HDITEM) phdi) {
	return SendMessage(ш, HDM_INSERTITEM, i, cast(LPARAM) phdi);
}

BOOL Header_DeleteItem(HWND ш, int i) {
	return cast(BOOL) SendMessage(ш, HDM_DELETEITEM, i, 0);
}

BOOL Header_GetItem(HWND ш, int i, LPHDITEM phdi) {
	return cast(BOOL) SendMessage(ш, HDM_GETITEM, i, cast(LPARAM) phdi);
}

BOOL Header_SetItem(HWND ш, int i, CPtr!(HDITEM) phdi) {
	return cast(BOOL) SendMessage(ш, HDM_SETITEM, i, cast(LPARAM) phdi);
}

BOOL Header_Layout(HWND ш, LPHDLAYOUT playout) {
	return SendMessage(ш, HDM_LAYOUT, 0, cast(LPARAM) playout);
}

static if (_WIN32_IE >= 0x300) {
	int Header_OrderToIndex(HWND ш, int i) {
		return SendMessage(ш, HDM_ORDERTOINDEX, i, 0);
	}

	BOOL Header_GetItemRect(HWND ш, int i, RECT* r) {
		return cast(BOOL) SendMessage(ш, HDM_GETITEMRECT, i, cast(LPARAM) r);
	}

	BOOL Header_GetOrderArray(HWND ш, int iSize, LPINT lpiArray) {
		return cast(BOOL) SendMessage(ш, HDM_GETORDERARRAY, iSize,
		  cast(LPARAM) lpiArray);
	}

	BOOL Header_SetOrderArray(HWND ш, int iSize, LPINT lpiArray) {
		return cast(BOOL) SendMessage(ш, HDM_SETORDERARRAY, iSize,
		  cast(LPARAM) lpiArray);
	}

	HIMAGELIST Header_CreateDragImage(HWND ш, int i) {
		return cast(HIMAGELIST) SendMessage(ш, HDM_CREATEDRAGIMAGE, i, 0);
	}

	HIMAGELIST Header_SetImageList(HWND ш, HIMAGELIST himl) {
		return cast(HIMAGELIST) SendMessage(ш, HDM_SETIMAGELIST, 0,
		  cast(LPARAM) himl);
	}

	HIMAGELIST Header_GetImageList(HWND ш) {
		return cast(HIMAGELIST) SendMessage(ш, HDM_GETIMAGELIST, 0, 0);
	}
}

static if (_WIN32_IE >= 0x400) {
	BOOL Header_GetUnicodeFormat(HWND ш) {
		return cast(BOOL) SendMessage(ш, HDM_GETUNICODEFORMAT, 0, 0);
	}

	BOOL Header_SetUnicodeFormat(HWND ш, BOOL fUnicode) {
		return cast(BOOL) SendMessage(ш, HDM_SETUNICODEFORMAT, fUnicode, 0);
	}
}
...
COLORREF ListView_GetBkColor(HWND ш) {
	return cast(COLORREF) SendMessage(ш, LVM_GETBKCOLOR, 0, 0);
}

HIMAGELIST ListView_GetImageList(HWND ш, int i) {
	return cast(HIMAGELIST) SendMessage(ш, LVM_GETIMAGELIST, i, 0);
}

int ListView_GetItemCount(HWND ш) {
	return SendMessage(ш, LVM_GETITEMCOUNT, 0, 0);
}

BOOL ListView_GetItem(HWND ш, LPLVITEM pitem) {
	return cast(BOOL) SendMessage(ш, LVM_GETITEM, 0, cast(LPARAM) pitem);
}

BOOL ListView_SetBkColor(HWND ш, COLORREF c) {
	return cast(BOOL) SendMessage(ш, LVM_SETBKCOLOR, 0, cast(LPARAM) c);
}

HIMAGELIST ListView_SetImageList(HWND ш, HIMAGELIST в, int i) {
	return cast(HIMAGELIST) SendMessage(ш, LVM_SETIMAGELIST, i,
	  cast(LPARAM) в);
}

BOOL ListView_SetItem(HWND ш, CPtr!(LV_ITEM) i) {
	return cast(BOOL) SendMessage(ш, LVM_SETITEM, 0, cast(LPARAM) i);
}

int ListView_InsertItem(HWND ш, CPtr!(LV_ITEM) i) {
	return SendMessage(ш, LVM_INSERTITEM, 0, cast(LPARAM) i);
}

BOOL ListView_DeleteItem(HWND ш, int i) {
	return cast(BOOL) SendMessage(ш, LVM_DELETEITEM, i, 0);
}

BOOL ListView_DeleteAllItems(HWND ш) {
	return cast(BOOL) SendMessage(ш, LVM_DELETEALLITEMS, 0, 0);
}

UINT ListView_GetCallbackMask(HWND ш) {
	return cast(UINT) SendMessage(ш, LVM_GETCALLBACKMASK, 0, 0);
}

BOOL ListView_SetCallbackMask(HWND ш, UINT m) {
	return cast(BOOL) SendMessage(ш, LVM_SETCALLBACKMASK, m, 0);
}

int ListView_GetNextItem(HWND ш, int i, UINT f) {
	return SendMessage(ш, LVM_GETNEXTITEM, i, MAKELPARAM(cast(ushort)f, 0));
}

int ListView_FindItem(HWND ш, int i, CPtr!(LV_FINDINFO) p) {
	return SendMessage(ш, LVM_FINDITEM, i, cast(LPARAM) p);
}

BOOL ListView_GetItemRect(HWND ш, int i, LPRECT p, int c) {
	return cast(BOOL) SendMessage(ш, LVM_GETITEMRECT, i, p ?
	  (p.left = c, cast(LPARAM) p) : 0);
}

BOOL ListView_SetItemPosition(HWND ш, int i, int x, int y) {
	return cast(BOOL) SendMessage(ш, LVM_SETITEMPOSITION, i, MAKELPARAM(cast(ushort)x, cast(ushort)y));
}

BOOL ListView_GetItemPosition(HWND ш, int i, POINT* p) {
	return cast(BOOL) SendMessage(ш, LVM_GETITEMPOSITION, i, cast(LPARAM) p);
}

DWORD ListView_GetItemSpacing(HWND ш, BOOL f) {
	return cast(DWORD) SendMessage(ш, LVM_GETITEMSPACING, f, 0);
}

int ListView_GetStringWidth(HWND ш, LPCSTR s) {
	return SendMessage(ш, LVM_GETSTRINGWIDTH, 0, cast(LPARAM) s);
}

int ListView_HitTest(HWND ш, LPLVHITTESTINFO p) {
	return SendMessage(ш, LVM_HITTEST, 0, cast(LPARAM) p);
}

BOOL ListView_EnsureVisible(HWND ш, int i, BOOL f) {
	return cast(BOOL) SendMessage(ш, LVM_ENSUREVISIBLE, i, MAKELPARAM(cast(ushort)f, 0));
}

BOOL ListView_Scroll(HWND ш, int dx, int dy) {
	return cast(BOOL) SendMessage(ш, LVM_SCROLL, dx, dy);
}

BOOL ListView_RedrawItems(HWND ш, int f, int l) {
	return cast(BOOL) SendMessage(ш, LVM_REDRAWITEMS, f, l);
}

BOOL ListView_Arrange(HWND ш, UINT c) {
	return cast(BOOL) SendMessage(ш, LVM_ARRANGE, c, 0);
}

HWND ListView_EditLabel(HWND ш, int i) {
	return cast(HWND) SendMessage(ш, LVM_EDITLABEL, i, 0);
}

HWND ListView_GetEditControl(HWND ш) {
	return cast(HWND) SendMessage(ш, LVM_GETEDITCONTROL, 0, 0);
}

BOOL ListView_GetColumn(HWND ш, int i, LPLVCOLUMN p) {
	return cast(BOOL) SendMessage(ш, LVM_GETCOLUMN, i, cast(LPARAM) p);
}

BOOL ListView_SetColumn(HWND ш, int i, CPtr!(LV_COLUMN) p) {
	return cast(BOOL) SendMessage(ш, LVM_SETCOLUMN, i, cast(LPARAM) p);
}

int ListView_InsertColumn(HWND ш, int i, CPtr!(LV_COLUMN) p) {
	return SendMessage(ш, LVM_INSERTCOLUMN, i, cast(LPARAM) p);
}

BOOL ListView_DeleteColumn(HWND ш, int i) {
	return cast(BOOL) SendMessage(ш, LVM_DELETECOLUMN, i, 0);
}

int ListView_GetColumnWidth(HWND ш, int i) {
	return SendMessage(ш, LVM_GETCOLUMNWIDTH, i, 0);
}

BOOL ListView_SetColumnWidth(HWND ш, int i, int x) {
	return cast(BOOL) SendMessage(ш, LVM_SETCOLUMNWIDTH, i, MAKELPARAM(cast(ushort)x, 0));
}

HIMAGELIST ListView_CreateDragImage(HWND ш, int i, LPPOINT p) {
	return cast(HIMAGELIST) SendMessage(ш, LVM_CREATEDRAGIMAGE, i,
	  cast(LPARAM) p);
}

BOOL ListView_GetViewRect(HWND ш, RECT* p) {
	return cast(BOOL) SendMessage(ш, LVM_GETVIEWRECT, 0, cast(LPARAM) p);
}

COLORREF ListView_GetTextColor(HWND ш) {
	return cast(COLORREF) SendMessage(ш, LVM_GETTEXTCOLOR, 0, 0);
}

BOOL ListView_SetTextColor(HWND ш, COLORREF c) {
	return cast(BOOL) SendMessage(ш, LVM_SETTEXTCOLOR, 0, cast(LPARAM) c);
}

COLORREF ListView_GetTextBkColor(HWND ш) {
	return cast(COLORREF) SendMessage(ш, LVM_GETTEXTBKCOLOR, 0, 0);
}

BOOL ListView_SetTextBkColor(HWND ш, COLORREF c) {
	return cast(BOOL) SendMessage(ш, LVM_SETTEXTBKCOLOR, 0, cast(LPARAM) c);
}

int ListView_GetTopIndex(HWND ш) {
	return SendMessage(ш, LVM_GETTOPINDEX, 0, 0);
}

int ListView_GetCountPerPage(HWND ш) {
	return SendMessage(ш, LVM_GETCOUNTPERPAGE, 0, 0);
}

BOOL ListView_GetOrigin(HWND ш, LPPOINT p) {
	return cast(BOOL) SendMessage(ш, LVM_GETORIGIN, 0, cast(LPARAM) p);
}

BOOL ListView_Update(HWND ш, WPARAM i) {
	return cast(BOOL) SendMessage(ш, LVM_UPDATE, i, 0);
}

void ListView_SetItemState(HWND ш, int i, UINT d, UINT m) {
	LV_ITEM _lvi;
	_lvi.stateMask = m;
	_lvi.state = d;
	SendMessage(ш, LVM_SETITEMSTATE, i, cast(LPARAM) &_lvi);
}

UINT ListView_GetItemState(HWND ш, int i, UINT m) {
	return cast(UINT) SendMessage(ш, LVM_GETITEMSTATE, i, m);
}

void ListView_GetItemText(HWND ш, int i, int iS, LPTSTR s, int n) {
	LV_ITEM _lvi;
	_lvi.iSubItem = iS;
	_lvi.cchTextMax = n;
	_lvi.pszText = cast(char*)s;
	SendMessage(ш, LVM_GETITEMTEXT, i, cast(LPARAM) &_lvi);
}

void ListView_SetItemText(HWND ш, int i, int iS, LPTSTR s) {
	LV_ITEM _lvi;
	_lvi.iSubItem = iS;
	_lvi.pszText = cast(char*) s;
	SendMessage(ш, LVM_SETITEMTEXT, i, cast(LPARAM) &_lvi);
}

void ListView_SetItemCount(HWND ш, int n) {
	SendMessage(ш, LVM_SETITEMCOUNT, n, 0);
}

BOOL ListView_SortItems(HWND ш, PFNLVCOMPARE f, LPARAM l) {
	return cast(BOOL) SendMessage(ш, LVM_SORTITEMS, l, cast(LPARAM) f);
}

void ListView_SetItemPosition32(HWND ш, int i, int x, int y) {
	POINT p;
	p.x = x;
	p.y = y;
	SendMessage(ш, LVM_SETITEMPOSITION32, i, cast(LPARAM) &p);
}

UINT ListView_GetSelectedCount(HWND ш) {
	return cast(UINT) SendMessage(ш, LVM_GETSELECTEDCOUNT, 0, 0);
}

UINT ListView_GetCheckState(HWND ш, UINT i) {
	return ((cast(UINT) SendMessage(ш, LVM_GETITEMSTATE, i, LVIS_STATEIMAGEMASK)) >> 12) - 1;
}

void ListView_SetCheckState(HWND ш, UINT i, BOOL f) {
	ListView_SetItemState(ш, i, INDEXTOSTATEIMAGEMASK(f ? 2 : 1),
	  LVIS_STATEIMAGEMASK);
}

BOOL ListView_GetISearchString(HWND ш, LPSTR lpsz) {
	return cast(BOOL) SendMessage(ш, LVM_GETISEARCHSTRING, 0,
	  cast(LPARAM) lpsz);
}

void ListView_CancelEditLabel(HWND ш) {
	SendMessage(ш, LVM_CANCELEDITLABEL, 0, 0);
}

int ListView_EnableGroupView(HWND ш, BOOL i) {
	return cast(int) SendMessage(ш, LVM_ENABLEGROUPVIEW, i, 0);
}

static if (_WIN32_WINDOWS >= 0x410 && (_WIN32_WINNT >= 0x500 || _WIN32_IE >= 0x500)) {
	BOOL ListView_SortItemsEx(HWND ш, PFNLVCOMPARE c, LPARAM p) {
		return SendMessage(ш, LVM_SORTITEMSEX, cast(WPARAM) p, cast(LPARAM)c);
	}
}

static if (_WIN32_WINNT >= 0x501) {
	int ListView_GetGroupInfo(HWND ш, int i, PLVGROUP p) {
		return SendMessage(ш, LVM_GETGROUPINFO, i, cast(LPARAM) p);
	}

	void ListView_GetGroupMetrics(HWND ш, PLVGROUPMETRICS p) {
		SendMessage(ш, LVM_GETGROUPMETRICS, 0, cast(LPARAM) p);
	}

	BOOL ListView_GetInsertMark(HWND ш, PLVINSERTMARK p) {
		return SendMessage(ш, LVM_GETINSERTMARK, 0, cast(LPARAM) p);
	}

	COLORREF ListView_GetInsertMarkColor(HWND ш) {
		return SendMessage(ш, LVM_GETINSERTMARKCOLOR, 0, 0);
	}

	int ListView_GetInsertMarkRect(HWND ш, LPRECT p) {
		return SendMessage(ш, LVM_GETINSERTMARKRECT, 0, cast(LPARAM) p);
	}

	COLORREF ListView_GetOutlineColor(HWND ш) {
		return SendMessage(ш, LVM_GETOUTLINECOLOR, 0, 0);
	}

	UINT ListView_GetSelectedColumn(HWND ш) {
		return SendMessage(ш, LVM_GETSELECTEDCOLUMN, 0, 0);
	}

	void ListView_GetTileInfo(HWND ш, PLVTILEINFO p) {
		SendMessage(ш, LVM_GETTILEINFO, 0, cast(LPARAM) p);
	}

	void ListView_GetTileViewInfo(HWND ш, PLVTILEVIEWINFO p) {
		SendMessage(ш, LVM_GETTILEVIEWINFO, 0, cast(LPARAM) p);
	}

	DWORD ListView_GetView(HWND ш) {
		return SendMessage(ш, LVM_GETVIEW, 0, 0);
	}

	BOOL ListView_HasGroup(HWND ш, int i) {
		return SendMessage(ш, LVM_HASGROUP, i, 0);
	}

	int ListView_InsertGroup(HWND ш, int i, PLVGROUP p) {
		return SendMessage(ш, LVM_INSERTGROUP, i, cast(LPARAM) p);
	}

	void ListView_InsertGroupSorted(HWND ш, PLVINSERTGROUPSORTED p) {
		SendMessage(ш, LVM_INSERTGROUPSORTED, cast(WPARAM) p, 0);
	}

	BOOL ListView_InsertMarkHitTest(HWND ш, LPPOINT p, PLVINSERTMARK t) {
		return SendMessage(ш, LVM_INSERTMARKHITTEST, cast(WPARAM) p, cast(LPARAM) t);
	}

	BOOL ListView_IsGroupViewEnabled(HWND ш) {
		return SendMessage(ш, LVM_ISGROUPVIEWENABLED, 0, 0);
	}

	UINT ListView_MapIDToIndex(HWND ш, UINT i) {
		return SendMessage(ш, LVM_MAPIDTOINDEX, i, 0);
	}

	/*	??? MSDN documents this as "Not implemented", except in relation to
	 *	Windows CE/Mobile.
	 */
	void ListView_MoveGroup(HWND ш, int i, int t) {
		SendMessage(ш, LVM_MOVEGROUP, i, t);
	}

	void ListView_RemoveAllGroups(HWND ш) {
		SendMessage(ш, LVM_REMOVEALLGROUPS, 0, 0);
	}

	int ListView_RemoveGroup(HWND ш, int i) {
		return SendMessage(ш, LVM_REMOVEGROUP, i, 0);
	}

	int ListView_SetGroupInfo(HWND ш, int i, PLVGROUP p) {
		return SendMessage(ш, LVM_SETGROUPINFO, i, cast(LPARAM) p);
	}

	void ListView_SetGroupMetrics(HWND ш, PLVGROUPMETRICS p) {
		SendMessage(ш, LVM_SETGROUPMETRICS, 0, cast(LPARAM) p);
	}

	BOOL ListView_SetInfoTip(HWND ш, PLVSETINFOTIP p) {
		return SendMessage(ш, LVM_SETINFOTIP, 0, cast(LPARAM) p);
	}

	BOOL ListView_SetInsertMark(HWND ш, PLVINSERTMARK p) {
		return SendMessage(ш, LVM_SETINSERTMARK, 0, cast(LPARAM) p);
	}

	COLORREF ListView_SetInsertMarkColor(HWND ш, COLORREF c) {
		return SendMessage(ш, LVM_SETINSERTMARKCOLOR, 0, c);
	}

	COLORREF ListView_SetOutlineColor(HWND ш, COLORREF c) {
		return SendMessage(ш, LVM_SETOUTLINECOLOR, 0, c);
	}

	void ListView_SetSelectedColumn(HWND ш, int i) {
		SendMessage(ш, LVM_SETSELECTEDCOLUMN, i, 0);
	}

	BOOL ListView_SetTileInfo(HWND ш, PLVTILEINFO p) {
		return SendMessage(ш, LVM_SETTILEINFO, 0, cast(LPARAM) p);
	}

	BOOL ListView_SetTileViewInfo(HWND ш, PLVTILEVIEWINFO p) {
		return SendMessage(ш, LVM_SETTILEVIEWINFO, 0, cast(LPARAM) p);
	}

	int ListView_SetView(HWND ш, DWORD i) {
		return SendMessage(ш, LVM_SETVIEW, i, 0);
	}

	int ListView_SortGroups(HWND ш, PFNLVGROUPCOMPARE c, LPVOID p) {
		return SendMessage(ш, LVM_SORTGROUPS, cast(WPARAM) c, cast(LPARAM) p);
	}
}
...
static if (_WIN32_WINNT >= 0x501) {
	...
	BOOL ComboBox_SetMinVisible(HWND ш, INT i) {
		return cast(BOOL) SendMessage(ш, CB_SETMINVISIBLE, cast(WPARAM) i, 0);
	}

	int ComboBox_GetMinVisible(HWND ш) {
		return SendMessage(ш, CB_GETMINVISIBLE, 0, 0);
	}
}
...
BOOL MonthCal_SetRange(HWND ш, DWORD f, LPSYSTEMTIME st) {
	return cast(BOOL) SendMessage(ш, MCM_SETRANGE, cast(WPARAM) f,
	  cast(LPARAM) st);
}

extern (Windows) BOOL ShowHideMenuCtl(HWND, UINT, PINT);

BOOL TabCtrl_GetItem(HWND ш, int i, LPTCITEM p) {
	return cast(BOOL) SendMessage(ш, TCM_GETITEM, i, cast(LPARAM) p);
}

BOOL TabCtrl_SetItem(HWND ш, int i, LPTCITEM p) {
	return cast(BOOL) SendMessage(ш, TCM_SETITEM, i, cast(LPARAM) p);
}

int TabCtrl_InsertItem(HWND ш, int i, CPtr!(TC_ITEM) p) {
	return SendMessage(ш, TCM_INSERTITEM, i, cast(LPARAM) p);
}

BOOL TabCtrl_DeleteItem(HWND ш, int i) {
	return cast(BOOL) SendMessage(ш, TCM_DELETEITEM, i, 0);
}

BOOL TabCtrl_DeleteAllItems(HWND ш) {
	return cast(BOOL) SendMessage(ш, TCM_DELETEALLITEMS, 0, 0);
}

BOOL TabCtrl_GetItemRect(HWND ш, int i, LPRECT p) {
	return cast(BOOL) SendMessage(ш, TCM_GETITEMRECT, i, cast(LPARAM) p);
}

int TabCtrl_GetCurSel(HWND ш) {
	return SendMessage(ш, TCM_GETCURSEL, 0, 0);
}

int TabCtrl_SetCurSel(HWND ш, int i) {
	return SendMessage(ш, TCM_SETCURSEL, i, 0);
}

int TabCtrl_HitTest(HWND ш, LPTCHITTESTINFO p) {
	return SendMessage(ш, TCM_HITTEST, 0, cast(LPARAM) p);
}

BOOL TabCtrl_SetItemExtra(HWND ш, int c) {
	return cast(BOOL) SendMessage(ш, TCM_SETITEMEXTRA, c, 0);
}

int TabCtrl_AdjustRect(HWND ш, BOOL b, LPRECT p) {
	return SendMessage(ш, TCM_ADJUSTRECT, b, cast(LPARAM) p);
}

DWORD TabCtrl_SetItemSize(HWND ш, int x, int y) {
	return cast(DWORD) SendMessage(ш, TCM_SETITEMSIZE, 0, MAKELPARAM(cast(ushort)x, cast(ushort)y));
}

void TabCtrl_RemoveImage(HWND ш, int i) {
	SendMessage(ш, TCM_REMOVEIMAGE, i, 0);
}

void TabCtrl_SetPadding(HWND ш, int x, int y) {
	SendMessage(ш, TCM_SETPADDING, 0, MAKELPARAM(cast(ushort)x, cast(ushort)y));
}

int TabCtrl_GetRowCount(HWND ш) {
	return SendMessage(ш, TCM_GETROWCOUNT, 0, 0);
}

HWND TabCtrl_GetToolTips(HWND ш) {
	return cast(HWND) SendMessage(ш, TCM_GETTOOLTIPS, 0, 0);
}

void TabCtrl_SetToolTips(HWND ш, HWND t) {
	SendMessage(ш, TCM_SETTOOLTIPS, cast(WPARAM) t, 0);
}

int TabCtrl_GetCurFocus(HWND ш) {
	return SendMessage(ш, TCM_GETCURFOCUS, 0, 0);
}

void TabCtrl_SetCurFocus(HWND ш, int i) {
	SendMessage(ш, TCM_SETCURFOCUS, i, 0);
}

HIMAGELIST TabCtrl_GetImageList(HWND ш) {
	return cast(HIMAGELIST) SendMessage(ш, TCM_GETIMAGELIST, 0, 0);
}

HIMAGELIST TabCtrl_SetImageList(HWND ш, HIMAGELIST в) {
	return cast(HIMAGELIST) SendMessage(ш, TCM_SETIMAGELIST, 0,
	  cast(LPARAM) в);
}

int TabCtrl_GetItemCount(HWND ш) {
	return SendMessage(ш, TCM_GETITEMCOUNT, 0, 0);
}

extern (Windows) BOOL _TrackMouseEvent(LPTRACKMOUSEEVENT);

HTREEITEM TreeView_InsertItem(HWND ш, LPTVINSERTSTRUCT i) {
	return cast(HTREEITEM) SendMessage(ш, TVM_INSERTITEM, 0, cast(LPARAM) i);
}

BOOL TreeView_DeleteItem(HWND ш, HTREEITEM i) {
	return cast(BOOL) SendMessage(ш, TVM_DELETEITEM, 0, cast(LPARAM) i);
}

BOOL TreeView_DeleteAllItems(HWND ш) {
	return cast(BOOL) SendMessage(ш, TVM_DELETEITEM, 0, cast(LPARAM) TVI_ROOT);
}

BOOL TreeView_Expand(HWND ш, HTREEITEM i, UINT c) {
	return cast(BOOL) SendMessage(ш, TVM_EXPAND, c, cast(LPARAM) i);
}

BOOL TreeView_GetItemRect(HWND ш, HTREEITEM i, LPRECT p, BOOL c) {
	*cast(HTREEITEM*) p = i;
	return cast(BOOL) SendMessage(ш, TVM_GETITEMRECT, c, cast(LPARAM) p);
}

UINT TreeView_GetCount(HWND ш) {
	return cast(UINT) SendMessage(ш, TVM_GETCOUNT, 0, 0);
}

UINT TreeView_GetIndent(HWND ш) {
	return cast(UINT) SendMessage(ш, TVM_GETINDENT, 0, 0);
}

BOOL TreeView_SetIndent(HWND ш, INT i) {
	return cast(BOOL) SendMessage(ш, TVM_SETINDENT, i, 0);
}

HIMAGELIST TreeView_GetImageList(HWND ш, INT i) {
	return cast(HIMAGELIST) SendMessage(ш, TVM_GETIMAGELIST, i, 0);
}

HIMAGELIST TreeView_SetImageList(HWND ш, HIMAGELIST в, INT i) {
	return cast(HIMAGELIST) SendMessage(ш, TVM_SETIMAGELIST, i,
	  cast(LPARAM) в);
}

HTREEITEM TreeView_GetNextItem(HWND ш, HTREEITEM i, UINT c) {
	return cast(HTREEITEM) SendMessage(ш, TVM_GETNEXTITEM, c, cast(LPARAM) i);
}

HTREEITEM TreeView_GetChild(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_CHILD);
}

HTREEITEM TreeView_GetNextSibling(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_NEXT);
}

HTREEITEM TreeView_GetPrevSibling(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_PREVIOUS);
}

HTREEITEM TreeView_GetParent(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_PARENT);
}

HTREEITEM TreeView_GetFirstVisible(HWND ш) {
	return TreeView_GetNextItem(ш, null, TVGN_FIRSTVISIBLE);
}

HTREEITEM TreeView_GetNextVisible(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_NEXTVISIBLE);
}

HTREEITEM TreeView_GetPrevVisible(HWND ш, HTREEITEM i) {
	return TreeView_GetNextItem(ш, i, TVGN_PREVIOUSVISIBLE);
}

HTREEITEM TreeView_GetSelection(HWND ш) {
	return TreeView_GetNextItem(ш, null, TVGN_CARET);
}

HTREEITEM TreeView_GetDropHilight(HTREEITEM ш) {
	return TreeView_GetNextItem(ш, null, TVGN_DROPHILITE);
}

HTREEITEM TreeView_GetRoot(HWND ш) {
	return TreeView_GetNextItem(ш, null, TVGN_ROOT);
}

BOOL TreeView_Select(HWND ш, HTREEITEM i, UINT c) {
	return cast(BOOL) SendMessage(ш, TVM_SELECTITEM, c, cast(LPARAM) i);
}

BOOL TreeView_SelectItem(HWND ш, HTREEITEM i) {
	return TreeView_Select(ш, i, TVGN_CARET);
}

BOOL TreeView_SelectDropTarget(HWND ш, HTREEITEM i) {
	return TreeView_Select(ш, i, TVGN_DROPHILITE);
}

BOOL TreeView_SelectSetFirstVisible(HWND ш, HTREEITEM i) {
	return TreeView_Select(ш, i, TVGN_FIRSTVISIBLE);
}

BOOL TreeView_GetItem(HWND ш, LPTVITEM i) {
 return cast(BOOL) SendMessage(ш, TVM_GETITEM, 0, cast(LPARAM) i);
}

BOOL TreeView_SetItem(HWND ш, CPtr!(TV_ITEM) i) {
	return cast(BOOL) SendMessage(ш, TVM_SETITEM, 0, cast(LPARAM) i);
}

HWND TreeView_EditLabel(HWND ш, HTREEITEM i) {
	return cast(HWND) SendMessage(ш, TVM_EDITLABEL, 0, cast(LPARAM) i);
}

HWND TreeView_GetEditControl(HWND ш) {
	return cast(HWND) SendMessage(ш, TVM_GETEDITCONTROL, 0, 0);
}

UINT TreeView_GetVisibleCount(HWND ш) {
	return cast(UINT) SendMessage(ш, TVM_GETVISIBLECOUNT, 0, 0);
}

HTREEITEM TreeView_HitTest(HWND ш, LPTVHITTESTINFO p) {
	return cast(HTREEITEM) SendMessage(ш, TVM_HITTEST, 0, cast(LPARAM) p);
}

HIMAGELIST TreeView_CreateDragImage(HWND ш, HTREEITEM i) {
	return cast(HIMAGELIST) SendMessage(ш, TVM_CREATEDRAGIMAGE, 0,
	  cast(LPARAM) i);
}

BOOL TreeView_SortChildren(HWND ш, HTREEITEM i, BOOL r) {
	return cast(BOOL) SendMessage(ш, TVM_SORTCHILDREN, r, cast(LPARAM) i);
}

BOOL TreeView_EnsureVisible(HWND ш, HTREEITEM i) {
	return cast(BOOL) SendMessage(ш, TVM_ENSUREVISIBLE, 0, cast(LPARAM) i);
}

BOOL TreeView_SortChildrenCB(HWND ш, LPTVSORTCB s, BOOL r) {
	return cast(BOOL) SendMessage(ш, TVM_SORTCHILDRENCB, r, cast(LPARAM) s);
}

BOOL TreeView_EndEditLabelNow(HWND ш, BOOL f) {
	return cast(BOOL) SendMessage(ш, TVM_ENDEDITLABELNOW, f, 0);
}

BOOL TreeView_GetISearchString(HWND ш, LPTSTR s) {
	return cast(BOOL) SendMessage(ш, TVM_GETISEARCHSTRING, 0, cast(LPARAM) s);
}

static if (_WIN32_IE >= 0x300) {
	DWORD ListView_ApproximateViewRect(HWND ш, int iw, int ih, int i) {
		return cast(DWORD) SendMessage(ш, LVM_APPROXIMATEVIEWRECT, i,
		  MAKELPARAM(cast(ushort)iw, cast(ushort)ih));
	}

	DWORD ListView_SetExtendedListViewStyle(HWND ш, DWORD s) {
		return cast(DWORD) SendMessage(ш, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, s);
	}

	DWORD ListView_GetExtendedListViewStyle(HWND ш) {
		return cast(DWORD) SendMessage(ш, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
	}

	BOOL ListView_SetColumnOrderArray(HWND ш, int i, int* a) {
		return cast(BOOL) SendMessage(ш, LVM_SETCOLUMNORDERARRAY,
		  cast(WPARAM) i, cast(LPARAM) a);
	}

	BOOL ListView_GetColumnOrderArray(HWND ш, int i, int* a) {
		return cast(BOOL) SendMessage(ш, LVM_GETCOLUMNORDERARRAY,
		  cast(WPARAM) i, cast(LPARAM) a);
	}

	HWND ListView_GetHeader(HWND ш) {
		return cast(HWND) SendMessage(ш, LVM_GETHEADER, 0, 0);
	}

	HCURSOR ListView_GetHotCursor(HWND ш) {
		return cast(HCURSOR) SendMessage(ш, LVM_GETHOTCURSOR, 0, 0);
	}

	INT ListView_GetHotItem(HWND ш) {
		return SendMessage(ш, LVM_GETHOTITEM, 0, 0);
	}

	BOOL ListView_GetSubItemRect(HWND ш, int i, int isi, int c, LPRECT p) {
		return cast(BOOL) SendMessage(ш, LVM_GETSUBITEMRECT, i,
		  p ? (p.left = c, p.top = isi, cast(LPARAM) p) : 0);
	}

	HCURSOR ListView_SetHotCursor(HWND ш, HCURSOR c) {
		return cast(HCURSOR) SendMessage(ш, LVM_SETHOTCURSOR, 0,
		  cast(LPARAM) c);
	}

	INT ListView_SetHotItem(HWND ш, INT i) {
		return SendMessage(ш, LVM_SETHOTITEM, cast(WPARAM) i, 0);
	}

	DWORD ListView_SetIconSpacing(HWND ш, int x, int y) {
		return cast(DWORD) SendMessage(ш, LVM_SETICONSPACING, 0,
		  MAKELONG(cast(ushort)x, cast(ushort)y));
	}

	INT ListView_SubItemHitTest(HWND ш, LPLVHITTESTINFO p) {
		return SendMessage(ш, LVM_SUBITEMHITTEST, 0, cast(LPARAM) p);
	}

	BOOL ListView_SetItemCountEx(HWND ш, int i, DWORD f) {
		return cast(BOOL) SendMessage(ш, LVM_SETITEMCOUNT, i, cast(LPARAM) f);
	}
	...
	HWND TreeView_GetToolTips(HWND ш) {
		return cast(HWND) SendMessage(ш, TVM_GETTOOLTIPS, 0, 0);
	}

	HWND TreeView_SetToolTips(HWND ш, HWND wt) {
		return cast(HWND) SendMessage(ш, TVM_SETTOOLTIPS, cast(WPARAM) wt, 0);
	}
}

static if (_WIN32_IE >= 0x400) {
	BOOL ListView_GetBkImage(HWND в, LPLVBKIMAGE plvbki) {
		return cast(BOOL) SendMessage(в, LVM_GETBKIMAGE, 0,
		  cast(LPARAM) plvbki);
	}

	BOOL ListView_SetBkImage(HWND в, LPLVBKIMAGE plvbki) {
		return cast(BOOL) SendMessage(в, LVM_SETBKIMAGE, 0,
		  cast(LPARAM) plvbki);
	}

	DWORD ListView_SetExtendedListViewStyleEx(HWND ш, DWORD m, DWORD s) {
		return cast(DWORD) SendMessage(ш, LVM_SETEXTENDEDLISTVIEWSTYLE, m, s);
	}

	VOID ListView_SetWorkAreas(HWND ш, INT n, LPRECT r) {
		SendMessage(ш, LVM_SETWORKAREAS, cast(WPARAM) n, cast(LPARAM) r);
	}

	VOID ListView_GetWorkAreas(HWND ш, INT n, LPRECT r) {
		SendMessage(ш, LVM_GETWORKAREAS, cast(WPARAM) n, cast(LPARAM) r);
	}

	BOOL ListView_GetNumberOfWorkAreas(HWND ш, LPUINT n) {
		return cast(BOOL) SendMessage(ш, LVM_GETNUMBEROFWORKAREAS, 0,
		  cast(LPARAM) n);
	}

	DWORD ListView_SetHoverTime(HWND ш, DWORD t) {
		return cast(DWORD) SendMessage(ш, LVM_SETHOVERTIME, 0,
		  cast(LPARAM) t);
	}

	DWORD ListView_GetHoverTime(HWND ш) {
		return cast(DWORD) SendMessage(ш, LVM_GETHOVERTIME, 0, 0);
	}

	INT ListView_GetSelectionMark(HWND ш) {
		return SendMessage(ш, LVM_GETSELECTIONMARK, 0, 0);
	}

	INT ListView_SetSelectionMark(HWND ш, INT i) {
		return SendMessage(ш, LVM_SETSELECTIONMARK, 0, cast(LPARAM) i);
	}

	HWND ListView_SetToolTips(HWND ш, HWND n) {
		return cast(HWND) SendMessage(ш, LVM_SETTOOLTIPS, cast(WPARAM) n, 0);
	}

	HWND ListView_GetToolTips(HWND ш) {
		return cast(HWND) SendMessage(ш, LVM_GETTOOLTIPS, 0, 0);
	}

	BOOL ListView_SetUnicodeFormat(HWND ш, BOOL f) {
		return cast(BOOL) SendMessage(ш, LVM_SETUNICODEFORMAT,
		  cast(WPARAM) f, 0);
	}

	BOOL ListView_GetUnicodeFormat(HWND ш) {
		return cast(BOOL) SendMessage(ш, LVM_GETUNICODEFORMAT, 0, 0);
	}
	...
	COLORREF TreeView_GetBkColor(HWND ш) {
		return cast(COLORREF) SendMessage(ш, TVM_GETBKCOLOR, 0, 0);
	}

	COLORREF TreeView_GetInsertMarkColor(HWND ш) {
		return cast(COLORREF) SendMessage(ш, TVM_GETINSERTMARKCOLOR, 0, 0);
	}

	int TreeView_GetItemHeight(HWND ш) {
		return SendMessage(ш, TVM_GETITEMHEIGHT, 0, 0);
	}

	UINT TreeView_GetScrollTime(HWND ш) {
		return cast(UINT) SendMessage(ш, TVM_GETSCROLLTIME, 0, 0);
	}

	COLORREF TreeView_GetTextColor(HWND ш) {
		return cast(COLORREF) SendMessage(ш, TVM_GETTEXTCOLOR, 0, 0);
	}

	COLORREF TreeView_SetBkColor(HWND ш, COLORREF c) {
		return cast(COLORREF) SendMessage(ш, TVM_SETBKCOLOR, 0,
		  cast(LPARAM) c);
	}

	COLORREF TreeView_SetInsertMarkColor(HWND ш, COLORREF c) {
		return cast(COLORREF) SendMessage(ш, TVM_SETINSERTMARKCOLOR, 0,
		  cast(LPARAM) c);
	}

	int TreeView_SetItemHeight(HWND ш, SHORT в) {
		return SendMessage(ш, TVM_SETITEMHEIGHT, cast(WPARAM) в, 0);
	}

	UINT TreeView_SetScrollTime(HWND ш, UINT t) {
		return cast(UINT) SendMessage(ш, TVM_SETSCROLLTIME, cast(WPARAM) t, 0);
	}

	COLORREF TreeView_SetTextColor(HWND ш, COLORREF c) {
		return cast(COLORREF) SendMessage(ш, TVM_SETTEXTCOLOR, 0,
		  cast(LPARAM) c);
	}

	BOOL TreeView_SetInsertMark(HWND ш, HTREEITEM i, BOOL a) {
		return cast(BOOL) SendMessage(ш, TVM_SETINSERTMARK, cast(WPARAM) a,
		  cast(LPARAM) i);
	}

	BOOL TreeView_SetUnicodeFormat(HWND ш, BOOL u) {
		return cast(BOOL) SendMessage(ш, TVM_SETUNICODEFORMAT,
		  cast(WPARAM) u, 0);
	}

	BOOL TreeView_GetUnicodeFormat(HWND ш) {
		return cast(BOOL) SendMessage(ш, TVM_GETUNICODEFORMAT, 0, 0);
	}

	HTREEITEM TreeView_GetLastVisible(HWND ш) {
		return TreeView_GetNextItem(ш, null, TVGN_LASTVISIBLE);
	}
}

static if (_WIN32_IE >= 0x500) {
	UINT TreeView_GetItemState(HWND ш, HTREEITEM i, UINT m) {
		return cast(UINT) SendMessage(ш, TVM_GETITEMSTATE, cast(WPARAM) i,
		  cast(LPARAM) m);
	}

	BOOL TreeView_SetItemState(HWND ш, HTREEITEM i, UINT d, UINT m) {
		TVITEM _tvi;
		_tvi.mask = TVIF_STATE;
		_tvi.hItem = i;
		_tvi.stateMask = m;
		_tvi.state = d;
		return cast(BOOL) SendMessage(ш, TVM_SETITEM, 0, cast(LPARAM) &_tvi);
	}
}
