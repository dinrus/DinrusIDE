...
bool SshShell::пуск(int mode_, const Ткст& terminal, размер pagesize, const Ткст& tmodes)
{
	mode  = mode_;
	psize = pagesize;
...
void SshShell::ReadWrite(Ткст& in, const void* out, int out_len)
{
	switch(mode) {
		case GENERIC: {
			if(out_len > 0)
...
#ifdef PLATFORM_POSIX
void SshShell::консЧмтай()
{
	if(!СобытиеWait(STDIN_FILENO, WAIT_READ, 0))
		return;
	Буфер<char> буфер(ssh->chunk_size);
	auto n = read(STDIN_FILENO, буфер, size_t(ssh->chunk_size));
	if(n > 0)
		Send(Ткст(буфер, n));
	else
	if(n == -1 && errno != EAGAIN)
...
void SshShell::консПиши(const void* буфер, int len)
{
	if(!СобытиеWait(STDOUT_FILENO, WAIT_WRITE, 0))
		return;
	auto n = write(STDOUT_FILENO, буфер, size_t(len));
	if(n == -1 && errno != EAGAIN)
...
void SshShell::х11Цикл()
{
#ifdef PLATFORM_POSIX
	if(xrequests.пустой())
		return;

	for(int i = 0; i < xrequests.дайСчёт(); i++) {
...
SshShell& SshShell::ForwardX11(const Ткст& host, int дисплей, int screen, int bufsize)
{
	if(!xenabled) {
		xenabled = true;
#ifdef PLATFORM_POSIX
