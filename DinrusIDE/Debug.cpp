...
void Ide::RunArgs() {
	...
	dlg.darkmode.SetLabel(IsSystemThemeDark() ? "Запустить в светлом режиме" : "Запустить в тёмном режиме");
#endif
	...
	auto Ins = [&](bool file) {
		...
		String s = file ? SelectFileOpen("Все файлы\t*.*") : SelectDirectory();
...
void Ide::LaunchTerminal(const char *dir)
{
	...
	h.ChDir(dir);
#ifdef PLATFORM_WIN32
	h.Launch(Nvl(HostConsole, "bash.exe"), false);
	...
	sdk(sdk)
{
	...
	devicesArray.AddColumn("Серийный Номер");
	devicesArray.AddColumn("Состояние");
