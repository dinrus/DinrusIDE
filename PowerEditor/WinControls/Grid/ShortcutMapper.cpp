...
void ShortcutMapper::initTabs()
{
    ...
    for (size_t i = 0; i < _nbTab; ++i)
    {
        ...
        tie.pszText = const_cast<LPWSTR>(_tabNames[i].Begin());
...
String ShortcutMapper::getTabString(size_t i) const
{
    if (i >= _nbTab)
        return TEXT("");
...
{
    if (_shortcutFilter.IsEmpty())
        return true;

    String shortcut_name = stringToLower(String(sc.getName()));
...
bool ShortcutMapper::isFilterValid(PluginCmdShortcut sc)
{
    ...
    bool match = false;
    String module_name = stringToLower(String(sc.getModuleName()));
...
void ShortcutMapper::fillOutBabyGrid()
{
    ...
    String nameStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnName", TEXT("Name"));
    String shortcutStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnShortcut", TEXT("Shortcut"));

    _babygrid.setText(0, 1, nameStr.Begin());
    _babygrid.setText(0, 2, shortcutStr.Begin());
    ...
    switch(_currentState)
    {
        case STATE_MENU:
        {
            ...
            String categoryStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnCategory", TEXT("Category"));
            _babygrid.setText(0, 3, categoryStr.Begin());
        }
        break;
        ...
        case STATE_PLUGIN:
        {
            ...
            _babygrid.setLineColNumber(nbItems, 3);
            String pluginStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnPlugin", TEXT("Plugin"));
            _babygrid.setText(0, 3, pluginStr.Begin());
        }
        break;
            ...
            for (size_t i = 0; i < nbItems; ++i)
            {
                if (isFilterValid(cshortcuts[i]))
                {
                    ...
                    if (cshortcuts[i].isEnabled()) //avoid empty strings for better performance
                        _babygrid.setText(cs_index, 2, cshortcuts[i].toString().Begin());
    ...
    switch (message)
    {
        ...
        case NPPM_INTERNAL_FINDKEYCONFLICTS:
        {
            ...
            *reinterpret_cast<bool*>(lParam) = isConflict;
            ...
            if (isConflict)
                ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(conflictInfo.Begin()));
            else
                ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(_conflictInfoEditing.Begin()));
                    ...
                    switch (HIWORD(wParam))
                    {
                        ...
                        case BGN_ROWCHANGED:
                        {
                            ...
                            if (conflictInfo.IsEmpty())
                                ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(_conflictInfoOk.Begin()));
                            else
                                ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(conflictInfo.Begin()));
...
bool ShortcutMapper::findKeyConflicts(__inout_opt String * const keyConflictLocation,
