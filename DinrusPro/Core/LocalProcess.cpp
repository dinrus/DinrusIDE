...
#endif
#ifdef PLATFORM_POSIX
	LLOG("\nLocalProcess::освободи, pid = " << (цел)getpid());
...
{ // provides conversion of charset for cmdline
	...
	cmd.добавь(0);
	return CreateProcessW(NULL, cmd, NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS, (проц *)envptr,
	                      cd ? вСисНабсимШ(cd).begin() : NULL, &si, &pi);
}
#endif

бул ЛокальнПроцесс::DoStart(кткст0 command, const Вектор<Ткст> *арг, бул spliterr, кткст0 envptr, кткст0 cd)
{
	...
	if(арг) {
		...
		for(цел i = 0; i < арг->дайСчёт(); i++) {
			...
			if(argument.дайСчёт() && argument.найдиПервыйИз(" \t\n\v\"") < 0)
				...
				кткст0 s = argument;
				for(;;) {
					цел num_backslashes = 0;
					while(*s == '\\') {
						s++;
						num_backslashes++;
					}
					if(*s == '\0') {
						cmdh.кат('\\', 2 * num_backslashes);
						break;
					}
					else
					if(*s == '\"') {
						cmdh.кат('\\', 2 * num_backslashes + 1);
						cmdh << '\"';
					}
					else {
						cmdh.кат('\\', num_backslashes);
						cmdh.кат(*s);
					}
					s++;
				}
	...
	бул h = Win32CreateProcess(command, envptr, si, pi, cd);
	...
	pid = fork();
#endif
	LLOG("\tfork, pid = " << (цел)pid << ", getpid = " << (цел)getpid());
	if(pid < 0)
		return false;
//		throw Искл(фмтЧ(t_("fork() Ошибка; Ошибка код_ = %d"), errno));
	...
	if (doublefork) {
		pid_t pid2 = fork();
		LLOG("\tfork2, pid2 = " << (цел)pid2 << ", getpid = " << (цел)getpid());
		if (pid2 < 0) {
			LLOG("fork2 failed");
			выход(1);
		}
...
//	rpipe[1] = wpipe[0] = -1;
	...
	for(цел a = 0; a < арги.дайСчёт(); a++)
		LLOG("[" << a << "]: <" << (арги[a] ? арги[a] : "NULL") << ">");
#endif

	if(cd)
		(проц)!chdir(cd); // that (проц)! strange thing is to silence GCC warning
...
//	printf("Ошибка running '%s', Ошибка код_ %d\n", command, errno);
	exit(-errno);
	return true;
#endif
}
...
#ifdef PLATFORM_POSIX
бул ЛокальнПроцесс::DecodeExitCode(цел status)
{
	if(WIFEXITED(status)) {
		exit_code = (ббайт)WEXITSTATUS(status);
		return true;
	}
...
#ifdef PLATFORM_POSIX
	if(пущен()) {
		LLOG("\nLocalProcess::глуши, pid = " << (цел)pid);
		exit_code = 255;
		kill(pid, SIGTERM);
		дайКодВыхода();
		цел status;
...
бул ЛокальнПроцесс::пущен() {
#ifdef PLATFORM_WIN32
	бцел кодвыхода;
	if(!hProcess)
		return false;
	if(GetExitCodeProcess(hProcess, &кодвыхода) && кодвыхода == STILL_ACTIVE)
		return true;
	бцел n;
	...
	if(!( (wp = waitpid(pid, &status, WNOHANG | WUNTRACED)) == pid &&
	      DecodeExitCode(status) ))
		return true;
	LLOG("пущен() -> no, just exited, exit код_ = " << exit_code);
	return false;
#endif
}

цел  ЛокальнПроцесс::дайКодВыхода() {
#ifdef PLATFORM_WIN32
	return пущен() ? (цел)Null : exit_code;
#endif
#ifdef PLATFORM_POSIX
	if(!пущен())
...
бул ЛокальнПроцесс::читай(Ткст& res) {
	Ткст dummy;
	return читай2(res, dummy);
}

бул ЛокальнПроцесс::читай2(Ткст& reso, Ткст& rese)
{
...
#ifdef PLATFORM_POSIX
	Ткст res[2];
	бул was_running = пущен() || wpipe[0] >= 0 || epipe[0] >= 0;
	for (цел wp=0; wp<2;wp++) {
		цел *pipe = wp ? epipe : wpipe;
		if (pipe[0] < 0) {
			LLOG("пайп["<<wp<<"] closed");
			continue;
		}
...
проц ЛокальнПроцесс::пиши(Ткст s)
{
	if(convertcharset)
		s = вСисНабсим(s);
#ifdef PLATFORM_WIN32
	if (hInputWrite) {
		бул ret = true;
		бцел n;
		for(цел wn = 0; ret && wn < s.дайДлину(); wn += n) {
	...
	if (rpipe[1] >= 0) {
		цел ret=1;
		for(цел wn = 0; (ret > 0 || errno == EINTR) && wn < s.дайДлину(); wn += ret) {
			Ткст ho = wreso;
			Ткст he = wrese;
			wreso = wrese = Null;
			читай2(wreso, wrese);
...
проц ЛокальнПроцесс::закройЧтен()
{
#ifdef PLATFORM_WIN32
	if(hOutputRead) {
		CloseHandle(hOutputRead);
...
цел ЛокальнПроцесс::финиш(Ткст& out)
{
	out.очисть();
	while(пущен()) {
		Ткст h = дай();
