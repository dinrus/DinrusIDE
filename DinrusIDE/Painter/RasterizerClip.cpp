...
{
	if(пусто_ли(x1) || пусто_ли(y1) || пусто_ли(x2) || пусто_ли(y2))
		return;
...
void Rasterizer::Move(const ТочкаПЗ& p)
{
	LLOG("Rasterizer::Move " << p);
	p0 = p;
}

void Rasterizer::Строка(const ТочкаПЗ& p)
{
	PAINTER_TIMING("Строка");
	LLOG("Rasterizer::Строка " << p);
