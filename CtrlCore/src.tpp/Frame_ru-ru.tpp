...
Ctrl: рамка может рисоваться над этой 
кромкой, несмотря на тот факт, что 
она не принадлежит к прямоугольнику 
Ctrl`'а. Это используется для определённых 
`"скиннинговых`" эффектов (типа сияния 
вокруг EditField). Дефолтная реализация 
равна нулю.  &]
[s3; &]
[s0; &]
[s0; &]
