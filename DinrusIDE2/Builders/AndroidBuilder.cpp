	...
	for(int i = 0; i < pkg.GetCount(); i++) {
		...
		if(BuilderUtils::IsXmlFile(filePath)) {
			if(isMainPackage && fileName == AndroidManifest::FILE_NAME) {
				if (manifest) {
					PutConsole("Файл манифеста уже существует. в проекте может быть только один AndroidManifest.xml.");
					return false;
				}
				...
				if (!manifest->Parse()) {
					PutConsole("Не удалось разобрать AndroidManifest.xml.");
					return false;
				}
			}
		}
