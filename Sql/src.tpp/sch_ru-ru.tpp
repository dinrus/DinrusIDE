...
ype][3  структуры для мапирования рядоб 
...
:: [s0; [* SQL`_NAME(][*/ id][* )]]
:: [s0; Предоставялет альнернативое имя 
...
с предельных числом символов [/ maxlen], 
тип String в C`+`+.]
...
совместимости между рахными движками 
...
:: [s0; [*/ list] i`- это лмтерал Си с именами колонок, 
разделёнными запятыми. `[sqlite3`]]
:: [s0; [* CHECK`_IN(][*/ list][* )]]
:: [s0; Значение колонки констрейнтов к 
списку [*/ list], литералу Си, разделённуму 
запятыми.`[sqlite3`]]
...
CHECK, [*/ check] `- оитерал Си с выражением 
...
:: [s0; Вставляет комментарий в данный сценарийt.]}}&]
[s0;%RU-RU &]
[s0;3 &]
[s0;3 &]
[s0;%RU-RU ]]