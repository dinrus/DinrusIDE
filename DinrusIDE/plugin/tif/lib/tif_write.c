 ...
 * TIFF Library.
 *
 * Scanline-oriented пиши Support
 */
#include "tiffiop.h"
#include <stdio.h>
...
TIFFWriteScanline(TIFF* tif, void* buf, uint32 row, uint16 sample)
{
	...
	if (!WRITECHECKSTRIPS(tif, module))
		return (-1);
	/*
	 * Handle delayed allocation of data буфер.  This
	...
	if (strip != tif->tif_curstrip) {
		...
		/*
		 * Watch out for a growing image.  The значение of strips/image
		...
		if (strip >= td->td_stripsperimage && imagegrew)
			td->td_stripsperimage =
                ...
                if (td->td_stripsperimage == 0) {
                        TIFFErrorExt(tif->tif_clientdata, module, "обнули strips per image");
                        return (-1);
                }
		tif->tif_row =
	...
	/* swab if needed - note that source буфер will be altered */
...
/* сделай sure that at the first attempt of rewriting a tile/strip, we will have */
/* more bytes available in the output буфер than the previous byte count, */
 ...
 * specified strip.
 *
 * NB: Рисунок length must be setup before writing.
 */
tmsize_t
...
/*
 * пиши the supplied data to the specified strip.
 *
 * NB: Рисунок length must be setup before writing.
 */
tmsize_t
TIFFWriteRawStrip(TIFF* tif, uint32 strip, void* data, tmsize_t cc)
{
	...
	if (strip >= td->td_nstrips) {
		...
		/*
		 * Watch out for a growing image.  The значение of
...
/*
 * пиши and compress a tile of data.  The
...
TIFFWriteTile(TIFF* tif, void* buf, uint32 x, uint32 y, uint32 z, uint16 s)
{
	 ...
	 *     after the output буфер is setup in TIFFWriteBufferSetup.
	 */
 ...
 *
 * NB: Рисунок length must be setup before writing; this
...
TIFFWriteCheck(TIFF* tif, int tiles, const char* module)
{
	if (tif->tif_mode == O_RDONLY) {
		TIFFErrorExt(tif->tif_clientdata, module, "Файл not open for writing");
		return (0);
	}
...
/*
 * настрой the raw data буфер used for encoding.
 */
int
TIFFWriteBufferSetup(TIFF* tif, void* bp, tmsize_t size)
{
	...
	if (size == (tmsize_t)(-1)) {
		...
		/*
		 * сделай raw data буфер at least 8K
		 */
		if (size < 8*1024)
			size = 8*1024;
	...
	if (bp == NULL) {
		bp = _TIFFmalloc(size);
		if (bp == NULL) {
			TIFFErrorExt(tif->tif_clientdata, module, "No space for output буфер");
			return (0);
		}
		tif->tif_flags |= TIFF_MYBUFFER;
	} else
...
/*
 * приставь the data to the specified strip.
 */
static int
TIFFAppendToStrip(TIFF* tif, uint32 strip, uint8* data, tmsize_t cc)
{
	...
	if (td->td_stripoffset_p[strip] == 0 || tif->tif_curoff == 0) {
            ...
            {
                /* 
                 * перейди to end of file, and set that as our location to 
                 * write this strip.
                 */
	...
	if (!WriteOK(tif, data, cc)) {
		TIFFErrorExt(tif->tif_clientdata, module, "пиши Ошибка at scanline %lu",
		    (unsigned long) tif->tif_row);
		    return (0);
	}
	tif->tif_curoff = m;
...
TIFFFlushData1(TIFF* tif)
{
	if (tif->tif_rawcc > 0 && tif->tif_flags & TIFF_BUF4WRITE ) {
		...
		if (!TIFFAppendToStrip(tif,
		    ...
		    tif->tif_rawdata, tif->tif_rawcc))
        {
            /* We update those variables even in case of Ошибка since there's */
