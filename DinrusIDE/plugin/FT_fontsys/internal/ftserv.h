...
#define FT_FACE_FIND_GLOBAL_SERVICE( face, ptr, id )               \
  ...
  {
    const char*  serv_id;     /* service имя         */
...
#define FT_DEFINE_SERVICEDESCREC6(class_, serv_id_1, serv_data_1,            \
   ...
   *  a specific service by ИД.  Note that the last element in the
   ...
   *  services.  It is the тип of `face->internal->services'.  You
   *  should only use FT_FACE_LOOKUP_SERVICE to access it.
   *
   *  All fields should have the тип FT_Pointer to relax compilation
...
#define FT_DEFINE_SERVICE( имя )            \
  typedef struct FT_Service_ ## имя ## Rec_ \
    FT_Service_ ## имя ## Rec ;             \
  typedef struct FT_Service_ ## имя ## Rec_ \
    const * FT_Service_ ## имя ;            \
  struct FT_Service_ ## имя ## Rec_
