...
void Ide::SetupEditor(int f, String hl, String path)
{
	...
	editor.WarnWhiteSpace(warnwhitespace &&
	                      findarg(hl, "cpp", "d", "java", "js", "cs", "json", "css", "lay", "sch", "t", "usc") >= 0);
...
void Ide::FileProperties()
{
	...
	d.font.Add(2, "Особый");
	d.font.Add(3, "Консоль");
	d.highlight.Add(Null, "Дефолт");
	...
	d.spellcheck_comments.Add(Null, "Дефолт");
...
void Ide::ChangeCharset()
{
	...
	for(;;) {
		...
		if(q >= 0) {
			if(PromptYesNo("Файл содержит символы, которые нельзя сохранить в текущей кодировке.&"
			               "Сохранить всё равно? (Если выберите Да, они будут заменены вопросительными знаками.)"))
								break;
			editor.SetCursor(q);
			return;
		}
