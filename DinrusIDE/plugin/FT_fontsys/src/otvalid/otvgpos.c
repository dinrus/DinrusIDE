...
#define MarkMarkPosFormat1Func  otv_u_O_O_u_O_O
  ...
  static FT_UInt
  otv_value_length( FT_UInt  формат )
  {
    FT_UInt  count;


    count = ( ( формат & 0xAA ) >> 1 ) + ( формат & 0x55 );
                            ...
                            OTV_Validator  valid )
  {
    ...
    /* дисплей `формат' in dual representation */
    ...
    for ( count = 4; count > 0; count-- )
    {
      if ( формат & 1 )
      {
        ...
        /* XXX: this значение is usually too small, especially if the current */
    ...
    case 1:     /* SinglePosFormat1 */
      {
        ...
        otv_ValueRecord_validate( p, ValueFormat, valid ); /* Значение */
      }
      break;
