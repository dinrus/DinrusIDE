...
typedef int sword;
typedef int sb4;
typedef РНЦП::dword uword;
typedef РНЦП::dword ub4;
typedef short sb2;
typedef РНЦП::word ub2;
typedef signed char sb1;
typedef РНЦП::byte ub1;
typedef РНЦП::byte OraText;
enum
{
	// OCIInitialize modes
	...
	OCI_DEFAULT      = 0x00,  // the default значение for parameters and attributes
	OCI_DATA_AT_EXEC = 0x02,  // data at execute time

	OCI_HTYPE_FIRST                = 1, // start значение of handle тип
	OCI_HTYPE_ENV                  = 1, // environment handle
	OCI_HTYPE_ERROR                = 2, // Ошибка handle
	...
	OCI_HTYPE_LAST                 = 17, // last значение of a handle тип
	...
	SQLT_RID = 11, // rowid
	SQLT_DAT = 12, // date in oracle формат
	SQLT_VBI = 15, // binary in VCS формат
	SQLT_BIN = 23, // binary data(DTYBIN)
	SQLT_LBI = 24, // long binary
	SQLT_UIN = 68, // unsigned integer
	SQLT_SLS = 91, // Дисплей sign leading separate
	SQLT_LVC = 94, // Longer longs (char)
	SQLT_LVB = 95, // Longer long binary
	SQLT_AFC = 96, // Ansi fixed char
	SQLT_AVC = 97, // Ansi Var char
	SQLT_CUR = 102, // cursor тип
	SQLT_RDD = 104, // rowid descriptor
	SQLT_LAB = 105, // label тип
	SQLT_OSL = 106, // oslabel тип

	SQLT_NTY = 108, // named object тип
	SQLT_REF = 110, // ref тип
	SQLT_CLOB = 112, // character lob
	SQLT_BLOB = 113, // binary lob
	SQLT_BFILEE = 114, // binary file lob
	SQLT_CFILEE = 115, // character file lob
	SQLT_RSET = 116, // result set тип
	SQLT_NCO = 122, // named collection тип (varray or nested table)
	SQLT_VST = 155, // OCIString тип
	SQLT_ODT = 156, // OCIDate тип

	// datetimes and intervals
	SQLT_DATE = 184, // ANSI Дата
	SQLT_TIME = 185, // TIME
	...
	OCI_DTYPE_FIRST = 50, // start значение of descriptor тип
	...
	OCI_DTYPE_FILE = 56, // Файл Lob locator
	...
	OCI_DTYPE_DATE = 65, // Дата
	OCI_DTYPE_TIME = 66, // Время
	OCI_DTYPE_TIME_TZ = 67, // Время with timezone
	...
	OCI_DTYPE_LAST = 71, // last значение of a descriptor тип
	...
	OCI_STMT_INSERT  = 4, // вставь Statement
	...
	OCI_STILL_EXECUTING      = -3123,  // OCI would block Ошибка
	...
	OCI_ATTR_DATA_TYPE     = 2, // the SQL тип of the column/argument
	OCI_ATTR_NAME          = 4, // the имя of the column/argument
	OCI_ATTR_PRECISION     = 5, // precision if number тип
	OCI_ATTR_SCALE         = 6, // scale if number тип
	OCI_ATTR_ROW_COUNT     = 9, // the rows processed so far
	OCI_ATTR_PREFETCH_ROWS = 11, // sets the number of rows to prefetch
	OCI_ATTR_STMT_TYPE     = 24, // statement тип
...
struct cda_def {
    ...
    ub2          ft;                                    /* SQL function тип */
    ub4          rpc;                                /* rows processed count */
    ub2          peo;                                  /* parse Ошибка offset */
    ...
    sword        ose;                                 /* OSD dependent Ошибка */
    ub1		 chk;
    dvoid        *rcsp;                          /* pointer to reserved area */
    ub1          rcs9[CDA_SIZE - sizeof (struct cda_head)];        /* filler */
};

typedef sb4 (*OCICallbackInBind)(dvoid *ictxp, РНЦП::OCIBind *bindp, ub4 iter,
	ub4 Индекс, dvoid **bufpp, ub4 *alenp,
	ub1 *piecep, dvoid **indp);

typedef sb4 (*OCICallbackOutBind)(dvoid *octxp, РНЦП::OCIBind *bindp, ub4 iter,
	ub4 Индекс, dvoid **bufpp, ub4 **alenp,
	ub1 *piecep, dvoid **indp,
	ub2 **rcodep);
