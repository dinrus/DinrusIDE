...
#define FT_COMPONENT  trace_pshrec
                       ...
                       PS_Hint       *ahint )
  {
    ...
    {
      Ошибка = ps_hint_table_ensure( table, count, memory );
      if ( Ошибка )
        goto Exit;
    }
  ...
  /* test a bit значение in a given mask */
  static FT_Int
                   ...
                   FT_Memory  memory )
  {
    ...
    if ( (FT_UInt)idx >= mask->num_bits )
    {
      Ошибка = ps_mask_ensure( mask, idx + 1, memory );
      if ( Ошибка )
        goto Exit;
                       ...
                       PS_Mask       *amask )
  {
    ...
    {
      Ошибка = ps_mask_table_ensure( table, count, memory );
      if ( Ошибка )
        goto Exit;
    }
                      ...
                      PS_Mask       *amask )
  {
    ...
    if ( count == 0 )
    {
      Ошибка = ps_mask_table_alloc( table, memory, &mask );
      if ( Ошибка )
        goto Exit;
    }
    else
                          ...
                          FT_Memory       memory )
  {
    ...
    Ошибка = ps_mask_table_last( table, memory, &mask );
    if ( Ошибка )
      goto Exit;

    Ошибка = ps_mask_ensure( mask, bit_count, memory );
    if ( Ошибка )
      goto Exit;
                       ...
                       FT_Memory      memory )
  {
    ...
    if ( index1 < index2 && index1 >= 0 && index2 < (FT_Int)table->num_masks )
    {
      ...
      if ( count2 > 0 )
      {
        ...
        if ( count2 > count1 )
        {
          Ошибка = ps_mask_ensure( mask1, count2, memory );
          if ( Ошибка )
            goto Exit;
                           ...
                           FT_Memory      memory )
  {
    ...
    for ( index1 = table->num_masks - 1; index1 > 0; index1-- )
    {
      for ( index2 = index1 - 1; index2 >= 0; index2-- )
      {
        ...
        {
          Ошибка = ps_mask_table_merge( table, index2, index1, memory );
          if ( Ошибка )
            goto Exit;
                             ...
                             FT_Memory     memory )
  {
    ...
    /* get last hint mask */
    Ошибка = ps_mask_table_last( &dim->masks, memory, &mask );
    if ( Ошибка )
      goto Exit;

    Ошибка = ps_mask_set_bit( mask, idx, memory );
  ...
  /* set the end point in a mask, called from "стоп" & "переустанов" methods */
  static void
                              ...
                              FT_Memory       memory )
  {
    ...
    Ошибка = ps_dimension_reset_mask( dim, end_point, memory );
    if ( Ошибка )
      goto Exit;

    /* set bits in new mask */
    Ошибка = ps_mask_table_set_bits( &dim->masks, source,
                           ...
                           FT_Int       *aindex )
  {
    ...
    /* now, lookup stem in the current hints table */
    {
      ...
      if ( idx >= max )
      {
        Ошибка = ps_hint_table_alloc( &dim->hints, memory, &hint );
        if ( Ошибка )
          goto Exit;
                            ...
                            FT_Memory     memory )
  {
    ...
    if ( count == 0 )
    {
      Ошибка = ps_mask_table_alloc( &dim->counters, memory, &counter );
      if ( Ошибка )
        goto Exit;
    }

    /* now, set the bits for our hints in the counter mask */
    Ошибка = ps_mask_set_bit( counter, hint1, memory );
    if ( Ошибка )
      goto Exit;

    Ошибка = ps_mask_set_bit( counter, hint2, memory );
    if ( Ошибка )
      goto Exit;

    Ошибка = ps_mask_set_bit( counter, hint3, memory );
    if ( Ошибка )
      goto Exit;
                 ...
                 PS_Hint_Type  hint_type )
  {
    ...
    default:
      hints->Ошибка     = PSH_Err_Invalid_Argument;
      hints->hint_type = hint_type;

      FT_TRACE0(( "ps_hints_open: invalid charstring тип\n" ));
      break;
    }
  }
                 ...
                 FT_Long*  stems )
  {
    if ( !hints->Ошибка )
    {
      ...
      case PS_HINT_TYPE_2:  /* Type 2 "hstem" or "vstem" operator */
        {
          ...
          for ( ; count > 0; count--, stems += 2 )
          {
            ...
            Ошибка = ps_dimension_add_t1stem(
      ...
      default:
        FT_TRACE0(( "ps_hints_stem: called with invalid hint тип (%d)\n",
                    hints->hint_type ));
        break;
      }
    }
                    ...
                    FT_Fixed*  stems )
  {
    ...
    if ( !hints->Ошибка )
    {
      ...
      if ( hints->hint_type == PS_HINT_TYPE_1 )
      {
        ...
        {
          Ошибка = ps_dimension_add_t1stem( dim,
        ...
        Ошибка = ps_dimension_add_counter( dim, idx[0], idx[1], idx[2],
                                          memory );
        if ( Ошибка )
          goto Fail;
      }
      ...
      {
        FT_ERROR(( "ps_hints_t1stem3: called with invalid hint тип\n" ));
        Ошибка = PSH_Err_Invalid_Argument;
        goto Fail;
      }
    }
                    ...
                    FT_UInt   end_point )
  {
    ...
    if ( !hints->Ошибка )
    {
      ...
      if ( hints->hint_type == PS_HINT_TYPE_1 )
      {
        Ошибка = ps_dimension_reset_mask( &hints->dimension[0],
                                         end_point, memory );
        if ( Ошибка )
          goto Fail;

        Ошибка = ps_dimension_reset_mask( &hints->dimension[1],
                                         end_point, memory );
        if ( Ошибка )
          goto Fail;
      }
                   ...
                   const FT_Byte*  bytes )
  {
    ...
    if ( !hints->Ошибка )
    {
      ...
      Ошибка = ps_dimension_set_mask_bits( &dim[0], bytes, count2, count1,
                                          end_point, memory );
      if ( Ошибка )
        goto Fail;

      Ошибка = ps_dimension_set_mask_bits( &dim[1], bytes, 0, count2,
                                          end_point, memory );
      if ( Ошибка )
        goto Fail;
    }
                      ...
                      const FT_Byte*  bytes )
  {
    ...
    if ( !hints->Ошибка )
    {
      ...
      /* set-up new horizontal and vertical hint mask now */
      Ошибка = ps_dimension_set_mask_bits( &dim[0], bytes, 0, count1,
                                          0, memory );
      if ( Ошибка )
        goto Fail;

      Ошибка = ps_dimension_set_mask_bits( &dim[1], bytes, count1, count2,
                                          0, memory );
      if ( Ошибка )
        goto Fail;
    }
                  ...
                  FT_UInt   end_point )
  {
    ...
    if ( !Ошибка )
    {
      ...
      Ошибка = ps_dimension_end( &dim[0], end_point, memory );
      if ( !Ошибка )
      {
        Ошибка = ps_dimension_end( &dim[1], end_point, memory );
      }
    }
