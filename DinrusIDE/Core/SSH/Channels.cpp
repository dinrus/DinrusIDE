...
bool SshChannel::SetEnv(const Ткст& variable, const Ткст& значение)
{
	return пуск([=]() mutable {
		int rc = libssh2_channel_setenv(*channel, variable, значение);
		if(!WouldBlock(rc) && rc < 0) выведиОш(rc);
		if(!rc)	LLOG("систСреда variable '" << variable << "' set to " << значение);
		return !rc;
	});
}
