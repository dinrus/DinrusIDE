...
static int JBIGDecode(TIFF* tif, uint8* буфер, tmsize_t size, uint16 s)
{
	 ...
	 * header.  I do not log when an Ошибка occurs because that will cause
	...
	if (JBG_EOK != decodeStatus)
	{
		 ...
		 * JBIG-KIT. Since the 2.0 the Ошибка reporting functions were
		 * changed. We will handle both cases here.
		 */
		TIFFErrorExt(tif->tif_clientdata,
			     "JBIG", "Ошибка (%d) decoding: %s",
			     decodeStatus,
#if defined(JBG_EN)
	...
	_TIFFmemcpy(буфер, pImage, decodedSize);
	jbg_dec_free(&decoder);
...
static void JBIGOutputBie(unsigned char* буфер, size_t len, void* userData)
{
	TIFF* tif = (TIFF*)userData;

	if (isFillOrder(tif, tif->tif_dir.td_fillorder))
	{
		TIFFReverseBits(буфер, (tmsize_t)len);
	}

	JBIGCopyEncodedData(tif, буфер, len, 0);
}

static int JBIGEncode(TIFF* tif, uint8* буфер, tmsize_t size, uint16 s)
{
...
int TIFFInitJBIG(TIFF* tif, int scheme)
{
	...
	/* настрой the function pointers for encode, decode, and cleanup. */
