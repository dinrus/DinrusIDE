...
void RasterFormat::пиши(byte *t, const КЗСА *s, int cx, const PaletteCv *palcv) const
{
	const КЗСА *e = s + cx;
	switch(тип) {
	case RASTER_1:
	...
	case RASTER_4:
		if(palcv) {
			const byte *e = t + (cx >> 1);
			while(t < e) {
				*t++ = (palcv->дай(s[0]) & 15) | ((palcv->дай(s[1]) & 15) << 4);
				s += 2;
			}
			if(cx & 1)
				*t = palcv->дай(*s) & 15;
	...
	case RASTER_4|RASTER_MSBFIRST:
		if(palcv) {
			const byte *e = t + (cx >> 1);
			while(t < e) {
				*t++ = (palcv->дай(s[1]) & 15) | ((palcv->дай(s[0]) & 15) << 4);
				s += 2;
			}
			if(cx & 1)
				*t = (palcv->дай(*s) & 15) << 4;
