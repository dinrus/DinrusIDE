...
void Coco_PaintCh(void *cgcontext, int тип, int значение, int state)
{
	...
	if(тип == COCO_NSCOLOR) {
		...
		CGContextSetFillColorWithColor(cg, РНЦП::decode(значение,
	...
	if(тип == COCO_NSIMAGE) {
		NSImage *img = [NSImage imageNamed:(значение ? NSImageNameInfo : NSImageNameCaution)];
		...
		if(РНЦП::findarg(тип, COCO_SCROLLTHUMB, COCO_SCROLLTRACK) >= 0) {
			...
			bc.allowsMixedState = тип == COCO_CHECKBOX;
			bc.title = @"";
			bc.controlSize = тип == COCO_RADIOBUTTON ? NSControlSizeSmall : NSControlSizeRegular;
			bc.frame = CGRectMake(0, 0, 140, 40);
			bc.buttonType = РНЦП::decode(тип, COCO_CHECKBOX, NSButtonTypeSwitch, COCO_RADIOBUTTON, NSButtonTypeRadio, NSButtonTypePushOnPushOff);
			bc.bezelStyle = тип == COCO_BUTTON ? NSBezelStyleRounded : NSBezelStyleRegularSquare;
			bc.state = РНЦП::decode(значение, 0, NSControlStateValueOff, 1, NSControlStateValueOn, NSControlStateValueMixed);
	        [bc highlight: state == РНЦП::CTRL_PRESSED];
			bc.enabled = state != РНЦП::CTRL_DISABLED;
