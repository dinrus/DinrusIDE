...
static ШТкст sReadLn(const wchar *p)
{
	ШТкстБуф wbuf;
	while(*p && *p <= ' ')
		p++;
...
void CSyntax::ScanSyntax(const wchar *ln, const wchar *e, int line, int tab_size)
{
	...
	if(!comment && *p == '#') {
		...
		|| idlen == 6 && id[2] == 'n' && id[3] == 'd' && id[4] == 'e' && id[5] == 'f')) {
			IfState& ifstate = ifstack.добавь();
...
void CSyntax::сериализуй(Поток& s)
{
	s % comment;
	s % linecomment;
	s % string;
