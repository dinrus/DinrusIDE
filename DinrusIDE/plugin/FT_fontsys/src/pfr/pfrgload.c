...
#define FT_COMPONENT  trace_pfr
                     ...
                     FT_Vector*  to )
  {
    ...
    Ошибка = FT_GLYPHLOADER_CHECK_POINTS( loader, 1, 0 );
    if ( !Ошибка )
    {
                      ...
                      FT_Vector*  to )
  {
    ...
    Ошибка = FT_GLYPHLOADER_CHECK_POINTS( loader, 3, 0 );
    if ( !Ошибка )
    {
                     ...
                     FT_Vector*  to )
  {
    ...
    Ошибка = FT_GLYPHLOADER_CHECK_POINTS( loader, 1, 1 );
    if ( !Ошибка )
      /* add new start point */
      Ошибка = pfr_glyph_line_to( glyph, to );
                         ...
                         FT_Byte*   limit )
  {
    ...
    /* now load a simple glyph */
    {
      ...
      for (;;)
      {
        FT_UInt  формат, format_low, args_format = 0, args_count, n;
        ...
        /*                                                             */
        PFR_CHECK( 1 );
        формат     = PFR_NEXT_BYTE( p );
        format_low = формат & 15;
        ...
        for ( n = 0; n < args_count; n++ )
        {
          ...
          case 1:                           /* 16-bit absolute значение */
            PFR_CHECK( 2 );
          ...
          /* read the additional формат flag for the general curve */
          if ( n == 0 && args_count == 4 )
          {
            PFR_CHECK( 1 );
        ...
        case 2:
        case 3:
          Ошибка = pfr_glyph_line_to( glyph, pos );
          goto Test_Error;

        case 4:                             /* move to inside contour  */
        case 5:                             /* move to outside contour */
          Ошибка = pfr_glyph_move_to( glyph, pos );
          goto Test_Error;

        default:                            /* curve operations */
          Ошибка = pfr_glyph_curve_to( glyph, pos, pos + 1, pos + 2 );

        Test_Error:  /* test Ошибка condition */
          if ( Ошибка )
            goto Exit;
        }
      } /* for (;;) */
                           ...
                           FT_Byte*   limit )
  {
    ...
    /* the PFR формат is dumb, using direct file offsets to point to the */
    ...
    for ( i = 0; i < count; i++, subglyph++ )
    {
      ...
      if ( формат & PFR_SUBGLYPH_2BYTE_SIZE )
      {
        PFR_CHECK( 2 );
      ...
      if ( формат & PFR_SUBGLYPH_3BYTE_OFFSET )
      {
        PFR_CHECK( 3 );
                      ...
                      FT_ULong   size )
  {
    ...
    if ( size > 0 && *p & PFR_GLYPH_IS_COMPOUND )
    {
      ...
      Ошибка = pfr_glyph_load_compound( glyph, p, limit );
      ...
      for ( n = 0; n < count; n++ )
      {
        ...
        Ошибка = pfr_glyph_load_rec( glyph, stream, gps_offset,
                                    subglyph->gps_offset,
                                    subglyph->gps_size );
        if ( Ошибка )
          break;
    ...
    else
    {
      ...
      /* load a simple glyph */
      Ошибка = pfr_glyph_load_simple( glyph, p, limit );
