...
#define AFM_STREAM_KEY_LEN( stream, ключ )       \
          ( (char*)(stream)->cursor - ключ - 1 )
...
#define AFM_STATUS_EOF( stream ) \
  ...
  /* read a ключ or значение in current column */
  static char*
                        ...
                        FT_UInt     n )
  {
    ...
    for ( i = 0; i < n; i++ )
    {
      ...
      if ( val->тип == AFM_VALUE_TYPE_STRING )
  ...
  {
    AFM_Stream  stream = parser->stream;
    char*       ключ    = 0;  /* make stupid compiler happy */
    ...
    if ( line )
    {
      while ( 1 )
      {
        ...
        ключ = afm_stream_read_one( stream );
    ...
    if ( len )
      *len = ( ключ ) ? (FT_Offset)AFM_STREAM_KEY_LEN( stream, ключ )
                     : 0;
    ...
    for ( n = 0; n < N_AFM_TOKENS; n++ )
    {
      if ( *( afm_key_table[n] ) == *ключ )
      {
        for ( ; n < N_AFM_TOKENS; n++ )
        {
          ...
          if ( ft_strncmp( afm_key_table[n], ключ, len ) == 0 )
            return (AFM_Token) n;
        }
      }
    }
                       ...
                       FT_Int*     aint )
  {
    ...
    val.тип = AFM_VALUE_TYPE_INTEGER;
  ...
  afm_parse_track_kern( AFM_Parser  parser )
  {
    ...
    while ( ( ключ = afm_parser_next_key( parser, 1, &len ) ) != 0 )
    {
      AFM_ValueRec  shared_vals[5];


      switch ( afm_tokenize( ключ, len ) )
      {
      case AFM_TOKEN_TRACKKERN:
        ...
        shared_vals[0].тип = AFM_VALUE_TYPE_INTEGER;
        shared_vals[1].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[2].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[3].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[4].тип = AFM_VALUE_TYPE_FIXED;
...
#define KERN_INDEX( g1, g2 )  ( ( (FT_ULong)g1 << 16 ) | g2 )
  ...
  afm_parse_kern_pairs( AFM_Parser  parser )
  {
    ...
    while ( ( ключ = afm_parser_next_key( parser, 1, &len ) ) != 0 )
    {
      AFM_Token  token = afm_tokenize( ключ, len );
      ...
      case AFM_TOKEN_KPY:
        {
          ...
          shared_vals[0].тип = AFM_VALUE_TYPE_INDEX;
          shared_vals[1].тип = AFM_VALUE_TYPE_INDEX;
          shared_vals[2].тип = AFM_VALUE_TYPE_INTEGER;
          shared_vals[3].тип = AFM_VALUE_TYPE_INTEGER;
  ...
  afm_parse_kern_data( AFM_Parser  parser )
  {
    ...
    while ( ( ключ = afm_parser_next_key( parser, 1, &len ) ) != 0 )
    {
      ...
      case AFM_TOKEN_STARTTRACKKERN:
        Ошибка = afm_parse_track_kern( parser );
        if ( Ошибка )
          return Ошибка;
        break;
      ...
      case AFM_TOKEN_STARTKERNPAIRS0:
        Ошибка = afm_parse_kern_pairs( parser );
        if ( Ошибка )
          return Ошибка;
        break;
                           ...
                           AFM_Token   end_section )
  {
    ...
    while ( n-- > 0 )
    {
      ключ = afm_parser_next_key( parser, 1, NULL );
      if ( !ключ )
        goto Fail;
    }
  ...
  afm_parser_parse( AFM_Parser  parser )
  {
    FT_Memory     memory = parser->memory;
    AFM_FontInfo  fi     = parser->FontInfo;
    FT_Error      Ошибка  = PSaux_Err_Syntax_Error;
    char*         ключ;
    FT_Offset     len;
    ...
    ключ = afm_parser_next_key( parser, 1, &len );
    if ( !ключ || len != 16                              ||
         ft_strncmp( ключ, "StartFontMetrics", 16 ) != 0 )
    ...
    while ( ( ключ = afm_parser_next_key( parser, 1, &len ) ) != 0 )
    {
      ...
      case AFM_TOKEN_METRICSSETS:
        ...
        {
          Ошибка = PSaux_Err_Unimplemented_Feature;
      ...
      case AFM_TOKEN_ISCIDFONT:
        shared_vals[0].тип = AFM_VALUE_TYPE_BOOL;
      ...
      case AFM_TOKEN_FONTBBOX:
        shared_vals[0].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[1].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[2].тип = AFM_VALUE_TYPE_FIXED;
        shared_vals[3].тип = AFM_VALUE_TYPE_FIXED;
      ...
      case AFM_TOKEN_STARTCHARMETRICS:
        {
          ...
          Ошибка = afm_parser_skip_section( parser, n,
                                           AFM_TOKEN_ENDCHARMETRICS );
          if ( Ошибка )
            return Ошибка;
        }
      ...
      case AFM_TOKEN_STARTKERNDATA:
        Ошибка = afm_parse_kern_data( parser );
        if ( Ошибка )
          goto Fail;
