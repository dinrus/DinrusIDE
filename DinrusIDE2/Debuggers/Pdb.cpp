...
#define KEYFILE      <DinrusIDE2/Debuggers/Pdb.key>
...
bool Pdb::Create(Host& local, const String& exefile, const String& cmdline, bool clang_)
{
	...
	if(!SymInitialize(hProcess, 0, FALSE)) {
		Error();
		Exclamation("Не удалось загрузить символы");
		return false;
	}
...
:	visual_display(this) {
	...
	tab.Add(autos.SizePos(), "Автосы");
	tab.Add(locals.SizePos(), "Локалы");
	tab.Add(self.SizePos(), "this");
	tab.Add(watches.SizePos(), "Вотчи");
	tab.Add(cpu.SizePos(), "ЦПБ");
	tab.Add(memory.SizePos(), "Память");
	tab.Add(bts.SizePos(), "Потоки");
	...
	if(!terminated) {
		...
		if(hProcess != INVALID_HANDLE_VALUE) {
			...
			if(exitcode == STILL_ACTIVE)
				Exclamation("Не удаётся заглушить отлаживаемый процесс. Перезапустите ИСР, пожалуйста.");
...
#define LAYOUTFILE <DinrusIDE2/Debuggers/Pdb.lay>
#include <CtrlCore/lay.h>

#define TOPICFILE <DinrusIDE2/Debuggers/app.tpp/all.i>
...
:	pdb(pdb)
{
	...
	help.SetQTF(GetTopic("DinrusIDE2/Debuggers/app/PDBExpressions_ru-ru"));
