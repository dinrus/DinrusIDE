...
t]&]
[s2;%RU-RU Возвращает the unique, per application run, 
identifier of Image. All Images with the same serial id can be 
considered equal (this is useful for caching images).&]
[s3; &]
[s4; &]
...
[s5;:Image`:`:operator`!`=`(const Image`&`)const: [@(0.0.255) bool]_operator!`=([@(0.0.255) c
onst]_[_^Image^ Image][@(0.0.255) `&]_[@3 img])_[@(0.0.255) const]&]
[s2;%RU-RU Tests whether two Images are equal (or not equal). Dimensions, 
...
[s2;%RU-RU Converts Image to Value.&]
[s3; &]
[s4; &]
[s5;:Upp`:`:Image`:`:IsPaintOnceHint`(`)const: [@(0.0.255) bool]_[* IsPaintOnceHint]()_[@(0.0.255) c
onst]&]
[s2;%RU-RU Signals that Image is supposed to be painted only once 
with Draw`::DrawImage.,&]
[s3; &]
[s0; &]
...
[s2;%RU-RU Uses Image created in ImageBuffer. ImageBuffer is emptied 
by this operation.&]
[s3;%RU-RU &]
[s0;%RU-RU &]
