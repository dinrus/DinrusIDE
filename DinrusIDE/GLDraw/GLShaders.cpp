...
static GLuint LoadShader(const char *src, GLenum тип) {
	GLuint shader;
	GLint compiled;

	shader = glCreateShader(тип);
	if(!shader)
		return 0;
	...
	if(!compiled) {
		Ткст Ошибка = тип == GL_VERTEX_SHADER ? "Vertex" : "Fragment";
		Ошибка << " shader failed to compile ";
		GLint infoLen = 0;
		glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &infoLen);
		if(infoLen > 1) {
			Буфер<char> infoLog(infoLen);
...
void GLProgram::Link()
{
	...
	if(!linked){
		...
		паника("Failed to link");
		очисть();
	}
}

void GLProgram::создай(const char *vertex_shader_, const char *fragment_shader_,
...
void GLProgram::создай(const char *vertex_shader, const char *fragment_shader,
                       int attr1, const char *attr1name)
{
	Tuple2<int, const char *> bind[] = {
		{ attr1, attr1name },
	};
	создай(vertex_shader, fragment_shader, bind, __countof(bind));
}
