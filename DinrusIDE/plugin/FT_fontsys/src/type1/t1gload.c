...
#define FT_COMPONENT  trace_t1gload
  ...
  {
    FT_Data   glyph_data;
    FT_Error  Ошибка = T1_Parse_Glyph_And_Get_Char_String(
                          ...
                          FT_Pos*  max_advance )
  {
    ...
    *max_advance = 0;
    ...
    for ( glyph_index = 0; glyph_index < type1->num_glyphs; glyph_index++ )
    {
      ...
      Ошибка = T1_Parse_Glyph( &decoder, glyph_index );
                   ...
                   FT_Fixed*  advances )
  {
    ...
    for ( nn = 0; nn < count; nn++ )
    {
      Ошибка = T1_Parse_Glyph( &decoder, first + nn );
      if ( !Ошибка )
                 ...
                 FT_Int32      load_flags )
  {
    ...
    Ошибка = decoder_funcs->init( &decoder,
                                 (FT_Face)face,
                                 (FT_Size)size,
                                 (FT_GlyphSlot)glyph,
                                 (FT_Byte**)type1->glyph_names,
    ...
    Ошибка = T1_Parse_Glyph_And_Get_Char_String( &decoder, glyph_index,
                                                &glyph_data );
    if ( Ошибка )
      goto Exit;
#ifdef FT_CONFIG_OPTION_INCREMENTAL
