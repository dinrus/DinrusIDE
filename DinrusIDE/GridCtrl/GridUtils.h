...
inline int расстояние(const Точка &p0, const Точка &p1)
{
	return max(абс(p0.x - p1.x), абс(p0.y - p1.y));
}
...
inline void LG_Dummy_Nop__(const char *fmt, ...) {} // about unused значение
#define LG LG_Dummy_Nop__
#endif
#define LGR LogGui

extern СтрокРедакт *dlog;
extern int       dlev;
