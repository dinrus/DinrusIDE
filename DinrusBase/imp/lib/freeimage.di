...
enum FREE_IMAGE_COLOR_TYPE {
	...
	FIC_RGBALPHA   = 4,		// RGB цвет model with альфа channel
...
enum FREE_IMAGE_COLOR_CHANNEL {
	...
	FICC_ALPHA	= 4,	// Use альфа channel
...
enum FREE_IMAGE_MDTYPE {
	...
	FIDT_ASCII		= 2,	// 8-bit bytes ш/ last byte null
...
const uint  BMP_DEFAULT         = 0,
            ...
            FAXG3_DEFAULT		= 0,
            GIF_DEFAULT			= 0,
            GIF_LOAD256			= 1,	// Load the image как 256 цвет image with ununsed палитра entries, if it's 16 or 2 цвет
            GIF_PLAYBACK		= 2,	// 'Play' the GIF to generate each frame (as 32bpp) instead of returning raw frame data when loading
            HDR_DEFAULT			= 0,
            ICO_DEFAULT         = 0,
            ICO_MAKEALPHA		= 1,	// convert to 32bpp and create an альфа channel from the AND-mask when loading
            IFF_DEFAULT         = 0,
...
void FreeImage_ConvertLine8To4(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
...
void FreeImage_ConvertLine1To16_555(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine4To16_555(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine8To16_555(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
...
void FreeImage_ConvertLine1To16_565(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine4To16_565(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine8To16_565(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
...
void FreeImage_ConvertLine1To24(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine4To24(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine8To24(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
...
void FreeImage_ConvertLine1To32(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine4To32(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
void FreeImage_ConvertLine8To32(BYTE *target, BYTE *source, int width_in_pixels, RGBQUAD *палитра);
...
FIBITMAP *FreeImage_ConvertFromRawBits(BYTE *bits, int ширина, int height, int питч, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL topdown = FALSE);
void FreeImage_ConvertToRawBits(BYTE *bits, FIBITMAP *dib, int питч, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL topdown = FALSE);
...
BOOL FreeImage_Paste(FIBITMAP *dst, FIBITMAP *src, int left, int top, int альфа);
