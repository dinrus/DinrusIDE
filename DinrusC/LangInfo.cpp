...
ШТкст DefaultGetIndexLetter(const wchar *text, int)
{
...
int DefaultLanguageCompare(const wchar *a, int a_length, const wchar *b, int b_length, int)
{
	...
	while(p1 < e1 && p2 < e2)
	{
		...
		int level1 = (буква_ли(c1) ? 3 : цифра_ли(c1) ? 2 : c1 == ' ' ? 0 : 1);
		int level2 = (буква_ли(c2) ? 3 : цифра_ли(c2) ? 2 : c2 == ' ' ? 0 : 1);
		if(level1 != level2)
...
ШТкст CSCZGetIndexLetter(const wchar *s, int)
{
	wchar temp[3];
...
static dword sGetLanguageDetails(int язык, Ткст *english_name, Ткст *native_name)
{
	int q = 0;
	byte cs = GetLNGCharset(язык);
	...
	for(const int *ptr = LanguageList; *ptr; ptr++, q++)
		if(*ptr == язык) {
			...
			if(native_name)
				*native_name = вНабсим(cs, Ткст(a + 1, b), CHARSET_UTF8);
...
void LanguageInfo::уст(int lang_)
{
	язык = lang_;
	dword q = sGetLanguageDetails(язык, &english_name, &native_name);
	if(!q)
		return;
...
static const char *NlsFindDigits(const char *ист, Ткст& dest)
{
	if(*ист && !цифра_ли(*ист))
	{
		const char *start = ист;
		while(*++ист && !цифра_ли(*ист))
			;
		dest.конкат(start, (int)(ист - start));
	}
	return ист;
}

static const char *NlsCopyDigits(const char *ист, Ткст& dest, Ткст thousands)
{
	if(цифра_ли(*(ист = NlsFindDigits(ист, dest))))
	{
		...
		while(цифра_ли(*++ист))
			;
		int first = ((int)(ист - p) + 2) % 3 + 1;
		while(p < ист)
		{
			dest.конкат(p, first);
			if((p += first) < ист)
			{
				dest.конкат(thousands);
				first = 3;
			}
		}
	}
...
static Ткст NlsFormatRaw(const char *n, Ткст thousands, Ткст decimals)
{
	if(*n == 0)
		return Null;
	Ткст result;
	...
	{ // decimal separator
		n++;
		result.конкат(decimals);
		const char *s = n;
		while(цифра_ли(*n))
			n++;
		result.конкат(s, (int)(n - s));
	}
...
Ткст LanguageInfo::фмтЦел(int значение) const
{
...
Ткст LanguageInfo::фмтДво(double значение, int digits, int FD_flags, int fill_exp) const
{
	if(пусто_ли(значение))
		return Null;
...
МассивМап<int, LanguageInfo>& LangMap()
{
	static МассивМап<int, LanguageInfo> x;
	return x;
}
...
const LanguageInfo& GetLanguageInfo(int lang)
{
	...
	МассивМап<int, LanguageInfo>& m = LangMap();
	int q = m.найди(lang);
	if(q >= 0)
		return m[q];
