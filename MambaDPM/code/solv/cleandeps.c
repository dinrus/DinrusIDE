...
#include <solv/solver_private.h>
#include <solv/bitmap.h>
#include <solv/pool.h>
#include <solv/util.h>
#include <solv/policy.h>
#include <solv/cplxdeps.h>
...
filter_unneeded(Solver *solv, Queue *unneededq, Map *unneededmap, int justone)
{
  ...
  for (i = 0; i < count; i++)
	...
	{
	  unsigned int off = pass == 0 ? s->requires_ : s->recommends;
	  Id p, pp, dep, *dp;
	  if (off)
...
solver_createcleandepsmap(Solver *solv, Map *cleandepsmap, int unneeded)
{
  ...
  if (1)
      ...
      while (dataiterator_step(&di))
	{
	  ...
	  if (!s->repo || !s->requires_)
	    continue;
	  if (s->repo != installed && !pool_installable(pool, s))
	    continue;
	  if (strncmp(pool_id2str(pool, s->name), "pattern:", 8) != 0)
	    continue;
	  for (dp = s->repo->idarraydata + s->requires_; *dp; dp++)
  ...
  for (;;)
	...
	continue;
      ...
      if (s->requires_)
	{
	  reqp = s->repo->idarraydata + s->requires_;
	  while ((req = *reqp++) != 0)
	    {
