#include <DinrusIDE2/Builders/AndroidBuilder.h>
...
void BuildMethods::MethodMenu(Bar& bar)
{
	method.StdBar(bar);
	bar.Separator();
	bar.Add("Импорт", THISBACK(Import));
}
...
void BuildMethods::Import()
{
	...
	for(int i = 0; i < fsel.GetCount(); i++) {
		...
		if(f.IsVoid()) {
			if(!PromptOKCancel(Format("Не удалось загрузить [* \1%s\1]. Продолжить?", fsel[i])))
				break;
			continue;
		}
		String nf = ConfigFile(GetFileNamePos(fsel[i]));
		if(FileExists(nf) && !PromptOKCancel(Format("Файл уже существует: [* \1%s\1]. Перзаписать?", nf)))
			continue;
		if(!SaveFile(nf, f))
			if(!PromptOKCancel(Format("Не удалось сохранить [* \1%s\1]. Продолжит?", nf)))
				break;
	}
	Load();
}
