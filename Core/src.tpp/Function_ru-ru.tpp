...
функции является NOP, а вероятное возвратное 
значение равно Res() (которая для фундаменталь
ных типов даёт 0). Вариант с Nuller используется 
при определении объявления функции 
с опциональными параметрами Function, 
типа [*C@5 void Foo(Function<void ()> `= Null);]&]
[s3;%- &]
[s4; &]
...
преобразовывать генерную лямбду 
(generic lambda) в значение Function.&]
[s3; &]
[s4; &]
...
[s2; Обычные конструкторы копирования/перемещен
ия и операторы присваивания.&]
[s3; &]
[s4; &]
[s5;:Function`<Res`(ArgTypes`.`.`.`)`> `:`:Proxy`(`)const:%- Function_[* Proxy]()_[@(0.0.255) c
onst]&]
[s2; Возвращает новую Function, которая, при 
вызове, вызывает эту Function. Применимо, 
когда какая`-то Function должна поздне 
измениться, но нам нужна другая Function, 
чтобы представлять её.&]
[s3; &]
[s4; &]
...
или функцию с лямбдой. Вначале вызывается 
оригинальная функция `'this`', а затем 
[%-*@3 fn]. Если любая из Functions возвращает 
значение, значение [%-*@3 fn] (аргумент 
справа от <<) представляет собой значени 
всей Function.&]
[s3; &]
[s4; &]
...
[s3;%- &]
[s0;%- Два наиболее частых типа Function в 
U`+`+ `- те, которые возвращают `'void`', 
и те, которые возвращают `'bool`'. В U`+`+ 
определено два удобных алиаса шаблонов:&]
[s0;%- &]
...
[s2; Создаёт алиас (псевдоним) для Function, 
возвращающей void. Например, [*C@5 Event<int, 
String>] эквивалентно [*C@5 Function<void (int, String)>].&]
[s3;%- &]
[s4;%- &]
...
возвращает bool(0), который равен false).&]
[s3;%- &]
[s0;%- &]
...
когда лучше поместить в функцию`-член 
`"вызывабельный`" код. Лямбда`-синтаксис 
C`+`+11 в таком случае может быть непригодным. 
Шаблонная функция MemFn и THISFN  служат 
для снижения типового переизбытка 
(typing overhead).&]
[s0;%- &]
...
([*@4 ArgTypes][@(0.0.255) ...]))&]
[s2; Эквивалентно std`::mem`_fn, возвращающему 
U`+`+ Function. Создаёт Function, представляющую 
собой вызов [%-*@3 member`_function] экземпляра 
[%-*@3 object].&]
[s3; &]
[s4;%- &]
[s5;:THISFN`(x`):%- [* THISFN](x)&]
[s2; Это макрос, определённый как MemFn(this, 
`&CLASSNAME`::x).&]
[s3;%- &]
[s0;@(0.0.255)3%- ]]