...
#Ошибка - Use GNU C compiler release 3 or higher
#endif

// for namespace problem in version 5.0
#if defined(_MSC_VER) && _MSC_VER < 1200        // check for V6.0
#Ошибка - Use Microsoft compiler version 6 or higher
#endif
...
typedef void (STDCALL *fpError)(int, char*);       // pointer to callback Ошибка handler
...
void importOptions(istream &in, vector<string> &optionsVector)
{
	...
	while (in)
	{
		...
		do
		{
			...
			// break options on пробелы, tabs or new-lines
			if (in.eof() || ch == ' ' || ch == '\t' || ch == '\n')
				break;
			else
...
void isOptionError(const string &arg, const string &errorInfo)
{
	...
	{
		(*_err) << errorInfo << endl;	// need main Ошибка message
		(*_err) << arg;					// output the option in Ошибка
	}
	else
		(*_err) << endl << arg;			// put endl after previous option
#else
	if (errorInfo.length() > 0)			// to avoid a compiler warning
		(*_err) << "Ошибка in param: " << arg << endl;
#endif
}
...
//		_modeManuallySet = true;
	...
	else if ( isParamOption(arg, "s", "indent=пробелы=") )
	{
		int spaceNum = 4;
		string spaceNumParam = GET_PARAMS(arg, "s", "indent=пробелы=");
		if (spaceNumParam.length() > 0)
			spaceNum = atoi(spaceNumParam.c_str());
		if (spaceNum < 2 || spaceNum > 20)
			isOptionError(arg, errorInfo);
	...
	else if ( IS_OPTION(arg, "indent=пробелы") )
	{
		formatter.setSpaceIndentation(4);
	}
...
//				_suffix = '.' + _suffix;
	...
	{
		(*_err) << "Artistic Стиль " << _version << endl;
		exit(0);
	}
	else
	{
...
extern "C" EXPORT char* STDCALL
	AStyleMain(const char* pSourceIn,          // pointer to the source to be formatted
	           ...
	           fpError fpErrorHandler,         // pointer to Ошибка handler function
	           fpAlloc fpMemoryAlloc)          // pointer to memory allocation function
{
	if (fpErrorHandler == NULL)			// cannot дисплей a message if no Ошибка handler
		return NULL;
	...
	parseOptions(formatter,
	             optionsVector.begin(),
	             optionsVector.end(),
	             "Unknown Artistic Стиль options\n"
...
void Ошибка(const char *why, const char* what)
{
	(*_err) << why << ' ' << what << '\n' << endl;
	exit(1);
}
...
void printHelp()
{
	...
	(*_err) << "original file-имя. The original pre-indented file is renamed, with a\n";
	(*_err) << "suffix of \".orig\" added to the original filename.\n";
	(*_err) << endl;
	(*_err) << "By default, astyle is set up to indent C/C++/C#/Java files, with 4 пробелы\n";
	(*_err) << "per indent, a maximal indentation of 40 пробелы inside continuous statements,\n";
	...
	(*_err) << endl;
	(*_err) << "    --style=linux\n";
	(*_err) << "    Linux mode (8 пробелы per indent, break definition-block\n";
	(*_err) << "    brackets but attach command-block brackets).\n";
	(*_err) << endl;
	(*_err) << "    --style=java\n";
	(*_err) << "    Java mode, with standard java style formatting/indenting.\n";
	(*_err) << endl;
	(*_err) << "Вкладка and Bracket Options:\n";
	(*_err) << "------------------------\n";
	(*_err) << "    default indent option\n";
	(*_err) << "    If no indentation option is set,\n";
	(*_err) << "    the default option of 4 пробелы will be used.\n";
	(*_err) << endl;
	(*_err) << "    --indent=пробелы=#   OR   -s#\n";
	(*_err) << "    Indent using # пробелы per indent. Not specifying #\n";
	(*_err) << "    will result in a default of 4 пробелы per indent.\n";
	(*_err) << endl;
	(*_err) << "    --indent=tab   OR   --indent=tab=#   OR   -t   OR   -t#\n";
	(*_err) << "    Indent using tab characters, assuming that each\n";
	(*_err) << "    tab is # пробелы long. Not specifying # will result\n";
	(*_err) << "    in a default assumption of 4 пробелы per tab.\n";
	(*_err) << endl;
	(*_err) << "    --force-indent=tab=#   OR   -T#\n";
	(*_err) << "    Indent using tab characters, assuming that each\n";
	(*_err) << "    tab is # пробелы long. Force tabs to be used in areas\n";
	(*_err) << "    Astyle would prefer to use пробелы.\n";
	(*_err) << endl;
	...
	(*_err) << "    прикрепи brackets to pre-block code (i.e. Java/K&R style).\n";
	(*_err) << endl;
	...
	(*_err) << "    добавь extra indentation to '{' and '}' block brackets.\n";
	(*_err) << endl;
	(*_err) << "    --indent-blocks   OR   -G\n";
	(*_err) << "    добавь extra indentation entire blocks (including brackets).\n";
	(*_err) << endl;
	...
	(*_err) << "    Indent a maximal # пробелы in a continuous statement,\n";
	(*_err) << "    relative to the previous line.\n";
	(*_err) << endl;
	(*_err) << "    --min-conditional-indent=#   OR   -m#\n";
	(*_err) << "    Indent a minimal # пробелы in a continuous conditional\n";
	(*_err) << "    belonging to a conditional header.\n";
	(*_err) << endl;
	(*_err) << "Formatting options:\n";
	(*_err) << "-------------------\n";
	(*_err) << "    --break-blocks   OR   -f\n";
	(*_err) << "    вставь empty lines around unrelated blocks, labels, classes, ...\n";
	(*_err) << endl;
	...
	(*_err) << endl;
	(*_err) << "    --pad=oper   OR   -p\n";
	(*_err) << "    вставь space paddings around operators.\n";
	(*_err) << endl;
	(*_err) << "    --pad=paren   OR   -P\n";
	(*_err) << "    вставь space padding around parenthesis on both the outside\n";
	(*_err) << "    and the inside.\n";
	(*_err) << endl;
	(*_err) << "    --pad=paren-out   OR   -d\n";
	(*_err) << "    вставь space padding around parenthesis on the outside only.\n";
	(*_err) << endl;
	(*_err) << "    --pad=paren-in   OR   -D\n";
	(*_err) << "    вставь space padding around parenthesis on the inside only.\n";
	(*_err) << endl;
	(*_err) << "    --unpad=paren   OR   -U\n";
	(*_err) << "    удали unnecessary space padding around parenthesis.  This\n";
	...
	(*_err) << "--------------\n";
	(*_err) << "    --suffix=####\n";
	(*_err) << "    приставь the suffix #### instead of '.orig' to original filename.\n";
	(*_err) << endl;
	...
	(*_err) << endl;
	(*_err) << "    --options=none\n";
	(*_err) << "    откл the default options file.\n";
	...
	(*_err) << endl;
	(*_err) << "дефолт options file:\n";
	(*_err) << "---------------------\n";
	(*_err) << "    Artistic Стиль looks for a default options file in the\n";
...
int main(int argc, char *argv[])
{
	...
	if (shouldParseOptionsFile)
	{
		...
		if (optionsFileName.compare("") != 0)
		{
			...
			if (!ok)
			{
				(*_err) << "For help on options, тип 'astyle -h' " << endl;
			}
		}
	}
	...
	if (!ok)
	{
		(*_err) << "For help on options, тип 'astyle -h' \n" << endl;
		exit(1);
	}
	...
	// this is used to формат text for text editors like TextWrangler
	// do NOT дисплей any console messages when this branch is used
	...
	else
	{
		...
		for (size_t i = 0; i < fileNameVector.size(); i++)
		{
			...
			if (!inCheck)
				Ошибка("Could not open input file", originalFileName.c_str());
			inCheck.close();

			if (rename(originalFileName.c_str(), inFileName.c_str()) < 0)
				Ошибка("Could not rename ", string(originalFileName + " to " + inFileName).c_str());

			ifstream in(inFileName.c_str(), ios::binary);
			if (!in)
				Ошибка("Could not open input file", inFileName.c_str());

			ofstream out(originalFileName.c_str(), ios::binary);
			if (!out)
				Ошибка("Could not open output file", originalFileName.c_str());
			...
			// дисплей file formatting message and save the filename
