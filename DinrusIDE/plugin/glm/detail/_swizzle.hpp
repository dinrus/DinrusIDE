...
namespace detail
{
	...
	protected:
		...
		// Use an opaque буфер to *ensure* the compiler doesn't call a constructor.
		// The size 1 буфер is assumed to aligned to the actual members so that the
		// elem()
		char    _buffer[1];
	};
	...
	/*
		...
		T			= тип of scalar values (e.g. float, double)
		N			= number of components in the vector (e.g. 3)
		E0...3		= what Индекс the n-th element of this swizzle refers to in the unswizzled vec

		DUPLICATE_ELEMENTS = 1 if there is a repeated element, 0 otherwise (used to specialize swizzles
			containing duplicate elements so that they cannot be used as r-values).
	*/
	template<int N, typename T, qualifier Q, int E0, int E1, int E2, int E3, int DUPLICATE_ELEMENTS>
	struct _swizzle_base2 : public _swizzle_base1<N, T, Q, E0,E1,E2,E3, detail::is_aligned<Q>::значение>
	{
		struct op_equal
		{
	...
	protected:
		...
		{
			// сделай a copy of the data in this == &that.
	...
	struct _swizzle_base2<N, T, Q, E0,E1,E2,E3, 1> : public _swizzle_base1<N, T, Q, E0,E1,E2,E3, detail::is_aligned<Q>::значение>
	{
		struct Stub {};
...
//
// Macro for wrapping a function taking one argument (e.g. абс())
//
...
namespace glm
{
	...
	//
	// Swizzles are distinct types from the unswizzled тип.  The below macros will
	// provide template specializations for the swizzle types for the given functions
	// so that the compiler does not have any ambiguity to choosing how to handle
	// the function.
	//
	// The alternative is to use the operator()() when calling the function in order
	// to explicitly convert the swizzled тип to the unswizzled тип.
	//

	//GLM_SWIZZLE_FUNCTION_1_ARGS(vec_type,    абс);
