...
void SelectPackageDlg::SyncFilter()
{
	...
	nest.Add(ALL, AttrText("Все").Italic().Bold().NormalInk(SBlue()));
	bool hub = false;
...
void SelectPackageDlg::ToolBase(Bar& bar)
{
...
#endif
	;
	bar.Add(base.IsCursor(), "Редактировать сботку..", THISBACK(OnBaseEdit))
		.Key(K_CTRL_ENTER);
	...
	if(d.GetCount()) {
		...
		for(int i = 0; i < d.GetCount(); i++)
			bar.Add("Синхронизовать " + d[i], IdeImg::svn_dir(), THISBACK1(SyncSvnDir, d[i]));
		bar.Add("Синхронизовать все..", IdeImg::svn(), THISBACK(SyncSvnDirs));
	}
}
...
void SelectPackageDlg::OnBaseRemove()
{
	...
	if(PromptOKCancel(Format("Удалить базовый файл [* \1%s\1]?", varpath))) {
		if(!FileDelete(varpath))
			Exclamation(Format("Ошибка при удалении файла [* \1%s\1].", varpath));
		else
			SyncBase(next);
	}
}
...
void SelectPackageDlg::SyncList(const String& find)
{
	...
	alist.ScrollTo(asc);
	clist.SetSbPos(csc);
	alist.HeaderTab(0).SetText("Пакет (" + AsString(alist.GetCount()) + ")");
}
