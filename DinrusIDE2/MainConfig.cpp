...
void MainConfigDlg::FlagDlg()
{
	...
	PPInfo pp;
	pp.SetIncludes(DinrusIde()->GetCurrentIncludePath() + ";" + GetClangInternalIncludes());
	...
	cfg.accepts.AddColumn("Набор").With([](One<Ctrl>& ctrl) {
		ctrl.Create<Option>().NoWantFocus();
	});
	cfg.accepts.AddColumn("Флаг");
	cfg.accepts.AddColumn("Пакеты");
