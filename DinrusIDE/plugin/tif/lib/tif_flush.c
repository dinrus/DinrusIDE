...
int TIFFForceStrileArrayWriting(TIFF* tif)
{
    ...
    if( !(tif->tif_flags & TIFF_DIRTYSTRIP) )
    {
        ...
        {
            TIFFErrorExt(tif->tif_clientdata, module,
                        "Функция not called together with "
...
/*
 * слей buffered data to the file.
 *
 * Frank Warmerdam'2000: I modified this to return 1 if TIFF_BEENWRITING
 * is not set, so that TIFFFlush() will proceed to write out the directory.
 * The documentation says returning 1 is an Ошибка indicator, but not having
 * been writing isn't exactly a an Ошибка.  Hopefully this doesn't cause
