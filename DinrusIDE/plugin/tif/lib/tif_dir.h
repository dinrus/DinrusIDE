...
/*
 * TIFF Рисунок Файл Directories are comprised of a table of field
 * descriptors of the form shown below.  The table is sorted in
 * ascending order by tag.  The values associated with each entry are
 * disjoint and may appear anywhere in the file (so long as they are
 * placed on a word boundary).
 *
 * If the значение is 4 bytes or less, in ClassicTIFF, or 8 bytes or less in
 * BigTIFF, then it is placed in the offset field to save space. If so,
 * it is left-justified in the offset field.
 */
typedef struct {
	uint16 tdir_tag;        /* see below */
	uint16 tdir_type;       /* data тип; see below */
...
/*
 * Internal формат of a TIFF directory entry.
 */
typedef struct {
#define FIELD_SETLONGS 4
	...
	/* even though the имя is misleading, td_stripsperimage is the number
...
struct _TIFFFieldArray {
	TIFFFieldArrayType тип;    /* array тип, will be used to determine if IFD is image and such */
...
struct _TIFFField {
	...
	TIFFSetGetFieldType set_field_type;     /* тип to be passed to TIFFSetField */
	TIFFSetGetFieldType get_field_type;     /* тип to be passed to TIFFGetField */
