...
debug (КэшКарта)
{
        ...
        проц main()
        {
                ...
                for (цел i=счёт; i--;)
                     тест.добавь (i, i);
                Стдвыв.фмтнс ("{} добавки: {}/s", счёт, счёт/ш.stop);

                // benchmark reading
                ш.старт;
                for (цел i=счёт; i--;)
                     тест.получи (i, знач);
                Стдвыв.фмтнс ("{} поиски: {}/s", счёт, счёт/ш.stop);

                // benchmark iteration
                ш.старт;
                foreach (ключ, значение; тест) {}
                Стдвыв.фмтнс ("{} обходов элементов: {}/s", тест.размер, тест.размер/ш.stop);
