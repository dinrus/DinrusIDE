...
use_merged_upsample (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->CCIR601_sampling)
    return FALSE;
  /* jdmerge.c only supports YCC=>дайКЗС color conversion */
 ...
 */
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
     ...
     */
    /* Размер in samples, after IDCT scaling */
 ...
 * 0..MAXJSAMPLE; the input значение may fall somewhat outside this range
 * due to noise introduced by quantization, roundoff Ошибка, etc.  These
 ...
 *
 * For most steps we can mathematically guarantee that the initial значение
 ...
 * initial limiting step (just after the IDCT), a wildly out-of-range значение
...
/* Allocate and fill in the sample_range_limit table */
{
  ...
  for (i = 0; i <= MAXJSAMPLE; i++)
    table[i] = (JSAMPLE) i;
  /* стоп of range limit table: limit[x] = MAXJSAMPLE for x > MAXJSAMPLE */
...
master_selection (j_decompress_ptr cinfo)
{
  ...
  /* For now, precision must match compiled-in значение... */
  ...
  /* устШирину of an output scanline must be representable as JDIMENSION. */
  ...
  /* пост-processing: in particular, color conversion first */
  if (! cinfo->raw_data_out) {
  ...
  /* Initialize principal буфер controllers. */
  use_c_buffer = cinfo->inputctl->has_multiple_scans || cinfo->buffered_image;
  jinit_d_coef_controller(cinfo, use_c_buffer);

  if (! cinfo->raw_data_out)
    jinit_d_main_controller(cinfo, FALSE /* never need full буфер here */);
...
prepare_for_output_pass (j_decompress_ptr cinfo)
{
  ...
  if (master->pub.is_dummy_pass) {
    ...
    if (cinfo->quantize_colors && cinfo->colormap == NULL) {
      /* выдели new quantization method */
...
/*
 * финиш up at end of an output pass.
 */
...
jpeg_new_colormap (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->quantize_colors && cinfo->enable_external_quant &&
      cinfo->colormap != NULL) {
    /* выдели 2-pass quantizer for external colormap use */
    cinfo->cquantize = master->quantizer_2pass;
