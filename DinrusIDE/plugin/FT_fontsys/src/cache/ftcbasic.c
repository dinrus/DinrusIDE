...
#define FTC_BASIC_ATTR_HASH( a )                                   \
                              ...
                              FTC_Manager  manager )
  {
    ...
    Ошибка = FTC_Manager_LookupFace( manager, family->attrs.scaler.face_id,
                                    &face );
                                ...
                                FT_Face     *aface )
  {
    ...
    Ошибка = FTC_Manager_LookupSize( manager, &family->attrs.scaler, &size );
    if ( !Ошибка )
    {
      FT_Face  face = size->face;


      Ошибка = FT_Load_Glyph( face, gindex,
                               ...
                               FT_Glyph   *aglyph )
  {
    ...
    Ошибка = FTC_Manager_LookupSize( cache->manager,
                                    scaler,
                                    &size );
    if ( !Ошибка )
    {
      ...
      Ошибка = FT_Load_Glyph( face, gindex, family->attrs.load_flags );
      if ( !Ошибка )
      {
        if ( face->glyph->формат == FT_GLYPH_FORMAT_BITMAP  ||
             face->glyph->формат == FT_GLYPH_FORMAT_OUTLINE )
        {
          /* ok, copy it */
          FT_Glyph  glyph;
  ...
  */
                         ...
                         FTC_Node       *anode )
  {
    ...
    *aglyph = NULL;
     ...
     */
    if ( (FT_ULong)тип->width >= 0x10000L )
    {
      FTC_OldImageDesc  desc = (FTC_OldImageDesc)тип;
    ...
    {
      if ( (FT_ULong)(тип->flags - FT_INT_MIN) > FT_UINT_MAX )
      {
        FT_TRACE1(( "FTC_ImageCache_Lookup: higher bits in load_flags" ));
        FT_TRACE1(( "0x%x are dropped\n", (тип->flags & ~((FT_ULong)FT_UINT_MAX)) ));
      }

      query.attrs.scaler.face_id = тип->face_id;
      query.attrs.scaler.width   = тип->width;
      query.attrs.scaler.height  = тип->height;
      query.attrs.load_flags     = (FT_UInt)тип->flags;
    }
    ...
    FTC_GCACHE_LOOKUP_CMP( cache,
                           ...
                           Ошибка );
#else
    Ошибка = FTC_GCache_Lookup( FTC_GCACHE( cache ),
                               hash, gindex,
                               FTC_GQUERY( &query ),
                               &node );
#endif
...
#define ftc_old_image_outline          ftc_old_image_format_outline
                                ...
                                FTC_OldImage_Desc*  desc )
  {
    ...
    /* convert image тип flags to load flags */
    {
      FT_UInt  load_flags = FT_LOAD_DEFAULT;
      FT_UInt  тип       = desc->image_type;


      /* determine load flags, depending on the font description's */
      /* image тип                                                */

      if ( FTC_OLD_IMAGE_FORMAT( тип ) == ftc_old_image_format_bitmap )
      {
        if ( тип & ftc_old_image_flag_monochrome )
          load_flags |= FT_LOAD_MONOCHROME;

        /* disable embedded bitmaps loading if necessary */
        if ( тип & ftc_old_image_flag_no_sbits )
      ...
      else
      {
        ...
        if ( тип & ftc_old_image_flag_unscaled )
      ...
      if ( тип & ftc_old_image_flag_unhinted )
        load_flags |= FT_LOAD_NO_HINTING;

      if ( тип & ftc_old_image_flag_autohinted )
