...
bool AssistEditor::IncludeAssist()
{
	...
	try {
		if(!p.Char('#') || !p.Id("include"))
			return false;
		if(p.Char('\"')) {
			include.Add(GetFileFolder(DinrusIDE2->editfile));
			include_local = true;
		}
		else {
			p.Char('<');
			include = SplitDirs(DinrusIDE2->GetIncludePath());
			include_local = false;
		}
		include_path.Clear();
		include_back = 0;
...
CurrentFileContext AssistEditor::CurrentContext(int pos)
{
	...
	cfx.filename = cfx.real_filename = NormalizePath(DinrusIDE2->editfile);
	cfx.includes = DinrusIDE2->GetCurrentIncludePath();
	cfx.defines = DinrusIDE2->GetCurrentDefines();
	...
	if(cfx.content.GetCount() && HasLibClang())
		SetCurrentFile(cfx, [=](const CppFileInfo& f, const Vector<Diagnostic>& ds) {
			...
			if(!IsCurrentFileDirty()) {
				...
				int di = 0;
				String path = NormalizePath(DinrusIDE2->editfile);
...
bool AssistEditor::DelayedTip(CodeEditor::MouseTip& mt)
{
	...
	String name;
	Point ref_pos;
	String ref_id = DinrusIDE2->GetRefId(mt.pos, name, ref_pos);
	...
	if(is_source_file) {
		...
		if(q >= 0) {
			...
			if(f.defines == cfx.defines && f.includes == cfx.includes && f.time >= GetFileTime(cfx.real_filename)) {
				annotating = false;
				PutAssist(cfx.real_filename + " аннотаций загружено из индекса");
			}
		}
		SyncCurrentFile(cfx);
	}
...
void AssistEditor::Abbr()
{
	...
	int len = s.GetCount();
	s = DinrusIDE2->abbr.Get(s, String());
	if(IsNull(s))
		return;
	NextUndo();
	SetCursor(c);
...
bool AssistEditor::Key(dword key, int count)
{
	...
	if(key == K_F12) {
		...
		String includes = DinrusIDE2->GetCurrentIncludePath() + ";" + GetClangInternalIncludes();
		ppi.SetIncludes(includes);
		ppi.GatherDependencies(DinrusIDE2->editfile, result, define_includes);
		DDUMP(includes);
		DDUMPM(result);
...
{
	navigatorframe.Show(navigator && DinrusIDE2 && !DinrusIDE2->designer && !DinrusIDE2->IsEditorMode());
}
