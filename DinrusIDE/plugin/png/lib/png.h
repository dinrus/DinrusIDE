 ...
 * and Группа 42, Inc. disclaim all warranties, expressed or implied,
 * including, without limitation, the warranties of merchantability and of
 * fitness for any purpose.  The Contributing Authors and Группа 42, Inc.
 ...
 *
 * The Contributing Authors and Группа 42, Inc. specifically permit, without
 * fee, and encourage the use of this source code as a component to
 * supporting the PNG file формат in commercial products.  If you use this
 ...
 *
 * Also, the PNG logo (in PNG формат, of course) is supplied in the
 * files "pngbar.png" and "pngbar.jpg (88x31) and "pngnow.png" (98x31).
 */

/*
 * Libpng is OSI Certified открой Source Software.  OSI Certified открой Source is
 * a certification mark of the открой Source Initiative. OSI has not addressed
 ...
 *    September 28, 2017
 *
 *    Since the PNG Development группа is an ad-hoc body, we can't make
 ...
 *
 *    Libpng only has three year fields.  Один is a 2-byte unsigned integer
 *    that will hold years up to 65535.  The other two hold the date in text
 *    формат, and will hold years up to 9999.
 *
 *    The integer is
 ...
 *          (formerly png_convert_to_rfc_1152() in Ошибка)
...
/* Careful here.  по one time, Guy wanted to use 082, but that would be octal.
...
/* Реф MSDN: Private as priority over особый
 * VS_FF_PRIVATEBUILD Файл *was not* built using standard release
 * procedures. If this значение is given, the StringFileInfo block must
 * contain a PrivateBuild string.
 *
 * VS_FF_SPECIALBUILD Файл *was* built by the original company using
 * standard release procedures but is a variation of the standard
 * file of the same version number. If this значение is given, the
...
/* Inhibit C++ имя-mangling for libpng functions but not for system calls. */
#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

/* This file is arranged in several sections.  The first section contains
 * structure and тип definitions.  The second section contains the external
...
{
   png_byte индекс;    /* used for palette files */
 ...
 * and whether that contents is compressed or not.  The "ключ" field
...
{
   int  compression;       /* compression значение:
                             -1: tEXt, none
                              0: zTXt, deflate
                              1: iTXt, none
                              2: iTXt, deflate  */
   png_charp ключ;          /* keyword, 1-79 character description of "text" */
...
#define PNG_TEXT_COMPRESSION_LAST     3  /* Not a valid значение */
...
{
    png_byte имя[PNG_CHUNK_NAME_LENGTH];
    png_byte *data;
    png_size_t size;
 ...
 *
 * ещё allocation details: all the chunk-reading functions that
...
typedef struct png_info_struct
{
   ...
   png_byte interlace_type PNG_DEPSTRUCT; /* Один of PNG_INTERLACE_NONE, PNG_INTERLACE_ADAM7 */
    ...
    * Данные is valid if (valid & PNG_INFO_gAMA) is non-zero.
    */
   float gamma PNG_DEPSTRUCT; /* gamma значение of image, if (valid & PNG_INFO_gAMA) */
#endif

#ifdef PNG_sRGB_SUPPORTED
    /* GR-P, 0.96a */
    /* Данные valid if (valid & PNG_INFO_sRGB) non-zero. */
    ...
    * unique, and the text string may be empty.  Любое number of text chunks may
    * be in an image.
    */
    ...
    * the low-order bits is not specified.  Данные is valid if
...
defined(PNG_READ_BACKGROUND_SUPPORTED)
    ...
    * same order as the palette colors, starting from Индекс 0.  Values
    * for the data are in the range [0, 255], ranging from fully transparent
    * to fully opaque, respectively.  For non-paletted images, there is a
    * single color specified that should be treated as fully transparent.
    * Данные is valid if (valid & PNG_INFO_tRNS) is non-zero.
    */
    ...
    * дисплей program does not have its own background color and the image
    * is needs to composited onto a background before дисплей.  The colors
    * in "background" are normally in the same color space/depth as the
    * pixel data.  Данные is valid if (valid & PNG_INFO_bKGD) is non-zero.
    */
    ...
    * and downwards from the top-left corner of the дисплей, page, or other
   ...
   png_byte offset_unit_type PNG_DEPSTRUCT; /* offset units тип */
#endif

#ifdef PNG_pHYs_SUPPORTED
   /* The pHYs chunk gives the physical pixel density of the image for
    * дисплей or printing in "phys_unit_type" units (see PNG_RESOLUTION_
    * defines below).  Данные is valid if (valid & PNG_INFO_pHYs) is non-zero.
    */
   ...
   png_byte phys_unit_type PNG_DEPSTRUCT; /* resolution тип (see PNG_RESOLUTION_ below) */
#endif

#ifdef PNG_hIST_SUPPORTED
   /* The hIST chunk contains the relative frequency or importance of the
    * various palette entries, so that a viewer can intelligently select a
    * reduced-color palette, if required.  Данные is an array of "num_palette"
    * values in the range [0,65535]. Данные valid if (valid & PNG_INFO_hIST)
    * is non-zero.
    */
    ...
    * [0.0, 0.8].  Данные valid if (valid & PNG_INFO_cHRM) non-zero.
    */
    ...
    * defines below, and the PNG-Группа's PNG extensions document for a
    * complete description of the transformations and how they should be
    * implemented, and for a description of the ASCII parameter strings.
    * Данные values are valid if (valid & PNG_INFO_pCAL) non-zero.
    */
   png_charp pcal_purpose PNG_DEPSTRUCT;  /* pCAL chunk description string */
   png_int_32 pcal_X0 PNG_DEPSTRUCT;      /* minimum значение */
   png_int_32 pcal_X1 PNG_DEPSTRUCT;      /* maximum значение */
   ...
   png_byte pcal_type PNG_DEPSTRUCT;      /* equation тип (see PNG_EQUATION_ below) */
   png_byte pcal_nparams PNG_DEPSTRUCT;   /* number of parameters given in pcal_params */
#endif

/* нов members added in libpng-1.0.6 */
 ...
 defined(PNG_HANDLE_AS_UNKNOWN_SUPPORTED)
   ...
   /* iCCP chunk data. */
   png_charp iccp_name PNG_DEPSTRUCT;     /* profile имя */
   png_charp iccp_profile PNG_DEPSTRUCT;  /* International Цвет Consortium profile data */
   ...
   /* Данные on sPLT chunks (there may be more than one). */
    ...
    * a byte значение, and two ASCII strings representing floating-point
    * values.  The values are width and height corresponsing to one pixel
    * in the image.  This external representation is converted to double
    * here.  Данные values are valid if (valid & PNG_INFO_sCAL) is non-zero.
    */
   ...
   /* Данные valid if (valid & PNG_INFO_IDAT) non-zero */
...
/* This is for compression тип. PNG 1.0-1.2 only define the single тип. */
#define PNG_COMPRESSION_TYPE_BASE 0 /* дефлируй method 8, 32K window */
#define PNG_COMPRESSION_TYPE_DEFAULT PNG_COMPRESSION_TYPE_BASE

/* This is for filter тип. PNG 1.0-1.2 only define the single тип. */
#define PNG_FILTER_TYPE_BASE      0 /* Single row per-byte filtering */
#define PNG_INTRAPIXEL_DIFFERENCING 64 /* Used only in MNG datastreams */
#define PNG_FILTER_TYPE_DEFAULT   PNG_FILTER_TYPE_BASE

/* These are for the interlacing тип.  These values should NOT be changed. */
#define PNG_INTERLACE_NONE        0 /* Non-interlaced image */
#define PNG_INTERLACE_ADAM7       1 /* Adam7 interlacing */
#define PNG_INTERLACE_LAST        2 /* Not a valid значение */

/* These are for the oFFs chunk.  These values should NOT be changed. */
#define PNG_OFFSET_PIXEL          0 /* смещение in pixels */
#define PNG_OFFSET_MICROMETER     1 /* смещение in micrometers (1/10^6 meter) */
#define PNG_OFFSET_LAST           2 /* Not a valid значение */
...
#define PNG_EQUATION_LAST         4 /* Not a valid значение */

/* These are for the sCAL chunk.  These values should NOT be changed. */
#define PNG_SCALE_UNKNOWN         0 /* unknown unit (image scale) */
#define PNG_SCALE_METER           1 /* meters per pixel */
#define PNG_SCALE_RADIAN          2 /* radians per pixel */
#define PNG_SCALE_LAST            3 /* Not a valid значение */

/* These are for the pHYs chunk.  These values should NOT be changed. */
#define PNG_RESOLUTION_UNKNOWN    0 /* pixels/unknown unit (aspect ratio) */
#define PNG_RESOLUTION_METER      1 /* pixels/meter */
#define PNG_RESOLUTION_LAST       2 /* Not a valid значение */
...
#define PNG_sRGB_INTENT_LAST       4 /* Not a valid значение */
...
typedef struct png_row_info_struct
{
   ...
   png_byte color_type; /* color тип of row */
 ...
 * own.  The png_error_ptr тип should match that of user-supplied warning
 * and Ошибка functions, while the png_rw_ptr тип should match that of the
...
struct png_struct_def
{
   ...
   png_voidp error_ptr PNG_DEPSTRUCT;       /* user supplied struct for Ошибка functions */
...
/* These were added in libpng-1.0.2 */
   ...
   png_bytep zbuf PNG_DEPSTRUCT;            /* буфер for zlib */
...
/* Added in libpng-1.2.43 */
   ...
   png_bytep prev_row PNG_DEPSTRUCT;        /* буфер to save previous (unfiltered) row */
   png_bytep row_buf PNG_DEPSTRUCT;         /* буфер to save current (unfiltered) row */
#ifndef PNG_NO_WRITE_FILTER
   png_bytep sub_row PNG_DEPSTRUCT;         /* буфер to save "sub" row when filtering */
   png_bytep up_row PNG_DEPSTRUCT;          /* буфер to save "up" row when filtering */
   png_bytep avg_row PNG_DEPSTRUCT;         /* буфер to save "avg" row when filtering */
   png_bytep paeth_row PNG_DEPSTRUCT;       /* буфер to save "Paeth" row when filtering */
#endif
   png_row_info row_info PNG_DEPSTRUCT;     /* used for transformation routines */

   png_uint_32 idat_size PNG_DEPSTRUCT;     /* current IDAT size for read */
   png_uint_32 crc PNG_DEPSTRUCT;           /* current chunk CRC значение */
   png_colorp palette PNG_DEPSTRUCT;        /* palette from the input file */
   png_uint_16 num_palette PNG_DEPSTRUCT;   /* number of color entries in palette */
   png_uint_16 num_trans PNG_DEPSTRUCT;     /* number of transparency values */
   png_byte chunk_name[5] PNG_DEPSTRUCT;    /* null-terminated имя of current chunk */
   png_byte compression PNG_DEPSTRUCT;      /* file compression тип (always 0) */
   png_byte filter PNG_DEPSTRUCT;           /* file filter тип (always 0) */
   ...
   png_byte color_type PNG_DEPSTRUCT;       /* color тип of file */
...
#  endif
   ...
   png_flush_ptr output_flush_fn PNG_DEPSTRUCT; /* Функция for flushing output */
...
#ifdef PNG_FLOATING_POINT_SUPPORTED
   float gamma PNG_DEPSTRUCT;          /* file gamma значение */
   float screen_gamma PNG_DEPSTRUCT;   /* screen gamma значение (display_exponent) */
#endif
#endif
 ...
 || defined(PNG_READ_EXPAND_SUPPORTED) || defined(PNG_READ_BACKGROUND_SUPPORTED)
   ...
   png_bytep save_buffer PNG_DEPSTRUCT;            /* буфер for previously read data */
   png_bytep current_buffer_ptr PNG_DEPSTRUCT;     /* current location in current_buffer */
   png_bytep current_buffer PNG_DEPSTRUCT;         /* буфер for recently used data */
   ...
   int cur_palette PNG_DEPSTRUCT;                  /* current push library palette Индекс */
...
#  ifdef PNG_TEXT_SUPPORTED
     ...
     png_charp current_text PNG_DEPSTRUCT;         /* current text chunk буфер */
...
/* for the Borland special 64K segment handler */
   ...
   png_bytep dither_index PNG_DEPSTRUCT;           /* Индекс translation for palette files */
#endif
   ...
   png_charp time_buffer PNG_DEPSTRUCT;            /* Ткст to hold RFC 1123 time text */
#endif
...
/* нов members added in libpng-1.0.3 */
...
/* нов member added in libpng-1.0.4 (renamed in 1.0.9) */
...
/* нов member added in libpng-1.0.7 */
#if defined(PNG_READ_GAMMA_SUPPORTED) || defined(PNG_READ_BACKGROUND_SUPPORTED)
   png_fixed_point int_gamma PNG_DEPSTRUCT;
#endif

/* нов member added in libpng-1.0.9, ifdef'ed out in 1.0.12, enabled in 1.2.0 */
#ifdef PNG_MNG_FEATURES_SUPPORTED
   png_byte filter_type PNG_DEPSTRUCT;
#endif

#ifdef PNG_1_0_X
/* нов member added in libpng-1.0.10, ifdef'ed out in 1.2.0 */
   png_uint_32 row_buf_size PNG_DEPSTRUCT;
#endif

/* нов members added in libpng-1.2.0 */
...
/* нов members added in libpng-1.0.2 but first enabled by default in 1.2.0 */
...
/* нов member added in libpng-1.0.13 and 1.2.0 */
   png_bytep big_row_buf PNG_DEPSTRUCT;        /* буфер to save current (unfiltered) row */

#ifdef PNG_READ_DITHER_SUPPORTED
/* The following three members were added at version 1.0.14 and 1.2.4 */
   png_bytep dither_sort PNG_DEPSTRUCT;        /* working sort array */
   png_bytep index_to_palette PNG_DEPSTRUCT;   /* where the original Индекс currently is */
                                 /* in the palette */
   png_bytep palette_to_index PNG_DEPSTRUCT;   /* which original Индекс points to this */
                                 /* palette color */
#endif

/* нов members added in libpng-1.0.16 and 1.2.6 */
...
/* нов member added in libpng-1.0.25 and 1.2.17 */
#ifdef PNG_UNKNOWN_CHUNKS_SUPPORTED
   /* Storage for unknown chunk that the library doesn't recognize. */
   png_unknown_chunk unknown_chunk PNG_DEPSTRUCT;
#endif

/* нов members added in libpng-1.2.26 */
  png_uint_32 old_big_row_buf_size PNG_DEPSTRUCT;
  png_uint_32 old_prev_row_size PNG_DEPSTRUCT;

/* нов member added in libpng-1.2.30 */
  png_charp chunkdata PNG_DEPSTRUCT;  /* буфер for reading chunk data */


};


/* This triggers a compiler Ошибка in png.c, if png.c and png.h
 ...
 * Handling more than 8 bytes from the beginning of the file is an Ошибка.
 */
...
/* переустанов the compression stream */
extern PNG_EXPORT(int,png_reset_zstream) PNGARG((png_structp png_ptr));

/* нов functions added in libpng-1.0.2 (not enabled by default until 1.2.0) */
#ifdef PNG_USER_MEM_SUPPORTED
...
/* пиши a PNG chunk - size, тип, (optional) data, CRC. */
extern PNG_EXPORT(void,png_write_chunk) PNGARG((png_structp png_ptr,
   png_bytep chunk_name, png_bytep data, png_size_t length));

/* пиши the start of a PNG chunk - length and chunk имя. */
extern PNG_EXPORT(void,png_write_chunk_start) PNGARG((png_structp png_ptr,
   png_bytep chunk_name, png_uint_32 length));

/* пиши the data of a PNG chunk started with png_write_chunk_start(). */
extern PNG_EXPORT(void,png_write_chunk_data) PNGARG((png_structp png_ptr,
   png_bytep data, png_size_t length));

/* финиш a chunk started with png_write_chunk_start() (includes CRC). */
...
/* читай the information before the actual image data. */
...
/* Преобр from a struct tm to png_time */
extern PNG_EXPORT(void,png_convert_from_struct_tm) PNGARG((png_timep ptime,
   struct tm FAR * ttime));

/* Преобр from time_t to png_time.  Uses gmtime() */
extern PNG_EXPORT(void,png_convert_from_time_t) PNGARG((png_timep ptime,
   time_t ttime));
#endif /* PNG_CONVERT_tIME_SUPPORTED */

#ifdef PNG_READ_EXPAND_SUPPORTED
/* расширь data to 24-bit дайКЗС, or 8-bit grayscale, with alpha if available. */
...
/* расширь the grayscale to 24-bit дайКЗС if necessary. */
extern PNG_EXPORT(void,png_set_gray_to_rgb) PNGARG((png_structp png_ptr));
#endif

#ifdef PNG_READ_RGB_TO_GRAY_SUPPORTED
/* Reduce дайКЗС to grayscale. */
#ifdef PNG_FLOATING_POINT_SUPPORTED
...
#if defined(PNG_READ_FILLER_SUPPORTED) || defined(PNG_WRITE_FILLER_SUPPORTED)
/* добавь a filler byte to 8-bit серый or 24-bit дайКЗС images. */
extern PNG_EXPORT(void,png_set_filler) PNGARG((png_structp png_ptr,
   png_uint_32 filler, int flags));
/* The values of the PNG_FILLER_ defines should NOT be changed */
#define PNG_FILLER_BEFORE 0
#define PNG_FILLER_AFTER 1
/* добавь an alpha byte to 8-bit серый or 24-bit дайКЗС images. */
#ifndef PNG_1_0_X
...
/* разверни bytes in 16-bit depth files. */
...
/* разверни packing order of pixels in bytes. */
...
#ifdef PNG_WRITE_FLUSH_SUPPORTED
/* уст how many lines between output flushes - 0 for no flushing */
extern PNG_EXPORT(void,png_set_flush) PNGARG((png_structp png_ptr, int nrows));
/* слей the current PNG output буфер */
...
/* читай one or more rows of image data. */
...
#ifndef PNG_NO_SEQUENTIAL_READ_SUPPORTED
/* читай the whole image into memory at once. */
extern PNG_EXPORT(void,png_read_image) PNGARG((png_structp png_ptr,
   png_bytepp image));
#endif

/* пиши a row of image data */
extern PNG_EXPORT(void,png_write_row) PNGARG((png_structp png_ptr,
   png_bytep row));

/* пиши a few rows of image data */
...
#ifndef PNG_NO_SEQUENTIAL_READ_SUPPORTED
/* читай the end of the PNG file. */
extern PNG_EXPORT(void,png_read_end) PNGARG((png_structp png_ptr,
   png_infop info_ptr));
#endif

/* освободи any memory associated with the png_info_struct */
extern PNG_EXPORT(void,png_destroy_info_struct) PNGARG((png_structp png_ptr,
   png_infopp info_ptr_ptr));

/* освободи any memory associated with the png_struct and the png_info_structs */
extern PNG_EXPORT(void,png_destroy_read_struct) PNGARG((png_structpp
   png_ptr_ptr, png_infopp info_ptr_ptr, png_infopp end_info_ptr_ptr));

/* освободи all memory used by the read (old method - NOT DLL EXPORTED) */
...
/* освободи any memory used in png_ptr struct (old method - NOT DLL EXPORTED) */
extern void png_write_destroy PNGARG((png_structp png_ptr)) PNG_DEPRECATED;

/* уст the libpng method of handling chunk CRC errors */
 ...
 * chunk.  For versions prior to 0.90, the action was always Ошибка/quit,
 * whereas in version 0.90 and later, the action for CRC errors in ancillary
 * chunks is warn/discard.  These values should NOT be changed.
 *
 *      значение                       action:critical     action:ancillary
 */
#define PNG_CRC_DEFAULT       0  /* Ошибка/quit          warn/discard data */
#define PNG_CRC_ERROR_QUIT    1  /* Ошибка/quit          Ошибка/quit        */
#define PNG_CRC_WARN_DISCARD  2  /* (INVALID)           warn/discard data */
#define PNG_CRC_WARN_USE      3  /* warn/use data       warn/use data     */
#define PNG_CRC_QUIET_USE     4  /* quiet/use data      quiet/use data    */
#define PNG_CRC_NO_CHANGE     5  /* use current значение   use current значение */
...
/* уст the filtering method(s) used by libpng.  Currently, the only valid
 * значение for "method" is 0.
 */
...
/* фильтруй values (not flags) - used in pngwrite.c, pngwutil.c for now.
...
#define PNG_FILTER_HEURISTIC_LAST       3  /* Not a valid значение */

/* уст the library compression level.  Currently, valid values range from
...
/* These next functions are called for input/output, memory, and Ошибка
...
/* замени the (Ошибка and abort), and warning functions with user
 * supplied functions.  If no messages are to be printed you must still
 * write and use replacement functions. The replacement error_fn should
 * still do a longjmp to the last setjmp location if you are using this
 * method of Ошибка handling.  If error_fn or warning_fn is NULL, the
...
/* Return the user pointer associated with the Ошибка functions */
extern PNG_EXPORT(png_voidp,png_get_error_ptr) PNGARG((png_structp png_ptr));

/* замени the default data output functions with a user supplied one(s).
...
/* замени the default data input function with a user supplied one. */
...
#ifdef PNG_USER_MEM_SUPPORTED
/* замени the default memory allocation functions with user supplied one(s). */
...
/* Функция to be called when data becomes available */
extern PNG_EXPORT(void,png_process_data) PNGARG((png_structp png_ptr,
   png_infop info_ptr, png_bytep буфер, png_size_t buffer_size));

/* Функция that combines rows.  Not very much different than the
 * png_combine_row() call.  является this even used?????
 */
...
#ifdef PNG_1_0_X
/* Функция to allocate memory for zlib. */
...
/* освободи data that was allocated internally */
...
extern PNG_EXPORT(png_voidp,png_memset_check) PNGARG((png_structp png_ptr,
   png_voidp s1, int значение, png_uint_32 size)) PNG_DEPRECATED;
...
/* Fatal Ошибка in PNG image of libpng - can't continue */
extern PNG_EXPORT(void,png_error) PNGARG((png_structp png_ptr,
   png_const_charp error_message)) PNG_NORETURN;

/* The same, but the chunk имя is prepended to the Ошибка string. */
...
/* Non-fatal Ошибка in libpng.  Can continue, but may have a problem. */
extern PNG_EXPORT(void,png_warning) PNGARG((png_structp png_ptr,
   png_const_charp warning_message));

#ifdef PNG_READ_SUPPORTED
/* Non-fatal Ошибка in libpng, chunk имя is prepended to message. */
 ...
 * png_get_<chunk> functions return a non-zero значение if the data was available
...
png_infop info_ptr));
/* уст row_pointers, which is an array of pointers to scanlines for use
 * by png_write_png().
 */
...
extern PNG_EXPORT(png_uint_32,png_get_pCAL) PNGARG((png_structp png_ptr,
   ...
   int *тип, int *nparams, png_charp *units, png_charpp *params));
#endif
...
extern PNG_EXPORT(png_uint_32,png_get_iCCP) PNGARG((png_structp png_ptr,
   png_infop info_ptr, png_charpp имя, int *compression_type,
...
extern PNG_EXPORT(void,png_set_iCCP) PNGARG((png_structp png_ptr,
   png_infop info_ptr, png_charp имя, int compression_type,
...
/* Provide a list of chunks and how they are to be handled, if the built-in
   handling or default unknown chunk handling is not desired.  Любое chunks not
...
#      define png_debug(l,m) \
       ...
       int num_tabs=l; \
       char формат[256]; \
       snprintf(формат,256,"%s%s%s",(num_tabs==1 ? "\t" : \
         (num_tabs==2 ? "\t\t":(num_tabs>2 ? "\t\t\t":""))), \
         m,PNG_STRING_NEWLINE); \
       fprintf(PNG_DEBUG_FILE,формат); \
       }
#    endif
#    ifndef png_debug1
#      define png_debug1(l,m,p1) \
       ...
       snprintf(формат,256,"%s%s%s",(num_tabs==1 ? "\t" : \
         (num_tabs==2 ? "\t\t":(num_tabs>2 ? "\t\t\t":""))), \
         m,PNG_STRING_NEWLINE); \
       fprintf(PNG_DEBUG_FILE,формат,p1); \
       }
#    endif
#    ifndef png_debug2
#      define png_debug2(l,m,p1,p2) \
       ...
       snprintf(формат,256,"%s%s%s",(num_tabs==1 ? "\t" : \
         (num_tabs==2 ? "\t\t":(num_tabs>2 ? "\t\t\t":""))), \
         m,PNG_STRING_NEWLINE); \
       fprintf(PNG_DEBUG_FILE,формат,p1,p2); \
       }
#    endif
#  endif /* __STDC __ */
#endif /* (PNG_DEBUG > 1) */
...
/* Strip the prepended Ошибка numbers ("#nnn ") from Ошибка and warning
 * messages before passing them to the Ошибка or warning handler.
 */
...
extern PNG_EXPORT(png_uint_32,png_get_user_height_max) PNGARG((png_structp
   png_ptr));
#endif
/* Maintainer: помести new public prototypes here ^, in libpng.3, and in
 * project defs
 */
...
/* Inline macros to do direct reads of bytes from the input буфер.  These
 * require that you are using an architecture that uses PNG byte ordering
 * (MSB first) and supports unaligned data storage.  I think that PowerPC
 * in big-endian mode and 680x0 are the only ones that will support this.
 * The x86 line of processors definitely do not.  The png_get_int_32()
 * routine also assumes we are using two's complement формат for negative
...
/* Place a 32-bit number into a буфер in PNG byte order (big-endian).
 */
...
/* Place a 16-bit number into a буфер in PNG byte order.
 ...
 * functionality in libpng.  ещё information about most functions can
...
/* ещё modes of operation.  Note that after an init, mode is set to
...
/* сохрани typing and make code easier to understand */

#define PNG_COLOR_DIST(c1, c2) (абс((int)((c1).red) - (int)((c2).red)) + \
   абс((int)((c1).green) - (int)((c2).green)) + \
   абс((int)((c1).blue) - (int)((c2).blue)))
...
/* PNG_OUT_OF_RANGE returns true if значение is outside the range
 * ideal-delta..ideal+delta.  Each argument is evaluated twice.
 * "ideal" and "delta" should be constants, normally simple
 * integers, "значение" a variable. Added to libpng-1.2.6 JB
 */
#define PNG_OUT_OF_RANGE(значение, ideal, delta) \
        ( (значение) < (ideal)-(delta) || (значение) > (ideal)+(delta) )
 ...
 * define the имя here, and add an invocation of the macro in png.c and
...
PNG_EXTERN png_voidp png_create_struct PNGARG((int тип)) PNG_PRIVATE;

/* освободи memory from internal libpng struct */
PNG_EXTERN void png_destroy_struct PNGARG((png_voidp struct_ptr)) PNG_PRIVATE;

PNG_EXTERN png_voidp png_create_struct_2 PNGARG((int тип, png_malloc_ptr
  malloc_fn, png_voidp mem_ptr)) PNG_PRIVATE;
PNG_EXTERN void png_destroy_struct_2 PNGARG((png_voidp struct_ptr,
   png_free_ptr free_fn, png_voidp mem_ptr)) PNG_PRIVATE;

/* освободи any memory that info_ptr points to and reset struct. */
...
#ifdef PNG_SIZE_T
/* Функция to convert a sizeof an элт to png_sizeof элт */
   PNG_EXTERN png_size_t PNGAPI png_convert_size PNGARG((size_t size))
      PNG_PRIVATE;
#endif

/* следщ four functions are used internally as callbacks.  PNGAPI is required
...
PNG_EXTERN void PNGAPI png_push_fill_buffer PNGARG((png_structp png_ptr,
   png_bytep буфер, png_size_t length)) PNG_PRIVATE;
#endif
...
/* переустанов the CRC variable */
PNG_EXTERN void png_reset_crc PNGARG((png_structp png_ptr)) PNG_PRIVATE;

/* пиши the "data" буфер to whatever output you are using. */
PNG_EXTERN void png_write_data PNGARG((png_structp png_ptr, png_bytep data,
   png_size_t length)) PNG_PRIVATE;

/* читай data from whatever input you are using into the "data" буфер */
PNG_EXTERN void png_read_data PNGARG((png_structp png_ptr, png_bytep data,
   png_size_t length)) PNG_PRIVATE;

/* читай bytes into buf, and update png_ptr->crc */
...
/* читай "skip" bytes, read the file crc, and (optionally) verify png_ptr->crc */
PNG_EXTERN int png_crc_finish PNGARG((png_structp png_ptr, png_uint_32 skip)
   PNG_PRIVATE);

/* читай the CRC from the file and compare it to the libpng calculated CRC */
PNG_EXTERN int png_crc_error PNGARG((png_structp png_ptr)) PNG_PRIVATE;

/* Calculate the CRC over a section of data.  Note that we are only
 * passing a maximum of 64K on systems that have this as a memory limit,
 * since this is the maximum буфер size we can specify.
 */
...
/* пиши the IHDR chunk, and update the png_struct with the necessary
 * information.
 */
...
PNG_EXTERN png_size_t png_check_keyword PNGARG((png_structp png_ptr,
   png_charp ключ, png_charpp new_key)) PNG_PRIVATE;
#endif

#ifdef PNG_WRITE_tEXt_SUPPORTED
PNG_EXTERN void png_write_tEXt PNGARG((png_structp png_ptr, png_charp ключ,
   png_charp text, png_size_t text_len)) PNG_PRIVATE;
#endif

#ifdef PNG_WRITE_zTXt_SUPPORTED
PNG_EXTERN void png_write_zTXt PNGARG((png_structp png_ptr, png_charp ключ,
   png_charp text, png_size_t text_len, int compression)) PNG_PRIVATE;
#endif

#ifdef PNG_WRITE_iTXt_SUPPORTED
PNG_EXTERN void png_write_iTXt PNGARG((png_structp png_ptr,
   int compression, png_charp ключ, png_charp lang, png_charp lang_key,
...
PNG_EXTERN void png_write_pCAL PNGARG((png_structp png_ptr, png_charp purpose,
   png_int_32 X0, png_int_32 X1, int тип, int nparams,
...
/* GRR TO ДЕЛАЙ (2.0 or whenever):  simplify other internal calling interfaces */
...
/* пиши out the filtered row. */
PNG_EXTERN void png_write_filtered_row PNGARG((png_structp png_ptr,
   png_bytep filtered_row)) PNG_PRIVATE;
/* финиш a row while reading, dealing with interlacing passes, etc. */
...
/* The following decodes the appropriate chunks, and does Ошибка correction,
...
PNG_EXTERN void png_push_restore_buffer PNGARG((png_structp png_ptr,
   png_bytep буфер, png_size_t buffer_length)) PNG_PRIVATE;
...
/* Maintainer: помести new private prototypes here ^ and in libpngpf.3 */
