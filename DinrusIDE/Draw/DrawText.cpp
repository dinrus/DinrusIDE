#include "Draw.h"

namespace РНЦП {

#define LLOG(x)    // LOG(x)
#define LTIMING(x) // TIMING(x)

ШТкст TextUnicode(const char *s, int n, byte cs, Шрифт font)
{
	if(n < 0)
		n = (int)strlen(s);
#ifdef PLATFORM_WIN32
	if(font.GetFace() == Шрифт::SYMBOL) {
		ШТкстБуф b(n);
		wchar *t = b;
		while(n > 0) {
			*t++ = *s++;
			n--;
		}
		return ШТкст(b);
	}
#endif
	if(cs == CHARSET_UTF8)
		return вУтф32(s, n);
	return вЮникод(s, n, cs);
}

void Draw::DrawText(int x, int y, int angle, const wchar *text, Шрифт font,
		            Цвет ink, int n, const int *dx)
{
	if(пусто_ли(ink)) return;
	ink = ResolveInk(ink);
	if(n < 0)
		n = strlen__(text);
	Std(font);
	double sina = 0;
	double cosa = 1;
	int    posx = 0;
	if(angle)
		Draw::SinCos(angle, sina, cosa);
	Шрифт font0 = font;
	if(GetInfo() & DRAWTEXTLINES)
		font.Underline(false).Strikeout(false);
	for(int i = 0; i < n; i++) {
		wchar chr = text[i];
		GlyphInfo gi = GetGlyphInfo(font, chr);
		if(gi.IsNormal())
			if(angle || chr >= 0x10000)
				DrawTextOp(int(x + cosa * posx), int(y - sina * posx), angle, &chr, font, ink, 1, NULL);
			else {
				int c = 1;
				int dd = 0;
				while(i + c < n && c < 1000) {
					GlyphInfo gi2 = GetGlyphInfo(font, text[i + c]);
					if(!gi2.IsNormal())
						break;
					dd += dx ? dx[c - 1] : gi.width;
					c++;
					gi = gi2;
				}
				DrawTextOp(x + posx, y, 0, text + i, font, ink, c, dx);
				posx += dd;
				i += c - 1;
				if(dx)
					dx += c - 1;
			}
		else
		if(gi.IsReplaced()) {
			Шрифт fnt = font;
			fnt.Face(gi.lspc);
			fnt.устВысоту(gi.rspc);
			if(angle)
				DrawTextOp(int(x + cosa * posx), int(y - sina * (font.GetAscent() - fnt.GetAscent() + posx)),
				             angle, &chr, fnt, ink, 1, NULL);
			else
				DrawTextOp(x + posx, y + font.GetAscent() - fnt.GetAscent(), 0, &chr, fnt, ink, 1, NULL);
			GlyphMetrics(gi, font, chr);
		}
		else
		if(gi.IsComposed()) {
			ComposedGlyph cg;
			Compose(font, chr, cg);
			if(angle) {
				DrawTextOp(int(x + cosa * posx), int(y - sina * posx), angle, &cg.basic_char, font, ink, 1, NULL);
				DrawTextOp(int(x + cosa * (posx + cg.mark_pos.x)), int(y - sina * (cg.mark_pos.y + posx)), angle, &cg.mark_char, cg.mark_font, ink, 1, NULL);
			}
			else {
				DrawTextOp(x + posx, y, 0, &cg.basic_char, font, ink, 1, NULL);
				DrawTextOp(x + cg.mark_pos.x + posx, y + cg.mark_pos.y, 0, &cg.mark_char, cg.mark_font, ink, 1, NULL);
			}
			GlyphMetrics(gi, font, chr);
		}
		else {
			Шрифт fnt = Arial(font.дайВысоту());
			wchar chr = 0x25a1;
			gi = GetGlyphInfo(fnt, chr);
			if(!gi.IsNormal()) {
				chr = ' ';
				gi = GetGlyphInfo(fnt, chr);
			}
			if(angle)
				DrawTextOp(int(x + cosa * posx), int(y - sina * posx), angle, &chr, fnt, ink, 1, NULL);
			else
				DrawTextOp(x + posx, y, 0, &chr, fnt, ink, 1, NULL);
		}
		posx += dx ? *dx++ : gi.width;
	}

	if((GetInfo() & DRAWTEXTLINES) && (font0.IsUnderline() || font0.IsStrikeout())) {
		int hg = абс(font0.GetCy());
		if(hg == 0) hg = 10;
		int thickness = max(hg / 20, 1);

		int ascent = font0.GetAscent();
		Размер offset = Точка(0, ascent);
		if(angle) {
			offset.cx = fround(ascent * sina);
			offset.cy = fround(ascent * cosa);
		}

		x += offset.cx;
		y += offset.cy;
		if(font0.IsUnderline()) {
			int p = max(hg / 15, int(font0.Info().GetDescent() > 0));
			DrawLine(
				int(x + p * sina),
				int(y + p * cosa),
				int(x + posx * cosa + p * sina),
				int(y - posx * sina + p * cosa),
				thickness,
				ink
			);
		}
		if(font0.IsStrikeout()) {
			int p = -ascent / 3;
			DrawLine(
				int(x + p * sina),
				int(y + p * cosa),
				int(x + posx * cosa + p * sina),
				int(y - posx * sina + p * cosa),
				thickness,
				ink
			);
		}
	}
}

// ----------------------------

void Draw::DrawText(int x, int y, const wchar *text, Шрифт font,
                    Цвет ink, int n, const int *dx)
{
	DrawText(x, y, 0, text, font, ink, n, dx);
}

// ---------------------------

void Draw::DrawText(int x, int y, int angle, const ШТкст& text, Шрифт font,
                    Цвет ink, const int *dx)
{
	DrawText(x, y, angle, ~text, font, ink, text.дайДлину(), dx);
}

void Draw::DrawText(int x, int y, const ШТкст& text, Шрифт font, Цвет ink, const int *dx)
{
	DrawText(x, y, 0, text, font, ink, dx);
}

// ---------------------------

void Draw::DrawText(int x, int y, int angle, const char *text, byte charset, Шрифт font,
                    Цвет ink, int n, const int *dx)
{
	DrawText(x, y, angle, TextUnicode(text, n, charset, font), font, ink, dx);
}

void Draw::DrawText(int x, int y, const char *text, byte charset, Шрифт font,
                    Цвет ink, int n, const int *dx)
{
	DrawText(x, y, 0, text, charset, font, ink, n, dx);
}

// ---------------------------

void Draw::DrawText(int x, int y, int angle, const char *text,
                    Шрифт font, Цвет ink, int n, const int *dx)
{
	DrawText(x, y, angle, text, CHARSET_DEFAULT, font, ink, n, dx);
}

void Draw::DrawText(int x, int y, const char *text, Шрифт font,
                    Цвет ink, int n, const int *dx)
{
	DrawText(x, y, text, CHARSET_DEFAULT, font, ink, n, dx);
}

// ---------------------------

void Draw::DrawText(int x, int y, int angle, const Ткст& text, Шрифт font,
                    Цвет ink, const int *dx)
{
	DrawText(x, y, angle, text, font, ink, text.дайДлину(), dx);
}

void Draw::DrawText(int x, int y, const Ткст& text, Шрифт font, Цвет ink, const int *dx)
{
	ШТкст h = TextUnicode(text, text.дайДлину(), CHARSET_DEFAULT, font);
	DrawText(x, y, h, font, ink, h.дайДлину(), dx);
}

// --------------------------

Размер дайРазмТекста(const wchar *text, Шрифт font, int n)
{
	FontInfo fi = font.Info();
	if(n < 0)
		n = strlen__(text);
	Размер sz;
	sz.cx = 0;
	const wchar *wtext = (const wchar *)text;
	while(n > 0) {
		if(*wtext >= ' ')
			sz.cx += fi[*wtext];
		n--;
		wtext++;
	}
	sz.cy = fi.дайВысоту();
	return sz;
}

Размер дайРазмТекста(const ШТкст& text, Шрифт font)
{
	return дайРазмТекста(text, font, text.дайДлину());
}

Размер дайРазмТекста(const char *text, byte charset, Шрифт font, int n)
{
	return дайРазмТекста(TextUnicode(text, n, charset, font), font);
}

Размер дайРазмТекста(const char *text, Шрифт font, int n)
{
	return дайРазмТекста(text, CHARSET_DEFAULT, font, n);
}

Размер дайРазмТекста(const Ткст& text, Шрифт font)
{
	return дайРазмТекста(text, font, text.дайДлину());
}

}
