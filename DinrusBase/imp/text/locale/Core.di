 ...
 * Примечания: text.locale adopts the RFC 1766 стандарт for культура names in the фмт &lt;language&gt;"-"&lt;region&gt;.
 ...
 *
 *   Стдвыв.фмтнс("англИмя: {}", культура.англИмя);
 *   Стдвыв.фмтнс("исконноеИмя: {}", культура.исконноеИмя);
 *   Стдвыв.фмтнс("имя: {}", культура.имя);
 *   Стдвыв.фмтнс("родитель: {}", культура.родитель.имя);
 *   Стдвыв.фмтнс("нейтрален_ли: {}", культура.нейтрален_ли);
 * }
 *
...
extern(D) class Культура : ИСлужбаФормата
{
     ...
     * Возвращает: A ткст containing the имя of the Культура in the фмт &lt;language&gt;"-"&lt;region&gt;.
     */
    ...
    /**
     * $(ANCHOR Culture_name)
     * $(I Свойство.) Retrieves the имя of the Культура in the фмт &lt;language&gt;"-"&lt;region&gt;.
     * Возвращает: The имя of the текущ экземпляр. Например, the имя of the UK English культура is "en-GB".
     */
    public ткст имя();

    /**
     * $(I Свойство.) Retrieves the имя of the Культура in the фмт &lt;languagename&gt; (&lt;regionname&gt;) in English.
    ...
    /**
     * $(I Свойство.) Retrieves the имя of the Культура in the фмт &lt;languagename&gt; (&lt;regionname&gt;) in its исконный language.
     ...
     * $(I Свойство.) Retrieves a ФорматЧисла defining the culturally appropriate фмт for displaying numbers и валюта.
     * Возвращает: A ФорматЧисла defining the culturally appropriate фмт for displaying numbers и валюта.
    */
    public ФорматЧисла форматЧисла();
	
    /**
     * $(I Свойство.) Assigns a ФорматЧисла defining the culturally appropriate фмт for displaying numbers и валюта.
     * Параметры: значения = A ФорматЧисла defining the culturally appropriate фмт for displaying numbers и валюта.
     */
    public проц форматЧисла(ФорматЧисла значение);

    /**
     * $(ANCHOR Культура_форматДатыВремени)
     * $(I Свойство.) Retrieves a ФорматДатыВремени defining the culturally appropriate фмт for displaying dates и times.
     * Возвращает: A ФорматДатыВремени defining the culturally appropriate фмт for displaying dates и times.
     */
    public ФорматДатыВремени форматДатыВремени();
	
    /**
     * $(I Свойство.) Assigns a ФорматДатыВремени defining the culturally appropriate фмт for displaying dates и times.
     * Параметры: значения = A ФорматДатыВремени defining the culturally appropriate фмт for displaying dates и times.
     */
 ...
 * Примечания: Numbers are formatted using фмт образцы retrieved из_ a ФорматЧисла экземпляр.
...
extern(D) class ФорматЧисла : ИСлужбаФормата
{
    ...
    /**
     * Retrieves an объект defining как в_ фмт the specified тип.
     ...
     *   цел n = 5678;
     *   Println(Форматировщик.фмт(фмт, "{0:N}", n));
     *
    ...
    /**
     * $(I Свойство.) Retrieves the фмт образец for негатив numbers.
     * Возвращает: The фмт образец for негатив numbers. For инвариантныйФормат, the default is 1 (representing "-n").
    ...
    /**
     * $(I Свойство.) Assigns the фмт образец for негатив numbers.
     * Параметры: значение = The фмт образец for негатив numbers.
     * Примеры:
     ...
     * Возвращает: The фмт образец в_ use for негатив валюта значения.
     */
    public final цел валютнОтрицатОбразец();
	
    /**
     * $(I Свойство.) Assigns the formal образец в_ use for негатив валюта значения.
     * Параметры: значение = The фмт образец в_ use for негатив валюта значения.
     */
     ...
     * Возвращает: The фмт образец в_ use for positive валюта значения.
     */
...
extern(D) class ФорматДатыВремени : ИСлужбаФормата
{
     ...
     * Retrieves the стандарт образцы in which Время значения can be formatted using the specified фмт character.
     * Возвращает: An Массив of strings containing the стандарт образцы in which Время значения can be formatted using the specified фмт character.
     */
    public final ткст[] дайВсеОбразцыДатыВремени(сим фмт);
     ...
     * $(I Свойство.) Retrieves the фмт образец for a крат дата значение.
     * Возвращает: The фмт образец for a крат дата значение.
     */
    public final ткст краткийОбразецДаты();
    /**
     * $(I Свойство.) Assigns the фмт образец for a крат дата _value.
     * Параметры: значение = The фмт образец for a крат дата _value.
     */
    public final проц краткийОбразецДаты(ткст значение);

    /**
     * $(ANCHOR DateTimeFormat_shortTimePattern)
     * $(I Свойство.) Retrieves the фмт образец for a крат время значение.
     * Возвращает: The фмт образец for a крат время значение.
     */
    public final ткст краткийОбразецВремени();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a крат время _value.
     * Параметры: значение = The фмт образец for a крат время _value.
     */
    public final проц краткийОбразецВремени(ткст значение);

    /**
     * $(ANCHOR DateTimeFormat_longDatePattern)
     * $(I Свойство.) Retrieves the фмт образец for a дол дата значение.
     * Возвращает: The фмт образец for a дол дата значение.
     */
    public final ткст длинныйОбразецДаты();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a дол дата _value.
     * Параметры: значение = The фмт образец for a дол дата _value.
     */
     ...
     * $(I Свойство.) Retrieves the фмт образец for a дол время значение.
     * Возвращает: The фмт образец for a дол время значение.
     */
    public final ткст длинныйОбразецВремени();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a дол время _value.
     * Параметры: значение = The фмт образец for a дол время _value.
     */
    public final проц длинныйОбразецВремени(ткст значение);

    /**
     * $(ANCHOR DateTimeFormat_monthDayPattern)
     * $(I Свойство.) Retrieves the фмт образец for a месяц и день значение.
     * Возвращает: The фмт образец for a месяц и день значение.
     */
    public final ткст образецДняМесяца();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a месяц и день _value.
     * Параметры: значение = The фмт образец for a месяц и день _value.
     */
    public final проц образецДняМесяца(ткст значение);

    /**
     * $(ANCHOR DateTimeFormat_yearMonthPattern)
     * $(I Свойство.) Retrieves the фмт образец for a год и месяц значение.
     * Возвращает: The фмт образец for a год и месяц значение.
     */
    public final ткст образецМесяцаГода();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a год и месяц _value.
     * Параметры: значение = The фмт образец for a год и месяц _value.
     */
     ...
     * $(I Свойство.) Retrieves the фмт образец for a дол дата и a дол время значение.
     * Возвращает: The фмт образец for a дол дата и a дол время значение.
     */
    public final ткст полныйОбразецДатыВремени();
	
    /**
     * $(I Свойство.) Assigns the фмт образец for a дол дата и a дол время _value.
     * Параметры: значение = The фмт образец for a дол дата и a дол время _value.
     */
    public final проц полныйОбразецДатыВремени(ткст значение);

    /**
     * $(ANCHOR DateTimeFormat_rfc1123Pattern)
     * $(I Свойство.) Retrieves the фмт образец based on the IETF RFC 1123 specification, for a время значение.
     * Возвращает: The фмт образец based on the IETF RFC 1123 specification, for a время значение.
     */
    public final ткст образецРФС1123();

    /**
     * $(ANCHOR DateTimeFormat_sortableDateTimePattern)
     * $(I Свойство.) Retrieves the фмт образец for a sortable дата и время значение.
     * Возвращает: The фмт образец for a sortable дата и время значение.
     */
    public final ткст сортируемыйОбразецДатыВремени();

    /**
     * $(ANCHOR DateTimeFormat_universalSortableDateTimePattern)
     * $(I Свойство.) Retrieves the фмт образец for a universal дата и время значение.
     * Возвращает: The фмт образец for a universal дата и время значение.
     */
