...
void RichTxt::FormatInfo::уст(const RichPara::фмт& fmt)
{
	(RichPara::фмт&)*this = fmt;
	charvalid = paravalid = 0xffffffff;
}

void RichTxt::FormatInfo::уст(const RichPara::CharFormat& fmt)
{
...
void RichTxt::FormatInfo::Combine(const RichPara::фмт& fmt)
{
	if(align != fmt.align)
		paravalid &= ~ALIGN;
	...
	if(NumberingDiffers(*this, fmt))
		paravalid &= ~NUMBERING;
	if(tab.дайСчёт() != fmt.tab.дайСчёт()) {
		paravalid &= ~TABS;
		tab.очисть();
	}
	else
...
void RichTxt::FormatInfo::ApplyTo(RichPara::фмт& fmt) const
{
