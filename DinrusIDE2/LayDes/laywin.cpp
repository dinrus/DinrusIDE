...
#include <DinrusIDE2/DinrusIDE2.h>

#define LTIMING(x) //TIMING(x)

#define KEYNAMESPACE LayoutKeys
#define KEYGROUPNAME "Layout"
#define KEYFILE      <DinrusIDE2/LayDes/laydes.key>
...
void LayDes::EditBar(Bar& bar)
{
	...
	bar.Add(islayout, "Выбрать все", CtrlImg::select_all(), THISBACK(SelectAll))
	   .Key(K_CTRL_A);
	...
	bar.Separator();
	bar.Add(islayout && CurrentLayout().IsUndo(), "Отменить", CtrlImg::undo(), THISBACK(Undo))
	   .Key(K_ALT_BACKSPACE)
	   .Key(K_CTRL_Z);
	bar.Add(islayout && CurrentLayout().IsRedo(), "Восстановить", CtrlImg::redo(), THISBACK(Redo))
...
void LayDes::GotoUsing()
{
	...
	if(item.IsCursor()) // TODO not for label
		DinrusIde()->FindDesignerItemReferences("With" + CurrentLayout().name + "::" + ~item.Get(1), ~item.Get(1));
	else
		DinrusIde()->FindDesignerItemReferences(CurrentLayout().name + "__layid", "With" + CurrentLayout().name);
}
...
void LayDes::MainMenuBar(Bar& bar)
{
	...
	bar.Add("Элемент", THISBACK(ItemBar));
...
LayDes::LayDes()
{
	...
	item.AddColumn("Перемен / ярл", 10).Margin(0);
