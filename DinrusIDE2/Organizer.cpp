...
bool UsesDlg::New()
{
	String s = SelectPackage("Выберите пакет");
	text <<= s;
	return !IsNull(s);
}
...
void PackageEditor::Prepare(WithUppOptDlg<TopWindow>& dlg, int type)
{
	...
	if(type == FLAG) {
		...
		dlg.info.SetLabel("Используйте префикс '-', чтобы удалить этот флаг");
	}
}
...
void PackageEditor::EditOption(bool duplicate)
{
	...
	if(type >= FLAG && type <= PKG_LAST) {
		...
		if(i >= 0 && i < m.GetCount()) {
			...
			Prepare(dlg, type);
			if(duplicate)
				dlg.Title(GetTitle().ToString() + " - дубликат");
			dlg.when <<= m[i].when;
			dlg.text <<= m[i].text;
			if(dlg.Run() != IDOK)
				return;
...
PackageEditor::PackageEditor()
{
	...
	Add("Статические библиотеки", actual.static_library);
	Add("Опции компоновки", actual.link);
	Add("Опции компилятора", actual.option);
