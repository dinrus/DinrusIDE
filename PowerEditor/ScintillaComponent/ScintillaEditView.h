...
struct LanguageNameInfo {
    const char* _langName = nullptr;
    const char* _shortName = nullptr;
...
public:
    ...
    void getGenericText(char *dest, size_t deslen, size_t start, size_t end, intptr_t* mstart, intptr_t* mend) const;
    String getGenericTextAsString(size_t start, size_t end) const;
    ...
    char * getGenericWordOnCaretPos(char * txt, int size);
    char * getGenericSelectedText(char * txt, int size, bool expand = true);
    intptr_t searchInTarget(const char * Text2Find, size_t lenOfText2Find, size_t fromPos, size_t toPos) const;
    void appandGenericText(const char * text2Append) const;
    void addGenericText(const char * text2Append) const;
    void addGenericText(const char * text2Append, intptr_t* mstart, intptr_t* mend) const;
    intptr_t replaceTarget(const char * str2replace, intptr_t fromTargetPos = -1, intptr_t toTargetPos = -1) const;
    intptr_t replaceTargetRegExMode(const char * re, intptr_t fromTargetPos = -1, intptr_t toTargetPos = -1) const;
    void showAutoComletion(size_t lenEntered, const char * list);
    void showCallTip(size_t startPos, const char * def);
    String getLine(size_t lineNumber);
    void getLine(size_t lineNumber, char * line, size_t lineBufferLen);
    ...
    void columnReplace(ColumnModeInfos & cmi, const char *str);
    ...
    SciBuffer * getCurrentBuffer() { return _currentBuffer; };
    void setCurrentBuffer(SciBuffer *buf2set) { _currentBuffer = buf2set; };
    void styleChange();

    void hideLines();

    bool markerMarginClick(size_t lineNumber);  //true if it did something
    void notifyMarkers(SciBuffer * buf, bool isHide, size_t location, bool del);
...
protected:
    ...
    SciBuffer * _currentBuffer = nullptr;
