...
&]
[s0;%- &]
[s0; [/ Ïğîèçâîäíûé îò] [^topic`:`/`/CtrlCore`/src`/Ctrl`$en`-us`#`:`:Ctrl`:`:class Ctrl^ C
trl]&]
[s3; &]
[s0;%- &]
...
[s2; Óñòàíàâëèâàåòthe minimum (thumb in leftmost position) and maximum (thumb 
in rightmost position) values. Âîçâğàùàåò `*this.&]
[s3; &]
[s4;%- &]
[s5;:SliderCtrl`:`:Range`(int`):%- [_^SliderCtrl^ SliderCtrl][@(0.0.255) `&]_[* Range]([@(0.0.255) i
nt]_[*@3 max])&]
[s2; Òî æå, ÷òî è [^topic`:`/`/CtrlLib`/src`/Slider`$en`-us`#SliderCtrl`:`:MinMax`(int`,int`)^ M
inMax](0, [%-*@3 max]).&]
[s3; &]
[s4;%- &]
[s5;:SliderCtrl`:`:GetMin`(`)const:%- [@(0.0.255) int]_[* GetMin]()_[@(0.0.255) const]&]
[s2; Âîçâğàùàåò minimum value.&]
[s3; &]
[s4;%- &]
[s5;:SliderCtrl`:`:GetMax`(`)const:%- [@(0.0.255) int]_[* GetMax]()_[@(0.0.255) const]&]
[s2; Âîçâğàùàåò maximum value.&]
[s3; &]
[s4;%- &]
[s5;:SliderCtrl`:`:IsVert`(`)const:%- [@(0.0.255) bool]_[* IsVert]()_[@(0.0.255) const]&]
[s2; Óñòàíàâëèâàåòthe step used in Inc and Dec methods. If [%-*@3 r] is true, 
