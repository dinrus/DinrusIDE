...
HFONT GetWin32Font(Шрифт fnt, int angle)
{
	...
	be.hfont = CreateFontW(
		fnt.дайВысоту() ? -абс(fnt.дайВысоту()) : -12,
		fnt.дайШирину(), angle, angle, fnt.IsBold() ? FW_BOLD : FW_NORMAL,
		fnt.IsItalic(), fnt.IsUnderline(), fnt.IsStrikeout(),
		fnt.GetFace() == Шрифт::SYMBOL ? SYMBOL_CHARSET : DEFAULT_CHARSET,
		fnt.IsTrueTypeOnly() ? OUT_TT_ONLY_PRECIS : OUT_DEFAULT_PRECIS,
		CLIP_DEFAULT_PRECIS,
		fnt.IsNonAntiAliased() ? NONANTIALIASED_QUALITY : DEFAULT_QUALITY,
		DEFAULT_PITCH|FF_DONTCARE,
		вСисНабсимШ(fnt.GetFaceName())
	);
...
static void Win32_GetGlyphIndices(HDC hdc, LPCWSTR s, int n, LPWORD r, DWORD flag)
{
	...
	ONCELOCK
		if(HMODULE hDLL = LoadLibrary("gdi32"))
			фн = (GGIW) GetProcAddress(hDLL, "GetGlyphIndicesW");
	if(фн)
		фн(hdc, s, n, r, flag);
	else
...
static ВекторМап<Ткст, FaceInfo> *sList;

static int CALLBACK Win32_AddFace(const LOGFONTW *logfont, const TEXTMETRICW *, dword тип, LPARAM param)
{
	const char *facename = (const char *)param;

	Ткст имя = изСисНабсимаШ(logfont->lfFaceName);
	
	if(facename && stricmp(имя, facename))
		return 1;
	...
	if(найдиИндекс(разбей("Courier нов CE;Courier нов CYR;Courier нов Greek;"
	                   "Courier нов TUR;Courier нов Baltic;Arial CE;Arial CYR;"
	                   "Arial Greek;Arial TUR;Arial Baltic;Arial CE;Times нов Roman CE;"
	                   "Times нов Roman CYR;Times нов Roman Greek;Times нов Roman TUR;"
	                   "Times нов Roman Baltic;Times нов Roman CE", ';'), имя) >= 0)
		return 1;

	int q = sList->найди(имя);
	FaceInfo& f = q < 0 ? sList->добавь(имя) : (*sList)[q];
	f.имя = имя;

	if(q < 0)
		f.info = Шрифт::SCALEABLE;
	if((logfont->lfPitchAndFamily & 3) == FIXED_PITCH)
		f.info |= Шрифт::FIXEDPITCH;
	if(logfont->lfPitchAndFamily & FF_ROMAN)
		f.info |= Шрифт::SERIFSTYLE;
	if(logfont->lfPitchAndFamily & FF_SCRIPT)
		f.info |= Шрифт::SCRIPTSTYLE;
	if(!(тип & TRUETYPE_FONTTYPE))
		f.info &= ~Шрифт::SCALEABLE;
	if(logfont->lfCharSet == SYMBOL_CHARSET || logfont->lfCharSet == OEM_CHARSET)
		f.info |= Шрифт::SPECIAL;
	return facename ? 0 : 1;
}

static int Win32_EnumFace(HDC hdc, const char *face)
{
	return EnumFontFamiliesW(hdc, face ? вСисНабсимШ(face).begin() : NULL, Win32_AddFace, (LPARAM)face);
}
...
Вектор<FaceInfo> GetAllFacesSys()
{
	...
	sList = &list;
	list.добавь("STDFONT").имя = "STDFONT";
	list.верх().info = 0;
	HDC hdc = ::CreateIC("DISPLAY", NULL, NULL, NULL);
	Win32_ForceFace(hdc, "Times нов Roman", "Arial");
	Win32_ForceFace(hdc, "Arial", "Arial");
	Win32_ForceFace(hdc, "Courier нов", "Arial");
...
GlyphInfo GetUnicodeGlyphInfo(Шрифт fnt, wchar ch)
{
	struct FontRec {
		Шрифт         font = Null;
...
GlyphInfo  GetGlyphInfoSys(Шрифт font, int chr)
{
	...
	static Шрифт      fnt[GLYPHINFOCACHE];
	...
	int page = chr >> 8;
	int q = комбинируйХэш(font, page) % GLYPHINFOCACHE;
...
Ткст GetFontDataSys(Шрифт font, const char *table, int offset, int size)
{
	...
	if(hfont) {
		...
		if(sz == GDI_ERROR) {
			LLOG("PdfDraw::финиш: GDI_ERROR on font " << pdffont.дайКлюч(i));
		}
		else {
...
ТочкаПЗ fx_to_dbl(const ТочкаПЗ& pp, const POINTFX& p) {
	return ТочкаПЗ(pp.x + fx_to_dbl(p.x), pp.y - fx_to_dbl(p.y));
}
		...
		while(cur_poly < end_poly) {
			...
			if (pc->wType == TT_PRIM_LINE)
				for(int i = 0; i < pc->cpfx; i++)
					sw.Строка(fx_to_dbl(pp, pc->apfx[i]));
			if (pc->wType == TT_PRIM_QSPLINE)
				for(int u = 0; u < pc->cpfx - 1; u++) {
					ТочкаПЗ b = fx_to_dbl(pp, pc->apfx[u]);
					ТочкаПЗ c = fx_to_dbl(pp, pc->apfx[u + 1]);
					if (u < pc->cpfx - 2)
						c = середина(b, c);
					sw.Quadratic(b, c);
				}
