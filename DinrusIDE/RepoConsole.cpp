...
{
	CtrlLayoutExit(*this, "Консоль Синхронизации Репозиториев");
...
int UrepoConsole::System(const char *cmd)
{
	...
	if(!host.StartProcess(p, cmd)) {
		list.Add(AttrText("Не удалось запустить программу").SetFont(font().Bold()).Ink(SLtRed()));
		return -1;
	}
	String out;
	canceled = false;
...
int UrepoConsole::CheckSystem(const char *s)
{
	...
	if(exitcode) {
		if(exitcode < 0)
			AddResult("Ошибка при выполнении " + String(s));
		else
			AddResult("код выхода = " + FormatInt(exitcode));
	}
	return exitcode;
}
