...
//	editor2.SetFrame(NullFrame());
	...
	buildmode.AddButton().Tip("Метод построения").Left() <<= THISBACK(DropMethodList);
	buildmode.AddButton().Tip("Режим построения") <<= THISBACK(DropModeList);
	methodlist.Normal();
