...
#define FT_COMPONENT  trace_cffobjs
  ...
  cff_size_init( FT_Size  cffsize )         /* CFF_Size */
  {
    ...
    if ( funcs )
    {
      ...
      Ошибка = funcs->create( cffsize->face->memory, &priv,
                             &internal->topfont );
      if ( Ошибка )
        goto Exit;
  ...
  static void
  remove_subset_prefix( FT_String*  имя )
  {
    FT_Int32  idx             = 0;
    FT_Int32  length          = strlen( имя ) + 1;
  ...
  /* Remove the style part from the family имя (if present). */
                ...
                const FT_String*  style_name )
  {
    ...
    if ( family_name_length > style_name_length )
    {
      ...
      if ( idx > style_name_length )
      {
        ...
        /* between real family имя and style */
        while ( idx > 0                     &&
                ( family_name[idx] == '-' ||
                  family_name[idx] == ' ' ||
                  family_name[idx] == '_' ||
                 ...
                 FT_Parameter*  params )
  {
    ...
    if ( !Ошибка )
    {
      ...
      Ошибка = face->goto_table( face, TTAG_head, stream, 0 );
      if ( !Ошибка )
      {
        pure_cff = 0;

        /* load font directory */
        Ошибка = sfnt->load_face( stream, face, 0, num_params, params );
        if ( Ошибка )
          goto Exit;
      }
      else
      {
        ...
        Ошибка = sfnt->load_cmap( face, stream );
        if ( Ошибка )
          goto Exit;

        /* XXX: we don't load the GPOS table, as OpenType Выкладка     */
        /* support will be added later to a layout library on top of */
        /* FreeType 2                                                */
      }

      /* now load the CFF part of the file */
      Ошибка = face->goto_table( face, TTAG_CFF, stream, 0 );
      if ( Ошибка )
        goto Exit;
    }
    else
    ...
    /* now load and parse the CFF table in the file */
    {
      ...
      face->extra.data = cff;
      Ошибка = cff_font_load( library, stream, face_index, cff, pure_cff );
      if ( Ошибка )
        goto Exit;
      ...
      /* SFNT based fonts use the `имя' table instead.               */
      ...
      /* нормализуй the font matrix so that `matrix->xx' is 1; the */
      ...
      if ( pure_cff )
      {
        ...
        /* retrieve font family & style имя */
        ...
        if ( cffface->family_name )
        {
          ...
          /* We try to extract the style имя from the full имя.   */
          /* We need to ignore пробелы and dashes during the search. */
          if ( full && family )
          {
            while ( *fullp )
            {
              ...
              /* ignore пробелы and dashes in full имя during comparison */
              if ( *fullp == ' ' || *fullp == '-' )
              {
                fullp++;
                continue;
              ...
              /* ignore пробелы and dashes in family имя during comparison */
              if ( *family == ' ' || *family == '-' )
              {
                family++;
                continue;
              ...
              if ( !*family && *fullp )
              {
                /* The full имя begins with the same characters as the  */
                /* family имя, with пробелы and dashes removed.  In this */
  ...
  /* XXX: WE ДЕЛАЙ NOT SUPPORT KERNING METRICS IN THE GPOS TABLE FOR NOW */
#if 0
        /* kerning available? */
        if ( face->kern_pairs )
        ...
        if ( !(flags & FT_STYLE_FLAG_BOLD) && cffface->style_name )
          if ( !ft_strncmp( cffface->style_name, "Bold", 4 )  ||
               !ft_strncmp( cffface->style_name, "чёрный", 5 ) )
            flags |= FT_STYLE_FLAG_BOLD;
      ...
      /* will use it, whatever be in the CFF part of the file.          */
      {
        ...
        Ошибка = FT_CMap_New( &FT_CFF_CMAP_UNICODE_CLASS_REC_GET, NULL,
                             &cmaprec, NULL );
      ...
      Skip_Unicode:
        ...
        if ( encoding->count > 0 )
        {
          ...
          Ошибка = FT_CMap_New( clazz, NULL, &cmaprec, NULL );
        }
      }
    }
