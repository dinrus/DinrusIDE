...
/******************************************************************************
        ...
        и/or крат в_ medium length контент. Brute-принудить approach with
...
/******************************************************************************
        ...
        крат совпадает и/or крат контент (employs brute-принудить strategy)
...
private struct ИщиПлод(T)
{
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Construct отыщи таблица. We принудить the alphabet в_ be ткст
...
debug (Поиск)
{
        ...
        проц main()
        {
                ...
                Стдвыв.фмтнс ("не_совпадают {}", elapsed.stop);

                elapsed.старт;
                for (auto i=5000; i--;)
                     Util.индексУ (x.ptr, '@', cast(бцел) x.length);
                Стдвыв.фмтнс ("индексУ {}", elapsed.stop);

                elapsed.старт;
                for (auto i=5000; i--;)
                     Util.местоположениеОбразца (x, "индексУ {}");
                Стдвыв.фмтнс ("образец {}", elapsed.stop);

                elapsed.старт;
                auto f = найди ("индексУ {}");
                for (auto i=5000; i--;)
                     f.вперёд(x);
                Стдвыв.фмтнс ("найди {}", elapsed.stop);

                elapsed.старт;
                auto s = ищи ("индексУ {}");
                for (auto i=5000; i--;)
                     s.вперёд(x);
                Стдвыв.фмтнс ("ищи {}", elapsed.stop);
        }
}
