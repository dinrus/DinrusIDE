...
[s9;%RU-RU This class simplifies the task of creating XML output 
text. It provides means of formatting XML tags and attributes 
and outputs String.&]
[s3; &]
[s0; &]
...
[s2;%RU-RU Возвращает the completed XML tag block with 
[/ empty] content. E.g. <mytag></mytag>.&]
[s3;%RU-RU &]
[s4; &]
...
[s2;%RU-RU Возвращает the completed XML tag block with 
[*@3 text] content. E.g. <mytag>mytext</mytag>. Note that the [*@3 text] 
here is not altered in the output in any way `- it can contain 
other subtags.&]
[s3;%RU-RU &]
[s4; &]
...
[s2; [%RU-RU Возвращает the completed XML tag block with 
][*@3 text][%RU-RU  content. In this case, text is converted from 
][*@3 charset] to UTF`-8 encoding and escaped with entities.&]
[s3;%RU-RU &]
[s4; &]
...
`=_CHARSET`_DEFAULT)&]
[s2; Возвращает the completed XML tag block with [*@3 text] 
content. In this case, text is converted from [*@3 charset] to 
UTF`-8 encoding and escaped with entities. Unlike [* Text], this 
variant adds [C xml:space`=`"preserved`"] attribute and escapes 
`'`\t`' and `'`\n`' with entities too.&]
[s3;%RU-RU &]
[s4; &]
...
[s2;%RU-RU These methods add XML tag attributes.&]
[s3;%RU-RU &]
[s0;%RU-RU &]
...
ag])&]
[s2;%RU-RU Устанавливаетthe tag name to [%-*@3 tag].&]
[s3; &]
[s0; &]
[s0; &]
[s0; &]
...
[s2; [%RU-RU Создаёт the header of XML file. If ][*@3 version]_ 
is NULL, it is not included, [*@3 standalone] can be either `"yes`" 
or `"no`" or NULL (standalone attribute is then not included).&]
[s3;%RU-RU &]
[s4; &]
[s5;:XmlDecl`(const char`*`): [_^String^ String]_[* XmlDecl]([@(0.0.255) const]_[@(0.0.255) c
har]_`*[*@3 text])&]
[s2;%RU-RU Создаёт the declaration element of XML.&]
[s3;%RU-RU &]
[s4; &]
[s5;:XmlDocType`(const char`*`): [_^String^ String]_[* XmlDocType]([@(0.0.255) const]_[@(0.0.255) c
har]_`*[*@3 text])&]
[s2;%RU-RU Создаёт the DOCTYPE XML declaration element.&]
[s3;%RU-RU &]
[s4; &]
