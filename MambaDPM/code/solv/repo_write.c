...
collect_needed_solvable(struct cbdata *cbdata, Solvable *s, Id *keymap)
{
  ...
  if (s->requires_ && keymap[SOLVABLE_REQUIRES])
    {
      *sp++ = keymap[SOLVABLE_REQUIRES];
      target->keys[keymap[SOLVABLE_REQUIRES]].size += incneedidarray(idarraydata + s->requires_, needid);
    }
...
collect_data_solvable(struct cbdata *cbdata, Solvable *s, Id *keymap)
{
  ...
  if (s->requires_ && keymap[SOLVABLE_REQUIRES])
    data_adddepids(xda, pool, needid, idarraydata + s->requires_, SOLVABLE_PREREQMARKER);
  if (s->recommends && keymap[SOLVABLE_RECOMMENDS])
