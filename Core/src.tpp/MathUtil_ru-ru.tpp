...
word]_[*@3 seed])&]
[s2;%RU-RU `"Засевает случайностью`" с одним 
значением [%-*@3 seed]. Этого достаточно, 
чтобы всегда получать одинаковую 
последовательность чисел (для одного 
и того же семени (seed)). Обычно используется 
при тестировании.&]
[s3;%RU-RU &]
[s4; &]
[s5;:Upp`:`:SeedRandom`(`): [@(0.0.255) void]_[* SeedRandom]()&]
[s2;%RU-RU `"Засевает случайностью`", пользуясь 
энтропией хостовой платформы. Отлично 
подходит для `"re`-randomize`" (`"повторной 
рандомизации`") Random, если уже было 
фиксировано значение семени.&]
[s3; &]
[s4; &]
[s5;:ZeroArray`(x`): [* ZeroArray]([*@3 x])&]
[s2;%RU-RU Заполняет массив Си [%-*@3 x] нулями.&]
[s3; &]
[s4; &]
...
[s5;:iscalefloor`(int`,int`,int`): [@(0.0.255) int]_[* iscalefloor]([@(0.0.255) int]_[*@3 x],
 [@(0.0.255) int]_[*@3 y], [@(0.0.255) int]_[*@3 z])&]
[s2;%RU-RU Возвращает [%-*@3 x] `* [%-*@3 y] / [%-*@3 z][%-* ,][%-  
округлённый (rounded) в сторону отрицательной 
бесконечности].&]
[s3;%RU-RU &]
[s4; &]
...
(truncated) [%- в сторону отрицательной бесконечнос
ти].&]
[s3;%RU-RU &]
[s4; &]
...
[@(0.0.255) int]_[*@3 y])&]
[s2; Перемещает [*@3 x] к ближайшему верхнемумножите
лю [*@3 y].&]
[s3;%RU-RU &]
[s4; &]
[s5;:fround`(double`): [@(0.0.255) int]_[* fround]([@(0.0.255) double]_[*@3 x])&]
[s2; Преобразует double [*@3 x ]в ближайший integer. 
Проверяет пределы целого.&]
[s3;%RU-RU &]
[s4; &]
[s5;:ffloor`(double`): [@(0.0.255) int]_[* ffloor]([@(0.0.255) double]_[*@3 x])&]
[s2; Преобразует double [*@3 x ]в integer, округляя 
...
точкой от [%-*@3 x]/[%-*@3 y].&]
[s2;%RU-RU &]
[s2;%RU-RU Остаток после операции деления 
`- это результат вычитания интегрального 
коэффициента, умноженный на знаменатель 
от числителя:&]
[s2;%RU-RU &]
