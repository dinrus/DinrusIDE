...
void   DinrusBuilder::AddFlags(Index<String>& cfg)
{
}

String DinrusBuilder::CompilerName() const
{
	if(!IsNull(compiler)) return compiler;
	return "drc";
}

String DinrusBuilder::CmdLine(const String& package, const Package& pkg)
{
...
void DinrusBuilder::BinaryToObject(String objfile, CParser& binscript, String basedir,
...
bool DinrusBuilder::BuildPackage(const String& package, Vector<String>& linkfile, Vector<String>& immfile,
...
bool DinrusBuilder::CreateLib(const String& product, const Vector<String>& obj,
...
bool DinrusBuilder::Link(const Vector<String>& linkfile, const String& linkoptions, bool createmap)
{
	if(!Wait())
		return false;
	PutLinking();
...
bool DinrusBuilder::Preprocess(const String& package, const String& file, const String& target, bool asmout)
{
	Package pkg;
	String packagePath = PackagePath(package);
	pkg.Load(packagePath);
