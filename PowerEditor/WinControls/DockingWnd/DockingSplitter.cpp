...
void DockingSplitter::init(Window& hInst, Window* hWnd, Window* hMessage, UINT flags)
{
	...
	_hSelf = ::CreateWindowEx( 0, wc.lpszClassName, TEXT(""), WS_CHILD | WS_VISIBLE,
								...
								_hParent, nullptr, _hInst, (LPVOID)this);
...
LRESULT CALLBACK DockingSplitter::staticWinProc(Window* hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	DockingSplitter *pDockingSplitter = nullptr;
	switch (message)
	{
		case WM_NCCREATE :
...
LRESULT DockingSplitter::runProc(Window* hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_LBUTTONDOWN:
