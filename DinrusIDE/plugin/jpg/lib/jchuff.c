...
typedef struct {
  INT32 put_buffer;		/* current bit-accumulation буфер */
...
typedef struct {
  ...
  savable_state saved;		/* Bit буфер & DC state at start of MCU */
  ...
  /* Режим flag: TRUE for optimization, FALSE for actual data output */
  ...
  char * bit_buffer;		/* буфер for correction bits (1 per char) */
...
typedef struct {
  JOCTET * next_output_byte;	/* => next byte to write in буфер */
  size_t free_in_buffer;	/* # of byte пробелы remaining in буфер */
  savable_state cur;		/* Current bit буфер & DC state */
  j_compress_ptr cinfo;		/* dump_buffer needs access to this */
} working_state;

/* MAX_CORR_BITS is the number of bits the AC refinement correction-bit
 * буфер can hold.  Larger sizes may slightly improve compression, but
 * 1000 is already well into the realm of overkill.
 * The minimum safe size is 64 bits.
 */

#define MAX_CORR_BITS  1000	/* макс # of correction bits I can буфер */
			 ...
			 c_derived_tbl ** pdtbl)
{
  ...
  /* These are code and size indexed by symbol значение */
...
/* Empty the output буфер; return TRUE if successful, FALSE if must suspend */
{
  struct jpeg_destination_mgr * dest = state->cinfo->dest;

  if (! (*dest->empty_output_buffer) (state->cinfo))
    return FALSE;
  /* After a successful буфер dump, must reset буфер pointers */
...
/* Empty the output буфер; we do not support suspension in this case. */
{
 ...
 * left-justified in this part.  по most 16 bits can be passed to emit_bits
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  /* new number of bits in буфер */
  put_bits = size + state->cur.put_bits;

  put_buffer <<= 24 - put_bits; /* align incoming bits */

  /* and merge with old буфер contents */
...
flush_bits_s (working_state * state)
{
  if (! emit_bits_s(state, 0x7F, 7)) /* fill any partial byte with ones */
    return FALSE;
  state->cur.put_buffer = 0;	     /* and reset bit-буфер to empty */
...
flush_bits_e (huff_entropy_ptr entropy)
{
  ...
  entropy->saved.put_buffer = 0; /* and reset bit-буфер to empty */
...
/*
 * Emit bits from a correction bit буфер.
 */
...
emit_eobrun (huff_entropy_ptr entropy)
{
  ...
  if (entropy->EOBRUN > 0) {	/* if there is any pending EOBRUN */
    ...
    while ((temp >>= 1))
      nbits++;
    /* safety check: shouldn't happen given limited correction-bit буфер */
    if (nbits > 14)
      ERREXIT(entropy->cinfo, JERR_HUFF_MISSING_CODE);
...
encode_mcu_DC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* Compute the DC значение after the required point transform by Al.
    ...
    temp = temp2;
    if (temp < 0) {
      temp = -temp;		/* temp is abs значение of input */
      /* For a negative input, want temp2 = bitwise complement of abs(input) */
      /* This code assumes we are on a two's complement machine */
      temp2--;
    }

    /* найди the number of bits needed for the magnitude of the coefficient */
    nbits = 0;
    while (temp) {
      nbits++;
      temp >>= 1;
    ...
    /* Emit that number of bits of the значение, if positive, */
...
encode_mcu_AC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (k = cinfo->Ss; k <= Se; k++) {
     ...
     * in C, we shift after obtaining the absolute значение; so the code is
     * interwoven with finding the abs значение (temp) and output bits (temp2).
     */
    if (temp < 0) {
      temp = -temp;		/* temp is abs значение of input */
...
encode_mcu_DC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Encode the MCU data blocks */
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    /* We simply emit the Al'th bit of the DC coefficient значение. */
...
encode_mcu_AC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (k = cinfo->Ss; k <= Se; k++) {
    ...
    /* We must apply the point transform by Al.  For AC coefficients this
     * is an integer division with rounding towards 0.  To do this portably
     * in C, we shift after obtaining the absolute значение.
     */
    if (temp < 0)
      temp = -temp;		/* temp is abs значение of input */
    temp >>= Al;		/* apply the point transform */
    absvalues[k] = temp;	/* save abs значение for main pass */
    if (temp == 1)
  ...
  BR_buffer = entropy->bit_buffer + entropy->BE; /* приставь bits to буфер */
  ...
  for (k = cinfo->Ss; k <= Se; k++) {
     ...
     */
    if (temp > 1) {
      /* The correction bit is the next bit of the absolute значение. */
  ...
  if (r > 0 || BR > 0) {	/* If there are trailing zeroes, */
     ...
     * 2. overflow of the correction bit буфер during the next MCU.
     */
...
/*
 * финиш up at the end of a Huffman-compressed scan.
 */
...
finish_pass_huff (j_compress_ptr cinfo)
{
  ...
  if (cinfo->progressive_mode) {
    ...
    /* грузи up working state ... flush_bits needs it */
    state.next_output_byte = cinfo->dest->next_output_byte;
    state.free_in_buffer = cinfo->dest->free_in_buffer;
    ASSIGN_STATE(state.cur, entropy->saved);
    state.cinfo = cinfo;

    /* слей out the last data */
    if (! flush_bits_s(&state))
...
#define MAX_CLEN 32		/* assumed maximum initial code length */
   ...
   * that no real symbol is given code-значение of all ones, because 256
  ...
  /* This элт does not correspond to a valid symbol frequency and is used
   * as starting index.
   */
  j = 256;
  ...
  for (i = 0;; i++) {
    ...
    if (i > 255)
      break;
    others[j] = i;		/* this symbol значение */
    j = i;			/* previous symbol значение */
  }
  ...
  while ((c1 = others[256]) >= 0) {
    v = freq[c1];
    i = c1;			/* first symbol значение */
    j = 256;			/* pseudo symbol значение for starting index */
    while ((c2 = others[c1]) >= 0) {
      if (freq[c2] > v) {
	v = freq[c2];
	i = c2;			/* this symbol значение */
	j = c1;			/* previous symbol значение */
      }
      c1 = c2;
    }
  ...
  for (;;) {
    /* найди the smallest nonzero frequency, set c1 = its symbol */
    ...
    /* найди the next smallest nonzero frequency, set c2 = its symbol */
    /* In case of ties, take the larger symbol number */
    c2 = -1;
    v = 1000000000L;
    for (i = 0; i <= 256; i++) {
...
/*
 * финиш up a statistics-gathering pass and create the new Huffman tables.
 */
...
finish_pass_gather (j_compress_ptr cinfo)
{
  ...
  if (cinfo->progressive_mode)
    /* слей out buffered data (all we care about is counting the EOB symbol) */
    emit_eobrun(entropy);
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  if (cinfo->progressive_mode) {
	...
	/* AC refinement needs a correction bit буфер */
