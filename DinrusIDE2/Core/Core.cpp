...
int ReadLNG(CParser& p) {
	...
	if(p.Id("LNGC_")) {
		...
		p.ThrowError("неверный языковой код");
	return 0;
}
		...
		if(belem[0].flags & (BinObjInfo::Block::FLG_ARRAY | BinObjInfo::Block::FLG_MASK)) {
			...
			for(int i = 0; i < blockref.GetCount(); i++)
				if(blockref[i]) {
					...
					if(data.IsVoid())
						throw Exc(Format("Ошибка при чтении файла '%s'", b.file));
					if(data.GetLength() != b.length)
						throw Exc(Format("длина файла '%s' изменилась (%d -> %d) в ходе создания объекта",
...
//					fo << AsCString(data, 70, "\t", ASCSTRING_OCTALHI | ASCSTRING_SMART) << ";\n\n";
			...
			if(data.IsVoid())
				throw Exc(Format("Ошибка при чтении файла '%s'", b.file));
			if(data.GetLength() != b.length)
				throw Exc(Format("длина файла '%s' изменена (%d -> %d) в ходе создания объекта",
					b.file, b.length, data.GetLength()));
			b.Compress(data);
	...
	if(ff) {
		...
		if(brc && ext == ".brc") {
			try {
				...
				if(brcdata.IsVoid())
					throw Exc(Format("ошибка при чтении файла '%s'", s));
