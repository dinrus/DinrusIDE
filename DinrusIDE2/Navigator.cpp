...
void Navigator::SyncCursor()
{
	...
	if(!navigating) {
		AnnotationItem q = DinrusIDE2->editor.FindCurrentAnnotation();
		navigating = true;
...
void Navigator::Navigate()
{
	...
	if(DinrusIDE2 && ii >= 0 && ii < litem.GetCount()) {
		...
		if(m.kind == KIND_NEST) {
			...
			if(q >= 0) {
				search.Clear();
				DinrusIDE2->AddHistory();
				DinrusIDE2->EditFile(h.Mid(q + 1));
				DinrusIDE2->AddHistory();
			}
			else {
				String id = h;
		...
		if(m.kind == KIND_SRCFILE) {
			DinrusIDE2->AddHistory();
			DinrusIDE2->EditFile(m.id);
			DinrusIDE2->AddHistory();
		}
		else
		if(m.kind == KIND_LINE) {
			DinrusIDE2->GotoPos(Null, m.pos);
		...
		if(GetAlt())
			DinrusIDE2->Usage(m.id, m.name, m.pos);
		else
		if(IsNull(search)) // current file - do not cycle
			DinrusIDE2->GotoPos(Null, m.pos);
		else
			DinrusIDE2->Cycle(m, GetCurrentLine(), true);
	}
	navigating = false;
	SyncCursor();
}
...
void Navigator::Search()
{
	...
	if(IsNull(s) && !sorting) {
		bool sch = GetFileExt(DinrusIDE2->editfile) == ".sch";
		Ide *DinrusIDE2 = DinrusIde();
		if(DinrusIDE2)
			for(const AnnotationItem& m : DinrusIDE2->editor.annotations) {
				NavItem& n = nitem.Add();
				(AnnotationItem&)n = m;
				n.path = DinrusIDE2->editfile;
				nests.FindAdd(n.nest = Nest(m, DinrusIDE2->editfile));
			}
		SortIndex(nests);
	}
	else {
