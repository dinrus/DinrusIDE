...
void Ide::SetupFormat() {
	...
	dlg.Add(assist, "Ассист");
	dlg.Add(ide, "ИСР");
	dlg.Add(ast, "Форматирование кода");
	dlg.Add(web_search, "Веб-поиск");
	dlg.WhenClose = dlg.Acceptor(IDEXIT);

#ifdef PLATFORM_WIN32
	ide.xterm.SetLabel("bash");
	ide.xterm << [&] { ide.console <<= "bash.exe"; };
	...
	hlt.hlstyle.AddColumn("Жирный").Ctrls<Option>();
	hlt.hlstyle.AddColumn("Курсив").Ctrls<Option>();
	hlt.hlstyle.AddColumn("Подчёркнутый").Ctrls<Option>();
	...
	for(;;) {
		...
		rtvr.Retrieve();
		/*СДЕЛАТЬ: Количество ядер, используемых ИСР, зависит от фактора
		объём оперативки/количество ядер, если он около 2 ГБ (а лучше, 3!) на каждое ядро,
		тогда нужно оставить на работу системы хотя бы 1-2 ядра.
		Эту формулу нужно обязательно реализовать и устанавливать дефолт с учётом этого,
		иначе при "тяжёлой" задаче, когда, например, компилируется код, использующий более
		1 ГБ библиотек (для сравнения, все библиотеки LLVM, которые, например, обязательно грузятся
		в память при компиляции компилятора Фортрана), памяти на систему не остаётся, что приводит к
		зависанию и вынужденной перезагрузке.
		hydra1_threads - вот о чём речь...
		 */
