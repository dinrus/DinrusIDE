...
#define FT_COMPONENT  trace_gxvmodule
                  ...
                  FT_ULong*           table_len )
  {
    ...
    if ( Ошибка == GXV_Err_Table_Missing )
      return GXV_Err_Ok;
    if ( Ошибка )
      goto Exit;
...
#define GXV_TABLE_LOAD( _sfnt )                                     \
          ...
          {                                                         \
            Ошибка = gxv_load_table( face, TTAG_ ## _sfnt,           \
                                    &_sfnt, &len_ ## _sfnt );       \
            if ( Ошибка )                                            \
              goto Exit;                                            \
          }
...
#define GXV_TABLE_SET( _sfnt )                                        \
                         ...
                         FT_Bytes*  ckern_table )
  {
    ...
    /* without volatile on `Ошибка' GCC 4.1.1. emits:                         */
    /*  warning: variable 'Ошибка' might be clobbered by 'longjmp' or 'vfork' */
    /* this warning seems spurious but ---                                   */
    FT_Error volatile         Ошибка = GXV_Err_Ok;
    FT_ValidatorRec volatile  valid;


    *ckern_table = NULL;

    Ошибка = gxv_load_table( face, TTAG_kern, &ckern, &len_ckern );
    if ( Ошибка )
      goto Exit;
