...
void SpecialFolders::ButInstalledSoftware_Push() {
    ...
    SoftwareInstalled.AddColumn("Имя", 60);
    SoftwareInstalled.AddColumn("Версия", 15);
    SoftwareInstalled.AddColumn("Поставщик", 30);
    SoftwareInstalled.AddColumn("Дата Установки", 15);
...
void SpecialFolders::ButClose_Push(){ Close();}

void SystemInfo::ButClose_Push(){Close();}

void ProcessList::ButClose_Push(){Close();}

void WindowsList_::ButClose_Push(){Close();}
...
SysInfoGui::SysInfoGui()
{
    ...
    systemInfo.Fill();
    this.Add(systemInfo.SizePos(), "Системная Инфо");
    windowsList.Fill();
    this.Add(windowsList.SizePos(), "Список Окон");
    processList.Fill();
    this.Add(processList.SizePos(), "Список Процессов");
    specialFolders.Fill();
    this.Add(specialFolders.SizePos(), "Спецпапки/Исполнимые/Установленное ПО");

 #ifdef AS_MAIN
    screenGrab.Fill();
    this.Add(screenGrab.SizePos(), "Захват Экрана");
    mouseKeyboard.Fill();
    this.Add(mouseKeyboard.SizePos(), "Мышь & Клавиатура");
...
#ifdef AS_MAIN
void SysInfoGui::TimerFun() {
    mouseKeyboard.OnTimer();
}

void SysInfoGui::MainMenu(Bar& bar) {
    menu.Add("Меню", THISBACK(FileMenu));
}

void SysInfoGui::FileMenu(Bar& bar)
{
