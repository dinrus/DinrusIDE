...
struct Твистер
{
        ...
        private enum : бцел                     // Period параметры
                ...
                UPPER_MASK = 0x80000000,        //  most significant ш-r биты 
...
debug (Твистер)
{
        ...
        проц main()
        {
                ...
                for (цел i=счёт; --i;)
                     v1 = dbl.дво;
                Стдвыв.фмтнс ("{} дво, {}/s, {:f10}", счёт, счёт/ш.stop, v1);

                ш.старт;
                for (цел i=счёт; --i;)
                     v1 = dbl.дробьДоп;
                Стдвыв.фмтнс ("{} дробьДоп, {}/s, {:f10}", счёт, счёт/ш.stop, v1);
                ...
                for (цел i=счёт; --i;)
                    ...
                    if (знач <= 0.0 || знач >= 1.0)
                       {
                       Стдвыв.фмтнс ("дво {:f10}", знач);
                       break;
                       }
                    знач = dbl.дробьДоп;
                    if (знач <= 0.0 || знач >= 1.0)
                       {
                       Стдвыв.фмтнс ("дробьДоп {:f10}", знач);
                       break;
                       }
                    }
        }
