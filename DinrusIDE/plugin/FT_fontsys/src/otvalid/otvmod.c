...
#define FT_COMPONENT  trace_otvmodule
                  ...
                  FT_ULong*           table_len )
  {
    ...
    Ошибка = FT_Load_Sfnt_Table( face, tag, 0, NULL, table_len );
    if ( Ошибка == OTV_Err_Table_Missing )
      return OTV_Err_Ok;
    if ( Ошибка )
      goto Exit;
    ...
    Ошибка = FT_Load_Sfnt_Table( face, tag, 0, *table, table_len );
                ...
                FT_Bytes          *ot_jstf )
  {
    ...
    {
      Ошибка = otv_load_table( face, TTAG_BASE, &base, &len_base );
      if ( Ошибка )
        goto Exit;
    }
    ...
    {
      Ошибка = otv_load_table( face, TTAG_GDEF, &gdef, &len_gdef );
      if ( Ошибка )
        goto Exit;
    }
    ...
    {
      Ошибка = otv_load_table( face, TTAG_GPOS, &gpos, &len_gpos );
      if ( Ошибка )
        goto Exit;
    }
    ...
    {
      Ошибка = otv_load_table( face, TTAG_GSUB, &gsub, &len_gsub );
      if ( Ошибка )
        goto Exit;
    }
    ...
    {
      Ошибка = otv_load_table( face, TTAG_JSTF, &jstf, &len_jstf );
      if ( Ошибка )
        goto Exit;
    }
    ...
    {
      Ошибка = otv_load_table( face, TTAG_MATH, &math, &len_math );
      if ( Ошибка )
        goto Exit;
    }
