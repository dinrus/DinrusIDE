...
void SolidFiller::старт(int minx, int maxx)
{
	t += minx;
}

inline КЗСА InvertRGBA(const КЗСА& c)
{
	КЗСА a;
	a.r = ~c.r;
	a.g = ~c.g;
...
void SubpixelFiller::старт(int minx, int maxx)
{
	int x = minx / 3;
	if(x > 0) {
		begin = sbuffer;
...
void SubpixelFiller::пиши(int len)
{
	КЗСА *e = min(t + len, end);
	int16 *q = begin;
	while(t < e) {
		КЗСА c = ss ? Mul8(*s++, alpha) : invert ? InvertRGBA(*t) : color;
		int a;
		if(t->a != 255)
...
void SubpixelFiller::стоп()
{
	v[3] = v[4] = v[5] = 0;
	пиши(int(v + 3 - begin) / 3);
}

void SpanFiller::старт(int minx, int maxx)
{
	t += minx;
	ss->дай(буфер, minx, y, maxx - minx + 1);
	s = буфер;
}
...
void ClipFiller::иниц(int _cx)
{
	cx = _cx;
	буфер.размести(2 * cx);
}

void ClipFiller::очисть()
{
	t = ~буфер;
	x = 0;
	empty = true;
...
void ClipFiller::старт(int xmin, int xmax)
{
	Render(0, xmin);
}
...
void ClipFiller::финиш(ClippingLine& cl)
{
	if(empty)
		return;
	while(x < cx) {
	...
	ПРОВЕРЬ(t - ~буфер <= 2 * cx);
	if(full)
		cl.SetFull();
	else
		cl.уст(~буфер, int(t - ~буфер));
}
...
void MaskFillerFilter::старт(int minx, int maxx)
{
	t->старт(minx, maxx);
...
void NoAAFillerFilter::старт(int minx, int maxx)
{
	t->старт(minx, maxx);
}
