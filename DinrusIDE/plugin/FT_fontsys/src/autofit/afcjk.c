                              ...
                              FT_ULong       charcode )
  {
...

    {
      ...
      for ( dim = 0; dim < AF_DIMENSION_MAX; dim++ )
      {
        ...
        Ошибка = af_latin_hints_compute_segments( hints, (AF_Dimension)dim );
        if ( Ошибка )
          goto Exit;
...
#define AF_CJK_MAX_TEST_CHARACTERS  32
  ...
  /* Basically the Latin version with тип AF_CJKMetrics for metrics. */
  FT_LOCAL_DEF( void )
                                ...
                                AF_Dimension   dim )
  {
    ...
    Ошибка = af_cjk_hints_compute_segments( hints, dim );
    if ( !Ошибка )
    {
      af_cjk_hints_link_segments( hints, dim );

      Ошибка = af_cjk_hints_compute_edges( hints, dim );
    }
    return Ошибка;
  }
    ...
    for ( ; edge < edge_limit; edge++ )
    {
      ...
      for ( bb = 0; bb < cjk->blue_count; bb++ )
      {
        ...
        if ( is_top_right_blue ^ is_major_dir )
        {
          ...
          /* сравни the edge to the closest blue zone тип */
...
#define AF_LIGHT_MODE_MAX_DELTA_ABS  14
                      ...
                      AF_CJKMetrics  metrics )
  {
    ...
    {
      Ошибка = af_cjk_hints_detect_features( hints, AF_DIMENSION_HORZ );
      if ( Ошибка )
        goto Exit;
    ...
    {
      Ошибка = af_cjk_hints_detect_features( hints, AF_DIMENSION_VERT );
      if ( Ошибка )
        goto Exit;
