...
#define FT_COMPONENT  trace_synth
  ...
  FT_GlyphSlot_Embolden( FT_GlyphSlot  slot )
  {
    ...
    else /* slot->формат == FT_GLYPH_FORMAT_BITMAP */
    {
      ...
      Ошибка = FT_GlyphSlot_Own_Bitmap( slot );
      if ( Ошибка )
        return;

      Ошибка = FT_Bitmap_Embolden( library, &slot->bitmap, xstr, ystr );
      if ( Ошибка )
        return;
    }
