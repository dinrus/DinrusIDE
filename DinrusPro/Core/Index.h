...
public:
	...
	проц        добавь(T&& k)                  { AddS(pick(k), Smear(k)); }
	...
	цел         найдиДобавь(T&& k)              { return найдиДобавь(pick(k), []{}); }

	цел         помести(const T& k)             { return Put0(k, Smear(k)); }
	цел         помести(T&& k)                  { return Put0(pick(k), Smear(k)); }
	цел         найдиПомести(const T& k)         { return FindPut0(k); }
	цел         найдиПомести(T&& k)              { return FindPut0(pick(k)); }
	...
	проц        уст(цел i, T&& k)            { уст0(i, pick(k)); }
