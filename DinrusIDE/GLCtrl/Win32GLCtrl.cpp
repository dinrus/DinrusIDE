...
#pragma comment( lib, "opengl32.lib" )	// ищи For OpenGL32.lib While Linking
#pragma comment( lib, "glu32.lib" )		// ищи For GLu32.lib While Linking
...
void MakeWGLContext(int depthBits, int stencilBits, int samples)
{
	ONCELOCK {
		for(int pass = 0; pass < 2; pass++) {
			HWND hWND = CreateWindow("РНЦП-CLASS-A", "Fake Window",
...
void ГЛКтрл::CreateContext()
{
	MakeWGLContext(depthSize, stencilSize, numberOfSamples);
}

void ГЛКтрл::GLPane::State(int reason)
{
	DHCtrl::State(reason);
	
	if(reason == OPEN) {
		HWND hwnd = дайОУК();
		ГЛКтрл::CreateContext();
		HDC hDC = GetDC(hwnd);
		if(!SetPixelFormat(hDC, s_pixelFormatID, &s_pfd))
			return;
		ReleaseDC(hwnd, hDC);
...
void ГЛКтрл::GLPane::ExecuteGL(HDC hDC, Событие<> paint, bool swap_buffers)
{
	wglMakeCurrent(hDC, s_openGLContext);
	paint();
	if(swap_buffers)
...
void ГЛКтрл::GLPane::ExecuteGL(Событие<> paint, bool swap_buffers)
{
	HWND hwnd = дайОУК();
	ГЛКтрл::CreateContext();
	HDC hDC = GetDC(hwnd);
...
LRESULT ГЛКтрл::GLPane::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
