...
Pointf3 длина(Pointf3 p, double length)
{
	double l = max(длина(p), fpabsmax(p));
	if(l)
		return p * length / l;
	return p;
}

Ткст какТкст(const Pointf3& p)
{
	Ткст out;
	if(пусто_ли(p))
		out = "Pointf3(Null)";
...
Pointf3 ортогональ(Pointf3 v, Pointf3 against)
{
...
{
	double da = sqrt(вКвадрате(a) * вКвадрате(b));
	if(da == 0)
		return 0;
	da = (a ^ b) / da;
	if(da <= -1)
...
{
	double nt = max(длина(p.normal), fpabsmax(p.normal));
	if(nt)
		return Plane3(p.delta / nt, p.normal / nt);
	return p;
}

double пересек(Plane3 p, Pointf3 la, Pointf3 lb)
{
...
void Matrixf3::сериализуй(Поток& stream)
{
	stream % x % y % z % a;
}
...
Ткст Matrixf3::вТкст() const
{
	return Ткст().конкат() << "[x = " << x << ", y = " << y << ", z = " << z << ", a = " << a << "]";
}
...
Matrixf3 Matrixf3RotateX(double angle, Pointf3 fix)
{
	...
	return Matrixf3(1, 0, 0, 0, c, s, 0, -s, c).фиксируй(fix);
}

Matrixf3 Matrixf3RotateY(double angle, Pointf3 fix)
{
	double c = cos(angle), s = sin(angle);
	return Matrixf3(c, 0, s, 0, 1, 0, -s, 0, c).фиксируй(fix);
}

Matrixf3 Matrixf3RotateZ(double angle, Pointf3 fix)
{
	double c = cos(angle), s = sin(angle);
	return Matrixf3(c, s, 0, -s, c, 0, 0, 0, 1).фиксируй(fix);
}
...
void Camera::сериализуй(Поток& stream)
{
...
void Camera::Update()
{
	...
	Pointf3 up = ортогональ(upwards, direction);
	if(вКвадрате(up) <= 1e-10)
		up = ортогональ(FarthestAxis(direction), direction);
...
void Camera::SetPolar(Pointf3 dz, Pointf3 dx, double d, double a, double b, double r)
{
	...
	Pointf3 su = ортогональ(dy, dir);
	if(длина(su) <= 1e-10)
		su = ортогональ(FarthestAxis(dir), dir);
	su = UnitV(su);
...
{
	distance = расстояние(location, target);
	Pointf3 dy = dz % dx;
...
bool Camera::TransformClip(Pointf3 a, Pointf3 b, ТочкаПЗ& outa, ТочкаПЗ& outb) const
{
	...
	outb.x = b1.x / b1.z;
	outb.y = b1.y / b1.z;
	return ClipLine(outa, outb, ПрямПЗ(-view_px, -view_py, +view_px, +view_py));
}

Pointf3 Camera::InverseXY(ТочкаПЗ point, double z) const
{
