...
typedef struct {
	...
	lzma_check	check;			/* тип of the integrity check */
...
LZMAStrerror(lzma_ret ret)
{
	switch (ret) {
		...
		case LZMA_GET_CHECK:
		    return "integrity check тип is now available";
		case LZMA_MEM_ERROR:
		    return "cannot allocate memory";
		case LZMA_MEMLIMIT_ERROR:
		    return "memory usage limit was reached";
		case LZMA_FORMAT_ERROR:
		    return "file формат not recognized";
		...
		case LZMA_PROG_ERROR:
		    return "programming Ошибка";
		default:
		    return "unidentified liblzma Ошибка";
	}
}
...
LZMADecode(TIFF* tif, uint8* op, tmsize_t occ, uint16 s)
{
	...
	do {
		/*
		 * сохрани the current stream state to properly recover from the
...
LZMAPreEncode(TIFF* tif, uint16 s)
{
	...
	if (ret != LZMA_OK) {
		TIFFErrorExt(tif->tif_clientdata, module,
			"Ошибка in lzma_stream_encoder(): %s", LZMAStrerror(ret));
		return 0;
	}
	return 1;
}
...
LZMAEncode(TIFF* tif, uint8* bp, tmsize_t cc, uint16 s)
{
	...
	do {
		...
		if (ret != LZMA_OK) {
			TIFFErrorExt(tif->tif_clientdata, module,
				"Encoding Ошибка at scanline %lu, %s",
				(unsigned long) tif->tif_row, LZMAStrerror(ret));
			return 0;
		}
		if (sp->stream.avail_out == 0) {
...
LZMAPostEncode(TIFF* tif)
{
	...
	do {
		...
		case LZMA_OK:
			...
			break;
		default:
			TIFFErrorExt(tif->tif_clientdata, module, "Liblzma Ошибка: %s",
				     LZMAStrerror(ret));
			return 0;
		}
...
TIFFInitLZMA(TIFF* tif, int scheme)
{
	...
	sp->opt_delta.тип = LZMA_DELTA_TYPE_BYTE;
	/*
