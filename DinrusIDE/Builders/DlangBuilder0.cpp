...
//	}
	...
	if(!release)
		LDC2 << " --debug --gc -D_DEBUG " << debug_options;
	else
	...
	for(i = 0; i < sfile.GetCount(); i++) {
		...
		if(HdependFileTime(fn) > GetFileTime(objfile)) {
			...
			if(rc) {
				...
				if(LDC2.Find(" --m32 ") >= 0)
					exec << " --mtarget=pe-i386 ";
				exec << " -of" << GetPathQ(objfile) << Includes(" -I", package, pkg)
                           ...
                           const String& link_options)
{
	...
	if(is_shared) {
		lib = CompilerName();
		lib << " -shared --platformlib=\"libc++.a,libkernel32.a,libuser32.a\" -fPIC -fuse-cxa-atexit";
		if(HasFlag("GCC32"))
			lib << " --m32";
		Point p = ExtractVersion();
		if(!IsNull(p.x) && HasFlag("WIN32")) {
			lib << " --L=--major-image-version --L=" << p.x;
			if(!IsNull(p.y))
				lib << " --L=--minor-image-version --L=" << p.y;
		}
		lib << ' ' << link_options;
		lib << " -of";
	}
