...
void VerticalFileSwitcherListView::init(Window& hInst, Window* parent, HIMAGELIST hImaLst)
{
...
LRESULT VerticalFileSwitcherListView::runProc(Window* hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
...
void VerticalFileSwitcherListView::initList()
{
	...
	//add columns
	String nameStr = pNativeSpeaker->getAttrNameStr(TEXT("Name"), FS_ROOTNODE, FS_CLMNNAME);
	insertColumn(nameStr.Begin(), nameWidth, ++colIndex);
	if (isExtColumn)
	{
		String extStr = pNativeSpeaker->getAttrNameStr(TEXT("Ext."), FS_ROOTNODE, FS_CLMNEXT);
		insertColumn(extStr.Begin(), nppParams._dpiManager.scaleX(nppParams.getNppGUI()._fileSwitcherExtWidth), ++colIndex); //2nd column
	}
	if (isPathColumn)
	{
		String pathStr = pNativeSpeaker->getAttrNameStr(TEXT("Path"), FS_ROOTNODE, FS_CLMNPATH);
		insertColumn(pathStr.Begin(), nppParams._dpiManager.scaleX(nppParams.getNppGUI()._fileSwitcherPathWidth), ++colIndex); //2nd column if .ext is off
	}

	TaskListInfo taskListInfo;
	static Window* nppHwnd = ::GetParent(_hParent);
	...
	for (size_t i = 0, len = taskListInfo._tlfsLst.size(); i < len ; ++i)
	{
		...
		char fn[MAX_PATH];
		wcscpy_s(fn, ::PathFindFileName(fileNameStatus._fn.Begin()));
		...
		if (isExtColumn)
		{
			ListView_SetItemText(_hSelf, i, ++colIndex, ::PathFindExtension(fileNameStatus._fn.Begin()));
		}
		if (isPathColumn)
		{
			char dir[MAX_PATH], drive[MAX_PATH];
			_wsplitpath_s(fileNameStatus._fn.Begin(), drive, MAX_PATH, dir, MAX_PATH, nullptr, 0, nullptr, 0);
			wcscat_s(drive, dir);
...
void VerticalFileSwitcherListView::setItemIconStatus(BufferID bufferID)
{
	...
	for (int i = 0 ; i < nbItem ; ++i)
	{
		...
		if (tlfs->_bufID == bufferID)
		{
			...
			{
				char dir[MAX_PATH], drive[MAX_PATH];
				_wsplitpath_s(buf->getFullPathName(), drive, MAX_PATH, dir, MAX_PATH, nullptr, 0, nullptr, 0);
				wcscat_s(drive, dir);
				ListView_SetItemText(_hSelf, i, ++colIndex, drive);
			}
		}
...
String VerticalFileSwitcherListView::getFullFilePath(size_t i) const
{
...
void VerticalFileSwitcherListView::removeAll()
{
	...
	Window* colHeader = reinterpret_cast<Window*>(SendMessage(_hSelf, LVM_GETHEADER, 0, 0));
