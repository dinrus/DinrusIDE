...
int Hdepend::File(const String& f) {
	...
	if(!ff || ff.GetLastWriteTime() == info.time) {
		...
		if(info.time > GetSysTime() + 10)
			PutConsole(String().Cat() << "ПРЕДУПРЕЖДЕНИЕ: " << path << " имеет неверное (будущее) время " << info.time);
		ScanFile(path, ii);
	}
	return ii;
}
	...
	if(d >= 0) {
		...
		for(int i = 0; i < dep.GetCount(); i++) {
			...
			if(ff) {
				...
				if(tm > GetSysTime() + 10)
					PutConsole(String().Cat() << "ПРЕДУПРЕЖДЕНИЕ: " << dep[i] << " настроено неверное (будущее) время " << tm);
				if(tm > h)
					h = tm;
			}
		}
