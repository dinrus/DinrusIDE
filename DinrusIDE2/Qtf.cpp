...
QtfDlg::QtfDlg()
{
	...
	String path = AppendFileName(AppendFileName(PackageDirectory("RichText"), "srcdoc.tpp"), "QTF_ru-ru.tpp");
	if(FileExists(path)) {
...
void Ide::Qtf()
{
	...
	if(qtfsel && sel) {
		...
		dlg.qtfText.copy.SetLabel("Применить и закрыть");
		dlg.Run();
		editor.Remove(l, h - l);
		editor.Insert(l, (~dlg.qtfText.text).ToString());
	}
	else {
		dlg.qtfText.copy.SetLabel("Копировать и закрыть");
		dlg.Run();
	}
