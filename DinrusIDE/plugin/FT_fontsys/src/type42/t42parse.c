...
#define T1_Load_Field_Table( p, f, o, m, pf )                         \
                      ...
                      T42_Loader  loader )
  {
    ...
    if ( cur >= limit )
    {
      ...
      parser->root.Ошибка = T42_Err_Invalid_File_Format;
      return;
    }
                   ...
                   T42_Loader  loader )
  {
    ...
    /* if control reaches this point, the формат was not valid */
    Ошибка = T42_Err_Invalid_File_Format;
                         ...
                         T42_Loader  loader )
  {
    ...
    Ошибка = psaux->ps_table_funcs->init( code_table,
                                         loader->num_glyphs,
                                         memory );
    if ( Ошибка )
      goto Fail;

    Ошибка = psaux->ps_table_funcs->init( name_table,
                                         loader->num_glyphs,
                                         memory );
    if ( Ошибка )
      goto Fail;
    ...
    for (;;)
    {
      ...
      if ( *cur == '/' )
      {
        ...
        Ошибка = T1_Add_Table( code_table, n, cur, len + 1 );
        if ( Ошибка )
          goto Fail;
    ...
    while ( parser->root.cursor < limit )
    {
           ...
           ft_strncmp( (char*)cur, "FontDirectory", 13 ) == 0 )
      {
        ...
        if ( cur < limit )
        {
          ...
          if ( token.тип == T1_TOKEN_TYPE_ARRAY )
      ...
      else if ( *cur == '/' && cur + 2 < limit )
      {
        ...
        if ( len > 0 && len < 22 && parser->root.cursor < limit )
        {
          ...
          for ( i = 0; i < n_keywords; i++ )
          {
            ...
            FT_Byte   *имя   = (FT_Byte*)keyword->ident;
                 ...
                 ft_memcmp( cur, имя, len ) == 0                   )
            {
              ...
              parser->root.Ошибка = t42_load_keyword( face,
                                                     loader,
                                                     keyword );
              if ( parser->root.Ошибка )
