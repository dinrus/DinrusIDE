...
void Gdb::DebugBar(Bar& bar)
{
	...
	bar.Add("Остановить отладку", DbgImg::StopDebug(), THISBACK(Stop))
	   .Key(K_SHIFT_F5);
	bar.Separator();
	bool b = !IdeIsDebugLock();
	...
	bar.Add(b, "Копировать трассировку всех потоков", THISBACK(CopyStackAll));
...
{
	const auto phrase = String().Cat() << "(процесс " << pid << ") завершился нормально";
...
void Gdb::LoadFrames()
{
	...
	for(;;) {
		...
		if(n++ >= max_stack_trace_size) {
			frame.Add(Null, Sprintf("<загрузить ещё> (%d загружено)", frame.GetCount()));
			break;
		}
		frame.Add(i++, s);
	}
...
bool Gdb::Create(Host& host, const String& exefile, const String& cmdline, bool console)
{
	...
	if(!host.StartProcess(dbg, gdb_command)) {
		...
		ErrorOK("Ошибка при вызове gdb! Детали ищите в DinrusIDE2 логах.");
		return false;
	}
	...
	: gdb_utils(GdbUtilsFactory().Create())
{
	...
	CtrlLayoutOKCancel(quickwatch, "Вотч");
