...
#define FT_COMPONENT  trace_otvmath
                              ...
                              OTV_Validator  valid )
  {
    ...
    /* Italic correction and верх Accent Attachment have the same формат */
                              ...
                              OTV_Validator  valid )
  {
    ...
    p += 2;                           /* пропусти the Italics Correction значение */
    OTV_OPTIONAL_OFFSET( DeviceTableOffset );
    ...
    for ( i = 0; i < pcnt; ++i )
    {
      ...
      if ( gid >= valid->glyph_count )
        FT_INVALID_GLYPH_ID;
      p += 2*4;             /* skip the старт, стоп, Full, and Flags fields */
    }
                             ...
                             OTV_Validator  valid )
  {
    ...
    p += 2;                       /* пропусти the MinConnectorOverlap constant */
    ...
    for ( i = 0; i < vcnt; ++i )
    {
      OTV_OPTIONAL_OFFSET( смещение );
      OTV_SIZE_CHECK( смещение );
      otv_MathGlyphConstruction_validate( table + смещение, valid );
    }
