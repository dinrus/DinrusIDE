...
#define IMAGEFILE <DinrusIDE2/Browser/Topic.iml>
...
TopicEditor::TopicEditor()
{
	...
	topics_search.NullText(String(t_("Поиск")) + " " + "(Ctrl+Alt+F)");
...
#endif
	;
	bar.Add(topics_list.IsCursor(), "Переместить тематику..", THISBACK(MoveTopic));
	bar.Add(topics_list.IsCursor(), "Удалить тематику", THISBACK(RemoveTopic))
	   .Key(K_ALT_DELETE);
	bar.Separator();
	bar.Add(topics_list.IsCursor() && GetFileTitle(topicpath) != "$.tpp",
	        "Сохранить как шаблон..", THISBACK(SaveAsTemplate));
	bar.Add(topics_list.IsCursor(), "Применить шаблонный stylesheet..", THISBACK(ApplyStylesheet));
...
void TopicEditor::FileBar(Bar& bar)
{
	...
	bar.Add("Печать", CtrlImg::print(), THISBACK(Print))
	   .Key(K_CTRL_P);
	bar.Add("Экспорт в PDF..", THISBACK(ExportPdf));
	bar.Add("Экспорт группы  PDF..", THISBACK(ExportGroupPdf));
	bar.Add("Экспорт в HTML..", THISBACK(ExportHTML));
	bar.Add("Экспорт группы в HTML..", THISBACK(ExportGroupHTML));
}
