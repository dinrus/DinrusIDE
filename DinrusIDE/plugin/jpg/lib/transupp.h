 ...
 * hard, there is an asymmetry in the JPEG формат specification for images
 ...
 * flip transforms, lossless crop is restricted by the current JPEG формат: the
...
typedef struct {
   ...
   */
  JDIMENSION crop_width;	/* устШирину of selected region */
  JCROP_CODE crop_width_set;	/* (force disables adjustment) */
  JDIMENSION crop_height;	/* устВысоту of selected region */
  ...
  /* сбрось parameters: set by caller for drop request */
 ...
 * old source code that uses the original имя...
 */
...
/* копируй markers saved in the given source object to the destination object */
