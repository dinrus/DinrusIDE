...
void Ide::MacroSetSelection(EscEscape& e)
{
	...
	if(b < 0 || b > editor.GetLength() || c < 0 || c > editor.GetLength() || b + c > editor.GetLength())
		e.ThrowError(Format("Неверное выделение: начало = %d, счёт = %d (длина текста = %d)",
...
void Ide::MacroGet(EscEscape& e)
{
	if(e.GetCount() < 1 || e.GetCount() > 2)
		e.ThrowError("неверное число аргументов при вызове Get (1 или 2 ожидалось)");
	int pos = e.Int(0);
	int count = e.GetCount() > 1 ? e.Int(1) : 1;
	if(pos < 0 || pos > editor.GetLength() || count <= 0 || pos + count > editor.GetLength())
		e.ThrowError(Format("ошибка в Get(%d, %d), длина текста = %d", pos, count, editor.GetLength()));
...
void Ide::MacroInput(EscEscape& e)
{
	...
	cancel.SetLabel("Отмена") <<= dialog.Rejector(IDCANCEL);
	...
	dialog.Title("Бокс ввода макроса");
	if(dialog.Run() != IDOK) {
		e = EscValue();
		return;
	}
...
void Ide::MacroPackageDir(EscEscape& e)
{
	...
	if(!FileExists(pp))
		e.ThrowError("PackageDir: Пакет не найден.");
	e = GetFileFolder(pp);
}
...
void Ide::MacroPackageFiles(EscEscape& e)
{
	...
	if(!FileExists(pp))
		e.ThrowError("PackageFiles: Пакет не найден.");
	pkg.Load(pp);
	EscValue ret;
