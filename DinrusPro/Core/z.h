...
class Zlib {
	...
	проц          PutOut(const ук укз, цел size);
...
public:
	...
	проц Compress();
	проц Decompress();
	проц помести(const ук укз, цел size);
...
дол  копируйПоток(Поток& dest, Поток& ист, дол count, Врата<дол, дол> progress, цел chunk_size = 65536);
...
Ткст ZCompress(const ук данные, дол len, Врата<дол, дол> progress = Null);
...
Ткст ZDecompress(const ук данные, дол len, Врата<дол, дол> progress = Null);
...
Ткст GZCompress(const ук данные, цел len, Врата<дол, дол> progress = Null);
...
Ткст GZDecompress(const ук данные, цел len, Врата<дол, дол> progress = Null);
...
inline Ткст ZCompress(const ук данные, цел len, Лаз2<цел, цел> progress)   { return ZCompress(данные, len, AsGate64(progress)); }
...
inline Ткст ZDecompress(const ук данные, цел len, Лаз2<цел, цел> progress) { return ZDecompress(данные, len, AsGate64(progress)); }
...
inline Ткст GZCompress(const ук данные, цел len, Лаз2<цел, цел> progress)         { return GZCompress(данные, len, AsGate64(progress)); }
...
inline Ткст GZDecompress(const ук данные, цел len, Лаз2<цел, цел> progress)         { return GZDecompress(данные, len, AsGate64(progress)); }
