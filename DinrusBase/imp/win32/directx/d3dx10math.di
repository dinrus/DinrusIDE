...
float D3DXVec4Length(D3DXVECTOR4* pV) {
	...
	return sqrtf((pV.x * pV.x) + (pV.y * pV.y) + (pV.z * pV.z) + (pV.ш * pV.ш));
}
...
float D3DXVec4LengthSq(D3DXVECTOR4* pV) {
	...
	return (pV.x * pV.x) + (pV.y * pV.y) + (pV.z * pV.z) + (pV.ш * pV.ш);
}
...
float D3DXVec4Dot(D3DXVECTOR4* pV1, D3DXVECTOR4* pV2) {
	...
	return (pV1.x * pV2.x) + (pV1.y * pV2.y) + (pV1.z * pV2.z) + (pV1.ш * pV2.ш);
}
...
D3DXVECTOR4* D3DXVec4Minimize(D3DXVECTOR4* pOut, D3DXVECTOR4* pV1, D3DXVECTOR4* pV2) {
	...
	pOut.y = pV1.y < pV2.y ? pV1.y : pV2.y;
	pOut.z = pV1.z < pV2.z ? pV1.z : pV2.z;
	pOut.ш = pV1.ш < pV2.ш ? pV1.ш : pV2.ш;
	return pOut;
}
...
extern(Windows) {
	...
	D3DXMATRIX* D3DXMatrixPerspectiveRH(D3DXMATRIX* pOut, FLOAT ш, FLOAT в, FLOAT zn, FLOAT zf);
	D3DXMATRIX* D3DXMatrixPerspectiveLH(D3DXMATRIX* pOut, FLOAT ш, FLOAT в, FLOAT zn, FLOAT zf);
	...
	D3DXMATRIX* D3DXMatrixOrthoRH(D3DXMATRIX* pOut, FLOAT ш, FLOAT в, FLOAT zn, FLOAT zf);
	D3DXMATRIX* D3DXMatrixOrthoLH(D3DXMATRIX* pOut, FLOAT ш, FLOAT в, FLOAT zn, FLOAT zf);
...
float D3DXQuaternionLength(D3DXQUATERNION *pQ) {
    ...
    return sqrtf((pQ.x * pQ.x) + (pQ.y * pQ.y) + (pQ.z * pQ.z) + (pQ.ш * pQ.ш));
}
...
float D3DXQuaternionLengthSq(D3DXQUATERNION *pQ) {
    ...
    return (pQ.x * pQ.x) + (pQ.y * pQ.y) + (pQ.z * pQ.z) + (pQ.ш * pQ.ш);
}
...
float D3DXQuaternionDot(D3DXQUATERNION *pQ1, D3DXQUATERNION *pQ2) {
    ...
    return (pQ1.x * pQ2.x) + (pQ1.y * pQ2.y) + (pQ1.z * pQ2.z) + (pQ1.ш * pQ2.ш);
}
...
bool D3DXQuaternionIsIdentity(D3DXQUATERNION *pQ) {
    ...
    return (pQ.x == 0.0f) && (pQ.y == 0.0f) && (pQ.z == 0.0f) && (pQ.ш == 1.0f);
}
...
float D3DXPlaneDot(D3DXPLANE *pP, D3DXVECTOR4 *pV) {
    ...
    return (pP.a * pV.x) + (pP.b * pV.y) + (pP.c * pV.z) + (pP.d * pV.ш);
}
