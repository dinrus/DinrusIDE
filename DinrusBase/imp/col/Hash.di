...
struct Хэш(З, alias хэшФункц, alias обновлФункц, плав факторЗагрузки=ДефолтыХэш.факторЗагрузки, бцел исходнРазмерТаблицы=ДефолтыХэш.размерТаблицы, alias Разместитель=ДефолтныйРазместитель, бул допускатьДубликаты=нет, бул обновить_ли=да)
{
    ...
    бул добавь(З з)
    {
        ...
        auto в = хэшФункц(з) % таблица.length;
        узел хвост = таблица[в];
        if(хвост is пусто)
        {
            //
            // Узла пока ещё нет, добавим здесь новый узел.
            //
            хвост = таблица[в] = размести(з);
        ...
        if(ёмкость > таблица.length)
        {
            ...
            foreach(голова; таблица)
            {
                if(голова)
                {
                    ...
                    {
                        следщ = тек.следщ;
                        auto в = хэшФункц(тек.значение) % новТабл.length;
                        узел новГолова = новТабл[в];
    ...
    Позиция найди(З з)
    {
        ...
        узел укз;
        if(таблица[в] is пусто || (укз = найдиВБакете(таблица[в], з, таблица[в])) is пусто)
            return конец;
        Позиция p;
        p.владелец = this;
        p.инд = в;
    ...
    static if(допускатьДубликаты)
    {
        ...
        Позиция найди(З з, Позиция начатьС)
        {
            ...
            if(начатьС.инд < в)
            {
                ...
                if(таблица[в] is пусто)
                    return конец;

                // начать с бакета, в котором должно находиться значение.
                начатьС.инд = в;
                начатьС.ptr = таблица[в];
            }
            else if(начатьС.инд > в)
                // за бакетом, вернуть конец.
                return конец;

            if((начатьС.ptr = найдиВБакете(таблица[в], з, начатьС.ptr)) !is
                    пусто)
                return начатьС;
            return конец;
        }
