	...
	if(infile && ffs != STYLE_NO_REPLACE)
	{
		...
		if(!editor.IsReadOnly()) {
			...
			if(ffs == STYLE_CONFIRM_REPLACE)
			{
				...
				switch(PromptYesNoCancel(Format("Заменить %d строки в [* \1%s\1]?", infile, fn)))
				{
				case 1:  break;
			...
			if(doit)
			{
				...
				editor.BlockReplace();
				SaveFile();
				FFound().Add(fn, Null, AsString(infile) + " замен сделано");
				FFound().Sync();
			}
		}
	}
...
void Ide::FindInFiles(bool replace) {
	...
	if(c == IDOK) {
		...
		FFound().HeaderTab(2).SetText("Текст исходника");
		Renumber();
		ff.find.AddHistory();
		ff.files.AddHistory();
		...
		if(!pi.Canceled()) {
			...
			FFound().HeaderTab(2).SetText(Format("Текст исходника (%d)", FFound().GetCount()));
			if(!IsNull(pattern))
...
void Ide::ConstructFindInFiles() {
	...
	ff.folder.AddButton().SetMonoImage(CtrlImg::smalldown()).Tip("Относящиеся тома") <<= THISBACK(FindStdDir);
	ff.folder.AddButton().SetMonoImage(CtrlImg::smallright()).Tip("Выберите том") <<= THISBACK(FindFolder);
	editor.PutI(ff.find);
...
{
	bar.Add("Копировать текст", THISBACK1(CopyFound, false));
	bar.Add("Копировать всё", THISBACK1(CopyFound, true));
}
