...
void addText2Combo(const char * txt2add, Window* hCombo)
{
    if (!hCombo) return;
...
{
    char str[FINDREPLACE_MAXLENGTH] = { '\0' };
    ::SendMessage(hCombo, WM_GETTEXT, FINDREPLACE_MAXLENGTH - 1, reinterpret_cast<LPARAM>(str));
    return String(str);
};

void delLeftWordInEdit(Window* hEdit)
{
    char str[FINDREPLACE_MAXLENGTH];
...
int Searching::convertExtendedToString(const char * query, char * result, int length)
{   //query may equal to result, since it always gets smaller
    int i = 0, j = 0;
    int charLeft = length;
    char current;
    while (i < length)
    {   //because the backslash escape quences always reduce the size of the String, no overflow checks have to be made for target, assuming parameters are correct
        current = query[i];
        --charLeft;
        if (current == '\\' && charLeft)
...
bool Searching::readBase(const char * str, int * value, int base, int size)
{
    int i = 0, temp = 0;
    *value = 0;
...
LONG_PTR FindReplaceDlg::originalFinderProc = nullptr;
LONG_PTR FindReplaceDlg::originalComboEditProc = nullptr;
...
void FindReplaceDlg::create(int dialogID, bool isRTL, bool msgDestParent)
{
    ...
    _statusBar.init(GetModuleHandle(nullptr), _hSelf, 0);
    _statusBar.display();
...
void FindReplaceDlg::fillComboHistory(int id, const Vector<String> & strings)
{
    Window* hCombo = ::GetDlgItem(_hSelf, id);

    for (Vector<String>::const_reverse_iterator i = strings.rbegin() ; i != strings.rend(); ++i)
    {
        addText2Combo(i->Begin(), hCombo);
    }

    //empty String is not added to CB items, so we need to set it manually
    if (!strings.IsEmpty() && strings.begin()->empty())
    {
...
int FindReplaceDlg::saveComboHistory(int id, int maxcount, Vector<String> & strings, bool saveEmpty)
{
    char text[FINDREPLACE_MAXLENGTH] = { '\0' };
    Window* hCombo = ::GetDlgItem(_hSelf, id);
...
{
    Window* hCombo = ::GetDlgItem(_hSelf, comboID);
    addText2Combo(getTextFromCombo(hCombo).Begin(), hCombo);
}
...
std::pair<intptr_t, intptr_t> Finder::gotoFoundLine(size_t nOccurrence)
{
    ...
    if (!::SendMessage(_hParent, WM_DOOPEN, 0, reinterpret_cast<LPARAM>(fInfo._fullPath.Begin()))) return emptyResult;
...
Vector<String> Finder::getResultFilePaths() const
{
    Vector<String> paths;
    ...
    for (size_t i = 0; i < len; ++i)
    {
        ...
        String& path2add = (*_pMainFoundInfos)[i]._fullPath;
        bool found = path2add.IsEmpty();
...
bool Finder::canFind(const char *fileName, size_t lineNumber) const
{
    size_t len = _pMainFoundInfos->size();
    for (size_t i = 0; i < len; ++i)
    {
...
{
    Window* hFindCombo = ::GetDlgItem(_hSelf, IDFINDWHAT_FIFOLDER);
    addText2Combo(_options._str2Search.Begin(), hFindCombo);
...
void FindReplaceDlg::resizeDialogElements(LONG newWidth)
{
    ...
    {
        Window* resizeHwnd = ::GetDlgItem(_hSelf, id);
        ...
        ::SetWindowPos(resizeHwnd, nullptr, 0, 0, rc.right + addWidth, rc.bottom, SWP_NOMOVE | flags);
    ...
    for (int moveWndID : moveWindowIDs)
    {
        ...
        ::MapWindowPoints(nullptr, _hSelf, (LPPOINT)&rc, 2);

        ::SetWindowPos(moveHwnd, nullptr, rc.left + addWidth, rc.top, 0, 0, SWP_NOSIZE | flags);
    }

    auto additionalWindowHwndsToResize = { _tab.getHSelf() , _statusBar.getHSelf() };
    for (Window* resizeHwnd : additionalWindowHwndsToResize)
    {
        ::GetClientRect(resizeHwnd, &rc);
        ::SetWindowPos(resizeHwnd, nullptr, 0, 0, rc.right + addWidth, rc.bottom, SWP_NOMOVE | flags);
    }
...
intptr_t CALLBACK FindReplaceDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_INITDIALOG :
        {
            ...
            Window* hFindCombo = ::GetDlgItem(_hSelf, IDFINDWHAT);
            Window* hReplaceCombo = ::GetDlgItem(_hSelf, IDREPLACEWITH);
            Window* hFiltersCombo = ::GetDlgItem(_hSelf, IDD_FINDINFILES_FILTERS_COMBO);
            Window* hDirCombo = ::GetDlgItem(_hSelf, IDD_FINDINFILES_DIR_COMBO);
            ...
            for (Window* hComboBox : { hFindCombo, hReplaceCombo, hFiltersCombo, hDirCombo })
            {
                LOGFONT lf = {};
            ...
            _replaceClosePos.top = p.y;
             ...
             String searchButtonTip = pNativeSpeaker->getLocalizedStrFromID("shift-change-direction-tip", TEXT("Use Shift+Enter to search in the opposite direction."));
             _shiftTrickUpTip = CreateToolTip(IDOK, _hSelf, _hInst, const_cast<PTSTR>(searchButtonTip.Begin()), _isRTL);

             String checkboxTip = pNativeSpeaker->getLocalizedStrFromID("two-find-buttons-tip", TEXT("2 find buttons mode"));
             _2ButtonsTip = CreateToolTip(IDC_2_BUTTONS_MODE, _hSelf, _hInst, const_cast<PTSTR>(checkboxTip.Begin()), _isRTL);

             String findInFilesFilterTip = pNativeSpeaker->getLocalizedStrFromID("find-in-files-filter-tip", TEXT("Find in cpp, cxx, h, hxx && hpp:\r*.cpp *.cxx *.h *.hxx *.hpp\r\rFind in all files except exe, obj && log:\r*.* !*.exe !*.obj !*.log\r\rFind in all files but exclude folders tests, bin && bin64:\r*.* !\\tests !\\bin*\r\rFind in all files but exclude all folders log or logs recursively:\r*.* !+\\log*"));
             _filterTip = CreateToolTip(IDD_FINDINFILES_FILTERS_STATIC, _hSelf, _hInst, const_cast<PTSTR>(findInFilesFilterTip.Begin()), _isRTL);
        ...
        case WM_HSCROLL :
        {
            if (reinterpret_cast<Window*>(lParam) == ::GetDlgItem(_hSelf, IDC_PERCENTAGE_SLIDER))
            {
...
//Single actions
                ...
                case IDCANCEL:
                    ...
                    setStatusbarMessage(String(), FSNoMessage);
                    display(false);
                    break;
                ...
                case IDOK : // Find Next : only for FIND_DLG and REPLACE_DLG
                {
                    ...
                    else
                    {
                        ...
                        processFindNext(_options._str2Search.Begin(), _env, &findStatus);

                        NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
                        if (findStatus == FSEndReached)
                        {
                            String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-end-reached", TEXT("Find: Found the 1st occurrence from the top. The end of the document has been reached."));
                            setStatusbarMessage(msg, FSEndReached);
                        }
                        else if (findStatus == FSTopReached)
                        {
                            String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-top-reached", TEXT("Find: Found the 1st occurrence from the bottom. The beginning of the document has been reached."));
                ...
                {
                    Window* hFindWhat = ::GetDlgItem(_hSelf, IDFINDWHAT);
                    String findWhatText = getTextFromCombo(hFindWhat);
                    Window* hPlaceWith = ::GetDlgItem(_hSelf, IDREPLACEWITH);
                    String replaceWithText = getTextFromCombo(hPlaceWith);
                    if ((!findWhatText.IsEmpty() || !replaceWithText.IsEmpty()) && (findWhatText != replaceWithText))
                    {
                        ::SendMessage(hFindWhat, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(replaceWithText.Begin()));
                        ::SendMessage(hPlaceWith, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(findWhatText.Begin()));
                    }
                }
                return TRUE;
                ...
                case IDREPLACE:
                {
                    ...
                    if (_currentStatus == REPLACE_DLG)
                    {
                        ...
                        processReplace(_options._str2Search.Begin(), _options._str4Replace.Begin());
                        nppParamInst._isFindReplacing = false;
                    }
                }
                return TRUE;
//Process actions
                ...
                case IDD_FINDINFILES_FIND_BUTTON:
                {
                    ...
                    if (_currentStatus == FINDINFILES_DLG)
                    {
                        ...
                        if (!_options._directory.IsEmpty())
                        {
                            addText2Combo(_options._directory.Begin(), ::GetDlgItem(_hSelf, IDD_FINDINFILES_DIR_COMBO));
                ...
                case IDREPLACEALL :
                {
                    ...
                    if (_currentStatus == REPLACE_DLG)
                    {
                        ...
                        {
                            NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
                            String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replace-readonly", TEXT("Replace: Cannot replace text. The current document is read only."));
...
//Option actions
                ...
                case IDD_FINDINFILES_FOLDERFOLLOWSDOC_CHECK :
                {
                    ...
                    if (findHistory._isFolderFollowDoc)
                    {
                        ...
                        String currPath;
                        const SciBuffer* buf = (*_ppEditView)->getCurrentBuffer();
                        if (!(buf->getStatus() & (DOC_UNNAMED | DOC_DELETED)))
                        {
                            currPath = buf->getFullPathName();
                            PathRemoveFileSpec(currPath);
                        }
                        if (currPath.IsEmpty() || !PathIsDirectory(currPath.Begin()))
                            currPath = NppParameters::getInstance().getWorkingDir();
                        ::SetDlgItemText(_hSelf, IDD_FINDINFILES_DIR_COMBO, currPath.Begin());
                    }
                ...
                case IDD_FINDINFILES_BROWSE_BUTTON :
                {
                    ...
                    {
                        NativeLangSpeaker* pNativeSpeaker = NppParameters::getInstance().getNativeLangSpeaker();
                        const String title = pNativeSpeaker->getLocalizedStrFromID("find-in-files-select-folder", TEXT("Select a folder to search from"));
                        folderBrowser(_hSelf, title, IDD_FINDINFILES_DIR_COMBO, _options._directory.Begin());
                    }
                }
                return TRUE;
...
bool FindReplaceDlg::processFindNext(const char *txt2find, const FindOption *options, FindStatus *oFindStatus, FindNextType findNextType /* = FINDNEXTTYPE_FINDNEXT */)
{
    if (oFindStatus)
        *oFindStatus = FSFound;
    ...
    if (posFind == -1) //no match found in target, check if a new target should be used
    {
        ...
        { // not found
            ...
            {
                String newTxt2find = stringReplace(txt2find, TEXT("&"), TEXT("&&"));
                NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
                String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-cannot-find", TEXT("Find: Can't find the text \"$STR_REPLACE$\""));
    ...
    {
        NativeLangSpeaker* pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
        String msg = pNativeSpeaker->getLocalizedStrFromID("find-regex-zero-length-match", TEXT("zero length match"));
        msg = TEXT("^ ") + msg;
        (*_ppEditView)->showCallTip(start, msg.Begin());
    }
...
bool FindReplaceDlg::processReplace(const char *txt2find, const char *txt2replace, const FindOption *options)
{
    bool moreMatches;
        ...
        if (nextFind.cpMin == currentSelection.cpMin && nextFind.cpMax == currentSelection.cpMax)
        {
            ...
            else
            {
                if (replaceOptions._searchType == FindExtended)
                {
                    ...
                    char *pText2ReplaceExtended = new char[stringSizeReplace + 1];
            ...
            {
                String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replaced-without-continuing", TEXT("Replace: 1 occurrence was replaced."));
            ...
            else
            {
                ...
                {
                    String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replace-end-reached", TEXT("Replace: Replaced the 1st occurrence from the top. The end of document has been reached."));
                    setStatusbarMessage(msg, FSEndReached);
                }
                else if (status == FSTopReached)
                {
                    String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replace-top-reached", TEXT("Replace: Replaced the 1st occurrence from the bottom. The begin of document has been reached."));
                    setStatusbarMessage(msg, FSTopReached);
                }
                else
                {
    ...
    {
        NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
        String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replace-not-found", TEXT("Replace: no occurrence was found."));
        setStatusbarMessage(msg, FSNotFound);
    }
...
int FindReplaceDlg::markAll(const char *txt2find, int styleID)
{
    ...
    int nbFound = processAll(ProcessMarkAllExt, &markAllOpt, true, nullptr, styleID);
    return nbFound;
}
...
int FindReplaceDlg::processAll(ProcessOperation op, const FindOption *opt, bool isEntire, const FindersInfo *pFindersInfo, int colourStyleID)
{
    ...
    {
        NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
        String msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-readonly", TEXT("Replace All: Cannot replace text. The current document is read only."));
        setStatusbarMessage(msg, FSNotFound);
        return 0;
    }

    const FindOption *pOptions = opt?opt:_env;
    const char *txt2find = pOptions->_str2Search.Begin();
    const char *txt2replace = pOptions->_str4Replace.Begin();
...
int FindReplaceDlg::processRange(ProcessOperation op, FindReplaceInfo & findReplaceInfo, const FindersInfo * pFindersInfo, const FindOption *opt, int colourStyleID, ScintillaEditView *view2Process)
{
    ...
    char *pTextFind = nullptr;
    if (!findReplaceInfo._txt2find)
    {
        Window* hFindCombo = ::GetDlgItem(_hSelf, IDFINDWHAT);
        String str2Search = getTextFromCombo(hFindCombo);
        stringSizeFind = str2Search.GetLength();
        pTextFind = new char[stringSizeFind + 1];
        wcscpy_s(pTextFind, stringSizeFind + 1, str2Search.Begin());
    }
    else
    {
    ...
    char *pTextReplace = nullptr;
    if (op == ProcessReplaceAll)
    {
        if (!findReplaceInfo._txt2replace)
        {
            Window* hReplaceCombo = ::GetDlgItem(_hSelf, IDREPLACEWITH);
            String str2Replace = getTextFromCombo(hReplaceCombo);
            stringSizeReplace = str2Replace.GetLength();
            pTextReplace = new char[stringSizeReplace + 1];
            wcscpy_s(pTextReplace, stringSizeReplace + 1, str2Replace.Begin());
        }
        else
        {
    ...
    while (targetStart >= 0)
    {
        ...
        switch (op)
        {
            case ProcessFindAll:
            {
                ...
                char lineBuf[SC_SEARCHRESULT_LINEBUFFERMAXLENGTH];
                ...
                _pFinder->add(FoundInfo(targetStart, targetEnd, lineNumber + 1, pFileName), srml, line.Begin(), totalLineNumber);
            ...
            case ProcessFindInFinder:
            {
                ...
                const char *pFileName = pFindersInfo->_pFileName ? pFindersInfo->_pFileName : TEXT("");
                ...
                if (processed)
                {
                    ...
                    pFindersInfo->_pDestFinder->add(FoundInfo(targetStart, targetEnd, lineNumber + 1, pFileName), srml, line.Begin(), totalLineNumber);
                }
                break;
            }
...
void FindReplaceDlg::findAllIn(InWhat op)
{
    ...
    if (!_pFinder)
    {
        ...
        NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
        String text = pNativeSpeaker->getLocalizedStrFromID("find-result-caption", TEXT(""));

        if (!text.IsEmpty())
        {
            _findResTitle = text;
            data.pszName = _findResTitle.Begin();
        }
    ...
    {
        String text = _pFinder->getHitsString(_findAllResult);
        wsprintf(_findAllResultStr, text.Begin());
...
void FindReplaceDlg::setSearchText(char * txt2find)
{
    Window* hCombo = ::GetDlgItem(_hSelf, IDFINDWHAT);
    if (txt2find && txt2find[0])
    {
...
{
    Window* h = ::GetDlgItem(_hSelf, dlgItemID);
    if (!h) return;
...
{
    cutString(_env->_filters.Begin(), patternVect);
}

void FindReplaceDlg::getAndValidatePatterns(Vector<String> & patternVect)
{
    getPatterns(patternVect);
    if (patternVect.size() == 0)
    {
        setFindInFilesDirFilter(nullptr, TEXT("*.*"));
        getPatterns(patternVect);
    }
...
void FindReplaceDlg::saveInMacro(size_t cmd, int cmdType)
{
    ...
    ::SendMessage(_hParent, WM_FRSAVE_STR, IDFINDWHAT,  reinterpret_cast<LPARAM>(cmd == IDC_CLEAR_ALL ? TEXT("") : _options._str2Search.Begin()));
    ...
    if (cmdType & FR_OP_REPLACE)
        ::SendMessage(_hParent, WM_FRSAVE_STR, IDREPLACEWITH, reinterpret_cast<LPARAM>(_options._str4Replace.Begin()));
    if (cmdType & FR_OP_FIF)
    {
        ::SendMessage(_hParent, WM_FRSAVE_STR, IDD_FINDINFILES_DIR_COMBO, reinterpret_cast<LPARAM>(_options._directory.Begin()));
        ::SendMessage(_hParent, WM_FRSAVE_STR, IDD_FINDINFILES_FILTERS_COMBO, reinterpret_cast<LPARAM>(_options._filters.Begin()));
...
void FindReplaceDlg::setStatusbarMessage(const String& msg, FindStatus staus, char const *pTooltipMsg)
{
    if (_statusbarTooltipWnd)
    {
    ...
    if (isVisible())
    {
        ...
        _statusBar.setOwnerDrawText(msg.Begin());
    }
}

String FindReplaceDlg::getScopeInfoForStatusBar(FindOption const *pFindOpt) const
{
    String scope;
...
void FindReplaceDlg::execSavedCommand(int cmd, uptr_t intValue, const char* stringValue)
{
    try
    {
        switch (cmd)
            ...
            case IDC_FRCOMMAND_EXEC:
            {
                ...
                switch (intValue)
                {
                    case IDOK:
                        ...
                        {
                            nppParamInst._isFindReplacing = true;
                            processFindNext(_env->_str2Search.Begin());
                            nppParamInst._isFindReplacing = false;
                        }
                        break;
                    ...
                    case IDREPLACE:
                        ...
                        {
                            nppParamInst._isFindReplacing = true;
                            processReplace(_env->_str2Search.Begin(), _env->_str4Replace.Begin(), _env);
                            nppParamInst._isFindReplacing = false;
                        }
                        break;
                    case IDC_FINDALL_OPENEDFILES:
    ...
    {
        MessageBoxA(nullptr, err.what(), "Play Macro Exception", MB_OK);
    }
}
...
void FindReplaceDlg::setFindInFilesDirFilter(const char *dir, const char *filters)
{
    if (dir)
    {
...
void FindReplaceDlg::enableProjectCheckmarks()
{
    NppParameters& nppParams = NppParameters::getInstance();
    FindHistory & findHistory = nppParams.getFindHistory();
    Menu* hMenu = (Menu*) ::SendMessage (_hParent, NPPM_INTERNAL_GETMENU, 0, 0);
    if (hMenu)
    {
...
LRESULT FAR PASCAL FindReplaceDlg::finderProc(Window* hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
LRESULT FAR PASCAL FindReplaceDlg::comboEditProc(Window* hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    Window* hwndCombo = reinterpret_cast<Window*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
...
{
    Window* hFindCombo = ::GetDlgItem(_hSelf, comboID);
    if (!hFindCombo) return;

    String str2transform = getTextFromCombo(hFindCombo);

    // Count the number of character '\n' and '\r'
    size_t nbEOL = 0;
    size_t str2transformLen = lstrlen(str2transform.Begin());
    ...
    if (nbEOL)
    {
        char * newBuffer = new char[str2transformLen + nbEOL*2 + 1];
        int j = 0;
        for (size_t i = 0 ; i < str2transformLen ; ++i)
        {
...
void FindReplaceDlg::drawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if (_statusbarTooltipMsg.GetLength() == 0) return;
    ...
    if (s > 0)
    {
        ...
        if (_statusbarTooltipIcon)
        {
            ...
            if (!_statusbarTooltipWnd)
            {
                ...
                _statusbarTooltipWnd = CreateToolTipRect(1, _statusBar.getHSelf(), _hInst, const_cast<PTSTR>(_statusbarTooltipMsg.Begin()), rect);
...
bool FindReplaceDlg::replaceInFilesConfirmCheck(String directory, String fileTypes)
{
    bool confirmed = false;

    NativeLangSpeaker* pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();

    String title = pNativeSpeaker->getLocalizedStrFromID("replace-in-files-confirm-title", TEXT("Are you sure?"));

    String msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-files-confirm-directory", TEXT("Are you sure you want to replace all occurrences in :"));
    msg += TEXT("\r\r");
    msg += directory;

    msg += TEXT("\r\r");

    String msg2 = pNativeSpeaker->getLocalizedStrFromID("replace-in-files-confirm-filetype", TEXT("For file type :"));
    msg2 += TEXT("\r\r");
    msg2 += fileTypes[0] ? fileTypes : TEXT("*.*");

    msg += msg2;

    int res = ::MessageBox(nullptr, msg.Begin(), title.Begin(), MB_OKCANCEL | MB_DEFBUTTON2 | MB_TASKMODAL);
...
bool FindReplaceDlg::replaceInProjectsConfirmCheck()
{
    ...
    String title = pNativeSpeaker->getLocalizedStrFromID("replace-in-projects-confirm-title", TEXT("Are you sure?"));

    String msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-projects-confirm-message", TEXT("Do you want to replace all occurrences in all documents in the selected Project Panel(s)?"));
    int res = ::MessageBox(nullptr, msg.Begin(), title.Begin(), MB_OKCANCEL | MB_DEFBUTTON2 | MB_TASKMODAL);
...
bool FindReplaceDlg::replaceInOpenDocsConfirmCheck(void)
{
    ...
    NativeLangSpeaker* pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
    String title = pNativeSpeaker->getLocalizedStrFromID("replace-in-open-docs-confirm-title", TEXT("Are you sure?"));
    String msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-open-docs-confirm-message", TEXT("Are you sure you want to replace all occurrences in all open documents?"));
...
String Finder::getHitsString(int count) const
{
    NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
    String text = pNativeSpeaker->getLocalizedStrFromID("find-result-hits", TEXT(""));
...
void Finder::addSearchLine(const char *searchName)
{
    NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
    String str = pNativeSpeaker->getLocalizedStrFromID("find-result-title", TEXT("Search"));
    str += TEXT(" \"");
    str += searchName;
    str += TEXT("\" \r\n");

    setFinderReadOnly(false);
    _scintView.addGenericText(str.Begin());
    setFinderReadOnly(true);
...
void Finder::addFileNameTitle(const char * fileName)
{
    String str = TEXT("  ");
    str += fileName;
    str += TEXT("\r\n");
...
void Finder::addFileHitCount(int count)
{
    ...
    setFinderReadOnly(false);
    _scintView.insertGenericTextFrom(_lastFileHeaderPos, text.Begin());
    setFinderReadOnly(true);
    ++_nbFoundFiles;
}
...
{
    String nbResStr = std::to_wstring(count);
    String nbFoundFilesStr = std::to_wstring(_nbFoundFiles);
    String nbSearchedFilesStr = std::to_wstring(countSearched);

    NativeLangSpeaker* pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();

    String text = pNativeSpeaker->getLocalizedStrFromID(
    ...
    if (text.IsEmpty())
    {
        ...
        String hitsIn = count == 1 ? TEXT("hit") : TEXT("hits");

        String fileOrSelection = searchedEntireNotSelection ? TEXT("file") : TEXT("selection");;
        if (_nbFoundFiles != 1)
        {
    ...
    if (isMatchLines)
    {
        String lineFilterModeInfo = pNativeSpeaker->getLocalizedStrFromID("find-result-title-info-extra", TEXT(" - Line Filter Mode: only display the filtered results"));
        text += lineFilterModeInfo;
    }

    setFinderReadOnly(false);
    _scintView.insertGenericTextFrom(_lastSearchHeaderPos, text.Begin());
    setFinderReadOnly(true);
}

void Finder::add(FoundInfo fi, SearchResultMarkingLine miLine, const char* foundline, size_t totalLineNumber)
{
    ...
    miLine._segmentPostions[0].first += headerStr.GetLength();
    miLine._segmentPostions[0].second += headerStr.GetLength();
    headerStr += foundline;
    WcharMbcsConvertor& wmc = WcharMbcsConvertor::getInstance();
    const char* text2AddUtf8 = wmc.wchar2char(headerStr.Begin(), SC_CP_UTF8, &miLine._segmentPostions[0].first, &miLine._segmentPostions[0].second); // certainly utf8 here
...
void Finder::openAll()
{
    ...
    {
        ::SendMessage(_hParent, WM_DOOPEN, 0, reinterpret_cast<LPARAM>(path.Begin()));
    }
}
...
void Finder::copyPathnames()
{
    ...
    if (!toClipboard.IsEmpty())
    {
        if (!str2Clipboard(toClipboard, _hSelf))
        {
            assert(false);
            ::MessageBox(nullptr, TEXT("Error placing pathnames into clipboard."), TEXT("Notepad++"), MB_ICONINFORMATION);
        }
    }
}
...
bool Finder::isLineActualSearchResult(const String& s) const
{
...
String& Finder::prepareStringForClipboard(String& s) const
{
...
void Finder::copy()
{
    ...
    Vector<String> lines;
    String previousResultLineStr(TEXT(""));
    for (size_t line = fromLine; line <= toLine; ++line)
    {
        String lineStr = _scintView.getLine(line);
    ...
    const String toClipboard = stringJoin(lines, TEXT("\r\n")) + TEXT("\r\n");
    if (!toClipboard.IsEmpty())
    {
        if (!str2Clipboard(toClipboard, _hSelf))
        {
            assert(false);
            ::MessageBox(nullptr, TEXT("Error placing text in clipboard."), TEXT("Notepad++"), MB_ICONINFORMATION);
        }
    }
}
...
void Finder::setFinderStyle()
{
    ...
    const char * lexerName = ScintillaEditView::_langNameInfoArray[L_SEARCHRESULT]._langName;
...
intptr_t CALLBACK Finder::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_CONTEXTMENU :
        {
            if (Window*(wParam) == _scintView.getHSelf())
            {
                POINT p;
                ::GetCursorPos(&p);
                ...
                String findInFinder = pNativeSpeaker->getLocalizedStrFromID("finder-find-in-finder", TEXT("Find in these search results..."));
                String closeThis = pNativeSpeaker->getLocalizedStrFromID("finder-close-this", TEXT("Close these search results"));
                String collapseAll = pNativeSpeaker->getLocalizedStrFromID("finder-collapse-all", TEXT("Collapse all"));
                String uncollapseAll = pNativeSpeaker->getLocalizedStrFromID("finder-uncollapse-all", TEXT("Uncollapse all"));
                String copyLines = pNativeSpeaker->getLocalizedStrFromID("finder-copy", TEXT("Copy Selected Line(s)"));
                String copyVerbatim = pNativeSpeaker->getLocalizedStrFromID("finder-copy-verbatim", TEXT("Copy"));
                copyVerbatim += TEXT("\tCtrl+C");
                String copyPaths = pNativeSpeaker->getLocalizedStrFromID("finder-copy-paths", TEXT("Copy Pathname(s)"));
                String selectAll = pNativeSpeaker->getLocalizedStrFromID("finder-select-all", TEXT("Select all"));
                selectAll += TEXT("\tCtrl+A");
                String clearAll = pNativeSpeaker->getLocalizedStrFromID("finder-clear-all", TEXT("Clear all"));
                String purgeForEverySearch = pNativeSpeaker->getLocalizedStrFromID("finder-purge-for-every-search", TEXT("Purge for every search"));
                String openAll = pNativeSpeaker->getLocalizedStrFromID("finder-open-all", TEXT("Open all"));
                String wrapLongLines = pNativeSpeaker->getLocalizedStrFromID("finder-wrap-long-lines", TEXT("Word wrap long lines"));
...
void FindIncrementDlg::init(Window& hInst, Window* hPere, FindReplaceDlg *pFRDlg, bool isRTL)
{
...
intptr_t CALLBACK FindIncrementDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_COMMAND :
        {
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDM_SEARCH_FINDINCREMENT:  // Accel table: Start incremental search
                    ...
                    {
                        Window* hFindTxt = ::GetDlgItem(_hSelf, IDC_INCFINDTEXT);
            ...
            String str2Search = getTextFromCombo(::GetDlgItem(_hSelf, IDC_INCFINDTEXT));
            if (updateSearch)
            {
                FindStatus findStatus = FSFound;
                bool isFound = _pFRDlg->processFindNext(str2Search.Begin(), &fo, &findStatus);
            ...
            if (updateHiLight)
            {
                bool highlight = !str2Search.IsEmpty() &&
        ...
        case WM_ERASEBKGND:
        {
            ...
            {
                Window* hParent = ::GetParent(_hSelf);
                ...
                ::SetWindowOrgEx(winDC, ptOrig.x, ptOrig.y, nullptr);
                return (BOOL)lResult;
            }
        }
    }
...
void FindIncrementDlg::markSelectedTextInc(bool enable, FindOption *opt)
{
    ...
    char text2Find[FINDREPLACE_MAXLENGTH];
...
{
    String statusStr2Display;

    static const char* const strFSNotFound = TEXT("Phrase not found");
    static const char* const strFSTopReached = TEXT("Reached top of page, continued from bottom");
    static const char* const strFSEndReached = TEXT("Reached end of page, continued from top");
    ...
    if (nbCounted >= 0)
    {
        ...
        if (statusStr2Display.IsEmpty())
        {
            char strFindFSFound[128] = TEXT("");

            if (nbCounted == 1)
                wsprintf(strFindFSFound, TEXT("%d match"), nbCounted);
            else
                wsprintf(strFindFSFound, TEXT("%s matches"), commafyInt(nbCounted).Begin());
    ...
    // get the Window* of the editor
    Window* hEditor = ::GetDlgItem(_hSelf, IDC_INCFINDTEXT);

    // invalidate the editor rect
    ::InvalidateRect(hEditor, nullptr, TRUE);

    ::SendDlgItemMessage(_hSelf, IDC_INCFINDSTATUS, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(statusStr2Display.Begin()));
}
...
const char Progress::cClassName[] = TEXT("NppProgressClass");
const char Progress::cDefaultHeader[] = TEXT("Operation progress...");
...
Progress::Progress(Window& hInst) : _hwnd(nullptr), _hCallerWnd(nullptr)
{
...
Window* Progress::open(Window* hCallerWnd, const char* header)
{
    if (_hwnd)
        return _hwnd;

    // Create manually reset non-signalled event
    _hActiveState = ::CreateEvent(nullptr, TRUE, FALSE, nullptr);
    if (!_hActiveState)
        return nullptr;
    ...
    for (Window* hwnd = _hCallerWnd; hwnd; hwnd = ::GetParent(hwnd))
        ::UpdateWindow(hwnd);
    ...
    _hThread = ::CreateThread(nullptr, 0, threadFunc, this, 0, nullptr);
    if (!_hThread)
    {
...
void Progress::setPercent(unsigned percent, const char *fileName) const
{
    if (_hwnd)
    {
...
int Progress::thread()
{
    ...
    // Window message loop
    MSG msg;
    while ((r = ::GetMessage(&msg, nullptr, 0, 0)) != 0 && r != -1)
    {
...
int Progress::createProgressWindow()
{
    ...
    _hwnd = ::CreateWindowEx(
        ...
        nullptr, nullptr, _hInst, (LPVOID)this);
    if (!_hwnd)
        return -1;
    ...
    _hPText = ::CreateWindowEx(0, TEXT("STATIC"), TEXT(""),
        ...
        xStartPos, yTextPos,
        ctrlWidth, textHeight, _hwnd, nullptr, _hInst, nullptr);
...
LRESULT APIENTRY Progress::wndProc(Window* hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
    switch (umsg)
    {
        case WM_CREATE:
