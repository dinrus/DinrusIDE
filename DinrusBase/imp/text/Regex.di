/*******************************************************************************
    ...
    $(TR $(TD \ш) $(TD альфа-numeric characters or underscore) )
    $(TR $(TD \W) $(TD opposite of \ш) )
    $(TR $(TD \d) $(TD цифры) )
...
private final class TNFA(т_сим)
{
    ...
    **********************************************************************************************/
    debug(TangoRegex) проц выведи()
    {
        ...
        {
            Стдвыв.фмт("{}{:d2}{}", s is старт?">":" ", i, s.прими?"*":" ");

            бул первый=да;
            Стдвыв(" {");
            foreach ( t; s.transitions )
            {
                Стдвыв.фмт("{}{}{}:{}->{}", первый?"":", ", t.priority, "gnrx"[t.priorityClass], t.predicate.вТкст, t.мишень is пусто?-1:t.мишень.индекс);
                if ( t.тэг > 0 )
                {
                    Стдвыв.фмт(" t{}", t.тэг);
                }
    ...
    проц разбор(бул без_якоря)
    {
        ...
        ******************************************************************************************/
        бул выполни(Оператор nрасш_op, бул explicit_operator=да)
        {
            ...
            debug(tnfa) Стдвыв.фмтнс("\t{}:{} -> {}  {} frag(s)",
...
private:
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructCharFrag {}", c);
    ...
    {
        debug(tnfa) Стдвыв.фмт("constructChars тип={}", тип);
        ...
        debug(tnfa) Стдвыв.фмтнс("-> {}", trans.predicate.вТкст);
    ...
    {
        debug(tnfa) Стдвыв.фмт("constructCharClass тип={}", тип);
        ...
        if ( последний != т_сим.init )
            trans.predicate.добавьВвод(т_диапазон(последний));
        debug(tnfa) Стдвыв.фмтнс(" {}", образец[oldCursor..cursor]);
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructBracket");
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructOneMore");
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructZeroMore");
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructZeroOne");
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructOccur {},{}", minOccur, maxOccur);
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructAltern");
    ...
    {
        debug(tnfa) Стдвыв.фмтнс("constructConcat");
...
Louter:
    ...
    this(TNFA!(т_сим) tnfa)
    {
        ...
        while ( !unmarked.пустой )
        {
            ...
            foreach ( пред; disjointPredicates(состояние) )
            {
                ...
                debug(tdfa) Стдвыв.фмт("из_ {} with {} reach", состояние.dfa_state.индекс, пред.вТкст);
                SubsetState мишень = reach(состояние, пред);
                if ( мишень is пусто )
                {
                    continue;
                    debug(tdfa) Стдвыв.фмтнс(" nothing - lookbehind at beginning, skИПping");
                }
                debug(tdfa) Стдвыв.фмтнс(" {}", мишень.вТкст);
                ...
                debug (tdfa_new_trans) Стдвыв.фмтнс("Creating with пред: {}", пред);
                ...
                debug(tdfa)
                {
                    Стдвыв.фмтнс("=> из_ {} with {} reach {}", состояние.dfa_state.индекс, пред.вТкст, мишень.dfa_state.индекс);
                }
            }
    ...
    **********************************************************************************************/
    debug(TangoRegex) проц выведи()
    {
        ...
        foreach ( r, иот; tis )
        {
            Стдвыв.фмтнс("тэг({},{}) in reg {}", иот.тэг, иот.индекс, r);
        }
        Стдвыв.фмтнс("Initializer:");
        foreach ( cmd; initializer )
        {
            Стдвыв.фмтнс("{}", cmd.вТкст);
        }
        Стдвыв.фмтнс("Delta function:");
        foreach ( цел i, s; states )
        {
            Стдвыв.фмт("{}{:d2}{}", s is старт?">":" ", i, s.прими?"*":" ");
            ...
            foreach ( t; s.transitions )
            {
                Стдвыв.фмт("{}{}->{} (", первый?"":", ", t.predicate.вТкст, t.мишень.индекс);
                бул firstcmd=да;
...
private:
    ...
    **********************************************************************************************/
    ...
    {
        alias ДиапазонСимволов!(т_сим) т_диапазон;
        debug(tdfa) Стдвыв.фмтнс("disjointPredicates()");
        ...
        foreach ( t; состояние )
        {
            ...
            {
                debug(tdfa) Стдвыв.фмтнс("{}", t.predicate.вТкст);
        ...
        debug(tdfa)
        {
            ...
            foreach ( m; marks_[0 .. num_marks] )
            {
                ...
                if ( m.c > 0x20 && m.c < 0x7f )
                    Стдвыв.фмт("{}{}", m.конец?"e":"s", m.c);
                else
                    Стдвыв.фмт("{}{:x}", m.конец?"e":"s", cast(цел)m.c);
            }
            Стдвыв.нс;
        }
...
Lmerge:
        ...
        debug(tdfa) Стдвыв.фмтнс("disjointPredicates() конец");
        return preds;
    }
        ...
        while ( !стэк.пустой )
        {
            ...
            foreach ( t; se.nfa_state.transitions )
            {
                ...
                if ( врем !is пусто )
                {
                    ...
                    {
//                         debug(tdfa) Стдвыв.фмтнс("maxPrio({}) {} beats {}, continuing", t.мишень.индекс, врем.maxPriority, new_maxPri);
                        continue;
                    }
                    else if ( врем.maxPriority == new_maxPri )
                    {
                        ...
                        {
//                             debug(tdfa) Стдвыв.фмтнс("lastPrio({}) {} beats {}, continuing", t.мишень.индекс, врем.lastPriority, t.priority);
                            continue;
                        }
//                         else
//                             debug(tdfa) Стдвыв.фмтнс("lastPrio({}) {} beats {}", t.мишень.индекс, t.priority, врем.lastPriority);
                    }
//                     else
//                         debug(tdfa) Стдвыв.фмтнс("maxPrio({}) {} beats {}", t.мишень.индекс, new_maxPri, врем.maxPriority);
                }
...
Louter:
    ...
    **********************************************************************************************/
    проц generateFinishers(SubsetState r)
    {
        ...
        {
            debug (Finishers) Стдвыв.фмтнс("Finisher: {}", se);
    ...
    **********************************************************************************************/
    проц minimizeDFA()
    {
        ...
        debug(tdfa) Стдвыв.фмтнс("Minimizing TDFA");
...
Ltrans:
        ...
        foreach ( i, a; states[0 .. $-1] )
        {
            foreach ( j, b; states[i+1 .. $] )
            {
                if ( !рознь(i, j+i+1) )
                {
                    debug(tdfa) Стдвыв.фмтнс("Состояние {} == {}", i, j+i+1);
                    // remap b в_ a
...
class RegExpT(т_сим)
{
        ...
        Пример:
            ...
            проц main()
            {
                foreach(m; Regex("ab").ищи("qwerabcabcababqwer"))
                    Стдвыв.фмтнс("{}[{}]{}", m.pre, m.сверь(0), m.post);
            }
            // Prints:
    ...
    **********************************************************************************************/
    бул тест()
    {
        ...
        debug(TangoRegex) Стдвыв.фмтнс("{}{}: {}", s.прими?"*":" ", s.индекс, inp);
LmainLoop:
...
Lprocess_char:
            debug(TangoRegex) Стдвыв.фмтнс("{} (0x{:x})", c, cast(цел)c);
            ...
            case s.Mode.LOOKUP:
                ...
                {
                    debug(TangoRegex) Стдвыв.фмтнс("отыщи");
                    auto i = s.отыщи[c];
            ...
            case s.Mode.MIXED:
                if ( c < s.LOOKUP_LENGTH )
                {
                    debug(TangoRegex) Стдвыв.фмтнс("mixed");
                    auto i = s.отыщи[c];
                    if ( i == s.INVALID_STATE )
                        break;
                    t = s.transitions[ i ];
...
Ltrans_loop:
                    ...
                    tp < tp_end; ++tp )
            {
                ...
                case predicate_t.ПРежимСверки.single_char:
                    debug(TangoRegex) Стдвыв.фмтнс("single сим 0x{:x} == 0x{:x}", cast(цел)c, cast(цел)t.predicate.data_chr);
                ...
                case predicate_t.ПРежимСверки.bitmap:
                    debug(TangoRegex) Стдвыв.фмтнс("bitmap {}\n{}", c, t.predicate.вТкст);
                ...
                case predicate_t.ПРежимСверки.string_search:
                    debug(TangoRegex) Стдвыв.фмтнс("ткст ищи {} in {}", c, t.predicate.data_str);
                ...
                case predicate_t.ПРежимСверки.генерный:
                    debug(TangoRegex) Стдвыв.фмтнс("генерный {}\n{}", c, t.predicate.вТкст);
...
Lno_consume:
                ...
                s = t.мишень;
                debug(TangoRegex) Стдвыв.фмтнс("{}{}: {}", s.прими?"*":" ", s.индекс, inp[p..$]);
                debug(TangoRegex) Стдвыв.фмтнс("{} commands", t.commands.length);
