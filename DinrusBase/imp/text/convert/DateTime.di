/*******************************************************************************
        ...
        образом. Смотрите МестнДатаВремя.фмт() с описанием процесса
 ...
 struct МестнДатаВремя
{
    ...
    /**********************************************************************
            ...
            "d" = "M/d/yyyy"
            "D" = "dddd, MMMM d, yyyy"
            "f" = "dddd, MMMM d, yyyy в:mm tt"
            "F" = "dddd, MMMM d, yyyy в:mm:ss tt"
            "g" = "M/d/yyyy в:mm tt"
            "G" = "M/d/yyyy в:mm:ss tt"
            "y"
            "Y" = "MMMM, yyyy"
            "r"
            ...
            ---
            Стдвыв.фмтнс ("{:t}", Часы.сейчас);
            Стдвыв.фмтнс ("{:D}", Часы.сейчас);
            Стдвыв.фмтнс ("{:dddd, dd MMMM yyyy HH:mm}", Часы.сейчас);
            ---

    **********************************************************************/

    ткст фмт (ткст вывод, Время датаВремя, ткст выкладка)
    {
    ...
    **********************************************************************/
    ...
    {
        static if (is (T == сим))
            return фмт (вывод, датаВремя, фмт);
        else
        {
            сим[128] tmp0 =void;
            сим[128] tmp1 =void;
            return Utf.изТкст8(фмт(tmp0, датаВремя, Utf.вТкст(фмт, tmp1)), вывод);
        }
    }
    ...
    else
    {
        ...
        **********************************************************************/
         ...
         static МестнДатаВремя создай ()
        {
            ...
            дв.длинныйОбразецВремени = дайТкстФормата(T_FMT, "в:mm:ss tt");
            дв.краткийОбразецВремени = "в:mm"; //default

            дв.образецМесяцаГода = "MMMM, yyyy"; //no posix equivalent?
            дв.полныйОбразецДатыВремени = дайТкстФормата(D_T_FMT, "dddd, MMMM d, yyyy в:mm:ss tt");
        ...
        **********************************************************************/
        ...
        private static ткст преобразуй(ткст фмт)
        {
            ...
            for (auto i = 0; i < фмт.length; ++i)
            {
                ...
                switch (c)
                {
                case 'a': //сокращённое имя дня недели локали.
    ...
    **********************************************************************/
    ...
    private ткст форматируйОсобо (ref Результат результат, Время датаВремя, ткст фмт)
    {
        бцел            длин,
        деньгода,
        деньнед,
        ...
        // смети фмт specifiers ...
        while (индекс < фмт.length)
        {
            сим c = фмт[индекс];
            ...
            // день
            case 'd':
                длин = повториРазбор (фмт, индекс, c);
                if (длин <= 2)
            ...
            case '\"':
            case '\'':
                длин = разборКавычек (результат, фмт, индекс);
                break;
    ...
    **********************************************************************/
    ...
    private static цел повториРазбор(ткст фмт, цел поз, сим c)
    {
        цел n = поз + 1;
        while (n < фмт.length && фмт[n] is c)
            n++;
        return n - поз;
    }
    ...
    **********************************************************************/
    ...
    private static ткст форматируйЦел (ткст врем, цел знач, цел минимум)
    {
        auto чис = Целое.itoa (врем, знач);
        if ((минимум -= чис.length) > 0)
        {
    ...
    **********************************************************************/
    ...
    private static цел разборКавычек (ref Результат результат, ткст фмт, цел поз)
    {
        цел старт = поз;
        сим chQuote = фмт[поз++];
        бул найдено;
        while (поз < фмт.length)
        {
            сим c = фмт[поз++];
            if (c is chQuote)
            {
...
краткийОбразецДаты                : "M/d/yyyy"
    ,
краткийОбразецВремени                : "в:mm"
    ,
длинныйОбразецДаты                 : "dddd, MMMM d, yyyy"
    ,
длинныйОбразецВремени                 : "в:mm:ss tt"
    ,
полныйОбразецДатыВремени             : "dddd, MMMM d, yyyy в:mm:ss tt"
    ,
общКраткийОбразецВремени         : "M/d/yyyy в:mm"
    ,
общДлинныйОбразецВремени          : "M/d/yyyy в:mm:ss tt"
    ,
...
debug (ДатаВремя)
{
    ...
    проц main()
    {
        ...
        Стдвыв.фмтнс ("d: {}", локаль.фмт (врем, время, "d"));
        Стдвыв.фмтнс ("D: {}", локаль.фмт (врем, время, "D"));
        Стдвыв.фмтнс ("f: {}", локаль.фмт (врем, время, "f"));
        Стдвыв.фмтнс ("F: {}", локаль.фмт (врем, время, "F"));
        Стдвыв.фмтнс ("g: {}", локаль.фмт (врем, время, "g"));
        Стдвыв.фмтнс ("G: {}", локаль.фмт (врем, время, "G"));
        Стдвыв.фмтнс ("r: {}", локаль.фмт (врем, время, "r"));
        Стдвыв.фмтнс ("s: {}", локаль.фмт (врем, время, "s"));
        Стдвыв.фмтнс ("t: {}", локаль.фмт (врем, время, "t"));
        Стдвыв.фмтнс ("T: {}", локаль.фмт (врем, время, "T"));
        Стдвыв.фмтнс ("y: {}", локаль.фмт (врем, время, "y"));
        Стдвыв.фмтнс ("u: {}", локаль.фмт (врем, время, "u"));
        Стдвыв.фмтнс ("@: {}", локаль.фмт (врем, время, "@"));
        Стдвыв.фмтнс ("{}", локаль.генерный.фмт (врем, время, "ddd, dd MMM yyyy HH':'mm':'ss zzzz"));
    }
}
