...
#define FTC_CMAP_UNKNOWN  ( (FT_UInt16)-1 )
                        ...
                        FT_UInt32      char_code )
  {
    ...
    if ( cmap_index < 0 )
    {
      /* Treat a negative cmap index as a special значение, meaning that you */
      ...
      /* sets the face's charmap to the appropriate значение.                */
      ...
      case FTC_OLD_CMAP_BY_ENCODING:
        {
          ...
          Ошибка = FTC_Manager_LookupFace( cache->manager, desc->face_id,
                                          &face );
          if ( Ошибка )
            return 0;
    ...
    FTC_CACHE_LOOKUP_CMP( cache, ftc_cmap_node_compare, hash, &query,
                          node, Ошибка );
#else
    Ошибка = FTC_Cache_Lookup( cache, hash, &query, &node );
#endif
    if ( Ошибка )
      goto Exit;
    ...
    if ( (FT_UInt)( char_code - FTC_CMAP_NODE( node )->first >=
                    FTC_CMAP_INDICES_MAX ) )
      return 0; /* XXX: should return appropriate Ошибка */
    ...
    if ( gindex == FTC_CMAP_UNKNOWN )
    {
      ...
      Ошибка = FTC_Manager_LookupFace( cache->manager,
