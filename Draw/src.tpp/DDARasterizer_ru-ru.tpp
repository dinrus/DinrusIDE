...
[s2; Простой несглаживающий (non`-antialiased) 
растризатор, способный чертить линии 
алгоритмом [^http`:`/`/en`.wikipedia`.org`/wiki`/Bresenham`%27s`_line`_algorithm^ D
DA] и многоугольники с правилом чётное`-нечётн
ое. Итог отображается чисто виртуальными 
методами, рисующими горизонтальные 
и вертикальные сегменты линий.&]
[s3;%- &]
...
[s2;%- [%RU-RU PЧисто виртуальный метод, от 
которого ожидается отрисовка вертикальной 
