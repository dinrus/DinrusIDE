...
int SqlError(
	         ...
	         HWND parent,
#endif
             const char *text, const char *Ошибка, const char *statement, bool retry) {
	WithSqlErrorLayout<ТопОкно> r;
	CtrlLayoutCancel(r, t_("Database Ошибка"));
	r.text = text;
	r.Ошибка.AutoHideSb().устТолькоЧтен() <<= Ошибка;
	r.statement.AutoHideSb().устТолькоЧтен() <<= statement;
	r.retry.покажи(retry);
	r.Acceptor(r.retry, IDRETRY);
	r.ActiveFocus(r);
#ifdef PLATFORM_WIN32
	if(parent) r.открой(parent);
#endif
	return r.пуск();
}
...
#ifdef PLATFORM_WIN32
int  SqlError(const char *text, const char *Ошибка, const char *statement, bool retry) {
	return SqlError(HWND(NULL), text, Ошибка, statement, retry);
}
...
bool OkCommit(SqlSession& session, const char *msg) {
	if(ErrorRollback(session, msg))
		return false;
	session.Commit();
	if(ShowError(session, msg ? msg : t_("SQL Ошибка!")))
		return false;
	return true;
}
