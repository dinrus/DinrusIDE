...
bool IsConflictFile(String path)
{
	...
	if(*ext == '.') {
		ext = ext.Mid(1);
		if(findarg(ext, "мой", "их", "рабочий") >= 0 || *ext == 'r' && IsDigit(ext[1])) {
			for(int i = 0; i < 3; i++) {
					...
					if(action == SVN_IGNORE) {
						...
						if(action == ADD && IsConflictFile(file)) {
							...
							static const char *as[] = {
								"Изменить", "Решено", "Добавить", "Удалить", "Заменить"
							};
...
void RepoSync::SyncList()
{
	...
	for(const auto& w : ~work) {
		...
		if(actions) {
			...
			list.Add(-1, Null, "", AttrText("Нечего делать").SetFont(StdFont().Italic()));
	}
}
