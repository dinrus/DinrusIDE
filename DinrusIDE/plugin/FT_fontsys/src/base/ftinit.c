...
#undef  FT_USE_MODULE
#ifdef __cplusplus
#define FT_USE_MODULE( тип, x )  extern "C" const тип  x;
#else
#define FT_USE_MODULE( тип, x )  extern const тип  x;
#endif
...
#undef  FT_USE_MODULE
#define FT_USE_MODULE( тип, x )  (const FT_Module_Class*)&(x),
...
#undef  FT_USE_MODULE
#define FT_USE_MODULE( тип, x )  MODULE_CLASS_##x,
...
#undef  FT_USE_MODULE
#define FT_USE_MODULE( тип, x )                \
  Ошибка = FT_Create_Class_##x(library, &clazz); \
  if (Ошибка) goto Exit;                         \
  classes[i++] = clazz;
...
Exit:    
    if (Ошибка) ft_destroy_default_module_classes( library );
  ...
  FT_Add_Default_Modules( FT_Library  library )
  {
    ...
    while ( *cur )
    {
      Ошибка = FT_Add_Module( library, *cur );
      /* notify errors, but don't stop */
      if ( Ошибка )
        FT_TRACE0(( "FT_Add_Default_Module:"
                    " Cannot install `%s', Ошибка = 0x%x\n",
                    (*cur)->module_name, Ошибка ));
      cur++;
    }
  }
  ...
  FT_Init_FreeType( FT_Library  *alibrary )
  {
    ...
    Ошибка = FT_New_Library( memory, alibrary );
    if ( Ошибка )
